...
对于不向 Internet 公开的内部业务线应用程序，使用前端专用 IP 地址配置网关的做法非常有效。 对于位于不向 Internet 公开的安全边界内的多层应用程序中的服务和层也很有用，但会出现以下情况：

- 仍需要轮循负载分布
- 会话粘性
- 或传输层安全性 (TLS) 终止 (之前称为安全套接字层 (SSL) ) 。
...
Azure 需要一个虚拟网络才能在创建的资源之间通信。 请创建一个新的虚拟网络或使用现有的虚拟网络。 

本示例将创建新的虚拟网络。 可以在创建应用程序网关的同时创建虚拟网络。 在独立的子网中创建应用程序网关实例。 在此示例中有两个子网：一个用于应用程序网关，另一个用于后端服务器。
...
4. 输入虚拟机的以下值：
   ...
   - 键入有效的 **用户名**。
   - 键入有效的 **密码**。
1. 接受剩余的默认值并选择 " **下一步：磁盘**"。
1. 接受默认值，然后选择“下一步:  网络”。
1. 确保为虚拟网络选择 **myVNet** ，子网为 **myBackendSubnet**。
...
3. 使用刚刚完成的步骤创建第二个虚拟机并安装 IIS。 使用 myVM2 作为虚拟机名称，并 `VMName` 在中使用 `Set-AzVMExtension` 。
...
4. 重复此过程以添加 MyVM2。
   ![将突出显示目标类型和目标的 "后端池" 窗格。](./media/configure-application-gateway-with-private-frontend-ip/private-frontendip-4.png)
5. 选择“保存”。
...
1. 在 "myAppGateway" 页上，选择 " **前端 IP 配置** " 以记下前端专用 IP 地址。
    ![突出显示了私有类型的 "前端 IP 配置" 窗格。](./media/configure-application-gateway-with-private-frontend-ip/private-frontendip-5.png)
2. 复制专用 IP 地址，然后将其粘贴到 myVM3 上的浏览器地址栏中，以访问应用程序网关后端池。

## <a name="next-steps"></a>后续步骤

如果要监视后端池的运行状况，请参阅 [应用程序网关的后端运行状况和诊断日志](application-gateway-diagnostics.md)。
