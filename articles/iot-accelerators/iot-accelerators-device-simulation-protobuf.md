...
* .NET Core。 可以下载[适用于 Mac、Linux 和 Windows 的 .NET Core](https://dotnet.microsoft.com/download)。
...
从 GitHub 下载[设备模拟微服务](https://github.com/Azure/azure-iot-pcs-device-simulation/archive/master.zip)并将其解压缩到本地计算机上的适当位置。
...
默认情况下，新设备模型的 JSON 和 JS 文件将不会被复制到生成的解决方案。 需要显式包含这些文件。
...
了解如何将设备模拟自定义为使用 Protobuf 发送遥测数据之后，接下来请访问 GitHub 存储库以详细了解[设备模拟](https://github.com/Azure/azure-iot-pcs-device-simulation)。
