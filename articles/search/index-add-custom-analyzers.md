...
ms.locfileid: "124784710"
---
# <a name="add-custom-analyzers-to-string-fields-in-an-azure-cognitive-search-index"></a>向 Azure 认知搜索索引中的字符串字段添加自定义分析器

*自定义分析器* 是 tokenizer、一个或多个标记筛选器的组合（由用户定义），以及在搜索索引中指定的一个或多个字符筛选器，然后引用需要自定义分析的字段定义。 Tokenizer 负责将文本分解成多个标记，标记筛选器负责修改 tokenizer 发出的标记。 字符筛选器在 tokenizer 处理输入文本之前准备输入文本。 有关概念和示例，请参阅[在 Azure 认知搜索中添加分析器](search-analyzers.md)。

自定义分析器可以使得允许通过选择要调用的分析或筛选类型以及它们出现的顺序来控制将文本转换为可索引和可搜索的标记的过程。 

如果默认 (Standard Lucence)、内置或语言分析器都不能满足你需求，则创建并分配自定义分析器。 如果要将内置分析器与自定义选项一同使用，还可以创建自定义分析器。 例如，如果要更改 Standard 上的 maxTokenLength，可以使用用户定义的名称创建自定义分析器以设置该选项。
...
> [!NOTE]  
> 自定义分析器不会在 Azure 门户中公开。 添加自定义分析器的唯一方法是通过定义索引的代码。 
...
认知搜索支持以下列表中的字符筛选器。 有关各自的详细信息，请参阅 Lucene API 参考。
...
认知搜索支持以下列表中的 tokenizer。 有关各自的详细信息，请参阅 Lucene API 参考。
