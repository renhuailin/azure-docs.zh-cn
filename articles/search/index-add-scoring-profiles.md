---
title: 添加计分概要文件以提升搜索分数
titleSuffix: Azure Cognitive Search
description: 通过将计分概要文件添加到搜索索引，提升 Azure 认知搜索结果的搜索相关性分数。
manager: nitinme
author: shmed
ms.author: ramero
...
ms.locfileid: "112321843"
---
# <a name="add-scoring-profiles-to-a-search-index"></a>将计分概要文件添加到搜索索引

对于全文搜索查询，搜索引擎将为每个匹配文档计算搜索分数，这样可以从高到低对结果进行排名。 Azure 认知搜索使用默认计分算法计算初始分数，但可以通过“计分概要文件”自定义计算。

计分概要文件嵌入在索引定义中并包括用于提升匹配分数的属性，其中概要文件中找到的附加条件提供提升逻辑。 例如，你可能想要根据创收能力提升匹配项、提升新项或提升库存时间太长的项。  
...
以下定义显示名为“geo”的简单概要文件。 此文件用于提升在“hotelName”字段中具有搜索词的结果。 它还使用 `distance` 函数优先提升在当前位置十公里范围内的结果。 如果有人搜索“inn”一词，而“inn”恰好是酒店名称的一部分，包含当前位置 10 公里范围内带有“inn”的酒店的文档会在搜索结果中的较高位置出现。  
...
若要使用此计分概要文件，查询要表述为指定请求中的 scoringProfile 参数。
...
此查询对“inn”一词进行搜索，并在当前位置中传递。 请注意，此查询包含其他参数，如 scoringParameter。 查询参数在[搜索文档 (REST API)](/rest/api/searchservice/Search-Documents) 中介绍。  

请参阅[扩展示例](#bkmk_ex)以查看计分概要文件的更详细示例。  

<a name=what-is-default-scoring></a>

## <a name="how-scores-are-computed"></a>如何计算分数
...
当默认排名行为不足以满足业务目标时，应创建一个或更多计分概要文件。 例如，可能会决定搜索相关性应更倾向于新添加的项。 同样，可能有一个包含利润率的字段，或其他某些指示营收潜力的字段。 提升对用户或业务更有意义的结果通常是采用计分概要文件的决定因素。
...
`boostGenre` 概要文件使用加权文本字段来提升 albumTitle、genre 和 artistName 字段中找到的匹配项。 这些字段各自提升 1.5、5 和 2。 genre 为何比其他字段提升更高？ 如果对在某种程度上为同类的数据执行搜索（正如 musicstoreindex 中的“genre”一样），可能需要在相对权重中产生较大差异。 例如，在 musicstoreindex 中，“rock”既作为流派显示，又显示在采用相同组句方式的流派说明中。 如果希望流派的权重在流派说明之上，genre 字段将需要更高的相对权重。
...
1. 开始使用索引定义。 可以在现有索引中添加和更新计分概要文件，而无需重新生成它。 使用[更新索引](/rest/api/searchservice/update-index)请求发布你的修订版本。

1. 粘贴到本主题中提供的[模板](#bkmk_template)中。  

1. 提供一个名称。 计分概要文件为可选项，但如果要添加一个概要文件，必须提供名称。 请务必遵循字段的认知搜索[命名约定](/rest/api/searchservice/naming-rules)（以字母开头，避免使用特殊字符和保留字）。  

1. 指定提升条件。 单个概要文件可以包含[加权字段](#weighted-fields)和/或[函数](#functions)。 

应以迭代方式使用数据集，该数据集将帮助你证明或推翻给定概要文件的效力。

计分概要文件可在 Azure 门户（如以下屏幕截图中所示）中定义，或通过 [REST API](/rest/api/searchservice/update-index) 或在 Azure SDK 中（例如 Azure SDK for .NET 中的 [ScoringProfile](/dotnet/api/azure.search.documents.indexes.models.scoringprofile) 类）以编程方式定义。

   :::image type="content" source="media/scoring-profiles/portal-add-scoring-profile-small.png" alt-text="“添加计分概要文件”页" lightbox="media/scoring-profiles/portal-add-scoring-profile.png" border="true":::

<a name="weighted-fields"></a>

### <a name="using-weighted-fields"></a>使用加权字段

当字段上下文非常重要且查询是全文搜索（也称为自由格式文本搜索）时，请使用加权字段。 例如，如果查询包括“airport”一词，则你可能希望“说明”字段中“airport”的权重比 HotelName 中的更多。 

加权字段由一个可搜索字段和一个用作乘数的正数组成。 如果 HotelName 的原始字段分数为 3，则该字段的提升分数将变为 6，这将导致父文档本身的总分更高。
...
当简单的相对权重不足或不适用时，请使用函数，如果使用的是 distance 和 freshness，则对数值数据进行计算。 可以为每个计分概要文件指定多个函数。

| 函数 | 说明 |
|-|-|
| “freshness” | 按日期时间字段中的值提升 (Edm.DataTimeOffset)。 此函数有一个 `boostingDuration` 属性，以便你可以指定一个表示提升的时间跨度的值。 | 
| “magnitude” | 按数值高低程度提升。 调用此函数的方案包括按照利润率、最高价格、最低价格或下载次数提升。 此函数只能与 Edm.Double 和 Edm.Int 字段一起使用。 对于 magnitude 函数，如果想要反转模式（例如，将价格较低项提升至价格较高项之上），可以将范围反转为从高到低。 假设价格范围从 100 美元到 1 美元，可以将“boostingRangeStart”设为 100、“boostingRangeEnd”设为 1 以提升价格较低的项。 | 
| “distance”  | 按距离或地理位置提升。 此函数只能与 Edm.GeographyPoint 字段一起使用。 | 
| “tag”  | 按搜索文档和查询字符串共有的标记提升。 `tagsParameter` 中提供了标记。 此函数只能与 Edm.String 和 Collection(Edm.String) 字段一起使用。 | 
...
## <a name="template"></a>模板
 ...
 本部分演示计分概要文件的语法和模板。 请参阅下一部分的[属性参考](#bkmk_indexref)，获取计分概要文件属性的说明。  
...
| text | 包含 weights 属性。|  
| weights | 可选。 指定可搜索字段的名称/值对以及用于提升字段分数的正整数或浮点数。 正整数或数字将变为排名算法生成的原始字段分数的乘数。 例如，如果字段分数为 2，权重值为 3，则字段的提升分数将变为 6。 然后，聚合各个字段分数以创建文档字段分数，该分数用于对结果集中的文档进行排名。 |  
...
| functions > magnitude | magnitude 计分函数用于改变基于数值字段的值范围的排名。 一些最常见的用法示例如下： </br></br>“星级评分”：根据“星级评分”字段中的值更改评分。 如果两个项相关，具有较高评分的项先显示。 </br>“利润”：当两个文档相关时，零售商可能希望先提升具有较高利润的文档。 </br>“点击次数”：对于跟踪产品或页面点击行为的应用程序，可使用 magnitude 提升容易获得最多流量的项。 </br>“下载次数”：对于跟踪下载的应用程序，magnitude 函数可提升下载次数最多的项。|  
...
| functions > freshness | freshness 计分函数用于改变基于 DateTimeOffset 字段值的项的排名分数。 例如，具有较新日期的项可以排在日期较旧的项之上。 </br></br>也可以排列日历事件等具有未来日期的项，以便接近当前日期的项可以排在距离当前日期较远的将来的项之上。 </br></br>在当前服务版本中，范围的一端将固定为当前时间。 另一端是基于 boostingDuration 的过去的时间。 要提升将来时间的范围，请使用负 boostingDuration。 </br></br>提升从最大范围和最小范围改变的比率由应用到计分概要文件的内插确定（请见下图）。 若要反转应用的提升系数，请选择不超过 1 的提升系数。|  
...
| defaultScoringProfile | 在执行搜索请求时，如果未指定任何计分概要文件，则使用默认计分（仅限 [tf-idf](http://www.tfidf.com/)）。 </br></br>可以替代内置默认值，以将自定义概要文件代替为在搜索请求中未给定任何特定概要文件时使用的概要文件。|  
...
+ [什么是计分概要文件？](https://social.technet.microsoft.com/wiki/contents/articles/26706.azure-search-what-are-scoring-profiles.aspx)