...
Azure 认知搜索中的“索引器”是一种爬网程序，它从外部 Azure 数据源提取可搜索的文本和元数据，并使用字段到字段映射在源数据与索引之间填充搜索索引。 由于不需要编写任何将数据添加到索引的代码，该服务就能拉取数据，因此这种方法有时也称为“拉取模式”。 索引器还驱动认知搜索的 [AI 扩充](cognitive-search-concept-intro.md)功能，将外部在途内容处理与索引进行集成。
...
您可以使用索引器作为唯一的数据引入一方法，也可以将其作为加载和选择性地转换或扩充内容的技术组合的一部分。 下表汇总了主要方案。
...
| 多个索引器 | 如果使用多个数据源，而且需要改变运行时参数、计划或字段映射，则可能还需要多个索引器。 虽然多个索引器数据源集可以同一索引为目标，但要注意可能会覆盖索引中现有值的索引器运行。 如果第二个索引器数据源以相同的文档和字段为目标，则将覆盖第一次运行中的任何值。 字段值将被完全替换；索引器无法将多次运行中的值合并为同一字段。</br></br>另一个多索引器用例是[认知搜索的跨区域横向扩展](search-performance-optimization.md#use-indexers-for-updating-content-on-multiple-services)。 你可能有位于不同区域的相同搜索索引的副本。 若要同步搜索索引内容，可从同一数据源中拉取多个索引器，其中每个索引器都以不同的搜索索引为目标。</br></br>对非常大的数据集进行[并行索引](search-howto-large-index.md#parallel-indexing)还需要多索引器策略。 每个索引器都针对数据的一个子集。 |
| 内容转换 | 认知搜索支持可选的 [AI 扩充](cognitive-search-concept-intro.md)行为，这些行为可添加图像分析和自然语言处理，以创建新的可搜索内容和结构。 AI 扩充通过附加的[技能组](cognitive-search-working-with-skillsets.md)由索引器驱动。 若要执行 AI 扩充，索引器仍需要索引和 Azure 数据源，但在此方案中，索引器执行添加了技能组处理。 |
...
使用 Azure 虚拟网络访问客户端应用时，可以使用标准 Internet 连接（公共）或加密的专用连接来建立索引器与远程数据源之间的连接。 还可以建立使用可信服务标识进行身份验证的连接。 有关安全连接的详细信息，请参阅[通过专用终结点授予访问权限](search-indexer-securing-resources.md#granting-access-via-private-endpoints)和[使用托管标识连接到数据源](search-howto-managed-identities-data-sources.md)。
...
对于索引器接收的每个文档，它将执行或协调多个步骤来编制索引，从文档检索到最终的搜索引擎“移交”。 （可选）如果定义了技能组，索引器还有助于推动技能组的执行和输出。
...
技能组执行是一个可选步骤，它调用内置或自定义 AI 处理。 如果源数据是二进制图像，可能需要对其进行图像分析形式的光学字符识别 (OCR)，如果内容采用不同的语言，则可能需要语言翻译。 
...
如果包括技能组，则很可能需要包含输出字段映射。 技能组的输出实际上是一个名为“扩充文档”的信息树。 通过输出字段映射，可以选择此树中哪些部分要映射到索引中的字段。 了解如何[定义输出字段映射](cognitive-search-output-field-mapping.md)。
...
索引器需要提供连接字符串和可能的凭据的“数据源”对象。 调用[创建数据源 (REST)](/rest/api/searchservice/create-data-source) 或 [SearchIndexerDataSourceConnection 类](/dotnet/api/azure.search.documents.indexes.models.searchindexerdatasourceconnection)以创建数据源。

数据源的配置和管理独立于使用数据源的索引器，这意味着多个索引器可使用一个数据源，同时加载多个索引。

### <a name="step-2-create-an-index"></a>步骤 2：创建索引

索引器会自动执行某些与数据引入相关的任务，但通常不会自动创建索引。 先决条件是必须具有预定义的索引，且索引的字段必须与外部数据源中的字段匹配。 字段需按名称和数据类型进行匹配。 如果无法使用这些匹配，可以[定义字段映射](search-indexer-field-mappings.md)以建立关联。 有关创建索引的详细信息，请参阅[创建索引 (REST)](/rest/api/searchservice/Create-Index) 或 [SearchIndex 类](/dotnet/api/azure.search.documents.indexes.models.searchindex)。
...
首次在搜索服务中[创建索引器](/rest/api/searchservice/Create-Indexer)时，将运行索引器。 只有在创建或运行索引器时，才能确定数据源是否可访问，或者技能组是否有效。 首次运行后，可以使用[运行索引器](/rest/api/searchservice/run-indexer)按需重新运行，也可以[定义定期计划](search-howto-schedule-indexers.md)。 

可以通过门户或[获取索引器状态 API](/rest/api/searchservice/get-indexer-status) 监视索引器状态。 还应[对索引运行查询](search-query-create.md)，以验证结果是否符合预期。

## <a name="next-steps"></a>后续步骤

现在，您已了解入门知识，下一步是查看索引器属性和参数、计划和索引器监视。 或者，您可以返回[支持的数据源](#supported-data-sources)列表，以获取有关特定数据源的详细信息。
