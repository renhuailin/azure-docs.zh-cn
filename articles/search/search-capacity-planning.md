...
Azure 认知搜索中的可伸缩性体系结构基于副本和分区的灵活组合，因此，根据是否需要更多的查询或索引功能，可以改变容量。 创建服务后，可以单独增加或减少副本或分区数。 每增加一个物理资源，成本就会增加，但是一旦完成大的工作负载，就可以减小规模以降低费用。 增加或减少容量所需的时间为 15 分钟到几个小时，具体取决于调整的层和大小。
