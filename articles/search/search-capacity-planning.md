...
## <a name="about-queries-per-second-qps"></a>关于每秒查询 (QPS)

由于大量因素会影响查询性能，因此 Microsoft 不会发布预期的 QPS 次数。 QPS 估算值必须由每个客户使用对其应用程序有效的服务层、配置、索引和查询结构独立开发。 索引大小和复杂性、查询大小和复杂性以及流量大小是 QPS 的主要决定因素。 当此类因素未知时，没有方法能提供有意义的估计值。
