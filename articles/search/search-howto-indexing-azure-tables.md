---
title: 从 Azure 表存储索引数据
titleSuffix: Azure Cognitive Search
description: 设置搜索索引器，以对 Azure 表存储中存储的数据编制索引，以便在 Azure 认知搜索中进行全文搜索。
manager: nitinme
author: mgottein
ms.author: magottei
ms.devlang: rest-api
...
ms.locfileid: "112982953"
---
# <a name="index-data-from-azure-table-storage"></a>从 Azure 表存储索引数据

本文介绍如何配置 Azure 表索引器以提取内容，并使内容在 Azure 认知搜索中可搜索。 此工作流会在 Azure 认知搜索上创建搜索索引，并为从 Azure 表存储中提取的现有内容加载此索引。

可以使用以下任一客户端设置 Azure 表存储索引器：
...
## <a name="configure-an-indexer"></a>配置索引器

### <a name="step-1-create-a-data-source"></a>步骤 1：创建数据源

[创建数据源](/rest/api/searchservice/create-data-source)指定要索引的数据、访问数据所需的凭据和支持 Azure 认知搜索有效标识数据更改的策略。
...
- 托管标识连接字符串：`ResourceId=/subscriptions/<your subscription ID>/resourceGroups/<your resource group name>/providers/Microsoft.Storage/storageAccounts/<your storage account name>/;` 此连接字符串不需要帐户密钥，但你必须按照[使用托管标识设置与 Azure 存储帐户的连接](search-howto-managed-identities-storage.md)的说明操作。
...
[创建索引](/rest/api/searchservice/create-index)指定文档、属性和其他构造中可以塑造搜索体验的字段。
...
[创建索引器](/rest/api/searchservice/create-indexer)将数据源与目标搜索索引关联，并提供自动执行数据刷新的计划。 
...
此索引器每两小时运行一次。 （已将计划间隔设置为“PT2H”）。若要每隔 30 分钟运行一次索引器，可将间隔设置为“PT30M”。 支持的最短间隔为 5 分钟。 计划是可选的；如果省略，则索引器在创建后只运行一次。 但是，可以随时根据需要运行索引器。 有关定义索引器计划的详细信息，请参阅[为 Azure 认知搜索计划索引器](search-howto-schedule-indexers.md)。

## <a name="handle-field-name-discrepancies"></a>处理字段名称差异
