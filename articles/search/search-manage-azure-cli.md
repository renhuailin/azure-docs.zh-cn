---
title: 使用 az search 模块 Azure CLI 脚本
...
可以在 Windows、macOS、Linux 或 [Azure Cloud Shell](../cloud-shell/overview.md) 上运行 Azure CLI 命令和脚本，以创建和配置 Azure 认知搜索。 [**Az search**](/cli/azure/search)模块将具有完整奇偶校验的 [Azure CLI](/cli/)扩展为 [搜索管理 REST api](/rest/api/searchmanagement)和执行以下任务的能力：
...
以下命令来自 [**az 资源**](/cli/azure/resource)，返回有关订阅中已设置的现有资源和服务的信息。 如果你不知道已经创建了多少个搜索服务，则这些命令会返回该信息，省得要在门户中查找。
...
你可以在 CLI 中查看 [**az search**](/cli/azure/search) 中可用的子组和命令的有关信息。 或者，你可以查看 [文档](/cli/azure/search)。

若要查看中的可用子组 `az search` ，请运行以下命令。
...
每个子组中有多个命令可用。 可以 `service` 通过运行以下行来查看子组的可用命令。
...
如果知道包含搜索服务的资源组，请运行 [**az search service show**](/cli/azure/search/service#az_search_service_show) 以返回服务定义，包括名称、区域、层和副本和分区计数。
...
根据你的安全要求，你可能想要创建一个 [配置了 IP 防火墙](service-configure-firewall.md)的搜索服务。 为此，请向参数传递公共 IP (v4) 地址或 CIDR 范围，如下 `ip-rules` 所示。 规则应该用逗号分隔 (`,`) 或分号 (`;`) 。
...
在某些情况下，例如 [使用托管标识连接到数据源](search-howto-managed-identities-storage.md)时，需要打开 [系统分配的托管标识](../active-directory/managed-identities-azure-resources/overview.md)。 这是通过将添加 `--identity-type SystemAssigned` 到命令来完成的。
...
有关在 PowerShell 中创建专用终结点的详细信息，请参阅此 [专用链接快速入门](https://docs.microsoft.com/azure/private-link/create-private-endpoint-cli)

### <a name="manage-private-endpoint-connections"></a>管理专用终结点连接

除了创建专用终结点连接之外，您还可以 `show` 使用、 `update` 和 `delete` 连接。

若要检索专用终结点连接并查看其状态，请使用 [**az search 专用终结点-连接显示**](/cli/azure/search/private-endpoint-connection#az_search_private_endpoint_connection_show)。
...
若要更新连接，请使用 [**az search 专用终结点-连接更新**](/cli/azure/search/private-endpoint-connection#az_search_private_endpoint_connection_update)。 下面的示例将专用终结点连接设置为 "已拒绝"：
...
若要删除专用终结点连接，请使用 [**az search 专用终结点-连接删除**](/cli/azure/search/private-endpoint-connection#az_search_private_endpoint_connection_delete)。
...
若要滚动管理 [API 密钥](search-security-api-keys.md)，请使用 [**az search 管理密钥续订**](/cli/azure/search/admin-key#az_search_admin_key_renew)。 两个管理密钥是使用每个服务创建的，用于进行身份验证访问。 需要在每个请求中提供密钥。 这两个管理密钥在功能上是等效的，授予对搜索服务的完全写入访问权限，并可以检索任何信息，或创建和删除任何对象。 这两个密钥可以换用。 
...
若要 [增加或减少副本和分区，](search-capacity-planning.md) 请使用 [**az search 服务更新**](/cli/azure/search/service#az_search_service_update)。 增加副本或分区会增大费用，两者都提供固定和可变的费率。 如果你暂时需要更大的处理能力，可以增加副本和分区来处理工作负荷。 “概述”门户页中的监视区域提供有关查询延迟、每秒查询数和限制的磁贴，指示当前容量是否足够。
...
除了更新副本和分区计数之外，还可以更新 `ip-rules` 、 `public-access` 和 `identity-type` 。
...
若要创建共享的专用链接资源，请使用 [**az search shared-resource create**](/cli/azure/search/shared-private-link-resource#az_search_shared_private_link_resource_list)。 请记住，在运行此命令之前，可能需要对数据源进行一些配置。
...
若要检索共享的专用链接资源并查看其状态，请使用 [**az search shared-link**](/cli/azure/search/shared-private-link-resource#az_search_shared_private_link_resource_list)资源列表。
...
需要使用以下命令批准连接，然后才能使用。 需要从子资源中检索专用终结点连接的 ID。 在此示例中，我们从 az storage 获取连接 ID。
...
若要删除共享的专用链接资源，请使用 [**az search 共享--链接资源删除**](/cli/azure/search/shared-private-link-resource#az_search_shared_private_link_resource_delete)。
