...
Azure 认知搜索是一项搜索服务，它支持基于用户定义索引的索引和查询工作负载，其中包含在云中托管的远程可搜索内容。 为了提高性能，需将可搜索内容与查询引擎共置在一起，使返回结果的速度与用户预期的搜索查询速度相当。
...
可以直接在存储帐户门户页中开始。 在左侧导航页面中的“Blob 服务”下，选择“添加 Azure 认知搜索”，以创建新服务或选择现有服务 。 
...
索引器[破解文档](search-indexer-overview.md#document-cracking)，会打开一个 Blob 来检查内容。 这是连接到数据源后，在管道中发生的第一个步骤。 对于 Blob 数据，此步骤会检测 PDF、Office 文档和其他内容类型。 文档破解和文本提取是免费的。 如果 Blob 包含图像内容，则除非[添加 AI 扩充](search-blob-ai-integration.md)，否则会忽略图像。 标准索引仅适用于文本内容。
