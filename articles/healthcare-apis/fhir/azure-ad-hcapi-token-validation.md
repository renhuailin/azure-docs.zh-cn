---
title: 用于 FHIR 访问令牌验证的 Azure API
description: 演练令牌验证，并提供有关如何解决访问问题的提示
...
ms.locfileid: "103017898"
---
# <a name="azure-api-for-fhir-access-token-validation"></a>用于 FHIR 访问令牌验证的 Azure API

适用于 FHIR 的 Azure API 如何验证访问令牌将取决于实现和配置。 在本文中，我们将演练验证步骤，这在排查访问问题时非常有用。

## <a name="validate-token-has-no-issues-with-identity-provider"></a>验证令牌对于标识提供者没有问题

令牌验证的第一步是验证令牌是否由正确的标识提供者颁发，并且是否尚未修改。 FHIR 服务器将配置为使用特定的标识提供程序（称为颁发机构） `Authority` 。 FHIR 服务器将从终结点检索有关标识提供程序的信息 `/.well-known/openid-configuration` 。 使用 Azure AD 时，完整的 URL 将为：

```
GET https://login.microsoftonline.com/<TENANT-ID>/.well-known/openid-configuration
```

其中 `<TENANT-ID>` ，特定 Azure AD 租户 (租户 ID 或域名) 。

Azure AD 将在 FHIR 服务器上返回如下所示的文档。
...
``` 
FHIR 服务器的重要属性为 `jwks_uri` ，告诉服务器在何处提取验证令牌签名所需的加密密钥 `issuer` ，并告诉服务器颁发者声明中的内容 (`iss`) 此服务器颁发的令牌。 FHIR 服务器可以使用它来验证它是否正在接收可信令牌。

## <a name="validate-claims-of-the-token"></a>验证令牌的声明

服务器验证令牌的真实性后，FHIR 服务器将继续验证客户端是否拥有访问令牌所需的声明。

使用用于 FHIR 的 Azure API 时，服务器将验证：
