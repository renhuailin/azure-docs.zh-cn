...
本文介绍了如何使用服务主体登录方法连接到 Azure 媒体服务 v3 node.js SDK。 将要用到 *media-services-v3-node-tutorials* 示例存储库中的文件。 *HelloWorld-ListAssets* 示例包含了用于连接的代码，并会列出帐户中的资产。
...
> [!IMPORTANT]
> 查看 Azure 媒体服务[命名约定](media-services-apis-overview.md#naming-conventions)，以了解实体的重要命名限制。

## <a name="clone-the-nodejs-samples-repo"></a>克隆 Node.JS 示例存储库

将会用到 Azure 示例中的某些文件。 请克隆 Node.JS 示例存储库。
...
请安装最低版本的“@azure/ms-rest-nodeauth”：“^3.0.0”。

```bash
npm install @azure/ms-rest-nodeauth@"^3.0.0"
```

在此示例中，将使用 `package.json` 文件中的以下包。

|程序包|说明|
|---|---|
|`@azure/arm-mediaservices`|Azure 媒体服务 SDK。 <br/>为确保使用的是最新的 Azure 媒体服务包，请选中 [npm 安装 @azure/arm-mediaservices](https://www.npmjs.com/package/@azure/arm-mediaservices)。|
|`@azure/ms-rest-nodeauth` | 使用服务主体或托管标识进行 AAD 身份验证时，必然需要|
|`@azure/storage-blob`|存储 SDK。 将文件上传到资产时使用。|
|`@azure/ms-rest-js`| 用于登录。|
|`@azure/storage-blob` | 用于将文件上传和下载到 Azure 媒体服务中的“资产”以进行编码。|
|`@azure/abort-controller`| 与存储客户端一起使用，令长时间运行的下载操作处于超时状态|

### <a name="create-the-packagejson-file"></a>创建 package.json 文件

1. 使用常用的编辑器，创建 `package.json` 文件。
1. 打开该文件并粘贴以下代码：
...
将此文件的内容复制到名为 *.env* 的文件中。 应存储在工作存储库的根目录中。 这些是从门户中媒体服务帐户的“API 访问”页上获得的值。

创建了 *.env* 文件后，就可以开始使用这些示例。
...
4. 从 AMSv3Samples 文件夹中启动 Visual Studio Code。 从“.vscode”文件夹和 tsconfig.json 文件所在的文件夹中启动时，必须执行此操作
...
在 index.ts 文件中，按 F5 启动调试器。 如果帐户中已有资产，此时将显示资产列表。 如果帐户为空，则将显示空列表。  

若要快速查看所列出的资产，请使用门户上传一些视频文件。 这些资产将逐个自动创建，再次运行此脚本后，即会返回每个资产的名称。

### <a name="a-closer-look-at-the-helloworld-listassets-sample"></a>*HelloWorld-ListAssets* 示例的详细介绍

*HelloWorld-ListAssets* 示例说明了如何使用服务主体连接到媒体服务客户端，以及如何列出帐户中的资产。 有关代码作用的详细说明，请参阅该代码中的注释。
...
|项目名称|用例|
|---|---|
|Live/index.ts| 实时传送视频流的基本示例。 **警告**：在使用实时传送视频流时，请务必检查所有资源是否都已清理，并且不再在门户中计费|
|StreamFilesSample/index.ts| 上传本地文件或从源 URL 编码的基本示例。 该示例演示了如何使用存储 SDK 下载内容，以及如何流式传输到播放机 |
|StreamFilesWithDRMSample/index.ts| 演示如何使用 Widevine 和 PlayReady DRM 进行编码和流式传输 |
|VideoIndexerSample/index.ts| 使用视频和音频分析器预设以生成视频或音频文件的元数据和见解的示例 |
