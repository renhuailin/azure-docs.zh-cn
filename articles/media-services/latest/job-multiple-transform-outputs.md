...
ms.locfileid: "106108840"
---
# <a name="create-a-job-with-multiple-transform-outputs"></a>创建具有多个转换输出的作业

[!INCLUDE [media services api v3 logo](./includes/v3-hr.md)]

本主题说明如何创建具有两个转换输出的转换。 第一个转换输出需要使用内置的 [AdaptiveStreaming](encode-concept.md#builtinstandardencoderpreset) 预设对要编码的输入进行编码，以便进行自适应比特率流式处理。 第二个转换输出需要通过 [AudioAnalyzerPreset](analyze-video-audio-files-concept.md#built-in-presets) 处理输入视频中的音频信号。 创建转换后，可以提交一个作业来相应地处理视频。 由于在此示例中，我们指定了两个转换输出，因此必须指定两个作业输出。 可以选择将两个作业输出定向到相同的资产（如下所示），也可以将结果写入单独的资产。
