...
创建媒体服务帐户后，会将 **默认**[流式处理终结点](streaming-endpoint-concept.md)添加到帐户的 "**已停止**" 状态。 若要开始对内容进行流式处理并利用[动态打包](dynamic-packaging-overview.md)和[动态加密](content-protection-overview.md)功能，必须确保要从其流式获取内容的流式处理终结点处于“正在运行”状态。

本文介绍如何使用不同的技术在流式处理终结点上执行 " [启动](/rest/api/media/streamingendpoints/start) " 命令。 
...
* [开始 StreamingEndpoint](/rest/api/media/streamingendpoints/start)参考文档。
* 启动流式处理终结点是一个异步操作。 
...
有关详细信息，请参阅 [az ams 流式处理终结点启动](/cli/azure/ams/streaming-endpoint#az-ams-streaming-endpoint-start)。
