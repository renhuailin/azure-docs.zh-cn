...
ms.locfileid: "98797786"
---
# <a name="embed-video-indexer-widgets-in-your-apps"></a>将视频索引器小组件嵌入应用

本文介绍如何将视频索引器小组件嵌入应用。 视频索引器支持将三类小组件嵌入应用：认知见解、播放器和编辑器。
...
### <a name="private-content"></a>私有内容

若要嵌入私有视频，必须在 iframe 的 `src` 属性中传递访问令牌：

`https://www.videoindexer.ai/embed/[insights | player]/<accountId>/<videoId>/?accessToken=<accessToken>`
    
若要获取认知见解小组件内容，请使用以下方法之一：

- [获取见解小组件](https://api-portal.videoindexer.ai/docs/services/operations/operations/Get-Video-Insights-Widget?&pattern=widget) API。<br/>
- [获取视频访问令牌](https://api-portal.videoindexer.ai/docs/services/Operations/operations/Get-Video-Access-Token?)。 将其作为查询参数添加到 URL。 如前面所示，指定此 URL 作为 iframe 的 `src` 值。

若要在嵌入的小组件中提供编辑见解功能，必须传递包含编辑权限的访问令牌。 请在[获取见解小组件](https://api-portal.videoindexer.ai/docs/services/operations/operations/Get-Video-Insights-Widget?&pattern=widget)或[获取视频访问令牌](https://api-portal.videoindexer.ai/docs/services/operations/operations/Get-Video-Access-Token?)中使用 `&allowEdit=true`。
...
1. 可以在时间线中编辑脚本。
1. 视频索引器获取这些更新，并将它们保存在语言模型的[来自脚本编辑](customize-language-model-with-website.md#customize-language-models-by-correcting-transcripts)中。
1. 更新字幕：

    * 如果使用视频索引器的播放器小组件 - 它会自动更新。
...
本部分介绍如何在两个视频索引器小组件之间实现交互，这样当某个用户选择应用中的见解控件时，播放器就会跳到相关的时刻。
...
有关详细信息，请参阅[视频索引器 - 嵌入两个小组件演示](https://codepen.io/videoindexer/pen/NzJeOb)。
...
### <a name="embed-the-video-indexer-cognitive-insights-widget-and-use-a-different-video-player"></a>嵌入视频索引器认知见解小组件并使用其他视频播放器
...
如果将视频索引器见解嵌入到你自己的 [Azure Media Player](https://aka.ms/azuremediaplayer) 中，则可使用 `GetVttUrl` 方法获取隐藏式字幕。 也可从视频索引器 AMP 插件 `getSubtitlesUrl`（如前所示）调用 JavaScript 方法。
...
另请参阅[视频索引器 CodePen](https://codepen.io/videoindexer/pen/eGxebZ)。
