---
title: 对访问 Azure SignalR 服务的应用程序进行身份验证
description: 本文提供有关对使用 Azure Active Directory 访问 Azure SignalR 服务的应用程序进行身份验证的信息
author: terencefan
ms.author: tefa
ms.service: signalr
ms.topic: conceptual
...
---
# <a name="authenticate-an-application-with-azure-active-directory-to-access-azure-signalr-service"></a>对使用 Azure Active Directory 访问 Azure SignalR 服务的应用程序进行身份验证
...
将角色分配到 Azure AD 安全主体后，Azure 会向该安全主体授予对这些资源的访问权限。 访问权限可以局限到订阅、资源组或 Azure SignalR 资源的级别。 Azure AD 安全主体可向用户、组、应用程序服务主体或 [Azure 资源的托管标识](../active-directory/managed-identities-azure-resources/overview.md)分配角色。 
...
使用 Azure AD 授权访问 Azure SignalR 服务资源的第一步是，通过 [Azure 门户](https://portal.azure.com/)在 Azure AD 租户中注册应用程序。 注册应用程序时，需要向 AD 提供关于应用程序的信息。 Azure AD 随后会提供客户端 ID（也称为应用程序 ID）。在运行时，可以使用该 ID 将应用程序与 Azure AD 关联。 若要详细了解客户端 ID，请参阅 [Azure Active Directory 中的应用程序对象和服务主体对象](../active-directory/develop/app-objects-and-service-principals.md)。 
...
![上传认证](./media/authenticate/certification.png)
...
1. 在 [Azure 门户](https://portal.azure.com/)中导航到你的 SignalR 资源。
...
1. 在“添加角色分配”  页上，执行以下步骤：
    ...
    1. 分配有该角色的标识列出在该角色下。 例如，下图显示了应用程序 `signalr-dev` 和 `signalr-service` 在 SignalR 应用服务器角色中。 
        
        ![角色分配列表](./media/authenticate/role-assignment-list.png)

可以遵循类似的步骤来分配限定为资源组或订阅范围的角色。 定义角色及其范围后，可以使用[此 GitHub 位置](https://github.com/Azure/azure-event-hubs/tree/master/samples/DotNet/Microsoft.Azure.EventHubs/Rbac)提供的示例测试此行为。
...
或者只需在 `appsettings.json` 文件中配置 `ConnectionString`。
...
使用 `certificate` 时，请将 `clientSecret` 更改为 `clientCert`，如下所示：
...
请参阅以下相关文章：
- [对使用 Azure Active Directory 访问 Azure SignalR 服务的托管标识进行身份验证](authenticate-managed-identity.md)
