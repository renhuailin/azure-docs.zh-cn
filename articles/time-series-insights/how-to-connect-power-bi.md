...
## <a name="export-data-from-azure-time-series-insights-into-power-bi-desktop"></a>将 Azure 时序见解中的数据导出到 Power BI 桌面
...
1. 设置导出参数：
   ...
   * **存储类型**：选择是要对“暖存储”还是“冷存储”运行所选查询 。 如果已选择跨冷存储区的范围，则默认情况下，查询将路由到冷存储，因为温存储只包含最新的数据。 允许手动更改 storeType 参数，但不建议使用此参数来获得最佳体验。 

    > [!TIP] 
    > Azure 时序见解资源管理器将根据你选择导出的数据的搜索范围和视图自动选择建议的参数。 
