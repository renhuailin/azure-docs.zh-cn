...
- **通过自动复制确保映像可用性**
    ...
    将映像从实验室保存到共享映像库时，它会自动复制到[同一地域中的其他区域](https://azure.microsoft.com/global-infrastructure/regions/)。 如果某个区域发生中断，则将映像发布到实验室不受影响，因为可以使用来自另一个区域的映像副本。  从多个副本发布 VM 还可以帮助提高性能。
