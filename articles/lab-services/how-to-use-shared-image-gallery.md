...
4. 如果导出操作成功，则会看到以下消息：
    ...
    > [!IMPORTANT]
    > 在 Azure 实验室服务中[将实验室的模板映像保存](how-to-use-shared-image-gallery.md#save-an-image-to-the-shared-image-gallery)到共享映像库时，会将该映像作为专用映像上传到库。 [专用映像](../virtual-machines/shared-image-galleries.md#generalized-and-specialized-images)保留计算机特定的信息和用户配置文件。 仍可以直接将通用映像上传到 Azure 实验室服务外部的库。    
