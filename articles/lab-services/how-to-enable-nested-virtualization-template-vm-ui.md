...
7. 在向导的最后一页上，单击“完成”。
8. 显示“启动服务”对话框时，单击“启动服务”。
9. 等到服务启动。

## <a name="update-network-adapter-settings"></a>更新网络适配器设置

网络适配器将与用于之前创建的 NAT 网络的默认网关 IP 的 IP 关联。  在本示例中，我们将创建子网掩码为 255.255.255.0 的 IP 地址 192.168.0.1。  我们将使用之前创建的虚拟交换机。
...
6. 在“Internet 协议版本 4 (TCP/IPv4)属性”对话框中，选择“使用下面的 IP 地址”。  对于 IP 地址，请输入 192.168.0.1。 对于子网掩码，请输入 255.255.255.0。  将默认网关留空。  同时将 DNS 服务器留空。

    >[!NOTE]
    > NAT 网络的范围将是采用 CIDR 表示法的 192.168.0.0/24。  这会创建范围从 192.168.0.1 到 192.168.0.254 的可用 IP 地址。  按照约定，网关具有子网范围内的第一个 IP 地址。

7. 单击“确定”。

## <a name="create-dhcp-scope"></a>创建 DHCP 作用域

以下步骤说明如何添加 DHCP 作用域。  在本文中，NAT 网络是采用 CIDR 表示法的 192.168.0.0/24。  这会创建范围从 192.168.0.1 到 192.168.0.254 的可用 IP 地址。  创建的范围必须在可用地址的范围内，不包括之前创建的 IP 地址。

1. 打开“管理工具”并打开“DHCP”管理工具。
2. 在“DHCP”工具中，展开当前服务器的节点，然后选择“IPv4”。
3. 从“操作”菜单中，选择“新建作用域...”
4. 显示“新建作用域向导”时，单击“欢迎”页上的“下一步”。
...
9. 在“租用期限”页上，单击“下一步”。
10. 在“配置 DHCP 选项”页上，选择“是，我想现在配置这些选项”。 单击“下一步”。
11. 在“路由器(默认网关)”上
12. 添加 192.168.0.1（如果尚未添加）。 单击“下一步”。
13. 在“域名和 DNS 服务器”页上，将 168.63.129.16 添加为 DNS 服务器 IP 地址（如果尚未添加）。  168.63.129.16 是 Azure 静态 DNS 服务器的 IP 地址。 单击“下一步”。
14. 在“WINS 服务器”页上，单击“下一步”。
15. 在“激活作用域”页上，选择“是，我要立刻激活这个作用域”。  单击“下一步”。
16. 在“完成新建作用域向导”页上，单击“完成”。
