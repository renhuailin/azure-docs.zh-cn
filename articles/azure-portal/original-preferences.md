---
title: 管理 Azure 门户设置和首选项（较旧版本）
description: 你可以根据自己的偏好更改 Azure 门户的默认设置。 本文档介绍了较旧版本的设置体验。
ms.date: 06/17/2021
ms.topic: how-to
...
ms.locfileid: "112372170"
---
# <a name="manage-azure-portal-settings-and-preferences-older-version"></a>管理 Azure 门户设置和首选项（较旧版本）

你可以根据自己的偏好更改 Azure 门户的默认设置。 

> [!IMPORTANT]
> 我们正在将所有 Azure 用户移到更新的体验。 本主题介绍较旧的体验。 有关最新信息，请参阅[管理 Azure 门户设置和首选项](set-preferences.md)。

全局页眉中的“设置”菜单中提供了大多数设置。

![显示全局页头图标的屏幕截图，其中突出显示了“设置”](./media/original-preferences/header-settings.png)
...
:::image type="content" source="media/original-preferences/filter-subscription-default-view.png" alt-text="按订阅筛选资源列表。":::

1. 在全局页眉中选择“目录和订阅筛选器”图标。

1. 选择要在启动门户时用作默认订阅的订阅。 

    :::image type="content" source="media/original-preferences/default-directory-subscription-filter.png" alt-text="选择要在启动门户时用作默认订阅的订阅。"::: 
...
![显示 Azure 门户设置的屏幕截图，其中突出显示了默认视图](./media/original-preferences/default-view.png)
...
![屏幕截图，显示如何设置门户菜单的默认模式。](./media/original-preferences/menu-mode.png)
...
![显示 Azure 门户设置的屏幕截图，其中突出显示了“主题”](./media/original-preferences/theme.png)
...
![显示 Azure 门户设置的屏幕截图，其中突出显示了弹出式通知](./media/original-preferences/pop-up-notifications.png)

若要阅读在当前会话中收到的所有通知，请从全局页头中选择“通知”。

![显示 Azure 门户全局页头的屏幕截图，其中显示突出了“通知”](./media/original-preferences/read-notifications.png)
...
![显示门户设置的屏幕截图，其中突出显示了非活动超时设置](./media/original-preferences/inactive-sign-out-user.png)

更改将自动保存。 空闲持续时间达到设置的值后，将注销你的 Azure 门户会话。

如果你的管理员已经启用了非活动状态超时策略，你仍然可以设置自己的超时设置，只是它必须小于目录级别设置。 选中“替代目录非活动状态超时策略”，然后设置时间间隔。

![显示门户设置的屏幕截图，其中突出显示了“替代目录非活动超时策略”设置](./media/original-preferences/inactive-sign-out-override.png)

### <a name="change-the-directory-timeout-setting-admin"></a>更改目录超时设置（管理员）

具有[全局管理员角色](../active-directory/roles/permissions-reference.md#global-administrator)的管理员可以在会话注销之前强制执行最长空闲时间。非活动超时设置在目录级别应用。 设置对新会话生效。 它不会立即应用于任何已登录的用户。 有关目录的详细信息，请参阅 [Active Directory 域服务概述](/windows-server/identity/ad-ds/get-started/virtual-dc/active-directory-domain-services-overview)。

如果你是全局管理员，并且需要为 Azure 门户的所有用户强制执行空闲超时设置，请执行以下步骤：

1. 选择链接文本“配置目录级别超时”。

    ![显示门户设置的屏幕截图，其中突出显示了链接文本](./media/original-preferences/settings-admin.png)
...
1. 选择“应用”。
    ...
    ![屏幕截图，显示用于设置目录级别非活动超时的页面](./media/original-preferences/configure.png)

若要确认是否正确设置了非活动超时策略，请从全局页眉中选择“通知”。 验证是否列出了成功通知。

![屏幕截图，显示用于目录级别非活动超时的成功通知消息](./media/original-preferences/confirmation.png)

## <a name="restore-default-settings"></a>还原默认设置

如果你对 Azure 门户设置做了更改，但想要放弃这些更改，请选择“还原默认设置”。 对门户设置所做的所有更改将会丢失。 此选项不影响仪表板自定义。

![显示如何还原默认设置的屏幕截图](./media/original-preferences/useful-links-restore-defaults.png)
...
![显示如何导出设置的屏幕截图](./media/original-preferences/useful-links-export-settings.png)
...
![显示如何删除设置的屏幕截图](./media/original-preferences/useful-links-delete-settings.png)
...
选择“应用”以更新语言和区域格式设置。
   ...
   ![显示语言和区域格式设置的屏幕截图](./media/original-preferences/language.png)
