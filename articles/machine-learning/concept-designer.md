...
description: 了解什么是 Azure 机器学习设计器，以及可用它来执行什么任务。 拖放 UI 支持模型训练和部署。
...
Azure 机器学习设计器是一个拖放界面，用于在 Azure 机器学习中训练和部署模型。 本文介绍可在设计器中执行的任务。
 ...
 - 若要开始使用设计器，请参阅[教程：训练无代码回归模型](tutorial-designer-automobile-price-train-score.md)。 
 - 若要了解设计器中可用的组件，请参阅[算法和组件参考](./algorithm-module-reference/module-reference.md)。
...
使用可视化画布生成端到端机器学习工作流。 在设计器中训练、测试和部署模型：
...
[管道](concept-azure-machine-learning-architecture.md#ml-pipelines)包含所连接的数据集和算法模块。 管道有许多用途：可以创建训练单个模型的管道，或训练多个模型的管道。 可以创建实时或批量进行预测的管道，或者创建仅清理数据的管道。 借助管道，可以重复使用你的工作并组织你的项目。
...
模块可能提供一组参数用于配置模块的内部算法。 在画布上选择模块时，模块的参数会显示在画布右侧的“属性”窗格中。 可以在该窗格中修改参数来调整管道。 可以在设计器中设置各个模块的计算资源。 
