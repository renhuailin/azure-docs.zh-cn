...
> [!Note]
> 本部分的过程需要定价层 S0 或 S1 中的 Azure Maps 帐户。
...
> [!Note]
> 本部分的过程需要定价层 S1 中的 Azure Maps 帐户。
...
6. 若要获取 udId，请在 Postman 应用中打开一个新选项卡。 在生成器选项卡上选择 GET HTTP 方法。通过状态 URI 发出 GET 请求。 如果数据上传成功，将在响应正文中收到一个 udId。 复制 udId。
