...
5. 设置邮件服务器参数，然后选择“复制”。 传感器树中的每个项旁边都显示了一个复选框。
...
6. 在“传感器引擎配置”部分标记你要更新的传感器，然后选择“自动更新” 。
...
8. 在“传感器版本升级”窗格中选择 :::image type="icon" source="media/how-to-manage-sensors-from-the-on-premises-management-console/plus-icon.png" border="false":::。

   :::image type="content" source="media/how-to-manage-sensors-from-the-on-premises-management-console/display-files.png" alt-text="显示文件的“传感器版本升级”屏幕。":::
...
10. 在更新过程中，每个传感器的更新状态将显示在“站点管理”窗口中。
...
5. 在“传感器引擎配置”部分，选择应接收更新的传感器。  

6. 在“选择威胁情报数据”部分，选择加号图标 ( **+** )。 

7. 上传从 Defender for IoT 的“更新”页下载的包。
...
2. 在“传感器引擎配置”部分，选择引擎对应的“启用”或“禁用”  。
...
3. 选择日历间隔、日期和时区。 时间格式基于 24 小时制。 例如，输入 6:00 PM 会显示为 18:00。 

4. 在“备份存储分配”字段中，输入要为备份分配的存储。 如果超过最大空间，你将收到通知。

5. 在“保留最后一个”字段中，指定你要为每个传感器保留的备份数。 超过此限制时，将删除最旧的备份。  
...
3. 在 Defender for IoT 中，为备份创建目录： 
...
8. 在 Defender for IoT 传感器上的共享文件夹中配置一个备份目录：  
