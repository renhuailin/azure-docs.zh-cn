...
如果用户在 30 分钟内没有使用控制台鼠标或键盘，将强制注销会话。
...
## <a name="resetting-passwords"></a>重置密码

### <a name="cyberx-or-support-user"></a>CyberX 或 Support 用户

只有 CyberX 和 Support 用户才能访问密码恢复功能  。 如果 CyberX 或 Support 用户忘记了密码，则可以通过 Defender for IoT 登录页上的“密码恢复”选项重置密码  。

重置 CyberX 或 Support 用户的密码：
...
1. 输入在“密码恢复”屏幕上收到的唯一标识符，然后选择“恢复” 。 下载 `password_recovery.zip` 文件。
...
### <a name="administrator-security-analyst-and-read-only-user"></a>管理员、安全分析师和只读用户

只读用户和安全分析师无法重置自己的密码，需要与 Administrator、Support 或 CyberX 角色的用户联系才能重置密码。 Administrator 用户必须联系 CyberX 或 Support 用户以重置密码 。

重置传感器上的用户密码：

1. Administrator、Support 或 CyberX 角色用户应登录传感器。
