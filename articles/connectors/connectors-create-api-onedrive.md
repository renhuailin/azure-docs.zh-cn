...
description: 通过在 Azure 逻辑应用中创建自动化工作流，在 OneDrive 中上传和管理文件
services: logic-apps
...
1. 在逻辑应用设计器中，键入 `onedrive` 获取触发器列表：  

   ![标题为“显示 Microsoft 托管的 API”的对话框有一个包含“onedrive”的框。 下面是四个触发器的列表。 其中第一个是“OneDrive - 创建文件时”。 第二个“OneDrive - 修改文件时”已被选中。](./media/connectors-create-api-onedrive/onedrive-1.png)
...
3. 选择“编辑”按钮并设置“频率”和“间隔”值。 例如，如果希望触发器每 15 分钟轮询一次，将“频率”设置为“分钟”，将“间隔”设置为“15”。 
   ...
   ![标题为“修改文件时”的对话框显示了五个分别标记为“文件夹”、“频率”、“间隔”、“时区”和“开始时间”的框。 “频率”和“时区”字段有下拉列表。](./media/connectors-create-api-onedrive/trigger-properties.png)
...
1. 选择加号。 可看到多个选项：“添加操作”、“添加条件”或“更多”选项之一。
   ...
   ![屏幕截图显示了四个按钮：“+ 新建步骤”、“添加操作”、“添加条件”和“...更多”。](./media/connectors-create-api-onedrive/add-action.png)

2. 选择“添加操作”。

3. 在搜索框中，键入 `onedrive` 获取所有可用操作的列表。

   ![标题为“显示 Microsoft 托管的 API”的对话框有一个包含“onedrive”的框。 下面是八个操作的列表。 第一个是“OneDrive - 创建文件”，并且已选中。](./media/connectors-create-api-onedrive/onedrive-actions.png) 

4. 在我们的示例中，选择“OneDrive - 创建文件”。 如果连接已存在，选择“文件夹路径”放入文件、输入“文件名”，并选择所需的“文件内容”：  

   ![标题为“创建文件”的对话框显示了三个标记为“文件夹路径”、“文件名”和“文件夹内容”的框。 “文件夹路径”框旁边有一个目录浏览按钮。](./media/connectors-create-api-onedrive/sample-action.png)

   如果提示提供连接信息，则输入详细信息以按照本主题中所述[创建连接](#create-the-connection)。

   在此示例中，你在 OneDrive 文件夹中新建文件。 可使用来自其他触发器的输出创建 OneDrive 文件。 例如，添加 Office 365 Outlook“新电子邮件到达时”触发器。 然后添加可在 ForEach 内使用“附件”和“内容类型”字段的 OneDrive“创建文件”操作，在 OneDrive 中新建文件。

   ![标题为“For each”的对话框有一个标记为“选择前面步骤中的输出”的框，其中包含“附件”。 有一个覆盖了“For each”框的剩余部分的“创建文件”对话框，其中包含标记为“文件夹路径”、“文件名”和“文件内容”的框。 ](./media/connectors-create-api-onedrive/foreach-action.png)
