...
唯一的先决条件是 [Microsoft .NET Framework 4.8](https://aka.ms/net-framework-download)。
...
* **方法**：在此模式下，定义 `GetBalance` 用于描述屏幕导航路径的方法，例如。 您还可以在每个屏幕上选择将成为该方法的输入和输出参数的字段。
...
1. 如果尚未打开 3270 设计工具，请将其打开。 在工具栏上，选择 " **捕获** " 以便进入捕获模式。

1. 在“会话”菜单中，选择“连接”。

1. 若要开始录制，请从 " **录制** " 菜单中选择 " **开始记录**"。  (键盘： Ctrl + E) 
...
1. 若要停止记录，请从 " **录制** " 菜单中选择 " **停止录制**"。  (键盘： Ctrl + Shift + E) 
...
1. 若要针对实时宿主运行方法，同时仍处于方法模式，请按 F5 键，或从设计工具的工具栏中选择 " **测试**"。
...
1. 在3270设计工具的 " **工具** " 菜单中，选择 " **生成定义**"。  (键盘： F6) 
...
1. 在搜索框中，选择 " **企业**"。 在搜索框中，输入 `3270` 作为筛选器。 从 "操作" 列表中，选择名为 "**通过 TN3270 连接运行大型机程序**" 的操作
