...
唯一的必备组件是 [Microsoft .NET Framework 4.8](https://aka.ms/net-framework-download)。
...
1. 若要开始录制，在“录制”菜单中选择“开始录制”。  （键盘：Ctrl + E）
...
1. 若要停止录制，在“录制”菜单中选择“停止录制”。  （键盘：Ctrl + Shift + E）
...
1. 添加方法的所有参数后，请定义每个参数的以下属性：
   ...
   | **字段填充方法** | 参数支持以下填充类型，必要时留空： <p><p>- “类型”：在字段中按顺序输入字符。 <p>- “填充”：将字段内容替换为字符，必要时留空。 <p>- “EraseEofType”：清除字段，然后在字段中按顺序输入字符。 |
   | **格式字符串** | 某些参数数据类型使用格式字符串，告知 3270 连接器如何将屏幕中的文本转换为 .NET 数据类型： <p><p>- “日期时间”：日期时间格式字符串遵循 [.NET 自定义日期和时间格式字符串](/dotnet/standard/base-types/custom-date-and-time-format-strings)。 例如，日期 `06/30/2019` 使用格式字符串 `MM/dd/yyyy`。 <p>- “小数”：小数格式字符串使用 [COBOL Picture 子句](https://www.ibm.com/support/knowledgecenter/ssw_ibm_i_73/rzasb/picture.htm)。 例如，数字 `100.35` 使用格式字符串 `999V99`。 |
   |||
...
1. 在要添加操作的最后一个步骤下，选择“新建步骤” **>** “添加操作”。  

1. 在搜索框下，选择“Enterprise”。 在搜索框中，输入 `3270` 作为筛选器。 从操作列表中，选择名为“通过 TN3270 连接运行大型机程序”的操作
