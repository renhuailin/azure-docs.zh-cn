...
description: 有关如何使用连接器通过 Azure 逻辑应用生成自动化工作流的概述。 了解不同触发器、操作和连接器的工作原理。
services: logic-apps
...
ms.locfileid: "110377032"
---
# <a name="connectors-for-azure-logic-apps"></a>适用于 Azure 逻辑应用的连接器

在 Azure 逻辑应用中，连接器可帮助你快速访问来自其他应用、服务、系统、协议和平台的数据、事件和其他资源。 使用连接器时，可生成支持跨云、本地和混合环境使用、处理和集成信息且通常无需编写任何代码的逻辑应用工作流。

你可从数百个连接器中选择要在工作流中使用的连接器。 因此，本文档重点介绍逻辑应用的一些热门和常用的连接器。 有关跨逻辑应用、Microsoft Power Automate 和 Microsoft Power Apps 使用的连接器的完整信息，请查看[连接器文档](/connectors)。 有关定价信息，请参阅[逻辑应用定价模型](../logic-apps/logic-apps-pricing.md)和[逻辑应用定价详细信息](https://azure.microsoft.com/pricing/details/logic-apps/)。

> [!NOTE]
> 要将工作流与没有连接器的服务或 API 集成，可通过 HTTP 等协议调用该服务，或[创建自定义连接器](#custom-apis-and-connectors)。

## <a name="what-are-connectors"></a>什么是连接器？

连接器可提供触发器和操作，用于执行逻辑应用工作流中的任务 。 每个触发器和操作都有可配置的属性。 某些触发器和操作要求[创建和配置连接](#connection-configuration)，以便工作流可以访问特定的服务或系统。

### <a name="triggers"></a>触发器

触发器指定用于启动工作流的事件，始终是所有工作流中的第一步。 每个触发器还遵循可控制触发器如何监视和响应事件的特定触发模式。 通常，触发器遵循“轮询”模式或“推送”模式，但有时触发器会同时提供这两个版本 。

- 轮询触发器按指定计划定期查看特定服务或系统，以检查新数据或特定事件。 如果有新数据可用或发生特定事件，这些触发器将创建并运行工作流的新实例。 然后，这个新实例再使用这些作为输入传递的数据。
- 推送触发器侦听新数据或即将发生的事件，而非轮询。 有新数据可用或事件发生时，这些触发器将创建并运行工作流的新实例。 然后，这个新实例再使用这些作为输入传递的数据。

例如，建议生成一个在文件上传到 FTP 服务器时执行操作的工作流。 作为工作流的第一个步骤，你可使用名为“添加或修改文件时”的 FTP 触发器，该触发器遵循轮询模式。 然后，可指定一个计划来定期检查上传事件。

触发器还会将任何输入和其他所需数据传递到工作流中，之后的操作可以在整个工作流中引用并使用这些数据。 例如，假设你想使用名为“收到新电子邮件时”的 Office 365 Outlook 触发器，在收到新电子邮件时启动工作流。 你可将此触发器配置为传递来自每封新电子邮件的内容，例如发件人、主题行、正文、附件等。 然后，工作流可使用其他操作来处理此信息。

### <a name="actions"></a>操作

操作是紧跟在触发器之后的操作，会在工作流中执行某种类型的任务。 可在工作流中使用多个操作。 例如，可使用检测 SQL 数据库中的新客户数据的 SQL 触发器启动工作流。 触发器触发后，工作流有一个可获取客户数据的 SQL 操作。 执行 SQL 操作后，工作流还有一个处理数据的操作（不一定是 SQL）。
...
[托管连接器](managed.md)由 Microsoft 部署、托管和管理。 这些连接器提供用于云服务和/或本地系统的触发器和操作。 托管连接器有以下类别：
...
### <a name="connection-security-and-encryption"></a>连接安全性和加密
...
只要目标服务或系统允许，建立的连接就可以访问该服务或系统。 对于使用 Azure AD OAuth 连接的服务（例如 Office 365 和 Dynamics），逻辑应用服务会无限期地刷新访问令牌。 其他服务可能会限制逻辑应用无需刷新即可使用令牌的时长。 某些操作（例如更改密码）会使所有访问令牌失效。

尽管是从工作流中创建连接，但连接是单独的 Azure资源，具有其自己的资源定义。 若要查看这些连接资源定义，请[将逻辑应用从 Azure 下载到 Visual Studio 中](../logic-apps/manage-logic-apps-with-visual-studio.md)。 要创建基本可以部署的有效参数化逻辑应用模板，这是最简单的方法。
...
如果使用限制流量的防火墙，并且逻辑应用工作流需要通过该防火墙进行通信，则必须设置防火墙，以允许访问逻辑应用工作流所在 Azure 区域中逻辑应用服务或运行时使用的[入站](../logic-apps/logic-apps-limits-and-config.md#inbound)和[出站](../logic-apps/logic-apps-limits-and-config.md#outbound) IP 地址。 如果工作流还使用托管连接器（例如 Office 365 Outlook 连接器或 SQL 连接器），或使用自定义连接器，则防火墙还需要允许访问逻辑应用所在 Azure 区域中的所有[托管连接器出站 IP 地址](../logic-apps/logic-apps-limits-and-config.md#outbound)。 有关详细信息，请查看[防火墙配置](../logic-apps/logic-apps-limits-and-config.md#firewall-configuration-ip-addresses-and-service-tags)。
...
定期内置触发器遵循你设置的计划，包括指定的任何时区。 但如果某个定期触发未指定任何其他高级计划选项（例如具体时间）来运行将来的定期触发，则将来的定期触发会遵照上一次的触发器执行。 因此，这些定期触发的开始时间可能会因存储调用期间的延迟等因素而发生偏移。
...
在基于连接的定期触发器（如 Office 365 Outlook）中，计划不是控制执行的唯一驱动因素。 时区仅确定初始开始时间。 后续运行取决于定期计划、上一次触发器执行以及其他可能导致运行时间发生偏差或产生意外行为的因素，例如：
...
* 若要使用“定期”触发器，请指定时区、开始日期和时间。 另外，还应在属性“在这些小时”和“在这些分钟”中（仅适用于“天”和“周”频率）指定运行后续定期触发的具体时间   。 但是，当时间发生调整时，有时 Windows 可能仍会导致问题。

* 请考虑使用[“滑动窗口”触发器](connectors-native-sliding-window.md)而不是“定期”触发器，以免错过定期触发 。
