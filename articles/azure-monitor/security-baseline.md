...
ms.locfileid: "101725611"
---
# <a name="azure-security-baseline-for-azure-monitor"></a>适用于 Azure Monitor 的 Azure 安全基线

此安全基线将 [Azure 安全基准版本 1.0](../security/benchmarks/overview-v1.md) 中的指南应用到 Azure Monitor。 Azure 安全基准提供有关如何在 Azure 上保护云解决方案的建议。
内容按“安全控件”分组，此类控件按适用于 Azure Monitor 的 Azure 安全基准和相关的指南定义。 排除了不适用于 Azure Monitor 的“控件”。 

 
若要查看 Azure Monitor 如何完全映射到 Azure 安全基准，请参阅[完整的 Azure Monitor 安全基线映射文件](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)。
...
若要允许流量到达 Azure Monitor，请使用“AzureMonitor”服务标记来允许入站和出站流量通过网络安全组。 若要允许“可用性监视”测试流量到达 Azure Monitor，请使用“ApplicationInsightsAvailability”服务标记来允许所有入站流量通过网络安全组。 若要允许警报通知到达客户终结点，请使用 "操作组" 服务标记允许通过网络安全组的入站流量。
...
- [如何为 Azure Monitor 设置专用链接](/azure/azure-monitor/platform/private-link-security)

- [在 Azure Monitor 中使用 Log Analytics 网关连接无法访问 Internet 的计算机](/azure/azure-monitor/platform/gateway)
...
- [针对 Azure Monitor 代理的网络要求](/azure/azure-monitor/platform/log-analytics-agent#network-requirements)
...
**Azure 策略内置定义-microsoft insights**：
...
**指南**：启用 Azure Active Directory (Azure AD) 多重身份验证，并遵循 Azure 安全中心的标识和访问建议。
...
**指南**：你有权访问 Azure Active Directory (Azure AD) 登录活动、审核和风险事件日志源，这允许你与任何 SIEM/监视工具集成。 可以通过创建 Azure AD 用户帐户的诊断设置并将审核日志和登录日志发送到 Log Analytics 工作区来简化此过程。 你可以在 Log Analytics 工作区中配置所需的警报。
...
- [在 Azure Monitor 中管理对日志数据和工作区的访问](/azure/azure-monitor/platform/manage-access)
...
**指导**：使用单独的订阅和管理组对各个安全域（如环境类型和数据敏感度级别）实现隔离。 你可以限制对应用程序和企业环境所需的 Azure Monitor 和相关资源的访问级别。 可以 (Azure RBAC) ，通过 Azure 基于角色的访问控制来控制对 Azure Monitor 的访问。
...
- [如何使用 TLS 1.2 安全地发送数据](/azure/azure-monitor/platform/data-security#sending-data-securely-using-tls-12)
...
- [Azure Monitor 中的角色、权限和安全性](/azure/azure-monitor/platform/roles-permissions-security)
...
- [Azure Monitor 客户管理的密钥](/azure/azure-monitor/platform/customer-managed-keys)
...
- [删除 Azure Log Analytics 工作区](/azure/azure-monitor/platform/delete-workspace)
...
- [保护 APM 实时指标流](https://docs.microsoft.com/azure/azure-monitor/app/live-stream#secure-the-control-channel)
...
- [Azure Policy 别名](https://docs.microsoft.com/azure/governance/policy/concepts/definition-structure#aliases)
...
- [如何使用托管标识提供 Key Vault 身份验证](/azure/key-vault/general/assign-access=policy-portal)
...
**指南**：使用托管标识，通过 Azure Active Directory (Azure AD) 中的自动托管标识提供 Azure 服务。 使用托管标识可以向支持 Azure AD 身份验证的任何服务（包括 Azure Monitor 资源）进行身份验证，无需在代码中放入任何凭据。
...
- [使用 Azure 资源管理器模板管理 Log Analytics 工作区](/azure/azure-monitor/samples/resource-manager-workspace)
...
**指南**：使用 Azure DevOps 安全地存储和管理代码，如自定义 Azure 策略、Azure 资源管理器模板。 若要保护在 Azure DevOps 中管理的资源，可以向特定用户、内置安全组或 Azure Active Directory (Azure AD)（如果与 Azure DevOps 集成）中定义的组或 Active Directory（如果与 TFS 集成）授予或拒绝授予权限。  使用 Azure 基于角色的访问控制 (Azure RBAC) 保护客户管理的密钥。
...
- [如何在密钥保管库中启用软删除和清除保护](../storage/blobs/soft-delete-blob-overview.md)
