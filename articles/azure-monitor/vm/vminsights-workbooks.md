...
## <a name="vm-insights-workbooks"></a>VM 见解工作簿
VM 见解包括以下工作簿。 你可以使用这些工作簿，也可以基于这些工作簿开始创建自定义工作簿，以满足你的特定需求。

### <a name="single-virtual-machine"></a>单个虚拟机

| 工作簿 | 说明 |
|----------|-------------|
| 性能 | 提供性能视图的可自定义版本，该版本利用所有已启用的 Log Analytics 性能计数器。 | 
| 连接 | “连接”提供 VM 建立的入站和出站连接的深入视图。 | 

### <a name="multiple-virtual-machines"></a>多台虚拟机

| 工作簿 | 说明 |
|----------|-------------|
| 性能 | 在单个工作簿中提供“前 N 项列表”和“图表”视图的可自定义版本，该版本利用所有已启用的 Log Analytics 性能计数器。|
| 性能计数器 | 基于众多性能计数器的“前 N 项”图表视图。 |
| 连接 | “连接”提供受监视 VM 建立的入站和出站连接的深入视图。 |
...
| 安全和审核 | TCP/IP 流量的分析，其中报告了连接总数、恶意连接数，以及 IP 终结点的全球位置。  若要启用所有功能，需要启用安全检测。 |
| TCP 流量 | 受监视 VM 的排名报告，在网格中以趋势线的形式显示它们已发送、接收的网络流量和总量。 |
| 流量比较 | 在此工作簿中可以比较一台计算机或一组计算机的网络流量趋势。 |
...
文本参数允许在任意位置使用文本框中的值。 这个值可能是表名、列名、函数名、运算符名等。文本参数类型包含“从分析查询获取默认值”设置，使工作簿作者能够使用查询填充该文本框的默认值。
