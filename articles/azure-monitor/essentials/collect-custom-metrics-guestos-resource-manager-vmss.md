...
使用 Azure Monitor [Windows Azure 诊断 (WAD) 扩展](../agents/diagnostics-extension-overview.md)，可以从作为虚拟机、云服务或 Azure Service Fabric 群集的一部分运行的来宾操作系统（来宾 OS）收集指标和日志。 该扩展可将遥测数据发送到之前链接的文章中列出的许多不同位置。  
