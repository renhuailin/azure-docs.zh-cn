...
> [!TIP]
> 最佳做法是使用并配置 [Azure 诊断扩展](../agents/diagnostics-extension-overview.md)，将来宾 OS 性能指标发送到存储平台指标的同一个 Azure Monitor 指标数据库中。 扩展通过[自定义指标](../essentials/metrics-custom-overview.md) API 路由来宾 OS 指标。 然后，你可以像使用平台指标那样，绘制图表、发送警报以及以其他方式使用来宾 OS 指标。 或者，还可以使用 Log Analytics 代理将来宾 OS 指标发送到 Azure Monitor 日志/Log Analytics。 可以在该位置结合非指标数据查询这些指标。 
...
|CommandPoolJobQueueLength|是|命令池作业队列长度|Count|平均值|命令线程池队列中的作业数。|ServerResourceType|
...
|ProcessingPoolBusyIOJobThreads|是|线程:处理池繁忙 I/O 作业线程数|Count|平均值|处理线程池中正在运行 I/O 作业的线程数。|ServerResourceType|
...
|qpu_metric|是|QPU|Count|平均值|QPU。 S1 范围为 0-100，S2 范围为 0-200，S4 范围为 0-400|ServerResourceType|
...
|ShortParsingJobQueueLength|是|线程:短分析作业队列长度|Count|平均值|短分析线程池队列中的作业数。|ServerResourceType|
...
|TotalUpdateDeploymentMachineRuns|是|更新部署计算机运行总数|Count|总计|软件更新部署运行中的软件更新部署计算机运行总数|SoftwareUpdateConfigurationName、Status、TargetComputer、SoftwareUpdateConfigurationRunId|
...
|IdleNodeCount|否|空闲节点计数|Count|总计|空闲节点数目|无维度|
...
|JobTerminateStartEvent|是|作业终止启动事件数|Count|总计|已请求终止的作业总数。|jobId|
...
|PoolDeleteStartEvent|是|池删除启动事件数|Count|总计|已启动的池删除活动总数|poolId|
...
|TotalNodeCount|否|专用节点计数|Count|总计|批处理帐户中的专用节点总数|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|活动核心数|是|活动核心数|Count|平均值|活动核心数|Scenario, ClusterName|
|活动节点数|是|活动节点数|Count|平均值|正在运行的节点数目|Scenario, ClusterName|
|空闲核心数|是|空闲核心数|Count|平均值|空闲核心数|Scenario, ClusterName|
|空闲节点数|是|空闲节点数|Count|平均值|空闲节点数目|Scenario, ClusterName|
|已完成的作业|是|已完成的作业|Count|总计|已完成的作业数|Scenario, ClusterName, ResultType|
|提交的作业|是|提交的作业|Count|总计|提交的作业数|Scenario, ClusterName|
|正在退出的核心数|是|正在退出的核心数|Count|平均值|正在退出的核心数|Scenario, ClusterName|
|正在退出的节点数|是|正在退出的节点数|Count|平均值|正在退出的节点数|Scenario, ClusterName|
|占用的核心数|是|占用的核心数|Count|平均值|占用的核心数|Scenario, ClusterName|
|占用的节点数|是|占用的节点数|Count|平均值|已占用节点数|Scenario, ClusterName|
|配额利用率百分比|是|配额利用率百分比|Count|平均值|已利用的配额百分比|Scenario, ClusterName, VmFamilyName, VmPriority|
|核心总数|是|核心总数|Count|平均值|核心总数|Scenario, ClusterName|
|节点总数|是|节点总数|Count|平均值|节点总数|Scenario, ClusterName|
|不可用的核心数|是|不可用的核心数|Count|平均值|不可用的核心数|Scenario, ClusterName|
|不可用的节点数|是|不可用的节点数|Count|平均值|不可用的节点数目|Scenario, ClusterName|

## <a name="microsoftbingaccounts"></a>microsoft.bing/accounts

|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|BlockedCalls|是|阻止的调用数|Count|总计|超过速率或配额限制的调用数|ApiName、ServingRegion、StatusCode|
|ClientErrors|是|客户端错误数|Count|总计|引发任何客户端错误（HTTP 状态代码 4xx）的调用数|ApiName、ServingRegion、StatusCode|
...
|ServerErrors|是|服务器错误数|Count|总计|引发任何服务器错误（HTTP 状态代码 5xx）的调用数|ApiName、ServingRegion、StatusCode|
|SuccessfulCalls|是|成功调用数|Count|总计|成功调用数（HTTP 状态代码 2xx）|ApiName、ServingRegion、StatusCode|
|TotalCalls|是|总调用数|Count|总计|调用总数|ApiName、ServingRegion、StatusCode|
|TotalErrors|是|错误总数|Count|总计|引发任何错误（HTTP 状态代码 4xx 或 5xx）的调用数|ApiName、ServingRegion、StatusCode|
...
|QueuedTransactions|是|已排队的事务数|Count|平均值|已排队的事务数|节点|
...
|cachehits5|是|缓存命中数(分片 5)|Count|总计||无维度|
...
|cachemisses3|是|缓存未命中数(分片 3)|Count|总计||无维度|
...
|cachemisses8|是|缓存未命中数(分片 8)|Count|总计||无维度|
...
|connectedclients|是|连接的客户端数|Count|最大值||ShardId|
...
|connectedclients9|是|连接的客户端数(分片 9)|Count|最大值||无维度|
...
|evictedkeys2|是|逐出的密钥数(分片 2)|Count|总计||无维度|
...
|expiredkeys1|是|过期的密钥数(分片 1)|Count|总计||无维度|
...
|expiredkeys6|是|过期的密钥数(分片 6)|Count|总计||无维度|
...
|getcommands5|是|Get 数(分片 5)|Count|总计||无维度|
...
|operationsPerSecond4|是|每秒操作数（分片 4）|Count|最大值||无维度|
...
|operationsPerSecond9|是|每秒操作数（分片 9）|Count|最大值||无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|cachehits|是|缓存命中数|Count|总计||无维度|
|cacheLatency|是|缓存延迟毫秒数（预览）|Count|平均值||InstanceId|
|cachemisses|是|缓存未命中数|Count|总计||InstanceId|
|cacheRead|是|缓存读取量|每秒字节数|最大值||InstanceId|
|cacheWrite|是|缓存写入量|每秒字节数|最大值||InstanceId|
|connectedclients|是|连接的客户端数|Count|最大值||InstanceId|
|错误|是|错误|Count|最大值||InstanceId, ErrorType|
|evictedkeys|是|逐出的密钥数|Count|总计||无维度|
|expiredkeys|是|过期的密钥数|Count|总计||无维度|
|getcommands|是|获取数|Count|总计||无维度|
|operationsPerSecond|是|每秒操作数|Count|最大值||无维度|
|percentProcessorTime|是|CPU|百分比|最大值||InstanceId|
|serverLoad|是|服务器负载|百分比|最大值||无维度|
|setcommands|是|设置数|Count|总计||无维度|
|totalcommandsprocessed|是|总操作数|Count|总计||无维度|
|totalkeys|是|总密钥数|Count|最大值||无维度|
...
|RequestCount|是|请求计数|Count|总计|HTTP/S 代理提供客户端请求数|HttpStatus, HttpStatusGroup, ClientRegion, ClientCountry, Endpoint|
...
|ContainerCount|是|Blob 容器计数|Count|平均值|存储帐户的 Blob 服务中的容器数。|无维度|
|流出量|是|流出量|字节|总计|流出的数据量（以字节为单位）。 此数字包括从外部客户端到 Azure 存储流出的数据量，以及流出 Azure 中的数据量。 因此，此数字不反映计费的流出量。|GeoType, ApiName, Authentication|
...
|FileShareCount|否|文件共享计数|Count|平均值|存储帐户的文件服务中的文件共享数。|无维度|
...
|QueueMessageCount|是|队列消息计数|Count|平均值|存储帐户的队列服务中的队列消息的大致数目。|无维度|
|SuccessE2ELatency|是|成功 E2E 延迟|毫秒|平均值|向存储服务或指定的 API 操作发出的成功请求的端到端延迟（以毫秒为单位）。 此值包括在 Azure 存储中读取请求、发送响应和接收响应确认所需的处理时间。|GeoType, ApiName, Authentication|
|SuccessServerLatency|是|成功服务器延迟|毫秒|平均值|由 Azure 存储用于处理成功请求的延迟（以毫秒为单位）。 此值不包括 SuccessE2ELatency 中指定的网络延迟。|GeoType, ApiName, Authentication|
|事务|是|事务|Count|总计|向存储服务或指定的 API 操作发出的请求数。 此数值包括成功和失败的请求数，以及引发错误的请求数。 针对不同类型的响应数使用 ResponseType 维度。|ResponseType, GeoType, ApiName, Authentication|
...
|TableEntityCount|是|表实体计数|Count|平均值|存储帐户的表服务中的表实体数。|无维度|
|事务|是|事务|Count|总计|向存储服务或指定的 API 操作发出的请求数。 此数值包括成功和失败的请求数，以及引发错误的请求数。 针对不同类型的响应数使用 ResponseType 维度。|ResponseType, GeoType, ApiName, Authentication|
...
|MatchedRewards|是|匹配的奖励数|计数|总计| 匹配的奖励数。|IsMatchBaseline, Mode, RunId|
|ObservedRewards|是|观察到的奖励数|计数|总计|观察到的奖励数。|IsMatchBaseline, Mode, RunId|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|复合磁盘读取字节数/秒|否|磁盘读取字节数/秒（预览版）|字节|平均值|在监视期间每秒从磁盘读取的字节数，请注意，此指标处于预览版阶段，在正式发布之前可能会发生更改||
|复合磁盘读取操作数/秒|否|磁盘读取操作数/秒（预览版）|字节|平均值|监视期间在磁盘上执行的读取 IO 数，请注意，此指标处于预览版阶段，在正式发布之前可能会发生更改||
|复合磁盘写入字节数/秒|否|磁盘写入字节数/秒（预览版）|字节|平均值|在监视期间每秒写入磁盘的字节数，请注意，此指标处于预览版阶段，在正式发布之前可能会发生更改||
|复合磁盘写入操作数/秒|否|磁盘写入操作数/秒（预览版）|字节|平均值|监视期间在磁盘上执行的写入 IO 数，请注意，此指标处于预览版阶段，在正式发布之前可能会发生更改||
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|已用 CPU 信用额度|是|已用 CPU 信用额度|Count|平均值|虚拟机使用的总信用点数。 仅在 B 系列可突发 VM 上可用|无维度|
|剩余 CPU 信用额度|是|剩余 CPU 信用额度|Count|平均值|可用于突发的总信用点数。 仅在 B 系列可突发 VM 上可用|无维度|
...
|cluster_autoscaler_cluster_safe_to_autoscale|否|群集运行状况|Count|平均值|确定群集自动缩放程序是否会对群集执行操作||
|cluster_autoscaler_scale_down_in_cooldown|否|纵向缩减散热设备|Count|平均值|确定是否在散热设备中进行纵向缩减 - 在此时间范围内不会删除任何节点||
|cluster_autoscaler_unneeded_nodes_count|否|不需要的节点|Count|平均值|群集自动缩放程序将这些节点标记为删除候选项，最终会将其删除||
|cluster_autoscaler_unschedulable_pods_count|否|不可计划的 Pod|Count|平均值|群集中当前不可计划的 Pod 数||
|kube_node_status_allocatable_cpu_cores|否|托管群集中可用 CPU 内核的总数|Count|平均值|托管群集中可用 CPU 内核的总数||
...
|kube_pod_status_ready|否|就绪状态下的 Pod 数|Count|平均值|就绪状态下的 Pod 数|命名空间、Pod、条件|
...
|TriggerFailedRuns|是|失败的触发器运行数指标|Count|总计||Name、FailureType|
|TriggerSucceededRuns|是|成功的触发器运行数指标|Count|总计||Name、FailureType|
...
|JobEndedCancelled|是|取消的作业数|Count|总计|取消的作业计数。|无维度|
|JobEndedFailure|是|失败作业数|Count|总计|失败作业计数。|无维度|
|JobEndedSuccess|是|成功作业数|Count|总计|成功作业计数。|无维度|
...
|active_connections|是|活动连接数|Count|最大值|活动连接数|无维度|
...
|cpu_credits_consumed|是|已用 CPU 信用额度|Count|平均值|数据库服务器使用的总信用点数|无维度|
|cpu_credits_remaining|是|剩余 CPU 信用额度|Count|平均值|可用来集中使用的总信用点数|无维度|
|cpu_percent|是|CPU 百分比|百分比|平均值|CPU 百分比|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|active_connections|是|活动连接数|Count|平均值|活动连接数|ServerName|
|cpu_percent|是|CPU 百分比|百分比|平均值|CPU 百分比|ServerName|
|iops|是|IOPS|Count|平均值|每秒 IO 操作次数|ServerName|
...
|c2d.commands.egress.complete.success|是|已完成的 C2D 消息传递次数|Count|总计|设备成功完成的云到设备消息传递次数|无维度|
|c2d.commands.egress.reject.success|是|拒绝的 C2D 消息数|Count|总计|设备拒绝的云到设备消息数|无维度|
...
|配置|是|配置指标|Count|总计|配置操作的指标|无维度|
...
|d2c.endpoints.egress.serviceBusTopics|是|路由：消息传送到服务总线主题的次数|Count|总计|IoT 中心路由成功将消息传送到服务总线主题终结点的次数。|无维度|
|d2c.endpoints.egress.storage|是|路由：消息传送到存储的次数|Count|总计|IoT 中心路由成功将消息传送到存储终结点的次数。|无维度|
...
|d2c.telemetry.egress.orphaned|是|路由：遥测消息孤立次数 |Count|总计|消息由于与任何路由规则（包括回退规则）都不匹配而被 IoT 中心路由孤立的次数。 |无维度|
...
|d2c.telemetry.ingress.success|是|发送的遥测消息数|Count|总计|成功发送到 IoT 中心的、设备到云的遥测消息数|无维度|
...
|jobs.listJobs.success|是|对列出作业的成功调用数|Count|总计|对列出作业的所有成功调用的计数。|无维度|
...
|DeviceAssignments|是|已分配设备|Count|总计|已分配给 IoT 中心的设备数|ProvisioningServiceName、IotHubName|
...
|CassandraConnectionClosures|否|Cassandra 连接关闭数|Count|总计|已关闭的 Cassandra 连接数，按 1 分钟粒度报告|Region、ClosureReason|
...
|DedicatedGatewayRequests|是|DedicatedGatewayRequests|Count|Count|专用网关上的请求数|DatabaseName, CollectionName, CacheExercised, OperationName, Region|
...
|IntegratedCacheEvictedEntriesSize|否|IntegratedCacheEvictedEntriesSize|字节|平均值|从集成缓存中逐出的条目的大小|CacheType, Region|
...
|MetadataRequests|否|元数据请求|Count|计数|元数据请求的计数。 Cosmos DB 为每个帐户维护系统元数据集合，允许你免费枚举集合、数据库及其配置等等。|DatabaseName、CollectionName、Region、StatusCode |
...
|MongoRequestsCount|否|（已弃用）Mongo 请求速率|每秒计数|平均值|每秒 Mongo 请求计数|DatabaseName, CollectionName, Region, ErrorCode|
|MongoRequestsDelete|否|（已弃用）Mongo 删除请求速率|每秒计数|平均值|每秒 Mongo 删除请求数|DatabaseName, CollectionName, Region, ErrorCode|
|MongoRequestsInsert|否|（已弃用）Mongo 插入请求速率|每秒计数|平均值|每秒 Mongo 插入计数|DatabaseName, CollectionName, Region, ErrorCode|
|MongoRequestsQuery|否|（已弃用）Mongo 查询请求速率|每秒计数|平均值|每秒 Mongo 查询请求数|DatabaseName, CollectionName, Region, ErrorCode|
|MongoRequestsUpdate|否|（已弃用）Mongo 更新请求速率|每秒计数|平均值|每秒 Mongo 更新请求数|DatabaseName, CollectionName, Region, ErrorCode|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ActiveConnections|否|ActiveConnections|Count|平均值|Microsoft.EventHub 的活动连接总数。||
|AvailableMemory|否|可用内存|百分比|最大值|事件中心群集的可用内存，以总内存量的百分比表示。|角色|
|CaptureBacklog|否|捕获积压工作(backlog)。|Count|总计|捕获有关 Microsoft.EventHub 的积压工作(backlog)。||
|CapturedBytes|否|已捕获的字节数。|字节|总计|Microsoft.EventHub 已捕获的字节数。||
|CapturedMessages|否|已捕获的消息数。|Count|总计|Microsoft.EventHub 已捕获的消息数。||
|ConnectionsClosed|否|已关闭的连接数。|Count|平均值|Microsoft.EventHub 已关闭的连接数。||
|ConnectionsOpened|否|打开的连接数。|Count|平均值|Microsoft.EventHub 打开的连接数。||
|CPU|否|CPU|百分比|最大值|事件中心群集的 CPU 使用率（百分比）|角色|
|IncomingBytes|是|传入字节数。|字节|总计|Microsoft.EventHub 传入的字节数。||
|IncomingMessages|是|传入消息数|Count|总计|Microsoft.EventHub 传入的消息数。||
|IncomingRequests|是|传入请求数|Count|总计|Microsoft.EventHub 传入的请求数。||
|OutgoingBytes|是|传出字节数。|字节|总计|Microsoft.EventHub 传出的字节数。||
|OutgoingMessages|是|传出消息数|Count|总计|Microsoft.EventHub 传出的消息数。||
|QuotaExceededErrors|否|超过限额错误。|Count|总计|Microsoft.EventHub 的超过限额错误数。|OperationResult|
|ServerErrors|否|服务器错误数。|Count|总计|Microsoft.EventHub 的服务器错误数。|OperationResult|
|大小|否|大小|字节|平均值|EventHub 的大小（以字节为单位）。|角色|
|SuccessfulRequests|否|成功的请求数|Count|总计|Microsoft.EventHub 成功请求数。|OperationResult|
|ThrottledRequests|否|限制的请求数。|Count|总计|Microsoft.EventHub 限制的请求数。|OperationResult|
|UserErrors|否|用户错误数。|Count|总计|Microsoft.EventHub 用户错误数。|OperationResult|
...
|INMSGS|是|传入的消息数（已过时）（已弃用）|Count|总计|命名空间的传入消息总数。 此指标已弃用。 请改用传入消息指标（已弃用）||
|INREQS|是|传入的请求数（已弃用）|Count|总计|命名空间的传入发送请求总数（已弃用）||
|INTERR|是|内部服务器错误数（已弃用）|Count|总计|命名空间的内部服务器错误总数（已弃用）||
|MISCERR|是|其他错误（已弃用）|Count|总计|命名空间的失败请求总数（已弃用）||
...
|OUTMSGS|是|传出的消息数（已过时）（已弃用）|Count|总计|命名空间的传出消息总数。 此指标已弃用。 请改用传出消息指标（已弃用）||
|QuotaExceededErrors|否|超过限额错误。|Count|总计|Microsoft.EventHub 的超过限额错误数。|EntityName, OperationResult|
|ServerErrors|否|服务器错误数。|Count|总计|Microsoft.EventHub 的服务器错误数。|EntityName, OperationResult|
|大小|否|大小|字节|平均值|EventHub 的大小（以字节为单位）。|EntityName|
|SuccessfulRequests|否|成功的请求数|Count|总计|Microsoft.EventHub 成功请求数。|EntityName, OperationResult|
|SUCCREQ|是|成功的请求数（已弃用）|Count|总计|命名空间的成功请求总数（已弃用）||
|SVRBSY|是|服务器繁忙错误数（已弃用）|Count|总计|命名空间的服务器繁忙错误总数（已弃用）||
|ThrottledRequests|否|限制的请求数。|Count|总计|Microsoft.EventHub 限制的请求数。|EntityName, OperationResult|
|UserErrors|否|用户错误数。|Count|总计|Microsoft.EventHub 用户错误数。|EntityName, OperationResult|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|CategorizedGatewayRequests|是|已分类的网关请求数|Count|总计|按类别（1xx/2xx/3xx/4xx/5xx）统计的网关请求数|HttpStatus|
...
|ObservedCapacity|是|观察到的容量|Count|平均值|自动缩放执行时报告的容量。||
...
|availabilityResults/count|否|可用性测试|Count|计数|可用性测试计数|availabilityResult/name, availabilityResult/location, availabilityResult/success|
...
|dependencies/count|否|依赖项调用|Count|计数|应用程序对外部资源所进行的调用计数。|dependency/type, dependency/performanceBucket, dependency/success, dependency/target, dependency/resultCode, operation/synthetic, cloud/roleInstance, cloud/roleName|
...
|dependencies/failed|否|依赖项调用失败次数|Count|计数|应用程序对外部资源所进行的依赖项调用失败的计数。|dependency/type, dependency/performanceBucket, dependency/target, dependency/resultCode, operation/synthetic, cloud/roleInstance, cloud/roleName|
|exceptions/browser|否|浏览器异常|Count|计数|浏览器中所引发未捕获异常的计数。|cloud/roleName|
|exceptions/count|是|异常|Count|计数|所有未捕获异常的已合并计数。|cloud/roleName, cloud/roleInstance, client/type|
|exceptions/server|否|服务器异常|Count|计数|服务器应用程序中引发的未捕获的异常计数。|cloud/roleName, cloud/roleInstance|
|pageViews/count|是|页面视图|Count|计数|页面视图计数。|operation/synthetic, cloud/roleName|
...
|requests/count|否|服务器请求数|Count|计数|已完成的 HTTP 请求计数。|request/performanceBucket, request/resultCode, operation/synthetic, cloud/roleInstance, request/success, cloud/roleName|
...
|ContinuousExportResult|是|连续导出结果|Count|计数|指示连续导出是成功还是失败|ContinuousExportName, Result, Database|
...
|IngestionResult|是|引入结果|Count|总计|引入操作的数量|IngestionResultDetails|
...
|---|---|---|---|---|---|---|
|活动核心数|是|活动核心数|Count|平均值|活动核心数|Scenario, ClusterName|
|活动节点数|是|活动节点数|Count|平均值|活动节点数。 这些节点是正在运行作业的节点。|Scenario, ClusterName|
...
|已完成的运行数|是|已完成的运行数|Count|总计|已成功为此工作区完成的运行数。 当运行完成并收集输出时，将更新计数。|Scenario、RunType、PublishedPipelineId、ComputeType、PipelineStepType、ExperimentName|
|CpuUtilization|是|CpuUtilization|计数|平均值|CPU 节点上的使用率百分比。 利用率每分钟报告一次。|Scenario, runId, NodeId, ClusterName|
|错误|是|错误|Count|总计|此工作区中的运行错误数。 运行遇到错误时，将更新计数。|方案|
|失败的运行次数|是|失败的运行次数|Count|总计|此工作区失败的运行数。 运行失败时，将更新计数。|Scenario、RunType、PublishedPipelineId、ComputeType、PipelineStepType、ExperimentName|
...
|空闲核心数|是|空闲核心数|Count|平均值|空闲核心数|Scenario, ClusterName|
|空闲节点数|是|空闲节点数|Count|平均值|空闲节点数。 空闲节点是指未运行任何作业，但可以接受新作业（如果可用）的节点。|Scenario, ClusterName|
|正在退出的核心数|是|正在退出的核心数|Count|平均值|正在退出的核心数|Scenario, ClusterName|
|正在退出的节点数|是|正在退出的节点数|Count|平均值|正在退出的节点数。 正在退出的节点是指刚刚完成了作业处理，将进入空闲状态的节点。|Scenario, ClusterName|
|失败的模型部署数|是|失败的模型部署数|Count|总计|此工作区中失败的模型部署数|Scenario, StatusCode|
|已启动的模型部署数|是|已启动的模型部署数|Count|总计|此工作区中已启动的模型部署数|方案|
|成功的模型部署数|是|成功的模型部署数|Count|总计|此工作区中成功的模型部署数|方案|
|失败的模型注册数|是|失败的模型注册数|Count|总计|此工作区中失败的模型注册数|Scenario, StatusCode|
...
|占用的核心数|是|占用的核心数|Count|平均值|占用的核心数|Scenario, ClusterName|
|占用的节点数|是|占用的节点数|Count|平均值|占用的节点数。 这些节点是已从可用节点池中取出的低优先级节点。|Scenario, ClusterName|
...
|配额利用率百分比|是|配额利用率百分比|Count|平均值|已利用的配额百分比|Scenario, ClusterName, VmFamilyName, VmPriority|
|已启动的运行数|是|已启动的运行数|Count|总计|为此工作区运行的运行数。 运行在所需资源上开始运行时，将更新计数。|Scenario、RunType、PublishedPipelineId、ComputeType、PipelineStepType、ExperimentName|
...
|核心总数|是|核心总数|Count|平均值|核心总数|Scenario, ClusterName|
|节点总数|是|节点总数|Count|平均值|节点总数。 此总数包括一些活动节点、空闲节点、不可用的节点、占用的节点和正在退出的节点|Scenario, ClusterName|
|不可用的核心数|是|不可用的核心数|Count|平均值|不可用的核心数|Scenario, ClusterName|
|不可用的节点数|是|不可用的节点数|Count|平均值|不可用的节点数。 不可用的节点是由于存在某种不可解决的问题而无法正常运行的节点。 Azure 将回收这些节点。|Scenario, ClusterName|
...
|MaxChannelsAndLiveEventsCount|是|最大实时事件配额|Count|最大值|当前媒体服务帐户中允许的最大实时事件数目|无维度|
|MaxRunningChannelsAndLiveEventsCount|是|运行的实时事件最大配额|Count|最大值|当前媒体服务帐户中允许运行的最大实时事件数目|无维度|
|RunningChannelsAndLiveEventsCount|是|正在运行的实时事件计数|计数|平均值|当前媒体服务帐户中正在运行的实时事件总数|无维度|
|StreamingPolicyCount|是|流式处理策略计数|Count|平均值|当前媒体服务帐户中已创建的流式处理策略数量|无维度|
...
|AvgRequestCountPerHealthyHost|否|每个正常主机每分钟的请求数|Count|平均值|池中每个正常后端主机每分钟的平均请求计数|BackendSettingsPool|
...
|BlockedCount|是|Web 应用程序防火墙阻止的请求规则分发|Count|总计|Web 应用程序防火墙阻止的请求规则分发|RuleGroup, RuleId|
...
|TlsProtocol|是|客户端 TLS 协议|Count|总计|与应用程序网关建立了连接的客户端发起的 TLS 和非 TLS 请求数。 若要查看 TLS 协议分布，请按“TLS 协议”维度进行筛选。|Listener, TlsProtocol|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|pingmesh|否|Bastion 通信状态|Count|平均值|如果所有通信正常，则通信状态显示 1；如果不正常，则显示 0。||
|会话|否|会话计数|Count|总计|Bastion 的会话计数。 按总计和每个实例查看。|host|
|total|是|内存总量|Count|平均值|总内存统计信息|host|
|usage_user|否|已用 CPU|Count|平均值|CPU 使用率统计信息|cpu, host|
|已使用|是|已用内存|Count|平均值|内存使用率统计信息|host|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|QueryVolume|否|查询量|Count|总计|为 DNS 区域提供服务的查询数|无维度|
|RecordSetCapacityUtilization|否|记录集容量使用率|百分比|最大值|DNS 区域利用的记录集容量的百分比|无维度|
|RecordSetCount|否|记录集计数|Count|最大值|DNS 区域中的记录集数|无维度|
...
|ExpressRouteGatewayCpuUtilization|是|CPU 使用率|Count|平均值|ExpressRoute 网关的 CPU 使用率|roleInstance|
...
|SNATConnectionCount|是|SNAT 连接计数|Count|总计|并发活动连接总数|Protocol, ConnectionState|
...
|TestResult|是|测试结果|Count|平均值|连接监视器测试结果|SourceAddress, SourceName, SourceResourceId, SourceType, Protocol, DestinationAddress, DestinationName, DestinationResourceId, DestinationType, DestinationPort, TestGroupName, TestConfigurationName, TestResultCriterion, SourceIP, DestinationIP, SourceSubnet, DestinationSubnet|
...
|P2SConnectionCount|是|P2S 连接计数|Count|总计|网关的点到站点连接计数|协议|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|QueryVolume|是|查询量|Count|总计|为专用 DNS 区域提供的查询数|无维度|
|RecordSetCapacityUtilization|否|记录集容量使用率|百分比|最大值|专用 DNS 区域使用的记录集容量的百分比|无维度|
|RecordSetCount|是|记录集计数|Count|最大值|专用 DNS 区域中的记录集数|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|PEBytesIn|是|传入字节|计数|总计|传出字节总数|PrivateEndpointId|
|PEBytesOut|是|传出字节|计数|总计|传出字节总数|PrivateEndpointId|
...
|TunnelNatAllocations|否|隧道 NAT 分配数|Count|总计|隧道上 NAT 规则的分配计数|NatRule, ConnectionName, RemoteIP|
|TunnelNatedBytes|否|隧道 NAT 处理字节数|字节|总计|NAT 规则在隧道上进行 NAT 处理的字节数 |NatRule, ConnectionName, RemoteIP|
|TunnelNatedPackets|否|隧道 NAT 处理数据包数|Count|总计|NAT 规则在隧道上进行 NAT 处理的数据包数|NatRule, ConnectionName, RemoteIP|
|TunnelNatFlowCount|否|隧道 NAT 流数|Count|总计|隧道上按流类型和 NAT 规则列出的 NAT 流数|NatRule, ConnectionName, RemoteIP, FlowType|
|TunnelNatPacketDrop|否|隧道 NAT 数据包丢包数|Count|总计|隧道上按丢包类型和 NAT 规则列出的 NAT 处理数据包数|NatRule, ConnectionName, RemoteIP, DropType|
|TunnelReverseNatedBytes|否|隧道反向 NAT 处理字节数|字节|总计|NAT 规则在隧道上进行反向 NAT 处理的字节数|NatRule, ConnectionName, RemoteIP|
|TunnelReverseNatedPackets|否|隧道反向 NAT 处理数据包数|Count|总计|NAT 规则在隧道上反向 NAT 处理的数据包数|NatRule, ConnectionName, RemoteIP|
...
|ListenerConnections-ClientError|否|ListenerConnections-ClientError|Count|总计|关于 Microsoft.Relay 的侦听程序连接的客户端错误。|EntityName, OperationResult|
|ListenerConnections-ServerError|否|ListenerConnections-ServerError|Count|总计|关于 Microsoft.Relay 的侦听程序连接的服务器错误。|EntityName, OperationResult|
|ListenerConnections-Success|否|ListenerConnections-Success|Count|总计|Microsoft.Relay 的侦听程序成功连接数。|EntityName, OperationResult|
...
|SenderConnections-ClientError|否|SenderConnections-ClientError|Count|总计|关于 Microsoft.Relay 的发送方连接的客户端错误。|EntityName, OperationResult|
|SenderConnections-ServerError|否|SenderConnections-ServerError|Count|总计|关于 Microsoft.Relay 的发送方连接的服务器错误。|EntityName, OperationResult|
|SenderConnections-Success|否|SenderConnections-Success|Count|总计|Microsoft.Relay 的发送方成功连接数。|EntityName, OperationResult|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ActiveConnections|否|ActiveConnections|Count|总计|Microsoft.ServiceBus 的活动连接总数。||
...
|ServerErrors|否|服务器错误数。|Count|总计|Microsoft.ServiceBus 的服务器错误数。|EntityName, OperationResult|
|大小|否|大小|字节|平均值|队列/主题的大小（以字节为单位）。|EntityName|
|SuccessfulRequests|否|成功的请求数|Count|总计|命名空间的成功请求总数|EntityName, OperationResult|
|ThrottledRequests|否|限制的请求数。|Count|总计|Microsoft.ServiceBus 限制的请求数。|EntityName, OperationResult|
|UserErrors|否|用户错误数。|Count|总计|Microsoft.ServiceBus 的用户错误数。|EntityName, OperationResult|
...
|InboundTraffic|是|入站流量|字节|总计|服务的入站流量|无维度|
|OutboundTraffic|是|出站流量|字节|总计|服务的出站流量|无维度|
|TotalConnectionCount|是|连接计数|Count|最大值|用户连接量。|无维度|
...
|tempdb_data_size|是|Tempdb 数据文件大小 (KB)|Count|最大值|tempdb 数据文件中已使用的空间 (KB)。 不适用于数据仓库。|无维度|
|tempdb_log_size|是|Tempdb 日志文件大小 (KB)|Count|最大值|tempdb 事务日志文件中已使用的空间 (KB)。 不适用于数据仓库。|无维度|
...
|tempdb_data_size|是|Tempdb 数据文件大小 (KB)|Count|最大值|tempdb 数据文件中已使用的空间 (KB)。|无维度|
|tempdb_log_size|是|Tempdb 日志文件大小 (KB)|Count|最大值|tempdb 事务日志文件中已使用的空间 (KB)。|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ClientIOPS|是|总客户端 IOPS|Count|平均值|缓存处理客户端文件操作的速率。|无维度|
...
|StorageTargetHealth|是|存储目标运行状况|Count|平均值|缓存和存储目标之间的连接测试的布尔结果。|无维度|
|StorageTargetIOPS|是|总 StorageTarget IOPS|Count|平均值|缓存发送到特定 StorageTarget 的所有文件操作的速率。|StorageTarget|
...
|StorageTargetWriteIOPS|是|StorageTarget 写入 IOPS|Count|平均值|缓存发送到特定 StorageTarget 的文件写入操作的速率。|StorageTarget|
|运行时间|是|运行时间|Count|平均值|缓存和监视系统之间的连接测试的布尔结果。|无维度|
...
|StorageSyncRecallComputedSuccessRate|是|云分层重调成功率|百分比|平均值|所有成功重调的百分比|SyncGroupName, ServerName|
...
|ProcessCPUUsagePercentage|是|CPU 利用率百分比(预览)|百分比|最大值|CPU 利用率百分比(预览)|LogicalName、PartitionId|
...
|SQLStreamingBackloggedInputEventSources|否|积压的输入事件数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 积压的输入事件源的数量。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingConversionErrors|否|数据转换错误数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 无法转换为预期输出架构的输出事件的数量。 可以将错误策略更改为“删除”，以删除遇到此情况的事件。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingDeserializationError|否|输入反序列化错误数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 不可反序列化的输入事件数。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingEarlyInputEvents|否|早期输入事件数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 根据提前到达策略，应用程序时间被视为早于到达时间的输入事件数。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingInputEventBytes|否|输入事件字节数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 流式处理作业收到的数据量（以字节为单位）。 这可以用于验证正在发送到输入源的事件。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingInputEvents|否|输入事件数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 输入事件数。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingInputEventsSourcesPerSecond|否|收到的输入源数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 每秒输入事件源的数量。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingLateInputEvents|否|延迟输入事件数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 根据延迟到达策略，应用程序时间被视为晚于到达时间的输入事件数。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingOutOfOrderEvents|否|无序事件数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 事件中心输入适配器收到的无序事件中心事件（序列化消息）的数目，系统根据事件排序策略来删除这些事件，或者为其提供一个经过调整的时间戳。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingOutputEvents|否|输出事件数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 输出事件数。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
|SQLStreamingOutputWatermarkDelaySeconds|否|水印延迟（预览）|Count|最大值|这是美国东部和西欧区域中提供的预览指标。 输出水印延迟（秒）。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
...
|SQLStreamingRuntimeErrors|否|运行时错误数（预览）|Count|总计|这是美国东部和西欧区域中提供的预览指标。 与查询处理相关的错误总数（不包括引入事件或输出结果时发现的错误）。|ResourceName、SQLPoolName、SQLDatabaseName、JobName、LogicalName、PartitionId、ProcessorInstance|
...
|DiskQueueLength|是|磁盘队列长度|Count|平均值|在存储上排队的读取和写入请求的平均数量。 过高的磁盘队列长度表示应用可能由于磁盘 I/O 过多而速度变慢。|实例|
|Http101|是|Http 101|Count|总计|导致 HTTP 状态代码 101 的请求计数。|实例|
|Http2xx|是|Http 2xx|Count|总计|导致 HTTP 状态代码大于等于 200 但小于 300 的请求计数。|实例|
|Http3xx|是|Http 3xx|Count|总计|导致 HTTP 状态代码大于等于 300 但小于 400 的请求计数。|实例|
|Http401|是|Http 401|Count|总计|导致 HTTP 401 状态代码的请求计数。|实例|
|Http403|是|Http 403|Count|总计|导致 HTTP 403 状态代码的请求计数。|实例|
|Http404|是|Http 404|Count|总计|导致 HTTP 404 状态代码的请求计数。|实例|
|Http406|是|Http 406|Count|总计|导致 HTTP 406 状态代码的请求计数。|实例|
|Http4xx|是|Http 4xx|Count|总计|导致 HTTP 状态代码大于等于 400 但小于 500 的请求计数。|实例|
|Http5xx|是|Http 服务器错误|Count|总计|导致 HTTP 状态代码大于等于 500 但小于 600 的请求计数。|实例|
|HttpQueueLength|是|Http 队列长度|Count|平均值|必须在队列排满之前排入队列中的 HTTP 请求的平均数量。 较高或不断增长的 HTTP 队列长度表示计划处于高负载状态。|实例|
...
|请求|是|请求|Count|总计|请求总数，不考虑是否导致 HTTP 状态代码。|实例|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|AppConnections|是|连接|Count|平均值|沙盒中存在的绑定套接字的数目（w3wp.exe 及其子进程）。 绑定套接字是通过调用 bind()/connect() API 创建的，并一直保留到通过 CloseHandle()/closesocket() 关闭所述的套接字。|实例|
...
|CpuTime|是|CPU 时间|秒|总计|应用消耗的 CPU 量，以秒为单位。 有关此指标的详细信息， 不适用于 Azure Functions。 请参阅 https://aka.ms/website-monitor-cpu-time-vs-cpu-percentage （CPU 时间与 CPU 百分比）。|实例|
|CurrentAssemblies|是|当前程序集|Count|平均值|此应用程序中的所有 AppDomain 中加载的程序集的当前数量。|实例|
|FileSystemUsage|是|文件系统使用情况|字节|平均值|应用消耗的文件系统配额的百分比。|无维度|
|FunctionExecutionCount|是|函数执行计数|Count|总计|函数执行计数。 仅适用于 Azure Functions。|实例|
|FunctionExecutionUnits|是|函数执行单位数|Count|总计|函数执行单位数。 仅适用于 Azure Functions。|实例|
|Gen0Collections|是|第 0 代垃圾回收|Count|总计|自应用进程启动以来对第 0 代对象进行垃圾回收的次数。 较高代系的垃圾回收包括所有较低代系的垃圾回收。|实例|
|Gen1Collections|是|第 1 代垃圾回收|Count|总计|自应用进程启动以来对第 1 代对象进行垃圾回收的次数。 较高代系的垃圾回收包括所有较低代系的垃圾回收。|实例|
|Gen2Collections|是|第 2 代垃圾回收|Count|总计|自应用进程启动以来对第 2 代对象进行垃圾回收的次数。|实例|
|句柄数|是|句柄计数|Count|平均值|应用进程当前打开的句柄总数。|实例|
...
|请求|是|请求|Count|总计|请求总数，不考虑是否导致 HTTP 状态代码。|实例|
|RequestsInApplicationQueue|是|应用程序队列中的请求数|Count|平均值|应用程序请求队列中的请求数。|实例|
|线程数|是|线程计数|Count|平均值|应用进程中当前处于活动状态的线程数。|实例|
|TotalAppDomains|是|应用程序域总数|Count|平均值|此应用程序中加载的 AppDomain 的当前数目。|实例|
|TotalAppDomainsUnloaded|是|卸载的应用程序域总数|Count|平均值|自应用程序启动以来卸载的 AppDomain 的总数。|实例|
