...
1. **配置** - 检查指标警报规则配置以确保它正确配置：
    - 检查 **聚合类型** 和 **聚合粒度 (period)** 是否按预期方式配置。 **聚合类型** 确定度量值的聚合方式 ([在此处](./metrics-aggregation-explained.md#aggregation-types) 了解详细信息) 和 **聚合粒度 (时间)** 控制每次运行警报规则时，计算聚合度量值的频率。
    -  检查是否按预期方式配置了 **阈值** 或 **敏感度** 。
    - 对于使用动态阈值的警报规则，请检查是否配置了 "高级设置"，因为 **冲突数** 可能会筛选警报并 **忽略数据** ，然后才能影响阈值的计算方式。
