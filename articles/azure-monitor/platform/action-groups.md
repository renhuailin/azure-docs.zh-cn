...
如果你没有在 *主电子邮件* 上收到通知，则可以尝试以下步骤：

1. 在 Azure 门户中转到 *Active Directory*"。
2. 单击左侧窗格中的 "所有用户 () ，你将在右窗格中看到" (用户的列表) "。
3. 选择要查看其 *主电子邮件* 信息的用户。

  :::image type="content" source="media/action-groups/active-directory-user-profile.png" alt-text="如何查看用户配置文件的示例。"border="true":::

4. 如果 "电子邮件" 选项卡为空白，则在用户配置文件中的 "联系人信息" 下，单击顶部的 " *编辑* " 按钮，并在顶部添加 *主电子邮件* 并单击 " *保存* " 按钮。
