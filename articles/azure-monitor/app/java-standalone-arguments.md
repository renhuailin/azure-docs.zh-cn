...
将 JVM 参数 `-javaagent:path/to/applicationinsights-agent-3.0.3.jar` 添加到 `-jar` 之前的某个位置，例如：
...
如果使用的是 exec 形式，请将参数 `"-javaagent:path/to/applicationinsights-agent-3.0.3.jar"` 添加到参数列表中 `"-jar"` 参数之前的某个位置，例如：

```
ENTRYPOINT ["java", "-javaagent:path/to/applicationinsights-agent-3.0.3.jar", "-jar", "<myapp.jar>"]
```

如果使用的是 shell 形式，请在 `-jar` 之前的某个位置添加 JVM 参数 `-javaagent:path/to/applicationinsights-agent-3.0.3.jar`，例如：
...
如果文件 `<tomcat>/bin/setenv.sh` 已存在，则修改该文件并将 `-javaagent:path/to/applicationinsights-agent-3.0.3.jar` 添加到 `CATALINA_OPTS`。
...
如果文件 `<tomcat>/bin/setenv.bat` 已存在，只需修改该文件并将 `-javaagent:path/to/applicationinsights-agent-3.0.3.jar` 添加到 `CATALINA_OPTS` 即可。

### <a name="running-tomcat-as-a-windows-service"></a>将 Tomcat 作为 Windows 服务运行

找到文件 `<tomcat>/bin/tomcat8w.exe`。  运行该可执行文件，并将 `-javaagent:path/to/applicationinsights-agent-3.0.3.jar` 添加到 `Java` 选项卡下的 `Java Options` 中。
...
将 `-javaagent:path/to/applicationinsights-agent-3.0.3.jar` 添加到文件 `JBOSS_HOME/bin/standalone.conf` (Linux) 或 `JBOSS_HOME/bin/standalone.conf.bat` (Windows) 中的现有 `JAVA_OPTS` 环境变量：
...
将 `-javaagent:path/to/applicationinsights-agent-3.0.3.jar` 添加到 `JBOSS_HOME/domain/configuration/host.xml` 中的现有 `jvm-options`：
...
将 `-javaagent:path/to/applicationinsights-agent-3.0.3.jar` 添加到 `glassfish/domains/domain1/config/domain.xml` 中的现有 `jvm-options`：
