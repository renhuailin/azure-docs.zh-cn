---
title: 适用于 .NET 应用的 Azure Application Insights Snapshot Debugger
...
可在门户中查看调试快照，查看调用堆栈并检查每个调用堆栈帧中的变量。 若要获取更强大的调试体验与源代码，请在 Visual Studio 2019 Enterprise 中打开快照。 在 Visual Studio 中，还可以[将快照设置为以交互方式拍摄快照](/visualstudio/debugger/debug-live-azure-applications)，而无需等待异常。
...
* 在 Windows 上运行 .NET Core 2.1 (LTS) 或 3.1 (LTS) 的 .NET Core 和 ASP.NET Core 应用程序。
* Windows 上的 .NET 5.0 应用程序。

不建议使用 .NET Core 2.0、2.2 或 3.0，因为它们不受支持。
...
> [!NOTE]
> 有关可用的不同符号选项的详细信息，请参阅 [Visual Studio 文档](/visualstudio/ide/reference/advanced-build-settings-dialog-box-csharp?view=vs-2019&preserve-view=true#output
