...
根据所使用的 Application Insights 日志包，可通过多种方法注册 `ApplicationInsightsLoggerProvider`。 `ApplicationInsightsLoggerProvider` 是 <xref:Microsoft.Extensions.Logging.ILoggerProvider> 的实现，负责提供 <xref:Microsoft.Extensions.Logging.ILogger> 和 <xref:Microsoft.Extensions.Logging.ILogger%601> 实现。

## <a name="aspnet-core-applications"></a>ASP.NET Core 应用程序

若要将 Application Insights 遥测添加到 ASP.NET Core 应用程序，请使用 NuGet `Microsoft.ApplicationInsights.AspNetCore` 包。 可以通过 [Visual Studio 将其配置为连接服务](/visualstudio/azure/azure-app-insights-add-connected-service)，也可以手动配置。

默认情况下，通过[代码](./asp-net-core.md)或[无代码](./azure-web-apps.md?tabs=netcore#enable-agent-based-monitoring)方法配置时，ASP.NET Core 应用程序注册 Application Insights 日志提供程序。 注册的提供程序配置为自动捕获严重级别为 <xref:Microsoft.Extensions.Logging.LogLevel.Warning?displayProperty=nameWithType> 或更高的日志事件。 可以自定义严重性和类别。 有关详细信息，请参阅[日志记录级别](#logging-level)。
...
从 Program.cs 和 Startup.cs 登录时，有几个限制 ：

* 通过 [InMemoryChannel](./telemetry-channels.md) 遥测通道发送遥测。
...
在前面的代码中，使用 `"APPINSIGHTS_CONNECTIONSTRING"` 连接字符串配置了 `ApplicationInsightsLoggerProvider`。 应用了筛选器，将日志级别设置为 <xref:Microsoft.Extensions.Logging.LogLevel.Trace?displayProperty=nameWithType>。

> [!IMPORTANT]
> 建议使用[连接字符串](./sdk-connection-string.md?tabs=net)替代检测密钥。 新的 Azure 区域要求使用连接字符串而不是检测密钥。 
>
> 连接字符串用于标识要与遥测数据关联的资源。 它还允许你修改可供你的资源将其用作遥测目标的终结点。 你需要复制连接字符串，并将其添加到应用程序的代码或环境变量。
...
前面的示例使用了 `Microsoft.Extensions.Logging.ApplicationInsights` 包。 默认情况下，此配置使用“最低要求”的 `TelemetryConfiguration` 设置将数据发送到 Application Insights：`InMemoryChannel` 通道。 不使用采样，也不使用标准 `TelemetryInitializer` 实例。 可为控制台应用程序替代此行为，如以下示例所示。
...
以下部分介绍如何通过使用 <xref:Microsoft.Extensions.Options.ConfigureOptions%601.Configure(%600)> 方法替代默认的 `TelemetryConfiguration` 设置。 此示例设置 `ServerTelemetryChannel` 和采样。 它将自定义 `TelemetryInitializer` 实例添加到 `TelemetryConfiguration`。
...
`ILogger` 实现具有内置机制，可应用[日志筛选](/dotnet/core/extensions/logging#how-filtering-rules-are-applied)。 通过此筛选，可以控制发送到每个已注册提供程序（包括 Application Insights 提供程序）的日志。 可以在配置中（例如使用 appsettings.json 文件）或者在代码中执行筛选。
...
以下代码片段为以下项配置要发送到 `ApplicationInsightsLoggerProvider` 的日志：
...
如果范围的类型为 `IReadOnlyCollection<KeyValuePair<string,object>>`，则集合中的每个键值对都将作为自定义属性添加到 Application Insights 遥测中。 在下面的示例中，日志将作为 `TraceTelemetry` 捕获，其属性中将包含 `("MyKey", "MyValue")`。
...
如果使用任何其他类型作为范围，它将存储在 Application Insights 遥测中的 `Scope` 属性下。 在下面的示例中，`TraceTelemetry` 将具有名为 `Scope` 的属性，该属性包含范围。
...
[Microsoft.ApplicationInsights.AspNet SDK](https://www.nuget.org/packages/Microsoft.ApplicationInsights.AspNetCore) 包含一个内置的 `ApplicationInsightsLoggerProvider` (`Microsoft.ApplicationInsights.AspNetCore.Logging.ApplicationInsightsLoggerProvider`) 实例，该实例是通过 `ILoggerFactory` 扩展方法启用的。 从版本 2.7.1 开始，此提供程序已标记为过时。 下一次主版本更改中将完全删除它。 

[Microsoft.ApplicationInsights.AspNetCore 2.6.1](https://www.nuget.org/packages/Microsoft.ApplicationInsights.AspNetCore) 包本身尚未过时。 需要启用对请求和依赖项等项的监视。

建议的替代项是新的独立包 [Microsoft.Extensions.Logging.ApplicationInsights](https://www.nuget.org/packages/Microsoft.Extensions.Logging.ApplicationInsights)，其中包含改进的 `ApplicationInsightsLoggerProvider` 实例 (`Microsoft.Extensions.Logging.ApplicationInsights.ApplicationInsightsLoggerProvider`) 和 `ILoggerBuilder` 上用于启用它的扩展方法。

[Microsoft.ApplicationInsights.AspNet SDK](https://www.nuget.org/packages/Microsoft.ApplicationInsights.AspNetCore) 2.7.1 版依赖于新包，并自动启用 `ILogger` 捕获。

### <a name="why-are-some-ilogger-logs-shown-twice-in-application-insights"></a>为何某些 ILogger 日志在 Application Insights 中会显示两次？

如果通过在 `ILoggerFactory` 上调用 `AddApplicationInsights` 启用了较旧（现已过时）的 `ApplicationInsightsLoggerProvider` 版本，则可能会发生重复。 检查 `Configure` 方法是否包含以下代码，并将其删除：
...
如果在 Visual Studio 中调试时看到重复的日志，请在启用 Application Insights 的代码中将 `EnableDebugLogger` 设置为 `false`，如下所示。 此重复和修复仅在调试应用程序时相关。
...
请参阅[日志记录级别](#logging-level)部分，了解一般如何筛选日志。 若要关闭 `ApplicationInsightsLoggerProvider`，请在调用中使用 `LogLevel.None` 来配置日志记录。 在以下命令中，`builder` 为 <xref:Microsoft.Extensions.Logging.ILoggingBuilder>。
...
Application Insights 使用其他所有遥测使用的相同 `TelemetryConfiguration` 信息来捕获和发送 `ILogger` 日志。 但存在一种例外情况。 默认情况下，从 Program.cs 或 Startup.cs 登录时，`TelemetryConfiguration` 未完全设置 。 来自这些地方的日志没有默认配置，因此它们不会运行所有 `TelemetryInitializer` 实例和 `TelemetryProcessor` 实例。
...
使用独立包时，不会将 `TelemetryClient` 注入依赖项注入 (DI) 容器。 你需要创建新的 `TelemetryClient` 实例并使用日志提供程序使用的相同配置，如以下代码所示。 这可确保对所有自定义遥测和来自 `ILogger` 的遥测使用相同的配置。
...
### <a name="what-application-insights-telemetry-type-is-produced-from-ilogger-logs-where-can-i-see-ilogger-logs-in-application-insights"></a>将从 ILogger 日志生成哪种类型的 Application Insights 遥测数据？ 在哪里可以看到 Application Insights 中的 ILogger 日志？
...
这些遥测项与 Application Insights 的任何其他 `TraceTelemetry` 或 `ExceptionTelemetry` 项（包括 Azure 门户、分析或 Visual Studio 本地调试器）位于相同的位置。
