...
ms.locfileid: "101733935"
---
# <a name="parse-text-data-in-azure-monitor-logs"></a>分析 Azure Monitor 日志中的文本数据
Azure Monitor 收集的某些日志数据会在单个属性中包括多条信息。 将此数据分析为多个属性可以更轻松地在查询中进行使用。 一个常见示例是收集在单个属性中包含多个值的整个日志项目的[自定义日志](../agents/data-sources-custom-logs.md)。 通过为不同值创建单独属性，可以对每个值进行搜索和聚合。
...
如果在收集时分析数据，则配置会在表中创建新属性的[自定义字段](../logs/custom-fields.md)。 查询不必包含任何分析逻辑，只需将这些属性用作表中的任何其他字段。
...
有关在收集时分析数据的详细信息，请参阅[在 Azure Monitor 中创建自定义字段](../logs/custom-fields.md)。 这会在表中创建可以由查询使用的自定义属性（就如同任何其他属性一样）。
...
可能具有对特定表执行相同分析的多个查询。 在这种情况下，[创建一个函数](../logs/functions.md)以返回经过分析的数据，而不是在每个查询中复制分析逻辑。 随后可以在其他查询中使用函数别名来代替原始表。
