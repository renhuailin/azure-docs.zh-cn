...
* 利用 Azure Monitor 工作簿之类的工具集中可视化和分析 [客户租户](cross-workspace-query.md) 中的数据可能会导致较慢的体验，尤其是在跨超过50个工作区分析数据时。
