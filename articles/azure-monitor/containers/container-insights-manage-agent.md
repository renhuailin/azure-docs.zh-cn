---
title: 如何管理容器 insights 代理 |Microsoft Docs
description: 本文介绍如何使用容器见解使用的容器化 Log Analytics 代理管理最常见的维护任务。
ms.topic: conceptual
ms.date: 07/21/2020
...
---
# <a name="how-to-manage-the-container-insights-agent"></a>如何管理容器 insights 代理

容器见解使用容器化版本的适用于 Linux 的 Log Analytics 代理。 初始部署之后，在其生命周期内可能需要执行一些例程或可选任务。 本文详细介绍如何手动升级代理并禁用来自特定容器的环境变量集合。 

## <a name="how-to-upgrade-the-container-insights-agent"></a>如何升级 Container insights 代理

容器见解使用容器化版本的适用于 Linux 的 Log Analytics 代理。 发布该代理的新版本时，在 Azure Kubernetes 服务 (AKS) 和 Azure Red Hat OpenShift 版本 3.x 上承载的托管 Kubernetes 群集上，该代理会自动升级。 对于[混合 Kubernetes 群集](container-insights-hybrid-setup.md)和 Azure Red Hat OpenShift 版本 4.x，未托管该代理，需要手动升级该代理。
...
升级 AKS 群集上的代理的过程包括两个明确的步骤。 第一步是使用 Azure CLI 禁用容器 insights 监视。 请遵循[禁用监视](container-insights-optout.md?#azure-cli)一文中介绍的步骤。 可以使用 Azure CLI 从群集的节点中删除代理，不会影响解决方案以及工作区中存储的相应数据。 
...
容器见解从在 pod 中运行的容器收集环境变量，并将其显示在 **容器** 视图中选定容器的属性窗格中。 可在部署 Kubernetes 集群期间或之后通过设置环境变量 AZMON_COLLECT_ENV 禁用特定容器的集合来控制此行为。 此功能可从代理版本 ciprod11292018 或更高版本获得。  
...
若要验证配置更改是否生效，请在容器见解的 " **容器** " 视图中选择一个容器，然后在 "属性" 面板中展开 " **环境变量**"。  此部分应只显示前面创建的变量 - AZMON_COLLECT_ENV=FALSE。 对于所有其他容器，环境变量部分应列出发现的所有环境变量。
