...
如果你不熟悉 Azure Monitor 警报，请在开始之前参阅 [Microsoft Azure 中的警报概述](../platform/alerts-overview.md)。 若要详细了解使用日志查询的警报，请参阅 [Azure Monitor 中的日志警报](../alerts/alerts-unified-log.md)。 有关指标警报的详细信息，请参阅 [Azure Monitor 中的指标警报](../alerts/alerts-metric-overview.md)。
...
本部分逐步介绍如何使用用于容器的 Azure Monitor 提供的性能数据来创建指标度量预警规则。 可以将此基本过程与各种日志查询一起使用，以发出针对不同性能计数器的警报。 首先使用前面提供的某个日志搜索查询。 若要使用 ARM 模板进行创建，请参阅[使用 Azure 资源模板创建日志警报的示例](../alerts/alerts-log-create-templates.md)。
