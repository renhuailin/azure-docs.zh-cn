...
混合连接可以在两个联网的应用程序之间进行双向、请求-响应和二进制流通信以及简单的数据报流。 一方或双方均可位于 Nat 或防火墙后面。
...
该交互模型倾向于使用由许多其他网络服务 API 创建的术语： 有一个侦听器，它首先指示准备处理传入的连接，并在这些连接到达时接受这些连接。 在另一端，某个客户端会连接到侦听器，预期该连接被接受以建立双向通信路径。 “连接”、“侦听”和“接受”与大多数套接字 API 中找到的术语相同。

任何中继通信模型都会使双方生成针对服务终结点的出站连接。 这使得“侦听器”也常被说成“客户端”，可能还会导致其他术语重载。 因此，用于混合连接的准确术语如下：

连接两端上的程序称为 "客户端"，因为它们是服务的客户端。 等待和接受连接的客户端是“侦听器”，或者称其在“侦听器角色”中。 通过服务向侦听器启动新连接的客户端称为“发送方”，或者称其在“发送方角色”中。
...
编码信息仅在短时间内有效，基本上为发送方愿意等待的端到端创建连接的时间。 假设最长时间为 30 秒。 该 URL 只能用于一次成功的连接尝试。 使用集合 URL 建立 Web 套接字连接后，该 Web 套接字上的所有后续活动都将与发送方来回中继。 此行为在没有任何干预或服务解释的情况下发生。
...
HTTP 帧标头元数据将转换为 JSON，以方便由侦听器框架处理；执行这种转换的另一个原因是，相比 JSON 分析器，HTTP 标头分析库极其少见。 仅与发送方和中继 HTTP 网关（包括授权信息）之间的关系相关的 HTTP 元数据不转发。 HTTP 请求正文以透明方式作为二进制 Web 套接字帧传输。
...
对于请求，服务会决定是否通过控制通道来路由请求。 这包括但不限于以下情况：请求超过 64 kB (标头和正文) ，或者，如果请求是使用 ["分块" 传输编码](https://tools.ietf.org/html/rfc7230#section-4.1) 发送的，并且服务的请求数超过 64 kB，则不是即时请求。 如果服务选择通过会合点传送请求，它只会将会合地址传递给侦听器。
...
* **id** – 该连接的唯一标识符。 如果 ID 由发送方客户端提供，则为发送方提供的值，否则为系统生成的值。
...
此参数使发送方客户端可以在无法包含 HTTP 头时将调度参数传递到接受侦听器。 侦听器框架可从路径中分析出固定路径部分和注册名称，并向应用程序提供其余部分（可能不含任何带 `sb-` 前缀的查询字符串参数），以决定是否接受连接。
...
如果没有正文，则会省略正文帧。 布尔值 `body` 属性指示请求消息中是否存在正文。
...
* **address** - URI 字符串。 它是用于此请求的集合地址。 如果传入的请求大于 64 kB，则此消息的余下部分将会留空，并且客户端必须启动等同于如下所述 `accept` 操作的会合握手。 然后，服务将在建立的 Web 套接字放置完整的 `request`。 如果响应预期可能超过 64 kB，则侦听器也必须启动会合握手，然后通过建立的 Web 套接字传输响应。
* **id** – 字符串。 此请求的唯一标识符。
...
* **requestTarget** – 字符串。 此属性保存请求的[“请求目标”（RFC7230 第 5.3 部分）](https://tools.ietf.org/html/rfc7230#section-5.3)。 它包含查询字符串部分，该部分将去除所有 `sb-hc-` 前缀参数。
* **method** - 字符串。 这是 [RFC7231 第 4 部分](https://tools.ietf.org/html/rfc7231#section-4)所述的请求方法。 不得使用 `CONNECT` 方法。
* **bool** – 布尔值。 指示是否跟随一个或多个二进制正文框架。
...
请求可以包含任意其他 HTTP 头，包括应用程序定义的头。 所有提供的标头（除了直接在 RFC7230 中定义的标头） (参阅 [请求消息](#request-message)) 流向侦听器，并可在 `requestHeader` **请求** 消息的对象上找到。
...
如果有错误，则服务可以按如下所示进行回复。 可以通过 `Via` 标头的存在状态来判断响应是来自服务还是侦听器。 如果存在该标头，则响应来自侦听器。
...
| 504  | 网关超时 | 请求已路由到侦听器，但侦听器未在所需时间内确认收到。
