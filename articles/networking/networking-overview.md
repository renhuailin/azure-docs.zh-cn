...
- [连接服务](#connect)：使用 Azure 中的以下任一网络服务或其组合连接 Azure 资源和本地资源 - 虚拟网络 (VNet)、虚拟 WAN、ExpressRoute、VPN 网关、虚拟网络 NAT 网关、Azure DNS、对等互连服务和 Azure Bastion。
- [应用程序保护服务](#protect)：使用 Azure 中的以下任一网络服务或其组合来保护应用程序 - 专用链接、DDoS 防护、防火墙、网络安全组、Web 应用程序防火墙和虚拟网络终结点。
...
Azure Edge Zone 是 Microsoft Azure 中的一系列产品/服务，可使用户能够方便地进行数据处理。 可以将 VM、容器和其他所选 Azure 服务部署到 Edge Zone，以满足应用程序的低延迟和高吞吐量的需要。 有关详细信息，请参阅[什么是 Azure Edge Zone？](edge-zones-overview.md)。
...
本部分介绍 Azure 中有助于监视网络资源的网络服务 - 网络观察程序、Azure 网络监视器、ExpressRoute 监视器、Azure Monitor 和虚拟网络 TAP。
