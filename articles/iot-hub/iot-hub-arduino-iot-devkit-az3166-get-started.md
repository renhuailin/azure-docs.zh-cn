...
1. 在 Azure Cloud Shell 中运行以下命令，以创建设备标识。
   ...
   **MyNodeDevice**：所注册的设备的名称。 如下所示请使用 MyNodeDevice 。 如果为设备选择其他名称，则需要在本文中从头至尾使用该名称，并在运行示例应用程序之前在其中更新设备名称。
   ...
   > [!NOTE]
   > 如果在运行 `device-identity` 时遇到错误，请安装[用于 Azure CLI 的 Azure IoT 扩展](https://github.com/Azure/azure-iot-cli-extension/blob/dev/README.md)。
...
1. 确保 IoT DevKit 通过 USB 连接到计算机。 打开文件资源管理器，有一个称为 AZ3166 的 USB 大容量存储设备。
...
1. 在 DevKit 上，按住按钮 B 不放，按下再松开重置按钮，然后松开按钮 B。DevKit 进入 AP 模式。 若要进行确认，屏幕将显示 DevKit 的服务集标识符 (SSID) 和配置门户 IP 地址。
...
1. 在另一个启用了 Wi-Fi 的设备（计算机或手机）上使用 Web 浏览器连接到上一步骤中显示的 IoT DevKit SSID。 如果要求输入密码，则将其留空。
...
1. 在“结果”页，你会看到 WiFi 信息和设备连接字符串将存储到 IoT DevKit 中。
...
1. IoT DevKit 在几秒钟后将重新启动。 在 DevKit 屏幕上，会看到 DevKit 的 IP 地址，后跟遥测数据，其中包括温度和湿度值以及发送到 Azure IoT 中心的消息计数。
...
1. 安装 [Arduino IDE](https://www.arduino.cc/en/Main/Software)。 此 IDE 提供必要的工具链用于编译和上传 Arduino 代码。
    * Windows：使用 Windows Installer 版本。 不要从 App Store 安装。
...
4. 在扩展市场中找到 [Azure IoT Tools](https://aka.ms/azure-iot-tools) 并安装它。
    ...
    ![显示扩展市场中的 Azure IoT 工具的屏幕截图。](media/iot-hub-arduino-devkit-az3166-get-started/getting-started/install-azure-iot-tools.png)
...
与从 Azure 门户预配 Azure IoT 中心和设备不同，你可以在 VS Code 中进行预配，而无需离开开发环境。
...
1. 在指定的资源组中，按照指南选择或创建新的 Azure IoT 中心。
    ...
    ![选择“IoT 中心”步骤](media/iot-hub-arduino-devkit-az3166-get-started/getting-started/iot-hub-provision.png)

    ![选择“IoT 中心”](media/iot-hub-arduino-devkit-az3166-get-started/getting-started/select-iot-hub.png)

    ![已选择“IoT 中心”](media/iot-hub-arduino-devkit-az3166-get-started/getting-started/iot-hub-selected.png)
...
1. 现已预配 Azure IoT 中心并在其中创建了设备。 设备连接字符串还会保存在 VS Code 中，以便稍后配置 IoT DevKit。
...
1. 在“输出”窗格中，可以看到传入 IoT 中心的 D2C 消息。
    ...
    ![显示 D2C 到 IoT 中心的传入消息的屏幕截图。](media/iot-hub-arduino-devkit-az3166-get-started/getting-started/d2c-output.png)
