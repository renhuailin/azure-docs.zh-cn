...
ms.locfileid: "104576879"
---
# <a name="azure-security-baseline-for-azure-iot-hub"></a>适用于 Azure IoT 中心的 Azure 安全基线

此安全基线将 [Azure 安全基准版本 1.0](../security/benchmarks/overview-v1.md) 中的指导应用于 Microsoft Azure IoT 中心。 Azure 安全基准提供有关如何在 Azure 上保护云解决方案的建议。
内容按“安全控件”分组，这些控件根据适用于 Azure IoT 中心的 Azure 安全基准和相关指南定义。 排除了不适用于 Azure IoT 中心的“控件”。

 
若要查看 Azure IoT 中心如何完全映射到 Azure 安全基准，请参阅[完整的 Azure IoT 中心安全基线映射文件](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)。
...
指导：IoT 中心是一种多租户平台即服务 (PaaS)，不同的客户共用相同的计算、网络和存储硬件资源池。 IoT 中心的主机名会映射到一个公共终结点，该终结点具有可通过 Internet 以公开方式路由的 IP 地址。 不同的客户将共享此 IoT 中心公共终结点，广域网和本地网络中的 IoT 设备均可对其进行访问。 Microsoft 对该服务进行了设计，以完全隔离每个租户的数据，并不断努力确保获得此结果。
...
建立专用访问后，为 IoT 中心禁用公用网络访问权限，以提高安全性。 对特定 IoT 中心资源强制执行此网络级控件，以确保隔离。 为了使服务对其他使用公共路径的客户资源保持活动状态，其公共终结点需保持可解析状态，IP 地址需保持可发现状态，端口需保持打开状态。 不必为此担忧，因为 Microsoft 集成了多层安全保护，可确保租户间完全隔离。
...
- [Azure 中的租户隔离](https://docs.microsoft.com/azure/security/fundamentals/isolation-choices#tenant-level-isolation)

- [loT 网络最佳做法](https://docs.microsoft.com/azure/iot-fundamentals/security-recommendations#networking)
...
指导：使用 IoT 中心 IP 筛选规则阻止已知的恶意 IP。 另外，通过适用于 IoT 的 Azure 安全中心记录恶意尝试行为并针对其发出警报。
...
- [适用于 IoT 的 Azure 安全中心可疑 IP 地址通信](/azure/asc-for-iot/concept-security-alerts)

- [管理 Azure DDoS 防护基础版](../ddos-protection/ddos-protection-overview.md)
...
- [设置 Azure IoT 日志](https://docs.microsoft.com/azure/iot-hub/monitor-iot-hub-reference#resource-logs)
...
- [设置 Azure IoT 中心日志](https://docs.microsoft.com/azure/iot-hub/monitor-iot-hub-reference#resource-logs)
...
Azure 安全中心监视：[Azure 安全基准](/home/mbaldwin/docs/asb/azure-docs-pr/articles/governance/policy/samples/azure-security-benchmark.md)是安全中心的默认策略计划，是[安全中心建议](/home/mbaldwin/docs/asb/azure-docs-pr/articles/security-center/security-center-recommendations.md)的基础。 安全中心会自动启用与此控件相关的 Azure Policy 定义。 与此控件相关的警报可能需要相关服务的 [Azure Defender](/home/mbaldwin/docs/asb/azure-docs-pr/articles/security-center/azure-defender.md) 计划。
...
- [适用于 IoT 的 Azure 安全中心警报](/azure/asc-for-iot/concept-security-alerts)
...
指导：对于访问 IoT 中心的用户，请使用 Azure Active Directory (Azure AD) SSO。 请使用 Azure 安全中心标识和访问建议。
...
指导：启用 Azure Active Directory (Azure AD) 多重身份验证，以保护整体 Azure 租户，使所有服务受益。 IoT 中心服务不支持多重身份验证。
...
指导：对于需要提升权限的管理任务，请使用安全的特权访问工作站 (PAW)。

- [了解安全的特权访问工作站](https://4sysops.com/archives/understand-the-microsoft-privileged-access-workstation-paw-security-model/)
...
**指南**：对于访问 IoT 中心的用户，不支持条件访问。 若要缓解这种情况，请使用 Azure Active Directory (Azure AD) 命名位置仅允许从 IP 地址范围或国家/地区的特定逻辑分组访问整个 Azure 租户，从而使包括 IoT 中心在内的所有服务受益。
...
- [IoT 中心安全令牌](https://docs.microsoft.com/azure/iot-fundamentals/iot-security-deployment#iot-hub-security-tokens)
...
- [为 IoT 中心配置资源日志](https://docs.microsoft.com/azure/iot-hub/monitor-iot-hub#collection-and-routing)
...
指导：使用 Azure Active Directory (Azure AD) 标识保护功能配置对检测到的与用户标识相关的可疑操作的自动响应。 还可将数据引入 Azure Sentinel 以做进一步调查。
...
- [如何使用 IoT 中心的托管标识](https://docs.microsoft.com/azure/iot-hub/virtual-network-support#turn-on-managed-identity-for-iot-hub)
...
- [如何为 IoT 中心配置托管标识](https://docs.microsoft.com/azure/iot-hub/virtual-network-support#turn-on-managed-identity-for-iot-hub)
...
- [IoT 中心高可用性和灾难恢复](https://docs.microsoft.com/azure/iot-hub/iot-hub-ha-dr#achieve-cross-region-ha)
