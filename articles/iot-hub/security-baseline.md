...
内容按安全控制措施分组，这些控制措施根据适用于 Azure IoT 中心的 Azure 安全基准和相关指南定义。 排除了不适用于 Azure IoT 中心的 **控制措施**。

 
若要查看 Azure IoT 中心如何完全映射到 Azure 安全基准，请参阅[完整的 Azure IoT 中心安全基准映射文件](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)。
...
**指导**：IoT 中心是一种多租户平台即服务 (PaaS)，不同的客户共用相同的计算、网络和存储硬件资源池。 IoT 中心的主机名会映射到一个公共终结点，该终结点具有可通过 Internet 以公开方式路由的 IP 地址。 不同的客户将共享此 IoT 中心公共终结点，广域网和本地网络中的 IoT 设备均可对其进行访问。 Microsoft 根据每个租户的数据完全隔离的原则设计了该服务，并会持续努力确保此效果。
...
建立专用访问后，为 IoT 中心禁用公用网络访问以进一步提高安全性。 针对特定的 IoT 中心资源强制实施这种网络级控制，以确保隔离。 为了使服务对其他使用公共路径的客户资源保持活动状态，其公共终结点需保持可解析状态，IP 地址需保持可发现状态，端口需保持打开状态。 不必为此担忧，因为 Microsoft 集成了多层安全保护，可确保租户间完全隔离。
...
- [IoT 网络最佳做法](https://docs.microsoft.com/azure/iot-fundamentals/security-recommendations#networking)
...
还可以使用 Azure Active Directory (Azure AD) Privileged Identity Management 和 Azure 资源管理器来启用对管理帐户的即时访问。
...
**指导**：启用 Azure Active Directory (Azure AD) 多重身份验证，以保护整个 Azure 租户并受益于所有服务。 IoT 中心服务不提供多重身份验证支持。
...
**指导**：对于需要提升的特权的管理任务，请使用安全特权访问工作站 (PAW)。

- [了解安全特权访问工作站](https://4sysops.com/archives/understand-the-microsoft-privileged-access-workstation-paw-security-model/)
...
- [配置 IoT 中心的资源日志](https://docs.microsoft.com/azure/iot-hub/monitor-iot-hub#collection-and-routing)
...
每个 IoT 中心都有一个标识注册表，可用于在服务中创建各设备的资源。 可将单个或一组设备标识添加到允许列表或阻止列表，以便完全控制设备访问。
