...
还可以通过 Azure IoT 中心设备预配服务使用注册组 (DPS) 来处理设备到集线器的预配。 有关使用 DPS 预配 x.509 证书设备的详细信息，请参阅 [教程：使用注册组预配多个 x.509 设备](../iot-dps/tutorial-custom-hsm-enrollment-group-x509.md)。
...
X 公司需要向 IoT 中心注册 X.509 CA，IoT 中心将在智能 X 小组件进行连接时对其进行身份验证。 这个过程是一次性的，完成后即可对任何数量的智能 X 小组件设备进行身份验证和管理。 这是一个一次性过程，因为 CA 证书和设备证书之间的一对多关系是由 CA 证书或中间证书签名的。 此关系构成了使用 x.509 CA 身份验证方法的主要优点之一。 另一种方法是为每个智能 X 小组件设备上传单个证书指纹，但这会增加运营成本。
