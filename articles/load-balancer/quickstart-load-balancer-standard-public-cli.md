...
使用 [az network vnet create](/cli/azure/network/vnet#az_network_vnet_createt) 创建虚拟网络：
...
使用 [az network nsg rule create](/cli/azure/network/nsg/rule#az_network_nsg_rule_create) 创建网络安全组规则：
...
使用 [az network nic create](/cli/azure/network/nic#az_network_nic_create) 创建三个网络接口：
...
使用 [az network public-ip create](/cli/azure/network/public-ip#az_network_public_ip_create) 执行以下操作：
...
使用 [az network nic ip-config address-pool add](/cli/azure/network/nic/ip-config/address-pool#az_network_nic_ip_config_address_pool_add) 将虚拟机添加到后端池：
...
使用 [az network public-ip create](/cli/azure/network/public-ip#az_network_public_ip_create) 为出站连接创建单个 IP。  
...
使用 [az network public-ip create](/cli/azure/network/public-ip/prefix#az_network_public_ip_prefix_create) 为出站连接创建公共 IP 前缀。
...
使用 [az network lb frontend-ip create](/cli/azure/network/lb/frontend-ip#az_network_lb_frontend_ip_create) 创建新的前端 IP 配置：
...
使用 [az network lb address create](/cli/azure/network/lb/address-pool#az_network_lb_address_pool_create) 创建新的出站池：
...
使用 [az network lb outbound-rule create](/cli/azure/network/lb/outbound-rule#az_network_lb_outbound_rule_create) 为出站后端池创建新的出站规则：
...
使用 [az network nic ip-config address-pool add](/cli/azure/network/nic/ip-config/address-pool#az_network_nic_ip_config_address_pool_add) 将虚拟机添加到出站池：
...
使用 [az vm availability-set create](/cli/azure/vm/availability-set#az_vm_availability_set_create) 创建可用性集：
...
若要获取负载均衡器的公共 IP 地址，请使用 [az network public-ip show](/cli/azure/network/public-ip#az_network_public_ip_show)。 
