...
间隔值确定运行状况探测探测来自后端池实例的响应的频率。 如果运行状况探测失败，它会立即将后端池实例标记为运行不正常。 同样，在下次启动行状况探测时，运行状况探测会立即将后端池实例再次标记为正常。

我们可以通过一个示例进一步说明该行为，其中运行状况探测间隔已设置为 5 秒。 由于发送探测的时间与应用程序可能更改状态的时间不同步，因此运行状况探测反映应用程序状态所花的总时间可以划分为两种情况：

1. 如果应用程序在下一个探测抵达之前的那一刻开始生成超时探测响应，则这些事件的检测需要 5 秒，再加上应用程序发出超时信号到下一个探测抵达时的持续时间。  可以假设此项检测花费的时间略微超过 5 秒。
2. 如果应用程序在下一个探测抵达之后的那一刻开始生成超时的探测响应，则这些事件的检测只会在探测（和超时）抵达时加上额外 5 秒之后才开始。  可以假设此项检测花费的时间略少于 10 秒。

对于此示例，一旦发生检测，平台就会花费少量的时间对此更改做出反应。  这意味着，根据以下条件： 

1. 当应用程序开始更改状态时，并且
2. 检测到此更改时（发送下一个运行状况探测时），以及
3. 在整个平台中传达检测时 

可以假设对超时探测响应做出反应的时间介于对应用程序信号中的更改做出反应的最短时间（略微超过 5 秒）与最长时间（略微超过 10 秒）之间。  此示例旨在演示发生的情况，但是，无法准确预测超出此示例所示大致指导范围的持续时间。
...
公共和内部[标准负载均衡器](./load-balancer-overview.md)都通过 [Azure Monitor](./monitor-load-balancer.md) 将每个终结点和后端终结点的运行状况探测状态公开为多维指标。 这些指标可由其他 Azure 服务或合作伙伴应用程序使用。 

Azure Monitor 日志不适用于公共和内部基本负载均衡器。
