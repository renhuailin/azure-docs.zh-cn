...
使用 [New-AzLoadBalancer](/powershell/module/az.network/new-azloadbalancer) 创建标准负载均衡器。 以下示例使用前面步骤中创建的 IPv4 和 IPv6 前端 IP 配置、后端池和负载均衡规则创建名为 *myLoadBalancer* 的公共标准负载均衡器：
...
使用 [New-AzVirtualNetwork](/powershell/module/az.network/new-azvirtualnetwork) 创建虚拟网络。 以下示例创建包含 *mySubnet* 的名为 *dsVnet* 的虚拟网络：
