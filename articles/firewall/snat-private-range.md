...
- 若要将 Azure 防火墙配置为 **永远不** 考虑目标 ip 地址，请使用 **0.0.0.0/0** 作为专用 ip 地址范围。 通过此配置，Azure 防火墙永远不能将流量直接路由到 Internet。 

- 若要将防火墙配置为 **始终** 不考虑目标地址，请使用 **255.255.255.255/32** 作为专用 IP 地址范围。
