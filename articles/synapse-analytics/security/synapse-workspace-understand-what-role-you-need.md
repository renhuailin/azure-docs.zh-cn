...
创建笔记本或作业定义|工作区上的 Synapse 用户、Azure 所有者、参与者或读者</br> *要运行、发布或保存，需要其他权限*|读取
...
将对笔记本或作业定义的更改提交到 Git 工作分支|Git 权限|无
...
创建管道 |Synapse 用户 </br>[**_按注意事项 + Synapse 上的凭据用户_*]</br>发布或保存 * _Additional 需要权限|读取、凭据/UseSecret/操作
创建数据流、数据集或触发器 |Synapse 用户</br>*发布或保存时需要其他权限*|读取
...
创建触发器，包括立即触发|WorkspaceSystemIdentity 上的 Synapse 用户 + Synapse Credential 用户|读取、凭据/useSecret/操作
...
将新的、更新或删除的管道、数据流或触发器发布到服务|工作区上的 Synapse 项目发布服务器|管道/写入、删除</br>数据流写入，删除</br>触发器/写入，删除
