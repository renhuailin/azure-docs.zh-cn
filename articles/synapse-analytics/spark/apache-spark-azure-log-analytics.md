---
title: 使用 Log Analytics 收集和可视化指标和日志（预览版）
...
ms.locfileid: "122768119"
---
# <a name="tutorial-use-log-analytics-to-collect-and-visualize-metrics-and-logs-preview"></a>教程：使用 Log Analytics 收集和可视化指标和日志（预览版）
...
复制以下 Spark 配置，将其保存为“spark_loganalytics_conf.txt”，并填入以下参数：
...
> [!NOTE]
> 需要向将提交 Spark 应用程序的用户授予读取机密权限。 有关详细信息，请参阅[使用 Azure 基于角色的访问控制提供对密钥保管库密钥、证书和机密的访问权限](../../key-vault/general/rbac-guide.md)。
...
4. 在“创建机密”屏幕上，选择以下值：
   ...
   - 让其他值保留默认设置。 然后选择“创建”。
5. 复制以下 Spark 配置，将其保存为“spark_loganalytics_conf.txt”，并填入以下参数：
...
| spark.synapse.logAnalytics.keyVault.key.secret      | SparkLogAnalyticsSecret      | Log Analytics 工作区密钥的密钥保管库机密名称。                                                                                                                                      |
...
你可以将配置文件上传到 Azure Synapse Analytics 中的 Spark 池。 在 Synapse Studio 中：
...
> [!NOTE] 
>
> 提交到 Spark 池的所有 Spark 应用程序都会使用该配置设置将 Spark 应用程序指标和日志推送到指定工作区。

## <a name="submit-a-spark-application-and-view-the-logs-and-metrics"></a>提交 Spark 应用程序并查看日志和指标

以下是操作方法：

1. 将 Spark 应用程序提交到上一步中配置的 Spark 池。 可以使用以下任何一个方法来实现此操作：
    - 在 Synapse Studio 中运行笔记本。 
    - 在 Synapse Studio 中，通过 Spark 作业定义提交一个 Apache Spark 批处理作业。
    - 运行包含 Spark 活动的管道。

1. 转到指定的 Log Analytics 工作区，然后在 Spark 应用程序开始运行时查看应用程序指标和日志。
...
下面是查询 Spark 应用程序驱动程序和执行程序日志的示例：
