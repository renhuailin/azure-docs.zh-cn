...
若要写入专用 SQL 池外部表，该专用 SQL 池上必须存在 EXTERNAL DATA SOURCE 和 EXTERNAL FILE FORMAT。  有关详细信息，请参阅在专用 SQL 池中创建[外部数据源](/sql/t-sql/statements/create-external-data-source-transact-sql?view=azure-sqldw-latest&preserve-view=true)和[外部文件格式](/sql/t-sql/statements/create-external-file-format-transact-sql?view=azure-sqldw-latest&preserve-view=true)。  以下是在专用 SQL 池中创建外部数据源和外部文件格式的示例。
