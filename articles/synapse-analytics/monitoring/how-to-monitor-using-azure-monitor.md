...
若要访问这些指标，请完成 [Azure Monitor 数据平台](../../azure-monitor/data-platform.md)中的说明。
...
| CPUPercent                          | CPU 使用百分比             | 百分比 | 最大 (默认值) 、Min、Avg | SQL 池中所有节点的 CPU 使用率 |
| 连接                         | 连接                     | 计数   | Sum (默认值)   | SQL 池的总登录数计数 |
| ActiveQueries                      | 活动查询数                 | 计数   | Sum (默认值)    | 活动查询数。 如果使用此指标时不进行筛选和拆分，会显示系统上运行的所有活动查询 |
...
| **指标**                           | **度量值类别，显示名称**                  | **单位** | **聚合类型** | **说明**                |
|--------------------------------------|------------------------------------------|----------|----------------------|--------------------------------|
| BigDataPoolApplicationsEnded  | 已结束的 Apache Spark 应用程序数  | 计数 | Sum (默认值)  | 已结束的 Apache Spark 池应用程序数 |
| BigDataPoolAllocatedCores     | 分配到 Apache Spark 池的 Vcore 数                 | 计数 | 最大 (默认值) 、Min、Avg | 为 Apache Spark 池分配的 Vcore 数 |
