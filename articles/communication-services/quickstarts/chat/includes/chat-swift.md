...
可通过调用 `ChatThreadClient` 中的 `listMessages()` 方法来从聊天会话接收消息。 列出的消息包括系统消息和用户发送的消息。 有关可接收消息的类型的详细信息，请参阅[消息类型](https://docs.microsoft.com/azure/communication-services/concepts/chat/concepts#message-types)
