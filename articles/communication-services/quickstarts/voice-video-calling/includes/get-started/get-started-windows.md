...
ms.locfileid: "111593395"
---
本快速入门介绍如何使用适用于 Windows 的 Azure 通信服务通话 SDK 开始通话。

> [!NOTE]
> 在 [GitHub](https://github.com/Azure-Samples/communication-services-dotnet-quickstarts/tree/main/VoiceCalling) 上查找此快速入门的最终代码
...
- 安装通用 Windows 平台开发工作负荷的 [Visual Studio 2019](https://visualstudio.microsoft.com/downloads/)。 
...
在 Visual Studio 中，使用 **Blank App（通用 Windows）** 模板创建一个新项目，以设置单页通用 Windows 平台 (UWP) 应用。

:::image type="content" source="../../media/windows/create-a-new-project.png" alt-text="显示 Visual Studio 中“新建项目”窗口的屏幕截图。":::

### <a name="install-the-package"></a>安装包

右键单击你的项目，并选择`Manage Nuget Packages`安装`Azure.Communication.Calling` 。 

### <a name="request-access"></a>请求访问权限

转到 `Package.appxmanifest`，并单击 `Capabilities`。
勾选 `Internet (Client & Server)`，获取互联网的入站和出站访问权限。 勾选 `Microphone`，以访问麦克风的音频源。 

:::image type="content" source="../../media/windows/request-access.png" alt-text="显示请求在 Visual Studio 中访问互联网和麦克风的屏幕截图。":::

### <a name="set-up-the-app-framework"></a>设置应用框架

我们需要配置基本布局以附加逻辑。 为了发起出站呼叫，需要 `TextBox` 提供被调用方的用户 ID。 还需要一个 `Start Call` 按钮和一个 `Hang Up` 按钮。 打开你的项目的 `MainPage.xaml`，然后将 `StackPanel` 节点添加到 `Page`： 
...
通过用户访问令牌初始化 `CallAgent` 实例，这使我们可以发出和接收呼叫。 向 `InitCallAgent` 函数添加以下代码。 
...
你可以在 Visual Studio 中构建并运行代码。 请注意，对于解决方案平台，我们支持 `ARM64`、`x64` 和 `x86`。 
