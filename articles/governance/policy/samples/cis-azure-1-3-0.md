...
下文详细说明了 Azure Policy 法规符合性内置计划定义如何映射到 CIS Microsoft Azure 基础基准 1.3.0 的符合性域和控制措施 。
...
然后，请查找并选择 CIS Microsoft Azure 基础基准 1.3.0 法规符合性内置计划定义。

> [!IMPORTANT]
> 下面的每个控件都与一个或多个 [Azure Policy](../overview.md) 定义关联。
> 这些策略可以帮助你[评估控件的符合性](../how-to/get-compliance-data.md)；但是，控件与一个或多个策略之间通常不是 1:1 或完全匹配。 因此，Azure Policy 中的符合性仅引用策略定义本身；这并不能确保你完全符合某个控制措施的所有要求。 此外，符合性标准包含目前未由任何 Azure Policy 定义处理的控件。 因此，Azure Policy 中的符合性只是整体符合性状态的部分视图。 此符合性标准的符合性域、控制措施和 Azure Policy 定义之间的关联可能会随着时间的推移而发生变化。 若要查看更改历史记录，请参阅 [GitHub 提交历史记录](https://github.com/Azure/azure-policy/commits/master/built-in-policies/policySetDefinitions/Regulatory%20Compliance/CISv1_3_0.json)。
...
### <a name="ensure-that-azure-defender-is-set-to-on-for-app-service"></a>确保已为应用服务将 Azure Defender 设置为“启用”
...
### <a name="ensure-that-azure-defender-is-set-to-on-for-sql-servers-on-machines"></a>确保已为计算机上的 SQL 服务器将 Azure Defender 设置为“启用”
...
### <a name="ensure-that-azure-defender-is-set-to-on-for-key-vault"></a>确保已为 Key Vault 将 Azure Defender 设置为“启用”

ID：CIS Azure 2.8 所有权：客户 
...
### <a name="ensure-that-public-access-level-is-set-to-private-for-blob-containers"></a>确保为 Blob 容器将“公共访问级别”设置为“专用”

ID：CIS Azure 3.5 所有权：客户 
...
### <a name="ensure-the-storage-container-storing-the-activity-logs-is-not-publicly-accessible"></a>确保存储活动日志的存储容器不可公开访问
...
### <a name="ensure-that-the-expiration-date-is-set-on-all-keys"></a>确保已对所有密钥设置到期日期

**ID**：CIS Azure 8.1 所有权：客户
...
### <a name="ensure-that-the-expiration-date-is-set-on-all-secrets"></a>确保已对所有机密设置到期日期

**ID**：CIS Azure 8.2 所有权：客户
