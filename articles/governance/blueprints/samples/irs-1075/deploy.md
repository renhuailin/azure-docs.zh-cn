...
ms.locfileid: "100571906"
---
# <a name="deploy-the-irs-1075-blueprint-sample"></a>部署 IRS 1075 蓝图示例

若要部署 Azure 蓝图 IRS 1075 (Rev.11-2016) 蓝图示例，必须执行以下步骤：
...
1. 在“其他示例”下找到“IRS 1075 (Rev.11-2016)”蓝图示例，然后选择“使用此示例”。

1. 输入该蓝图示例的“基本信息”：

   - **蓝图名称**：提供 IRS 1075 (Rev.11-2016) 蓝图示例副本的名称。
...
|项目名称|项目类型|参数名称|说明|
|-|-|-|-|
|审核 IRS 1075 (Rev.11-2016) 控件并部署特定 VM 扩展以支持审核要求|策略分配|应为 VM 配置的 Log Analytics 工作区 ID|这是应为 VM 配置的 Log Analytics 工作区的 ID (GUID)。|
|审核 IRS 1075 (Rev.11-2016) 控件并部署特定 VM 扩展以支持审核要求|策略分配|应启用诊断日志的资源类型列表|用于审核是否未启用诊断日志设置的资源类型列表。 [Azure Monitor 诊断日志架构](../../../../azure-monitor/essentials/resource-logs-schema.md#service-specific-schemas)中提供了可接受的值。|
|审核 IRS 1075 (Rev.11-2016) 控件并部署特定 VM 扩展以支持审核要求|策略分配|应该从 Windows VM 管理员组中排除的用户的列表|以分号分隔的应从管理员本地组中排除的成员列表。 例如：管理员；myUser1；myUser2|
|审核 IRS 1075 (Rev.11-2016) 控件并部署特定 VM 扩展以支持审核要求|策略分配|应该包括在 Windows VM 管理员组中的用户的列表|以分号分隔的应包括在管理员本地组中的成员列表。 例如：管理员；myUser1；myUser2|
...
了解 IRS 1075 (Rev.11-2016) 蓝图示例的部署步骤后，请访问以下文章来了解蓝图和控制映射：
