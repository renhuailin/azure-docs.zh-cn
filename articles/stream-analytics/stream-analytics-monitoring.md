...
| CPU 使用率 (预览)        | 作业使用的 CPU 使用率的百分比。 如果此指标持续高于80%，则可能表示作业在 CPU 使用率上出现瓶颈，并可能会导致输入事件累积。 可以增加分配给作业的 SUs 数量，以减轻此类问题。 |
...
| 流单元利用率 %       | 作业使用的内存百分比。 如果 SU% 使用率持续超过80%，则水位线延迟会上升，而囤积事件的数量会增加，因此请考虑增加流式处理单位。 高利用率指示作业使用的资源数接近分配的最大资源数。 |
