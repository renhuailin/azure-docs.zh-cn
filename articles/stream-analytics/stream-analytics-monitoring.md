...
| CPU 利用率百分比(预览)       | 作业使用的 CPU 使用率百分比。 即使此值非常高（90% 或以上），也不应只根据此指标增加 SU 数量。 如果积压输入事件或水印延迟数量增加，可以使用此 CPU% 利用率指标来确定是否是 CPU 造成了瓶颈。 此指标可能间歇性出现峰值。 建议进行规模测试，以确定在 CPU 瓶颈导致输入积压或水印延迟增加之前的作业上限。 |
...
| 流单元利用率 %       | 作业使用的内存使用百分比。 如果 SU % 利用率持续超过 80%，则水印延迟增加，积压的事件数增加，请考虑增加流单元。 高利用率指示作业使用的资源数接近分配的最大资源数。 |
