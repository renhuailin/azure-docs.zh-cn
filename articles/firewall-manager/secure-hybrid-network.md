...
1. 对于“优先级”，请键入 **100**。
1. 对于“操作”，请选择“允许”。  
1. 在“规则”下，为“名称”键入 **AllowWeb**。 
1. 对于 **源**，请键入 **192.168.1.0/24**。
1. 对于“协议”，请选择“TCP”。 
1. 对于“目标端口”，请键入 **80**。
1. 对于“目标类型”，请选择“IP 地址”。
...
1. 对于“IPv4 地址空间”，请键入 **10.5.0.0/16**。
1. 在“子网名称”下，选择“默认值”。
1.  将“子网名称”更改为 **AzureFirewallSubnet**。 防火墙位于此子网中，子网名称 **必须** 是 AzureFirewallSubnet。
...
1. 对于“IPv4 地址空间”，请键入“10.6.0.0/16” 。
1. 在“子网名称”下，选择“默认值”。
1. 将“子网名称”更改为 **SN-Workload**。
1. 对于“子网地址范围”，请键入“10.6.0.0/24” 。 
1. 接受其他默认设置，然后选择“保存”。
1. 选择“查看 + 创建”。
1. 选择“创建”。
...
1. 对于“IPv4 地址空间”，请键入“192.168.0.0/16” 。
1. 在“子网名称”下，选择“默认值”。
1. 将“子网名称”更改为 **SN-Corp**。
1. 对于“子网地址范围”，请键入“192.168.1.0/24” 。 
1. 接受其他默认设置，然后选择“保存”。
2. 选择“添加子网”。
3. 对于“子网名称”，请键入 **GatewaySubnet**。
...
1. 在“虚拟网络”下，选中“VNet-hub”对应的复选框。 
1. 选择“管理安全性”，然后选择“部署具有防火墙策略的防火墙”。 
1. 在“转换虚拟网络”页上的“防火墙策略”下，选中“Pol-Net01”对应的复选框。  
1. 选择“下一步: 查看 + 确认”
1. 检查详细信息，然后选择“确认”。
...
此步骤创建从中心虚拟网络到本地虚拟网络的连接。 示例中引用了共享密钥。 可以对共享密钥使用自己的值。 共享密钥必须与两个连接匹配，这一点非常重要。 创建连接需要花费一段时间。
...
5. 选择 **添加** 。
   ...
   :::image type="content" source="media/secure-hybrid-network/firewall-peering.png" alt-text="VNet 对等互连":::
...
3. 选择“路由表”。
4. 选择“创建”。
1. 选择“FW-Hybrid-Test”作为资源组。
1. 对于“区域”，请选择“美国东部”。
1. 对于“名称”，请键入 **UDR-Hub-Spoke**。
1. 选择“查看 + 创建”  。
1. 选择“创建”。
1. 创建路由表后，请选择它以打开路由表页。
1. 在左栏中选择“路由”。
1. 选择 **添加** 。
1. 键入 **ToSpoke** 作为路由名称。
1. 键入 **10.6.0.0/16** 作为地址前缀。
1. 选择“虚拟设备”作为下一跃点类型。
1. 键入前面记下的防火墙专用 IP 地址作为下一跃点地址。
1. 选择“确定”。
