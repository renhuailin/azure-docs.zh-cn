...
description: 了解如何使用 Azure 开放数据集中的 OJ Sales Simulated 数据集。
...
[查看原始数据集说明](http://www.cs.unitn.it/~taufer/QMMA/L10-OJ-Data.html)或[下载数据集](http://www.cs.unitn.it/~taufer/Data/oj.csv)。
...
> [!TIP]
> [改为下载笔记本](https://opendatasets-api.azure.com/discoveryapi/OpenDataset/DownloadNotebook?serviceType=AzureNotebooks&package=azureml-opendatasets&registryId=sample-oj-sales-simulated)。
...
### <a name="read-data-from-azure-open-datasets"></a>读取 Azure 开放数据集中的数据
...
### <a name="upload-the-individual-datasets-to-blob-storage"></a>将单个数据集上传到 Blob 存储
...
### <a name="create-the-file-dataset"></a>创建文件数据集 
需要定义数据的路径以创建 [FileDataset](/python/api/azureml-core/azureml.data.file_dataset.filedataset)。 
...
### <a name="register-the-file-dataset-to-the-workspace"></a>将文件数据集注册到工作区 
...
> [!TIP]
> [改为下载笔记本](https://opendatasets-api.azure.com/discoveryapi/OpenDataset/DownloadNotebook?serviceType=AzureDatabricks&package=azureml-opendatasets&registryId=sample-oj-sales-simulated)。
