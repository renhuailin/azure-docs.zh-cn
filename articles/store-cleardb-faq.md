...
* [Azure VM 上运行的 MySQL 群集](https://github.com/azure/azure-quickstart-templates/tree/master/application-workloads/mysql/mysql-replication)
...
* [企业协议 (EA)](https://azure.microsoft.com/pricing/enterprise-agreement/)：每季以单独的合并发票按照 EA 向客户从 Azure 市场（第三方）购买的所有商品收费。 对于任何市场购买，均以 Azure 预付款（以前称为“货币承诺付款”）以外的方式付费。 请注意，Azure 应用商店目前无法供在阿塞拜疆、克罗地亚、挪威和波多黎各注册的客户使用。 
...
默认数据库选项是 Titan，需 3.50 美元。 我们不会在数据库创建期间显示成本，因此，你可能会错误地购买到不想要的数据库。 我们正在设法改善此体验，但在那之前，需要先检查为 Web 应用和数据库选择的定价层，然后再单击“创建”并开始部署资源。
...
## <a name="in-which-countriesregions-are-cleardb-premium-mysql-clusters-supported"></a>支持 ClearDB Premium MySQL 群集的国家/地区有哪些？
...
不知道。 无法创建其他用户，但可以在自己的 ClearDB 数据库群集上创建其他数据库。  
...
使用 MySQL (ClearDB) 创建新 Web 应用或创建 MySQL 数据库 (ClearDB) 时，所选订阅决定服务的支付方式。 对于 EA 订阅，不会阻止用户在 Azure 门户采购 ClearDB 等第三方服务。 EA 订阅不按 Azure 预付款计费，而是按季度以欠款形式计费。 EA 客户必须设置付款方式，例如信用卡，才能支付任何第三方市场服务。

## <a name="where-can-i-see-the-charges-for-cleardb-resources-in-an-ea-subscription"></a>在何处可查看 EA 订阅中 ClearDB 资源的计费情况？
直接 EA 客户可以在企业门户上看到 Azure 市场费用。 请注意，所有市场购买和消费都不按 Azure 预付款计费，而是按季度以欠款形式计费。 EA 客户需要直接付费给第三方服务提供商，或者通过启用其 EA 帐户的付款方式（例如信用卡）完成付费。
