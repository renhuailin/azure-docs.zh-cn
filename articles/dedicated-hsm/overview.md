...
许多组织都有严格的行业法规，规定必须将加密密钥存储在通过 [FIPS 140-2 级别 3](https://csrc.nist.gov/publications/detail/fips/140/2/final) 验证的 HSM 中。 Azure 专用 HSM 和新的单租户产品/服务 [Azure Key Vault 托管 HSM](../key-vault/managed-hsm/index.yml) 可帮助各个行业（例如，金融服务行业、政府机构等）的客户达到 FIPS 140-2 级别 3 要求。 同时，Microsoft 的多租户 [Azure Key Vault](../key-vault/index.yml) 服务目前使用通过 FIPS 140-2 级别 2 验证的 HSM。 
...
Azure 专用 HSM 是否适合取决于你能否做出需求和妥协的复杂组合。 例如 FIPS 140-2 级别 3 要求。 这项要求很常见，Azure 专用 HSM 和新的单租户产品/服务 [Azure Key Vault 托管 HSM](../key-vault/managed-hsm/index.yml) 是目前满足此要求的唯一选择。 如果这些强制要求与你不相关，你通常可以在 Azure Key Vault 和 Azure 专用 HSM 之间进行选择。 在做出决定之前评估要求。
