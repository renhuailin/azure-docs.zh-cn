...
- 填补遮挡区域。 尽管 Azure Kinect DK 数据转换生成的是单个图像，但两个相机（深度和 RGB 相机）实际上保持着较小的一段距离。 这种偏移使得遮挡成为可能。 遮挡是指前景对象阻挡了设备上两个相机之一的背景对象的部分视角。 在生成的彩色图像中，前景对象看上去像是在背景对象上投射了一个阴影。  
   ...
   ![图中显示了两个定向到同一点的相机，其中一个被挡住。](./media/occlusion.png)  
   附加的同步设备可以提供遮挡的数据。
- 扫描三维对象。
...
> [!NOTE]  
> “同步输出”是 RGB 相机的“VSync”。 所有设备的时间戳都设置为零并递增计数。 Microsoft 尚未确定同步脉冲的最小和最大宽度的特征，建议模拟由 Azure Kinect DK 的“同步输出”生成的脉冲。
...
> [!NOTE]  
> 实际脉冲宽度为 125us，但我们声明 160us 以留出一些余地。 以 NFOV UNBINNED 为例，每个 125us 脉冲之后是 1450us 空闲。 把这些加起来 - (9 x 125) + (8 x 1450) - 得到的曝光时间为 12.8 毫秒。 可以交错曝光两台设备的秘诀是让第二个相机的第一个脉冲落在第一个相机的第一个空闲期内。 第一个相机和第二个相机之间的延迟可能低至 125us（脉冲宽度），但是我们建议留出一些余地，因此我们声明 160us。 给定 160us，可以让最多 10 个相机的曝光时间处于交错状态。
