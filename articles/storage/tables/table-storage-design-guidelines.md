...
description: 了解设计 Azure 存储表服务的相关准则，以有效地支持读写操作。
services: storage
ms.service: storage
author: tamram
ms.author: tamram
...
* ***在查询中同时指定 PartitionKey 和 RowKey。** _ _点查询 * 这类查询是最高效的表服务查询。  
* ***请考虑存储实体的重复副本。*** 表存储比较便宜，因此请考虑多次存储同一实体（使用不同键）以实现更高效的查询。  
* ***请考虑反规范化处理数据。*** 表存储比较便宜，因此请考虑反规范化数据。 例如，存储摘要实体，以便对聚合数据的查询只需访问单个实体。  
* ***使用复合键值。** _只有_ *PartitionKey** 和 **RowKey**。 例如，使用复合键值来启用对实体的备用键控访问路径。  
