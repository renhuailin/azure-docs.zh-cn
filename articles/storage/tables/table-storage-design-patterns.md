...
![员工实体图，其中客户端应用程序可使用点查询，通过部门名称和员工 ID（PartitionKey 和 RowKey 值）检索单个员工实体。](media/storage-table-design-guide/storage-table-design-IMAGE06.png)
...
![显示具有不同 RowKey 值的员工实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE07.png)
...
* 要通过以字母“a”开头的邮件地址查找销售部门中的所有雇员，请使用：$filter=(PartitionKey eq 'Sales') and (RowKey ge 'email_a') and (RowKey lt 'email_b')  
  ...
  上述示例中使用的筛选器语法源自表服务 REST API，有关详细信息，请参阅[查询实体](/rest/api/storageservices/Query-Entities)。  
...
* 不一定需要重复实体的所有属性。 例如，如果使用 **RowKey** 中的电子邮件地址查找实体的查询始终不需要员工的年龄，则这些实体可具有以下结构：
   ...
   ![员工实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE08.png)
...
![员工实体结构图。在使用该结构时，客户端应用程序可使用点查询，通过部门名称和员工 ID（PartitionKey 和 RowKey 值）检索单个员工实体。](media/storage-table-design-guide/storage-table-design-IMAGE09.png)
...
![显示具有主索引的员工实体和具有辅助索引的员工实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE10.png)
...
* 不一定需要重复实体的所有属性。 例如，如果使用 **RowKey** 中的电子邮件地址查找实体的查询始终不需要员工的年龄，则这些实体可具有以下结构：
   ...
   ![显示具有辅助索引的员工实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE11.png)
...
![最终一致性的解决方案图示](media/storage-table-design-guide/storage-table-design-IMAGE12.png)
...
![员工实体结构图，其中客户端应用程序可通过使用部门名称和员工 ID（PartitionKey 和 RowKey）高效检索单个员工实体。](media/storage-table-design-guide/storage-table-design-IMAGE13.png)
...
![显示带有包含相同姓氏的员工 ID 列表的字符串的员工实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE14.png)
...
![部门实体和员工实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE16.png)

### <a name="solution"></a>解决方案

不是将数据存储在两个不同的实体中，而是对数据进行反规范化，并在部门实体中保留经理详细信息的副本。 例如：  

![已反规范化且已合并的部门实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE17.png)
...
![员工实体结构图，你应该使用该结构将员工实体存储在表存储中。](media/storage-table-design-guide/storage-table-design-IMAGE18.png)

还需要存储有关员工为组织工作的每年的评价和绩效的历史数据，并且需要能够按年份访问此信息。 一种选择是创建另一个表，该表存储具有以下结构的实体：  

![员工评价实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE19.png)
...
![具有复合键的员工实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE20.png)
...
![登录尝试实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE21.png)
...
![消息统计实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE22.png)
...
![显示具有单独属性的消息统计实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE23.png)
...
![显示具有 Rowkey 01 的消息统计实体和具有 Rowkey 02 的消息统计实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE24.png)
...
![显示具有用于指向 Blob 存储的照片的字符串的员工实体的插图](media/storage-table-design-guide/storage-table-design-IMAGE25.png)
...
通过使用 filter 子句指定查询，可实现检索多个实体。 若要避免表扫描，应始终在 filter 子句中包括 PartitionKey 值，如有可能也包括 RowKey 值以避免表和分区扫描。 表服务支持在 filter 子句中使用一组有限的比较运算符（大于、大于等于、小于、小于等于、等于和不等于）。
...
针对表服务的查询一次最多可以返回 1,000 个实体，并且可以执行时间最长为五秒。 如果结果集包含超过 1,000 个的实体，则当查询未在 5 秒内完成或者查询跨越分区边界时，表服务将返回一个继续标记，客户端应用程序使用该标记可以请求下一组实体。 若要深入了解继续标记的工作方式，请参阅 [Query Timeout and Pagination](/rest/api/storageservices/Query-Timeout-and-Pagination)（查询超时和分页）。  

如果使用的是表客户端库，当它从表服务返回实体时，可以自动处理延续令牌。 以下 C# 代码示例使用表客户端库，在表服务于响应中返回延续令牌时将自动处理它们：  
...
第二个选项是使用 TableEntity 类型（属性包）而不是具体的 POCO 实体类型，该选项无需序列化实体和将实体反序列化为 .NET 类型，因此还可提升性能。 以下 C# 代码可能会从表中检索多个不同类型的实体，但会将所有实体都作为 TableEntity 实例返回。 然后，它使用 **EntityType** 属性确定每个实体的类型：  
...
无需知道实体的类型就可删除该实体，在插入实体时始终知道该实体的类型。 但是，可使用 TableEntity 类型来更新实体，无需知道其类型且无需使用 POCO 实体类。 以下代码示例检索单个实体，并在更新该实体前检查 **EmployeeCount** 属性是否存在。  
