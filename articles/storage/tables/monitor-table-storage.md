...
Azure 表存储使用 [Azure Monitor](../../azure-monitor/overview.md) 来创建监视数据，Azure Monitor 是 Azure 中的一项完整堆栈监视服务。 Azure Monitor 提供了一整套用于监视 Azure 资源以及其他云和本地资源的功能。 
...
Azure 表存储会收集与其他 Azure 资源类型相同的监视数据，如[监视 Azure 资源中的数据](../../azure-monitor/essentials/monitor-azure-resource.md#monitoring-data)中所述。 
...
1. 选中“存档到存储帐户”复选框，然后单击“配置”按钮。
   ...
   > [!div class="mx-imgBorder"]   
   > ![“诊断设置”页的存档存储](media/monitor-table-storage/diagnostic-logs-settings-pane-archive-storage.png)
...
2. 在“选择事件中心”窗格中，选择要将日志流式传输到的事件中心的命名空间、名称和策略名称。 
   ...
   > [!div class="mx-imgBorder"]
   > ![“诊断设置”页的事件中心](media/monitor-table-storage/diagnostic-logs-settings-pane-event-hub.png)
...
1. 选中“发送到 Log Analytics”复选框，选择 Log Analytics 工作区，然后单击“保存”按钮。
   ...
   > [!div class="mx-imgBorder"]   
   > ![“诊断设置”页的 Log Analytic](media/monitor-table-storage/diagnostic-logs-settings-pane-log-analytics.png)
...
将此代码片段中的 `<storage-service-resource--id>` 占位符替换为表服务的资源 ID。 通过打开存储帐户的“属性”页，可在 Azure 门户中找到资源 ID。
...
若要详细了解如何将资源日志存档到 Azure 存储，请参阅 [Azure 资源日志](../../azure-monitor/essentials/resource-logs.md#send-to-azure-storage)。
...
若要详细了解如何将资源日志发送到事件中心，请参阅 [Azure 资源日志](../../azure-monitor/essentials/resource-logs.md#send-to-azure-event-hubs)。
...
将此代码片段中的 `<storage-service-resource--id>` 占位符替换为表存储服务的资源 ID。 通过打开存储帐户的“属性”页，可在 Azure 门户中找到资源 ID。
...
有关所有 Azure Monitor 支持指标（包括 Azure 表存储）的列表，请参阅 [Azure Monitor 支持的指标](../../azure-monitor/essentials/metrics-supported.md)。
...
你可以列出存储帐户或表存储服务的指标定义。 请使用 [Get-AzMetricDefinition](/powershell/module/az.monitor/get-azmetricdefinition) cmdlet。
...
你可以读取存储帐户或表存储服务的帐户级别指标值。 使用 [Get-AzMetric](/powershell/module/Az.Monitor/Get-AzMetric) cmdlet。
...
你可以列出存储帐户或表存储服务的指标定义。 使用 [az monitor metrics list-definitions](/cli/azure/monitor/metrics#az_monitor_metrics_list_definitions) 命令。
...
你可以读取存储帐户或表存储服务的指标值。 请使用 [az monitor metrics list](/cli/azure/monitor/metrics#az_monitor_metrics_list) 命令。
...
若要详细了解这些日志中显示的字段的参考，请参阅 [Azure 表存储监视数据参考](monitor-table-storage-reference.md)。
...
仅在针对服务终结点发出请求时才会创建日志条目。 例如，如果存储帐户的表终结点中存在活动，而 Blob 或队列终结点中没有该活动，则仅创建与表服务有关的日志。 Azure 存储日志包含有关成功和失败的存储服务请求的详细信息。 可以使用该信息监视各个请求和诊断存储服务问题。 将最大程度地记录请求。
...
不会记录表存储服务本身发出的请求，如创建或删除日志。 若要查看所记录数据的完整列表，请参阅[存储记录的操作和状态消息](/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages)和[存储日志格式](monitor-table-storage-reference.md)。
...
你可以使用安全信息和事件管理以及监视工具来访问和读取发送到事件中心的日志数据。 有关详细信息，请参阅 [Azure 资源日志](../../azure-monitor/essentials/resource-logs.md#send-to-azure-event-hubs)。
...
有关详细信息，请参阅[将 Azure 监视数据流式传输到事件中心和外部合作伙伴](../../azure-monitor/essentials/stream-monitoring-data-event-hubs.md)。

数据存储在 StorageTableLogs 表中。 

#### <a name="sample-kusto-queries"></a>示例 Kusto 查询

你可以在“日志搜索”栏中输入下面这些查询，以帮助监视表存储。 这些查询使用[新语言](../../azure-monitor/logs/log-query-overview.md)。
...
- 如需查看 Azure 表存储创建的日志和指标的参考，请参阅 [Azure 表存储监视数据参考](monitor-table-storage-reference.md)。
