...
>
> 通过允许来自托管服务实例的子网的流量，可以授予对在 VNet 内运行的 Azure 服务相应的访问权限。 你还可以通过下面所述的异常机制启用有限数量的方案。 若要通过 Azure 门户访问存储帐户中的数据，需要从设置的受信任的边界（IP 或 VNet）内的计算机进行访问。
...
你可以在应用了网络规则的存储帐户中使用非托管磁盘，通过创建一个例外来备份和还原 Vm。 本文的 [管理例外](#manage-exceptions) 部分介绍了此过程。 防火墙例外不适用于托管磁盘，因为它们已由 Azure 托管。
...
4. 若要使用新网络规则授予对虚拟网络的访问权限，请在 " **虚拟** 网络" 下选择 " **添加现有虚拟网络**"，选择 " **虚拟网络** 和 **子网** " 选项，然后选择 " **添加**"。 若要创建新的虚拟网络并向其授予访问权限，请选择“添加新的虚拟网络”。 提供创建新的虚拟网络所需的信息，然后选择“创建”。
...
5. 若要删除 IP 网络规则，请选择地址范围旁的 "垃圾桶" 图标。
...
## <a name="grant-access-from-azure-resource-instances-preview"></a> (预览版授予 Azure 资源实例的访问权限) 

在某些情况下，应用程序可能依赖于无法通过虚拟网络或 IP 地址规则隔离的 Azure 资源。 但是，您仍要保护仅存储帐户对应用程序的 Azure 资源的访问权限。 可以通过创建资源实例规则来配置存储帐户，以允许访问某些 Azure 服务的特定资源实例。 
...
安装最新版本的 PowershellGet 模块。 然后，关闭并重新打开 PowerShell 控制台。

```powershell
install-Module PowerShellGet –Repository PSGallery –Force  
```

安装 **Az** preview module。
...
你可以使用 Azure CLI 命令来添加或删除资源网络规则。

#### <a name="install-the-preview-extension"></a>安装预览扩展
...
2. 然后，使用以下命令验证你安装的 Azure CLI 的版本是否为 `2.13.0` 或更高版本。
...
某些 Azure 服务通过无法包含在网络规则中的网络运行。 可向其中的一部分受信任 Azure 服务授予对存储帐户的访问权限，同时对其他应用保持使用网络规则。 这些受信任的服务将使用强身份验证安全地连接到你的存储帐户。

可以通过创建网络规则例外来授予对受信任的 Azure 服务的访问权限。 有关分步指南，请参阅本文的 [管理例外](#manage-exceptions) 部分。

当你向受信任的 Azure 服务授予访问权限时，你将授予以下访问权限类型：

- 对订阅中注册的资源的选择操作的受信任访问。
- 基于系统分配的托管标识对资源进行受信任的访问。

<a id="trusted-access-resources-in-subscription"></a>

### <a name="trusted-access-for-resources-registered-in-your-subscription"></a>已在订阅中注册的资源的受信任访问

某些服务的资源 **在注册到订阅** 后，可在 **同一订阅** 中访问存储帐户以执行特定的操作，例如写入日志或备份。  下表描述了每个服务和允许的操作。 
...
### <a name="trusted-access-based-on-system-assigned-managed-identity"></a>基于系统分配的托管标识的可信访问

下表列出了可访问你的存储帐户数据的服务（如果为这些服务的资源实例授予了相应的权限）。 若要授予权限，你必须 [将 Azure 角色显式分配](storage-auth-aad.md#assign-azure-roles-for-access-rights) 给每个资源实例的 [系统分配的托管标识](../../active-directory/managed-identities-azure-resources/overview.md) 。 在这种情况下，实例的访问范围对应于分配给托管标识的 Azure 角色。 

> [!TIP]
> 授予对特定资源的访问权限的建议方法是使用资源实例规则。 若要授予对特定资源实例的访问权限，请参阅本文 [ () 预览版中的 "授予访问权限 ](#grant-access-specific-instances) "。
...
| Azure 认知服务       | CognitiveService/帐户    | 使认知服务能够访问存储帐户。 |
...
| Azure Migrate                  | Microsoft. 迁移/migrateprojects      | 允许通过 Azure Migrate 访问存储帐户。 |
| Azure Purview                  | 监控范围/帐户             | 允许监控范围访问存储帐户。 |
| Azure 远程渲染         | Microsoft.MixedReality/remoteRenderingAccounts | 允许通过远程呈现访问存储帐户。 |
| Azure Site Recovery            | Microsoft.RecoveryServices/vaults      | 允许通过 Site Recovery 访问存储帐户。 |
| Azure SQL 数据库             | Microsoft.Sql                          | 允许将审核数据 [写入](../../azure-sql/database/audit-write-storage-account-behind-vnet-firewall.md) 防火墙后面的存储帐户。 |
| Azure Synapse Analytics        | Microsoft.Sql                          | 允许使用) 中的 COPY 语句或 PolyBase (或 `openrowset` 无服务器池中的函数和外部表，从特定的 SQL 数据库导入和导出数据。 [了解详细信息](../../azure-sql/database/vnet-service-endpoint-rule-overview.md)。 |
...
在某些情况下，需要从网络边界外访问读取资源日志和指标。 配置受信任的服务对存储帐户的访问权限时，可以通过创建网络规则例外来允许对日志文件、度量值表或两者进行读访问。 有关分步指南，请参阅下面的 **管理异常** 部分。 若要了解有关使用存储分析的详细信息，请参阅 [使用 Azure 存储分析收集日志和指标数据](./storage-analytics.md)。 
