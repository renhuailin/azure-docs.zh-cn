---
title: 使用 Azure Monitor 存储见解监视 Azure 存储服务 | Microsoft Docs
description: 本文介绍了 Azure Monitor 的存储见解功能，可便于存储管理员快速了解自己 Azure 存储帐户存在的性能和利用率问题。
ms.topic: conceptual
author: normesta
ms.author: normesta
ms.date: 07/11/2021
...
ms.locfileid: "128606126"
---
# <a name="monitoring-your-storage-service-with-azure-monitor-storage-insights"></a>使用 Azure Monitor 存储见解监视存储服务

通过提供 Azure 存储服务性能、容量和可用性的统一视图，存储见解提供对 Azure 存储帐户的全面监视。 可以通过两种方式来观察存储容量和性能：直接在存储帐户中查看，或在 Azure Monitor 中跨存储帐户组查看。

本文可帮助你了解存储见解提供的体验，此体验不仅可便于大规模获得关于存储帐户的运行状况和性能的可操作见解，还可便于关注热点和诊断延迟、限制和可用性问题。

## <a name="introduction-to-storage-insights"></a>存储见解简介

在深入了解此体验之前，你应该了解它如何呈现和可视化信息。 无论你是直接从存储帐户选择存储功能，还是从 Azure Monitor 中选择，存储见解都提供一致的体验。
...
此功能不需要启用或配置任何设置，存储帐户的存储指标是默认收集的。 如果不熟悉可用于 Azure 存储的指标，请查阅 [ Azure 存储指标](../blobs/monitor-blob-storage.md)，以查看 Azure 存储指标的说明和定义。
...
> [!NOTE]
> 若要详细了解哪些错误可以在报告中显示，请参阅[响应类型架构](../blobs/monitor-blob-storage-reference.md#metrics-dimensions)，并查找 ServerOtherError、ClientOtherError、ClientThrottlingError 等响应类型。 根据所选的存储帐户，如果报告的错误超过三种类型，则其他所有错误都在“其他”类别下表示。
...
## <a name="customize-storage-insights"></a>自定义存储见解
...
如需常规故障排除指南，请参阅专用的基于工作簿的见解[故障排除文章](../../azure-monitor/insights/troubleshoot-workbooks.md)。

本部分将帮助你诊断和排查在使用存储见解时可能会遇到的一些常见问题。 使用下面的列表来查找与具体问题相关的信息。

### <a name="resolving-performance-capacity-or-availability-issues"></a>解决性能、容量或可用性问题

若要获取帮助来排查在使用存储见解时遇到的任何存储相关问题，请参阅 Azure 存储[故障排除指南](storage-monitoring-diagnosing-troubleshooting.md#troubleshooting-guidance)。
...
### <a name="how-to-analyze-and-troubleshoot-the-data-shown-in-storage-insights"></a>如何对存储见解中显示的数据进行分析和故障排除？

 若要详细了解如何分析存储见解中显示的 Azure 存储数据并排查其中的问题，请参阅[监视、诊断和排查 Microsoft Azure 存储的问题](storage-monitoring-diagnosing-troubleshooting.md)一文。
...
- 配置[指标警报](../../azure-monitor/alerts/alerts-metric.md)和[服务运行状况通知](../../service-health/alerts-activity-log-service-notifications-portal.md)来设置自动警报，以帮助检测问题。

- 查看[使用 Azure Monitor 工作簿创建交互式报表](../../azure-monitor/visualize/workbooks-overview.md)，了解工作簿旨在支持的方案、创作新报表和自定义现有报表的方式，以及更多信息。
