...
1. **方案标识符**：`abfs` 协议用作方案标识符。 如果在末尾添加一个“s”(abfs<b><i>s</i></b>)，则无论选择何种身份验证方法，ABFS Hadoop 客户端驱动程序将<i>始终</i>使用传输层安全性 (TLS)。 如果选择 OAuth 作为身份验证，则即使指定了“abfs”而不是“abfss”，客户端驱动程序也始终使用 TLS，因为 OAuth 仅依赖于 TLS 层。 最后，如果选择使用较早的存储帐户密钥方法，则客户端驱动程序会将“abfs”解释为你不希望使用 TLS。 
