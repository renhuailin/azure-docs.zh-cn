...
| 数据保护选项 | 保护帐户不被删除 | 保护容器不被删除 | 保护 blob 不被删除 | 保护 blob 不被覆盖 |
|--|--|--|--|--|
| Azure 资源管理器锁定 | 是 | 否<sup>1</sup> | 否 | 否 |
| 容器上的不可变性策略 | 是<sup>2</sup> | 是 | 是 | 是 |
| 容器软删除 | 否 | 是 | 否 | 否 |
| Blob 版本管理<sup>3</sup> | 否 | 否 | 是 | 是 |
| Blob 软删除<sup>3</sup> | 否 | 否 | 是 | 是 |
| 时间点还原<sup>3</sup> | 否 | 否 | 是 | 是 |
| Blob 快照 | 否 | 否 | 否 | 是 |
| 将数据复制到第二个帐户的自创解决方案<sup>4</sup> | 否 | 是 | 是 | 是 |

<sup>1</sup> Azure 资源管理器锁定无法保护容器不被删除。<br />
<sup>2</sup> 当合法保留或锁定的基于时间的保留策略对某个容器生效时，存储帐户也会受到保护，无法删除。<br />
<sup>3</sup> 目前不支持 Data Lake Storage 工作负载。<br />
<sup>4</sup> blob 存储和 Data Lake Storage 工作负载支持的选项是AzCopy 和 Azure 数据工厂。 仅 blob 存储工作负载支持对象复制。<br />
...
| Blob（任何类型） | 恢复软删除的 blob<sup>1</sup><br />[了解详细信息...](soft-delete-blob-enable.md) | 已启用 blob 软删除，且软删除保留间隔尚未过期。 |
