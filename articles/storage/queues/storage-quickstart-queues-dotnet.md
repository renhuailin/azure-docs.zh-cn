...
创建 [`QueueClient`](/dotnet/api/azure.storage.queues.queueclient) 类的实例。 然后，通过调用 [`CreateAsync`](/dotnet/api/azure.storage.queues.queueclient.createasync) 方法，在存储帐户中创建队列。
...
通过调用 [`PeekMessagesAsync`](/dotnet/api/azure.storage.queues.queueclient.peekmessagesasync) 方法，查看队列中的消息。 此方法从队列前面检索一条或多条消息，但不更改消息的可见性。
...
通过调用 [`UpdateMessageAsync`](/dotnet/api/azure.storage.queues.queueclient.updatemessageasync) 方法来更新消息的内容。 此方法可以更改消息的可见性超时和内容。 消息内容必须是最大为 64 KB 的 UTF-8 编码字符串。 除了该消息的新内容，还会传入代码中之前保存的 `SendReceipt` 中的值。 `SendReceipt` 值标识要更新的消息。
