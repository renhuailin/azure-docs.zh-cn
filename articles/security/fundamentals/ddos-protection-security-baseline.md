...
ms.locfileid: "102615289"
---
# <a name="azure-security-baseline-for-azure-ddos-protection-standard"></a>Azure DDoS 防护标准的 Azure 安全基线

此安全基线将 [Azure 安全基准](../benchmarks/overview.md)中的指南应用到 Azure DDoS 防护标准。 Azure 安全基准提供有关如何在 Azure 上保护云解决方案的建议。 内容分为 Azure 安全基准定义的安全控制和适用于 DDoS 防护的相关指南。 不适用于 DDoS 防护的控制措施已排除。 若要查看 DDoS 防护如何完全映射到 Azure 安全基准，请参阅[完整的 DDoS 防护安全基线映射文件](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)。
...
- [有关如何访问针对 DDoS 防护标准服务的遥测、日志和攻击分析的信息](../../ddos-protection/telemetry.md)
...
此外，Azure Active Directory (AD) 具有必须显式分配且可查询的内置角色。 使用 Azure AD PowerShell 模块执行即席查询，以发现属于管理组成员的帐户。
...
指导：使用启用了 Azure AD 多重身份验证 (MFA) 的安全的 Azure 托管工作站来登录并配置 Azure 客户密码箱请求。
...
指导：使用 Azure AD 命名位置，仅允许从 IP 地址范围或国家/地区的特定逻辑分组访问 Azure 门户。
...
- [在 Azure DDoS 防护中管理 Azure 基于角色的访问控制 (Azure RBAC)](../../ddos-protection/manage-ddos-protection.md)
