...
ms.locfileid: "102504910"
---
# <a name="azure-security-baseline-for-customer-lockbox-for-microsoft-azure"></a>适用于 Microsoft Azure 的客户密码箱的 Azure 安全基线

适用于 Microsoft Azure 的客户密码箱的 Azure 安全基线包含的建议可帮助你改进部署的安全状况。

此服务的基线摘自 [Azure 安全基准版本 1.0](../benchmarks/overview.md)，其中提供了有关如何根据我们的最佳做法指导保护 Azure 上的云解决方案的建议。
...
**指导**：不适用；不能将虚拟网络、子网或网络安全组与客户密码箱相关联。
...
**指导**：不适用；不能将虚拟网络、子网或网络安全组与 Azure 客户密码箱相关联。
...
**指导**：不适用；不能将虚拟网络、子网或网络安全组与客户密码箱相关联。 没有要生成或监视的网络配置。
...
**指导**：不适用；Microsoft 会维护用于客户密码箱之类的资源的时间源。
...
* [客户密码箱的审核日志](./customer-lockbox-overview.md#auditing-logs)
...
**指导**：不适用；客户密码箱不会处理或生成与反恶意软件相关的日志。
...
**指导**：不适用；客户密码箱不会处理或生成与 DNS 相关的日志。
...
**指导**：维护对客户密码箱请求拥有管理访问权限的用户帐户的清单。 可以在 Azure 门户中为你的订阅使用“标识和访问控制(IAM)”窗格来配置 Azure 基于角色的访问控制 (Azure RBAC)。 角色将应用到 Azure Active Directory 中的用户、组、服务主体和托管标识。
...
**指导**：Azure Active Directory 没有默认密码的概念。 其他需要密码的 Azure 资源会强制创建具有复杂性要求和最小密码长度的密码，该长度因服务而异。 你对可能使用默认密码的第三方应用程序和市场服务负责。
...
**指导**：使用启用了 Azure AD 多重身份验证 (MFA) 的特权访问工作站 (PAW) 来登录并配置客户密码箱请求。
...
**指导**：当环境中出现可疑或不安全的活动时，请使用 Azure Active Directory Privileged Identity Management (PIM) 生成日志和警报。

此外，还可使用 Azure Active Directory 风险检测查看有关风险用户行为的警报和报告。
...
**指导**：在适用的情况下使用 Azure Active Directory 作为中央身份验证和授权系统。 Azure Active Directory 通过对静态数据和传输中数据使用强加密来保护数据。 Azure Active Directory 还会对用户凭据进行加盐操作、哈希操作，并安全地存储用户凭据。

* [如何创建和配置 Azure Active Directory 实例](../../active-directory/fundamentals/active-directory-access-create-new-tenant.md)
...
**指导**：Azure Active Directory 提供日志来帮助你发现过时的帐户。 此外，请使用 Azure Active Directory 访问评审来高效管理组成员身份、对企业应用程序的访问和角色分配。 可以定期评审用户的访问权限，确保只有适当的用户才持续拥有访问权限。

* [了解 Azure Active Directory 报告](../../active-directory/reports-monitoring/index.yml)

* [如何使用 Azure Active Directory 访问评审](../../active-directory/governance/access-reviews-overview.md)
...
你可以访问 Azure Active Directory 登录活动、审核和风险事件日志源，以便与 Azure Sentinel 或第三方 SIEM 集成。

你可以为 Azure Active Directory 用户帐户创建诊断设置，并将审核日志和登录日志发送到 Log Analytics 工作区以简化上述过程。 可以在 Log Analytics 中配置所需的日志警报。
...
**指导**：对于控制平面（例如 Azure 门户）中帐户登录行为的偏差，请使用 Azure Active Directory 标识保护和风险检测功能进行配置，实现在检测到与用户标识相关的可疑操作时自动进行响应。 还可将数据引入 Azure Sentinel 以做进一步调查。

* [如何查看 Azure Active Directory 风险登录](../../active-directory/identity-protection/overview-identity-protection.md)
...
**指导**：不适用；将在你要授予对其的访问权限的资源所在的订阅中预配客户密码箱。 没有要保护或隔离的公共终结点。 向在租户级别具有“所有者”角色的用户授予客户密码箱请求访问权限。
...
**指导**：Microsoft 管理客户密码箱的底层基础结构，并实施了严格的控制措施来防止客户数据丢失或泄露。

* [了解 Azure 中的客户数据保护](./protection-customer-data.md)
...
**指导**：默认情况下，当数据在云服务和客户之间传输时，Microsoft 会使用传输层安全性 (TLS) 协议对数据进行保护。 Microsoft 的数据中心与连接到 Azure 服务的客户端系统协商建立 TLS 连接。 TLS 提供严格的身份验证，消息隐私性和完整性强（允许检测消息篡改、拦截和伪造），具有良好的互操作性，算法灵活，易于部署和使用。

* [了解 Azure 传输中的加密](./encryption-overview.md#encryption-of-data-in-transit)
...
**指南**：不适用；此建议适用于计算资源。 Microsoft 管理客户密码箱的底层基础结构，并实施了严格的控制措施来防止客户数据丢失或泄露。
...
有关详细信息，请参阅[安全控制：漏洞管理](../benchmarks/security-control-vulnerability-management.md)。

### <a name="51-run-automated-vulnerability-scanning-tools"></a>5.1：运行自动漏洞扫描工具

**指导**：不适用；Microsoft 对支持客户密码箱的底层系统执行漏洞管理。
...
**指导**：不适用；对客户密码箱请求的访问仅限于容纳资源的 Azure 订阅的所有者。 以租户所有者身份登录后，无需密码、机密或密钥即可访问客户密码箱。
...
**指导**：不适用；此项指导适用于计算资源。 Microsoft Antimalware 在支持客户密码箱解决方案的基础主机上处于启用状态。
...
有关详细信息，请参阅[安全控制：数据恢复](../benchmarks/security-control-data-recovery.md)。

### <a name="91-ensure-regular-automated-back-ups"></a>9.1：确保定期执行自动备份

**指导**：不适用；客户密码箱本身不存储客户数据。
...
**指导**：不适用；客户密码箱本身不存储客户数据，也不使用密钥或密码进行访问。
