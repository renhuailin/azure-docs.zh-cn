...
description: 了解如何为 Avere vFXT for Azure 中的群集添加后端存储系统。 如果创建了包含群集的 Azure Blob 容器，则随时可以使用。
author: ekpgh
...
> [!TIP]
> 如果创建了新的 Azure Blob 容器和 Avere vFXT 群集，则该容器已配置并可供使用。

如果没有随群集创建新的 Blob 容器，或者想要添加其他硬件或基于云的存储系统，请按照这些说明进行操作。

有两个主要任务：

1. [创建核心文件管理器](#create-a-core-filer)，用于将 vFXT 群集连接到现有存储系统或 Azure 存储帐户容器。
...
* [Azure 存储云核心文件管理器](#azure-blob-storage-cloud-core-filer) - 介绍如何将 Azure Blob 存储容器添加为云核心文件管理器

### <a name="nas-core-filer"></a>NAS 核心文件管理器

NAS 核心文件管理器可以是本地 NetApp 或 Isilon 设备，也可以是云中的 NAS 终结点。 存储系统必须与 Avere vFXT 群集具有可靠的高速连接 - 例如，1GBps ExpressRoute 连接（非 VPN） - 并且它必须为群集根提供对正在使用的 NAS 导出的访问权限。

请按照以下步骤添加 NAS 核心文件管理器：
...
### <a name="azure-blob-storage-cloud-core-filer"></a>Azure Blob 存储云核心文件管理器
...
1. 创建新的 Blob 容器：单击概述页面上的“容器”，然后单击“+容器”。 使用任何容器名称，并确保访问权限设置为“专用”。

   ![存储 blob 页上圈出了“+容器”按钮，并且在弹出页上创建了新的容器](media/avere-vfxt-new-blob.png)

1. 单击“设置”下的“访问密钥”，获取 Azure 存储帐户密钥。  复制提供的其中一个密钥。
...
创建交接点后，客户端将使用命名空间路径从存储系统访问这些文件。

## <a name="next-steps"></a>后续步骤

* [装载 Avere vFXT 群集](avere-vfxt-mount-clients.md)
* 了解[将数据移动到新的 Blob 容器](avere-vfxt-data-ingest.md)的有效方法
