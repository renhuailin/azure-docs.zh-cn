---
title: PowerShell 脚本 - 撤销删除已删除的文件共享
...
如果意外删除了文件共享，此脚本可帮助你撤消该删除操作。 通过使用文件共享的软删除安全功能，可以选择在为期 14 天的保留期内撤消删除文件共享，从而能恢复所有文件共享内容、快照和恢复点。 若要详细了解软删除功能，请访问此[链接](../soft-delete-azure-file-share.md)。
...
## <a name="how-to-use-the-script-in-different-scenarios"></a>如何在不同方案中使用脚本

### <a name="prerequisites"></a>先决条件

1. 在运行脚本之前，请从[此链接](/powershell/azure/install-az-ps)安装最新的 Azure PowerShell Az 模块。
2. 请记住以下详细信息，因为需要将它们作为脚本的不同参数值传递：
...
1. 在计算机上使用所选名称保存上述脚本。 在此示例中，我们将其保存为“Undelete.ps1”
2. 按照满足要求的方案运行脚本。
