...
## <a name="restore-prerequisites"></a>还原必备组件

在还原数据库之前，请注意以下事项：

- 可将数据库还原到同一 Azure 区域中的 SQL Server 实例。
- 目标服务器必须注册到与源服务器相同的保管库。
- 如果在服务器上运行多个实例，则所有实例都应启动并运行。 否则，服务器将不会出现在目标服务器列表中，以便您可以将数据库还原到该列表中。 有关详细信息，请参阅 [故障排除步骤](backup-sql-server-azure-troubleshoot.md#faulty-instance-in-a-vm-with-multiple-sql-server-instances)。
