...
系统中心数据保护管理器和 Azure 备份服务器 (MABS) 与 Azure 备份集成，并使用多个可提升效率的内置功能，可在将数据初始完整备份到 Azure 时节省网络和存储成本。 初始完整备份通常会传输大量数据，且需要较多网络带宽，相比之下，后续备份只传输差异/增量部分。 Azure 备份会压缩初始备份。 通过脱机种子设定过程，Azure 备份可以使用磁盘将压缩后的初始备份数据脱机上传到 Azure。
...
>
> MABS UR1 更新还通过使用 MABS 中的 Azure Data Box 提供脱机备份预览。 请联系 [SystemCenterFeedback@microsoft.com](mailto:SystemCenterFeedback@microsoft.com) 了解更多信息。
...
* 请确保在 SC DPM 或 MABS 上仅安装[最新版本的 Microsoft Azure 恢复服务代理](https://aka.ms/azurebackup_agent)并向恢复服务保管库注册该代理。
