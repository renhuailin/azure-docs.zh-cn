...
1. 在“恢复服务保管库”窗格中，选择新保管库。
    ...
    选择保管库时，“恢复服务保管库”窗格会缩窄，“设置”窗格（顶部有保管库的名称）和“保管库详细信息”窗格会打开。

    ![查看新保管库的存储配置](./media/backup-try-azure-backup-in-10-mins/set-storage-configuration-2.png)
2. 在新保管库的“设置”窗格中，使用垂直滚动条向下滚动到“管理”部分，并选择“备份基础结构”。
    此时会打开“备份基础结构”窗格。
3. 在“备份基础结构”窗格中，选择“备份配置”以打开“备份配置”窗格 。
...
4. 为保管库选择合适的存储复制选项。
    ...
    默认情况下，保管库具有异地冗余存储。 如果使用 Azure 作为主要备份存储终结点，请继续使用“异地冗余”。 如果不使用 Azure 作为主要备份存储终结点，则选择“本地冗余” ，以减少 Azure 存储成本。 在[此存储冗余概述](../storage/common/storage-redundancy.md)中，深入了解[异地冗余](../storage/common/storage-redundancy.md#geo-redundant-storage)、[本地冗余](../storage/common/storage-redundancy.md#locally-redundant-storage)和[区域冗余](../storage/common/storage-redundancy.md#zone-redundant-storage)存储选项。

创建保管库之后，即可对其进行配置，以便备份 Windows 系统状态。

## <a name="configure-the-vault"></a>配置保管库

1. 在“恢复服务保管库”窗格（对应于刚创建的保管库）的“开始使用”部分选择“备份”，并在“开始使用备份”窗格上选择“备份目标”。  
...
> [!NOTE]
> 无法通过 Azure 门户启用备份功能。 使用 Microsoft Azure 恢复服务代理备份 Windows Server 的系统状态。
>
...
2. 完成 Microsoft Azure 恢复服务代理安装向导。 若要完成该向导，需完成以下操作：
   ...
   * 将加密通行短语保存在安全的位置。
     ...
     > [!NOTE]
     > 如果你丢失或忘记了该通行短语，Microsoft 无法帮助恢复备份数据。 请将文件保存在安全的位置。 还原备份时需要用到此文件。
     >
     >
...
如有疑问，请[向我们发送反馈](https://feedback.azure.com/forums/258995-azure-backup)。
