...
云服务（外延支持）不支持托管服务的逻辑概念，而托管服务就包括上述两个槽，即生产和暂存。 每个部署都是独立的云服务（外延支持）部署。 要测试云服务的新版本并对其进行暂存，请部署云服务部署（外延支持）并将其标记为可与其他云服务（外延支持）交换的 VIP
...
云服务（外延支持）使用 Azure 密钥保管库和基本 (ARM) 公共 IP 地址。 需要证书的客户必须使用 Azure 密钥保险库进行证书管理（[详细了解](https://azure.microsoft.com/pricing/details/key-vault/) Azure 密钥保险库定价。） 云服务（外延支持）的每个公共 IP 地址是单独收费的（[详细了解](https://azure.microsoft.com/pricing/details/ip-addresses/)公共 IP 地址定价。） 
