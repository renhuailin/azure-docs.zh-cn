---
title: Azure 云服务（外延支持）定义架构（.cscfg 文件）| Microsoft Docs
description: 与云服务（外延支持）定义架构相关的信息
ms.topic: article
...
---
# <a name="azure-cloud-services-extended-support-config-schema-cscfg-file"></a>Azure 云服务（外延支持）配置架构（cscfg 文件）

服务配置文件指定要为服务中每个角色部署的角色实例数、任何配置设置的值，以及与角色关联的任何证书的缩略图。 如果服务属于虚拟网络，必须在服务配置文件以及虚拟网络配置文件中提供网络的配置信息。 服务配置文件的默认扩展名为 csdef。

通过[云服务（外延支持）定义架构](schema-csdef-file.md)描述服务模型。
...
[Azure 云服务（外延支持）定义架构（csdef 文件）](schema-csdef-file.md)