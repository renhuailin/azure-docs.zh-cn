...
**ServiceDefinition.csdef** 文件指定 Azure 用于配置云服务的设置。 [Azure 服务定义架构（.csdef 文件）](schema-csdef-file.md)为服务定义文件提供允许的格式。 以下示例显示了可为 Web 角色和辅助角色定义的设置：
...
云服务设置的配置由 **ServiceConfiguration.cscfg** 文件中的值确定。 指定要为此文件中每个角色部署的实例数。 在服务定义文件中定义的配置设置值已添加到服务配置文件中。 与云服务相关联的所有管理证书的指纹也会添加到该文件中。 [Azure 服务配置架构（.cscfg 文件）](schema-cscfg-file.md)为服务配置文件提供允许的格式。
...
可以参考[服务配置架构](schema-cscfg-file.md)更好了解此处使用的 XML 架构，不过以下是元素的快速说明：
...
Azure 运行时库包括 Microsoft.WindowsAzure.ServiceRuntime 命名空间，它提供类用于与来自角色的 Azure 环境进行交互。 RoleEnvironment 类定义在配置更改前后引发的以下事件：
...
**CSPack** 使用服务定义文件和服务配置文件的内容来定义包的内容。 CSPack 生成可以使用 [Azure 门户](../cloud-services/cloud-services-how-to-create-deploy-portal.md#create-and-deploy) 上传到 Azure 的应用程序包文件 (.cspkg)。 默认情况下，该应用程序包名为 `[ServiceDefinitionFileName].cspkg`，但可以通过使用 **CSPack** 的 `/out` 选项指定不同的名称。
