...
本文使用 [Helm 3][helm] 在 [支持的 Kubernetes 版本][支持 aks 的版本] 上安装 NGINX 入口控制器。 确保使用最新版本的 Helm，并且有权访问 ingress-nginx Helm 存储库。 本文中概述的步骤可能与 Helm 图表、NGINX 入口控制器或 Kubernetes 的先前版本不兼容。
