...
ms.locfileid: "101740579"
---
# <a name="azure-security-baseline-for-azure-kubernetes-service"></a>适用于 Azure Kubernetes 服务的 Azure 安全基线

此安全基线将 [Azure 安全基准版本 1.0](../security/benchmarks/overview-v1.md) 中的指南应用到 azure Kubernetes。 Azure 安全基准提供有关如何在 Azure 上保护云解决方案的建议。
内容按照 Azure 安全基准定义的 **安全控件** 进行分组，以及适用于 azure Kubernetes 的相关指南。 不适用于 Azure Kubernetes 的 **控件** 已被排除。

 
若要查看 Azure Kubernetes 如何完全映射到 Azure 安全基准，请参阅 [完整的 Azure Kubernetes security 基线映射文件](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)。
...
在订阅中创建或更新虚拟网络时，网络观察程序将在虚拟网络的区域中自动启用。 你还可以使用 PowerShell、Azure CLI、REST API 或 Azure 资源管理器客户端方法创建网络观察程序的新实例。
...
- [了解使用 WAF 保护 AKS 群集的最佳做法](https://docs.microsoft.com/azure/aks/operator-best-practices-network#secure-traffic-with-a-web-application-firewall-waf)
...
**指导**：使用 Azure Policy 为与 Azure Kubernetes 服务 (AKS) 群集关联的网络资源定义并实现标准安全配置。 

在“Microsoft.ContainerService”和“Microsoft.Network”命名空间中使用 Azure Policy 别名创建自定义策略，以审核或强制实施 AKS 群集的网络配置。 
...
**指南**：对网络安全组和其他资源使用标记，使流量流入和流出 Azure Kubernetes SERVICE (AKS) 群集。 使用各个网络安全组规则的“说明”字段为允许流量传入/传出网络的任何规则指定业务需要和/或持续时间等。
...
- [启用和查看 Azure Kubernetes 服务 (AKS) 中 Kubernetes 主节点的日志](/azure/aks/view-master-logs)
...
- masterclient： MasterClientCertificate 的审核日志中的显示名称，从 az aks 获取的证书 
...
- [在此处查看包括日志角色的日志架构](/azure/aks/view-master-logs)

- [了解用于容器的 Azure Monitor](/azure/azure-monitor/insights/container-insights-overview)

- [如何为容器启用 Azure Monitor](/azure/azure-monitor/insights/container-insights-onboard)
...
**指导**：使用活动日志监视 Azure Kubernetes 服务 (AKS) 资源上的操作，查看所有活动及其状态。 使用活动日志确定对订阅中的资源执行的操作： 
...
- [如何启用和查看 AKS 中 Kubernetes 主节点的日志](/azure/aks/view-master-logs)
...
**指导**：将 Azure Kubernetes 服务 (AKS) 与安全中心一起使用，可以更深入地了解 AKS 节点。 

查看安全中心在主机和群集级别检测到的威胁和恶意活动警报。 安全中心实现对 AKS 群集中发生的原始安全事件的持续分析，例如网络数据、进程创建和 Kubernetes 审核日志。 确定此活动是否为预期行为，或者应用程序是否行为异常。 使用 Azure Monitor 中的指标和日志来证实发现结果。 
...
- [容器的警报 - Azure Kubernetes 服务群集](https://docs.microsoft.com/azure/security-center/alerts-reference#alerts-akscluster)
...
- [如何实时查看 Kubernetes 日志、事件和 Pod 指标](/azure/azure-monitor/insights/container-insights-livedata-overview)
...
启用 Azure AD 多重身份验证，并遵循安全中心的标识和访问管理建议。
...
- [如何在 Azure 中 (MFA) 启用多重身份验证](../active-directory/authentication/howto-mfa-getstarted.md)
...
- [如何使用 Azure Monitor 创建、查看和管理日志警报](/azure/azure-monitor/platform/alerts-log)
...
在安全中心使用 Qualys 扫描图像后，如果发现问题，你会在安全中心仪表板中收到通知。 容器注册表项绑定功能可以更深入地了解 Azure 资源管理器的注册表中所使用映像的漏洞。 
...
- [如何为使用 Windows Server 节点的 AKS 群集升级 AKS 节点池](https://docs.microsoft.com/azure/aks/use-multiple-node-pools#upgrade-a-node-pool)
...
- [指定节点池的排斥、标签或标记](https://docs.microsoft.com/azure/aks/use-multiple-node-pools#specify-a-taint-label-or-tag-for-a-node-pool)
...
检查并为隔离和多租户实现附加的 Kubernetes 功能和注意事项，包括以下内容：计划、网络、身份验证/授权和容器。 还使用单独的订阅和管理组进行开发、测试和生产。 将 AKS 群集与虚拟网络、适当标记的子网分开，并使用 Web 应用程序防火墙 (WAF) 进行保护。
...
- [了解 AKS 群集的状态配置](https://docs.microsoft.com/azure/aks/concepts-clusters-workloads#control-plane)
...
**指导**：使用“计算 &amp; 应用”部分下的安全中心容器建议，为 Azure Kubernetes 服务 (AKS) 群集执行基线扫描。 

当发现配置问题或漏洞时，安全中心仪表板中将显示通知。 这需要启用可选的容器注册表捆绑包，该捆绑包允许安全中心扫描映像。  
...
- [如何对 AKS 群集使用 Key Vault](https://docs.microsoft.com/azure/aks/developer-best-practices-pod-security#limit-credential-exposure)
...
- [了解 Azure Kubernetes Service (AKS) 的托管标识和 Key Vault](https://docs.microsoft.com/azure/aks/developer-best-practices-pod-security#limit-credential-exposure)

- [Azure AD Pod 标识](https://github.com/Azure/aad-pod-identity)
...
- [AKS 共担责任和守护程序集](https://docs.microsoft.com/azure/aks/support-policies#shared-responsibility)
...
- [如何还原 Key Vault 证书](https://docs.microsoft.com/powershell/module/az.keyvault/restore-azkeyvaultcertificate?view=azps-4.8.0&amp;preserve-view=true)

- [如何还原 Key Vault 密钥](https://docs.microsoft.com/powershell/module/az.keyvault/restore-azkeyvaultkey?view=azps-4.8.0&amp;preserve-view=true)
...
- [如何在 Key Vault 中启用“软删除”](https://docs.microsoft.com/azure/storage/blobs/soft-delete-blob-overview?tabs=azure-portal)
...
**指导**：在安全中心为警报分配严重性的情况下，确定必须首先调查哪些警报的优先级。 严重性取决于安全中心对调查结果或用于发出警报的分析的确信程度，以及对导致警报的活动背后存在恶意意图的确信程度。
...
**指导**：执行试验以定期测试系统的事件响应功能。 确定薄弱点和缺口，并根据需要修订事件响应计划。
...
根据需要，选择 "安全中心" 数据连接器，根据组织业务要求将警报流式传输到 Azure Sentinel。
...
**指导**：请遵循 Microsoft 互动规则，确保你的渗透测试不违反 Microsoft 政策。 针对 microsoft 托管的云基础结构、服务和应用程序，针对 microsoft 托管的云基础结构、服务和应用程序的 Microsoft 策略和执行的其他相关信息。
