...
有关 Azure Key Vault 的概述，请参阅[什么是 Azure Key Vault？](overview.md)。如果没有 Azure 订阅，请在开始之前创建一个[免费帐户](https://azure.microsoft.com/free/?WT.mc_id=A261C142F)。
...
现在，可以通过使用密钥的 URI，引用已创建或上传到 Azure 密钥保管库的密钥。 使用 `https://ContosoKeyVault.vault.azure.net/keys/ContosoFirstKey` 总能获得当前版本。 使用 https://[keyvault-name].vault.azure.net/keys/[keyname]/[key-unique-id] 会获取此特定版本。 例如，`https://ContosoKeyVault.vault.azure.net/keys/ContosoFirstKey/cgacf4f763ar42ffb0a1gca546aygd87`。 
...
有关如何生成此 BYOK 包的详细说明，请参阅[如何对 Azure 密钥保管库使用受 HSM 保护的密钥](../keys/hsm-protected-keys.md)。
...
- 有关 Azure Key Vault 和 HSM 的信息，请参阅[如何将受 HSM 保护的密钥与 Azure Key Vault 结合使用](../keys/hsm-protected-keys.md)。
