...
有关使用 Azure RBAC 提供对 Key Vault 数据平面的访问的详细信息，请参阅[使用 Azure 基于角色的访问控制提供对 Key Vault 密钥、证书和机密的访问](rbac-guide.md)
...
有关专用终结点的详细信息，请参阅 [Azure 专用链接与 Key Vault](./private-link-service.md)
...
| --- | --- | --- | --- |
| 安全团队 | [密钥保管库参与者](../../role-based-access-control/built-in-roles.md#key-vault-contributor) | 证书：所有操作 <br> 密钥：所有操作 <br> 机密：所有操作 | [密钥保管库管理员](../../role-based-access-control/built-in-roles.md#key-vault-administrator) |
| 开发人员和&nbsp;操作人员 | 密钥保管库部署权限<br><br> **注意**：此权限允许已部署的 VM 从密钥保管库提取机密。 | 无 | 无 |
| 审核人员 | 无 | 证书：list <br> 密钥：列出<br>机密：列出<br><br> **注意**：此权限让审核员能够检查日志中未发出的密钥和机密的属性（标记、激活日期、到期日期）。 | [密钥保管库读取者](../../role-based-access-control/built-in-roles.md#key-vault-reader) |
...
| 应用程序 | 无 | 机密：get、list <br> 证书：get、list | [密钥保管库读取者](../../role-based-access-control/built-in-roles.md#key-vault-reader)、[密钥保管库机密用户](../../role-based-access-control/built-in-roles.md#key-vault-secrets-user) |
...
[与 Key Vault 建立专用链接连接](private-link-service.md)
