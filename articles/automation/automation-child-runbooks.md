...
ms.locfileid: "129353424"
---
# <a name="create-modular-runbooks-in-automation"></a>在自动化中创建模块化 Runbook

在 Azure 自动化中，一种建议的做法是编写可重用的模块化 Runbook，其中使用由其他 Runbook 调用的离散函数。 父 Runbook 通常会调用一个或多个子 Runbook 来执行所需的功能。 

调用子 Runbook 的方法有两种，你应该了解其中存在的明显差异，以确定哪种方法最适合你的方案。 下表汇总了从一个 Runbook 调用另一个 Runbook 的两种方法的差异。
...
发布某个 Runbook 时，必须事先发布它所调用的任何子 Runbook。 原因是在编译 Runbook 时，Azure 自动化会生成与任何子 Runbook 的关联。 如果尚未发布子 Runbook，则父 Runbook 看似会正确发布，但会在启动时生成异常。 如果发生这种情况，可以重新发布父 Runbook，以正确引用子 Runbook。 如果由于已创建关联而更改了任何子 Runbook，无需重新发布父 Runbook。
...
由于计时原因，子 Runbook 的输出不会可靠地返回到父 Runbook。 另外，`$VerbosePreference`、`$WarningPreference` 等变量可能不会传播到子 Runbook。 为避免这些问题，可以结合 `Wait` 参数使用 `Start-AzAutomationRunbook` 将子 Runbook 作为单独的自动化作业调用。 此方法会阻止父 Runbook，直到子 Runbook 完成。
