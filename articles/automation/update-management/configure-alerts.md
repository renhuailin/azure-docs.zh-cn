...
1. 在“创建警报规则”页上，系统已选择你的自动化帐户作为资源。 如果要对其进行更改，请选择“编辑资源”。

1. 在“选择资源”页上，从“按资源类型筛选”下拉列表中选择“自动化帐户” 。

1. 选择要使用的自动化帐户，再选择“完成”。

1. 选择“添加条件”以选择适合要求的内容。

1. 对于维度，请从列表中选择一个有效值。 如果所需的值不在列表中，请选择维度旁边的 \+，并键入自定义名称。 然后选择要查找的值。 若要为某个维度选择所有值，请选择“选择 \*”按钮。 如果没有为某个维度选择值，更新管理将忽略该维度。

    ![配置信号逻辑](./media/manage-updates-for-vm/signal-logic.png)

1. 在“警报逻辑”中的“时间聚合”和“阈值”字段中输入值，然后选择“完成”   。

1. 在下一页上，输入警报的名称和描述。

1. 对于成功的运行，请将“严重性”字段设置为“信息(严重性 2)”；对于失败的运行，请将其设置为“信息(严重性 1)”  。
...
1. 选择警报，然后在“操作”下选择“添加操作组”。 此时将显示“选择要附加到此警报规则的操作组”窗格。

   :::image type="content" source="./media/manage-updates-for-vm/select-an-action-group.png" alt-text="使用情况和预估成本。":::

1. 选中要附加的操作组的复选框，然后按“选择”。
