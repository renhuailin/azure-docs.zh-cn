...
可以使用 Azure 命令行接口 (Azure CLI) 创建 Azure 资源组、服务器和单一数据库。 如果不想使用 Azure Cloud Shell，请在计算机上[安装 Azure CLI](/cli/azure/install-azure-cli)。
...
可以使用 Windows PowerShell 创建资源组、服务器和单一数据库。 如果不想使用 Azure Cloud Shell，请[安装 Azure PowerShell 模块](/powershell/azure/install-az-ps)。
