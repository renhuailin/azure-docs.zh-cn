...
使用 SQL Server 中的 [SQL Server 代理](/sql/ssms/agent/sql-server-agent)和 [SQL 托管实例](sql-managed-instance-paas-overview.md)，可以创建和计划可针对一个或多个数据库定期执行的作业，以运行 Transact-SQL (T-SQL) 查询和执行维护任务。 本文介绍如何使用 SQL 代理的 SQL 托管实例。

> [!Note]
> SQL 代理不适用于 Azure SQL 数据库或 Azure Synapse Analytics。 相反，建议[使用弹性作业实现作业自动化](../database/job-automation-overview.md)。

### <a name="sql-agent-job-limitations-in-sql-managed-instance"></a>SQL 托管实例中的 SQL 代理作业限制
...
- SQL 代理设置为只读。 
    - 不支持系统存储过程 `sp_set_agent_properties`。
...
- 自动完成管理任务，并将作业计划为在每个工作日、数小时之后或按其他频率运行。
  ...
  - 更新引用数据（所有数据库的公用信息）、从 Azure Blob 存储加载数据。 Microsoft 建议使用 [SHARED ACCESS SIGNATURE 身份验证向 Azure Blob 存储进行身份验证](/sql/t-sql/statements/bulk-insert-transact-sql#f-importing-data-from-a-file-in-azure-blob-storage)。
...
## <a name="sql-agent-jobs-in-sql-managed-instance"></a>SQL 托管实例中的 SQL 代理作业
...
> [!Note]
> 若要详细了解如何利用 Azure SSIS Integration Runtime 与 SQL 托管实例托管的 SSISDB，请参阅[在 Azure 数据工厂中将 Azure SQL 托管实例与 SQL Server Integration Services (SSIS) 配合使用](../../data-factory/how-to-use-sql-managed-instance-with-ir.md)。

[事务复制](../managed-instance/replication-transactional-overview.md)可将表中的更改复制到 SQL 托管实例、Azure SQL 数据库或 SQL Server 中的其他数据库。 若要了解信息，请参阅[在 Azure SQL 托管实例中配置复制](../../azure-sql/managed-instance/replication-between-two-instances-configure-tutorial.md)。 
...
如果尚未启用此功能，首先需要在 SQL 托管实例上配置[数据库邮件功能](/sql/relational-databases/database-mail/database-mail)：
...
如果将链接到非 sysadmin 登录名的用户添加到 msdb 系统数据库中三个 SQL 代理固定数据库角色中的任何一个角色，则会出现以下问题：需要向 master 数据库中的三个系统存储过程授予显式 EXECUTE 权限。 如果遇到此问题，将显示错误消息“在对象 <object_name> 中拒绝了 EXECUTE 权限(Microsoft SQL Server，错误:229)”。 
