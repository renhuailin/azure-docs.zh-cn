...
description: 在 Azure 虚拟机上使用高级文件共享 (PFS) 创建 (FCI) SQL Server 的故障转移群集实例。
...
本文介绍如何使用 [高级文件共享](../../../storage/files/storage-how-to-create-premium-fileshare.md)在 Azure 虚拟机 (vm) 中创建 (FCI) SQL Server 的故障转移群集实例。

在 Windows Server 2012 或更高版本上，高级文件共享是存储空间直通)  (支持的、与故障转移群集实例完全支持的、与故障转移群集 2012 SQL Server 实例完全支持的低延迟文件共享。 高级文件共享提供更高的灵活性，使你能在不停机的情况下对文件共享进行大小调整和缩放。
...
1. 通过使用 "文件资源管理器" 或 " **运行** " 对话框， (选择 "Windows + R) 来使用共享。 使用网络路径 `\\storageaccountname.file.core.windows.net\filesharename`。 例如： `\\sqlvmstorageaccount.file.core.windows.net\sqlpremiumfileshare`

1. 至少在新连接的文件共享上创建一个文件夹，将 SQL 数据文件放置到其中。
1. 在将加入群集的每个 SQL Server VM 上重复这些步骤。

  > [!IMPORTANT]
  > - 请考虑对备份文件使用单独的文件共享来保存每秒的输入/输出操作 (IOPS) 以及此共享的空间容量用于数据和日志文件。 你可以为备份文件使用高级或标准文件共享。
