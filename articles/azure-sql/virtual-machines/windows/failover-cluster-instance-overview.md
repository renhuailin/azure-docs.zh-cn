...
### <a name="lightweight-extension-support"></a>轻型扩展支持   

目前，仅支持将 Azure 虚拟机上的 SQL Server 故障转移群集实例用于 SQL Server IaaS 代理扩展的[轻型管理模式](sql-server-iaas-agent-extension-automate-management.md#management-modes)。 要从完全扩展模式更改为轻型模式，请先删除相应 VM 的 SQL 虚拟机资源，再在轻型模式下向 SQL IaaS 代理扩展注册它们。 使用 Azure 门户删除 SQL 虚拟机资源时，请取消勾选相应虚拟机旁边的复选框，从而避免删除该虚拟机。 

完全扩展支持自动备份、修补和高级门户管理等功能。 这些功能不适用于在轻型管理模式下注册的 SQL Server VM。
