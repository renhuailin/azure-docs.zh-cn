...
**指导**：透明数据加密 (TDE) 通过加密静态数据，帮助保护 Azure SQL 数据库、Azure SQL 托管实例和 Azure 数据仓库免受恶意脱机活动的威胁。 它可执行静态数据库、关联备份和事务日志文件的实时加密和解密，无需更改应用程序。 默认情况下，在 SQL 数据库和 SQL 托管实例中会为所有新部署的数据库启用 TDE。 Microsoft 或客户可以管理 TDE 加密密钥。
