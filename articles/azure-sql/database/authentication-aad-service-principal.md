...
1. 分配服务器标识。 分配的服务器标识代表托管服务标识 (MSI)。 目前，Azure SQL 的服务器标识不支持用户托管标识 (UMI)。
    ...
    > [!NOTE]
    > 也可以使用 CLI 命令分配服务器标识。 有关详细信息，请参阅 [az sql server create](/cli/azure/sql/server#az_sql_server_create) 和 [az sql server update](/cli/azure/sql/server#az_sql_server_update)。
...
- 仅支持以 SQL 托管实例的 Azure AD 管理员身份使用 CLI 命令以及 [Az.Sql 版本为 2.9.0](https://www.powershellgallery.com/packages/Az.Sql/2.9.0) 或更高的 PowerShell 命令设置 Azure AD 应用程序。 有关详细信息，请参阅 [az sql mi ad-admin create](/cli/azure/sql/mi/ad-admin#az_sql_mi_ad_admin_create) 和 [Set-AzSqlInstanceActiveDirectoryAdministrator](/powershell/module/az.sql/set-azsqlinstanceactivedirectoryadministrator) 命令。 
