...
在 Microsoft Azure 订阅中拆分 / 合并服务作为云服务运行。 因此会对你的服务实例收取云服务费用。 除非频繁地执行拆分/合并/移动操作，否则建议删除拆分/合并云服务。 这可以节省用于运行中的或已部署的云服务实例的成本。 只要需要执行拆分或合并操作，便可以重新部署和启用已准备好的可运行配置。
