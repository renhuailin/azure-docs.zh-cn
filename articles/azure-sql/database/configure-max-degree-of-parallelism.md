...
## <a name="overview"></a>概述
  在 Azure SQL 数据库中，对于每个新的单一数据库和弹性池数据库，默认的 MAXDOP 设置均为 8。 这意味着数据库引擎可以使用多个线程执行查询。 与 SQL Server 不同，服务器范围的 MAXDOP 默认为 0（无限制），Azure SQL 数据库中的新数据库默认设置为 MAXDOP 8。 此默认设置可防止利用不必要的资源，还可确保一致的客户体验。 通常无需在 Azure SQL 数据库工作负载中进一步配置 MAXDOP，但将其作为高级性能优化选项使用或许很有用处。

> [!Note]
>   2020 年 9 月，基于 Azure SQL 数据库服务多年的遥测数据，[MAXDOP 8 被选为](https://techcommunity.microsoft.com/t5/azure-sql/changing-default-maxdop-in-azure-sql-database-and-azure-sql/ba-p/1538528)新数据库的默认值，作为适合各种客户工作负载的最佳值。 此默认值有助于防止并行度过高导致的性能问题。 在此之前，新数据库的默认设置为 MAXDOP 0。 对于在 2020 年 9 月之前创建的现有数据库，MAXDOP 数据库范围的配置选项未发生更改。

  通常情况下，如果数据库引擎选择并行执行查询，则执行时间将更短。 但并行度过高可能会消耗过多的处理器资源，且不会提高查询性能。 从规模上看，并行度过高可能会对同一数据库引擎实例上执行的所有查询的查询性能产生负面影响，因此在 SQL Server 工作负载中，设置并行度上限是一项常见的性能优化操作。

  下表描述了在执行具有不同 MAXDOP 值的查询时，数据库引擎的行为：

| MAXDOP | 行为 | 
|--|--|
| = 1 | 数据库引擎不使用多个并发线程执行查询。 | 
| > 1 | 数据库引擎为并行线程数设置上限。 数据库引擎会选择要使用的额外工作线程数。 用于执行查询的工作线程总数可能高于指定的 MAXDOP 值。 |
| = 0 | 数据库引擎可使用许多并行线程，其上限取决于逻辑处理器的总数。 数据库引擎会选择要使用的并行线程数。| 
| | |
...
-   对于每项[任务](/sql/relational-databases/system-dynamic-management-views/sys-dm-os-tasks-transact-sql)，强制设置 MAXDOP。 对于每个[请求](/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-requests-transact-sql)或查询，不强制设置。 这意味着在并行执行查询期间，单个请求可以生成多项具有由 MAXDOP 确定的上限的任务。 有关详细信息，请参阅[线程和任务体系结构指南](/sql/relational-databases/thread-and-task-architecture-guide)中的“计划并行任务”部分。 
  
-   索引操作（如创建或重新生成索引、或删除聚集索引）可能会大量占用资源。 可以在 `CREATE INDEX` 或 `ALTER INDEX` 语句中指定 MAXDOP 索引选项，以替代数据库索引操作的最大并行度值。 MAXDOP 值在执行时应用于语句，但不存储在索引元数据中。 有关详细信息，请参阅 [配置并行索引操作](/sql/relational-databases/indexes/configure-parallel-index-operations)。  
  
-   除了查询和索引操作之外，数据库范围的 MAXDOP 配置选项还可控制 DBCC CHECKTABLE、DBCC CHECKDB 和 DBCC CHECKFILEGROUP 的并行度。 

##  <a name="recommendations"></a><a name="Security"></a> 建议  
...
### <a name="excessive-parallelism"></a>并行度过高
  ...
  建议客户避免使用 MAXDOP 0，即使它目前看起来不会导致问题。 当 CPU 和工作线程接收的并发请求数超过服务目标所能支持的数量时，并行度过高最容易导致问题。 避免使用 MAXDOP 0 可降低由于数据库扩展或 Azure SQL 数据库中硬件的未来代系为同一数据库服务对象提供更多核心导致并行度过高进而导致出现更多潜在问题的风险。

### <a name="modifying-maxdop"></a>修改 MAXDOP 

  如果确定其他 MAXDOP 设置是 Azure SQL 数据库工作负载的最佳设置，则可以使用 `ALTER DATABASE SCOPED CONFIGURATION` T-SQL 语句。 如需查看示例，请参阅下面的 [Transact-SQL 使用示例](#examples)部分。 将此步骤添加到部署过程，以便在创建数据库后更改 MAXDOP。

  如果非默认 MAXDOP 只对工作负载中的一部分查询有利，则可以通过添加 OPTION (MAXDOP) 提示在查询级别重写 MAXDOP。 如需查看示例，请参阅下面的 [Transact-SQL 使用示例](#examples)部分。 

  利用涉及实际并发查询负载的负载测试，全面测试 MAXDOP 配置更改。 

  主副本和次要副本的 MAXDOP 可以单独进行配置，以便为读写和只读工作负载使用不同的最优 MAXDOP 设置。 这适用于 Azure SQL 数据库[读取扩展](read-scale-out.md)、[异地复制](active-geo-replication-overview.md)和[超大规模次要副本](service-tier-hyperscale.md)。 默认情况下，所有次要副本都继承主副本的 MAXDOP 配置。
...
#### <a name="maxdop-database-scoped-configuration"></a>MAXDOP 数据库范围的配置   
  ...
  此示例演示如何使用 [ALTER DATABASE SCOPED CONFIGURATION](/sql/t-sql/statements/alter-database-scoped-configuration-transact-sql) 语句将 `max degree of parallelism` 选项配置为 `2`。 该设置将立即生效。 PowerShell cmdlet [Invoke-SqlCmd](/powershell/module/sqlserver/invoke-sqlcmd) 执行 T-SQL 查询以设置并返回 MAXDOP 数据库范围的配置。 
...
1.  连接到 Azure SQL 数据库。 你无法更改 master 数据库中数据库范围的配置。
...
```
  ...
  此示例演示如何使用 [ALTER DATABASE SCOPED CONFIGURATION](/sql/t-sql/statements/alter-database-scoped-configuration-transact-sql) 语句将 `max degree of parallelism` 选项配置为 `8`。 该设置将立即生效。  
  
```sql  
ALTER DATABASE SCOPED CONFIGURATION SET MAXDOP = 8;
```  

此示例适用于[已启用读取扩展副本](read-scale-out.md)、[异地复制](active-geo-replication-overview.md)和[超大规模次要副本](service-tier-hyperscale.md)的 Azure SQL 数据库。 例如，考虑到读写和只读工作负载可能存在差异，将主副本设置为与次要副本不同的默认 MAXDOP。 `sys.database_scoped_configurations` 的 `value_for_secondary` 列包含次要副本的设置。
