...
我们会按照[将 Azure SQL 数据库流量迁移到更新的网关](gateway-migration.md)中列出的过程，定期停用那些使用旧硬件的网关，并将流量迁移到新网关。 强烈建议客户使用网关 IP 地址范围，这样就不会受某个区域中的此活动的影响。

> [!IMPORTANT]
> SQL 数据库或 Azure Synapse 的登录名可以在某个区域中的任何网关上登陆。 为了一致地连接到 SQL 数据库或 Azure Synapse，请允许网络流量进出区域的所有网关 IP 地址或网关 IP 地址范围。

| 区域名称          | 网关 IP 地址 | 网关 IP 地址范围 |
| --- | --- | --- |
...
| 印度中部        | 104.211.96.159, 104.211.86.30 , 104.211.86.31 | 104.211.86.32/29、20.192.96.32/29 |
...
| 挪威东部          | 51.120.96.0, 51.120.96.33 | 51.120.96.32/29 |
