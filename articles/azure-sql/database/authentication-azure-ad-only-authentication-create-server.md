---
title: 在 Azure SQL 中创建启用了“仅限 Azure Active Directory 身份验证”的服务器
...
本操作指南概述了在预配期间创建启用了[仅限 Azure AD 身份验证](authentication-azure-ad-only-authentication.md)的 [Azure SQL 逻辑服务器](logical-servers.md)或 [Azure SQL 托管实例](../managed-instance/sql-managed-instance-paas-overview.md)的步骤。 “仅限 Azure AD 身份验证”功能可阻止用户使用 SQL 身份验证连接到服务器或托管实例，仅允许使用 Azure AD 身份验证进行连接。

## <a name="prerequisites"></a>先决条件

- 使用 Azure CLI 时，需要版本 2.26.1 或更高版本。 有关安装和最新版本的详细信息，请参阅[安装 Azure CLI](/cli/azure/install-azure-cli)。
...
以下部分提供了一些示例和脚本，介绍如何创建 SQL 逻辑服务器或托管实例，为服务器或实例设置 Azure AD 管理员，以及如何在服务器创建期间启用“仅限 Azure AD 身份验证”。 有关该功能的详细信息，请参阅[仅限 Azure AD 身份验证](authentication-azure-ad-only-authentication.md)。
...
若要在创建服务器或托管实例后更改现有属性，应使用其他现有 API。 有关详细信息，请参阅[使用 API 管理“仅限 Azure AD 身份验证”](authentication-azure-ad-only-authentication.md#managing-azure-ad-only-authentication-using-apis)和[使用 Azure SQL 配置和管理 Azure AD 身份验证](authentication-aad-configure.md)。
...
Azure CLI命令 `az sql server create` 用于预配新的 Azure SQL 逻辑服务器。 以下命令将预配启用了“仅限 Azure AD 身份验证”的新服务器。
...
PowerShell 命令 `New-AzSqlServer` 用于预配新的 Azure SQL 逻辑服务器。 以下命令将预配启用了“仅限 Azure AD 身份验证”的新服务器。 
...
[服务器 - 创建或更新](/rest/api/sql/2020-11-01-preview/servers/create-or-update) Rest API 可用于在预配期间创建启用了“仅限 Azure AD 身份验证”的 Azure SQL 逻辑服务器。 

以下脚本将预配 Azure SQL逻辑服务器，将 Azure AD 管理员设置为 `<AzureADAccount>`，并启用“仅限 Azure AD 身份验证”。 服务器 SQL 管理员登录名也将自动创建，而且密码将设置为随机密码。 由于在预配过程中禁用了 SQL 身份验证连接，因此不会使用 SQL 管理员登录。
...
若要使用 ARM 模板通过服务器的 Azure AD 管理员集预配 SQL 逻辑服务器并启用“仅限 Azure AD 身份验证”，请参阅[“仅限 Azure AD 身份验证”的 Azure SQL 逻辑服务器](https://github.com/Azure/azure-quickstart-templates/tree/master/quickstarts/microsoft.sql/sql-logical-server-aad-only-auth)快速启动模板。
...
- 目前不支持在预配期间使用 Azure 门户创建启用了“仅限 Azure AD 身份验证”的服务器或实例。
