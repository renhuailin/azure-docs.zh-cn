...
|每个池的最大并发辅助角色数（请求数）<sup>3</sup>|400|500|600|700|800|
|每个池的最大并发登录数（请求数）<sup>3</sup>|800|1000|1200|1400|1600|
...
|每个池的最大并发辅助角色数（请求数）<sup>3</sup>|900|1000|1200|1600|1800|3600|
|每个池的最大并发登录数（请求数）<sup>3</sup>|1800|2000|2400|3200|3600|7200|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|
...
|每个池的最大并发辅助角色数（请求数）<sup>3</sup>|168|336|504|672|
|每个池的最大并发登录数（请求数）<sup>3</sup>|168|336|504|672|
...
|存储类型|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|
|IO 延迟（近似）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|
|每个池的最大数据 IOPS <sup>2</sup>|12,499|15,624|18,748|21,873|24,998|28,123|
|每个池的最大日志速率 (MBps)|48|60|72|84|96|108|
|每个池的最大并发辅助角色数（请求数）<sup>3</sup>|800|1,000|1,200|1,400|1,600|1,800|
|每个池的最大并发登录数（请求数）<sup>3</sup>|800|1,000|1,200|1,400|1,600|1,800|
...
|存储类型|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|
|IO 延迟（近似）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|
|每个池的最大数据 IOPS <sup>2</sup>|31,248|37,497|49,996|99,993|160,000|
|每个池的最大日志速率 (MBps)|120|144|192|264|264|
|每个池的最大并发辅助角色数（请求数）<sup>3</sup>|2,000|2,400|3,200|6,400|12,800|
|每个池的最大并发登录数（请求数）<sup>3</sup>|2,000|2,400|3,200|6,400|12,800|
