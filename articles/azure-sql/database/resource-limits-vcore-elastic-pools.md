...
* [Transact-SQL](elastic-pool-scale.md)（通过 [ALTER DATABASE](/sql/t-sql/statements/alter-database-transact-sql#overview-sql-database)）
...
|每个池的最大数据 IOPS <sup>3</sup> |400|800|1200|1600|2000|2400|
|每个池的最大日志速率 (MBps)|6|12|18|24|30|36|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup> |210|420|630|840|1050|1260|
|每个池的最大并发登录数 <sup>4</sup> |210|420|630|840|1050|1260|
...
<sup>4</sup> 如需了解任何单个数据库的最大并发辅助角色数（请求数），请参阅[单一数据库资源限制](resource-limits-vcore-single-databases.md)。 例如，如果弹性池使用 Gen5 且每个数据库的最大 vCore 数设置为 2，则最大并发辅助角色数目值为 200。  如果每个数据库的最大 vCore 数设置为 0.5，则最大并发辅助角色数目值为 50，因为 Gen5 上每个 vCore 的最大并发辅助角色数为 100。 对于每个数据库的最大 vCore 设置小于 1 个 vCore 或更少的其他情况，最大并发辅助角色数会相应重新缩放。
...
|存储类型|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|
|IO 延迟（近似）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|
|每个池的最大数据 IOPS <sup>3</sup>|2800|3200|3600|4000|6400|9600|
|每个池的最大日志速率 (MBps)|42|48|54|60|62.5|62.5|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|1470|1680|1890|2100|3360|5040|
|最大并发登录池（请求数）<sup>4</sup>|1470|1680|1890|2100|3360|5040|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|
...
|每个池的最大数据 IOPS <sup>3</sup>|800|1600|2400|3200|4000|4800|5600|
|每个池的最大日志速率 (MBps)|12|24|36|48|60|62.5|62.5|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|210|420|630|840|1050|1260|1470|
|每个池的最大并发登录数（请求数）<sup>4</sup>|210|420|630|840|1050|1260|1470|
...
|存储类型|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|
|IO 延迟（近似）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|
|每个池的最大数据 IOPS <sup>3</sup> |6,400|7,200|8,000|9,600|12,800|16,000|16,000|
|每个池的最大日志速率 (MBps)|62.5|62.5|62.5|62.5|62.5|62.5|62.5|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|1680|1890|2100|2520|3360|4200|8400|
|每个池的最大并发登录数（请求数）<sup>4</sup>|1680|1890|2100|2520|3360|4200|8400|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|30,000|
...
|最大日志大小 (GB) <sup>2</sup>|336|336|336|336|512|
...
|每个池的最大数据 IOPS <sup>3</sup>|2560|3200|3840|4480|5120|
|每个池的最大日志速率 (MBps)|48|60|62.5|62.5|62.5|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|400|500|600|700|800|
|每个池的最大并发登录数（请求数）<sup>4</sup>|800|1000|1200|1400|1600|
...
|IO 延迟（近似）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|
|每个池的最大数据 IOPS <sup>3</sup>|5760|6400|7680|10240|11520|12800|
|每个池的最大日志速率 (MBps)|62.5|62.5|62.5|62.5|62.5|62.5|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|900|1000|1200|1600|1800|3600|
|每个池的最大并发登录数（请求数）<sup>4</sup>|1800|2000|2400|3200|3600|7200|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|
...
|每个池的最大数据 IOPS <sup>3</sup>|800|1600|2400|3200|
|每个池的最大日志速率 (MBps)|12|24|36|48|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|168|336|504|672|
|每个池的最大并发登录数（请求数）<sup>4</sup>|168|336|504|672|
...
|最大日志大小 (GB) <sup>2</sup>|307|307|307|307|307|
|TempDB 最大数据大小 (GB)|64|96|128|160|192|
|[最大本地存储大小](resource-limits-logical-server.md#storage-space-governance) (GB)|1356|1356|1356|1356|1356|
...
|每个池的最大数据 IOPS <sup>3</sup>|9,000|13,500|18,000|22,500|27,000|
|每个池的最大日志速率 (MBps)|20|30|40|50|60|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|420|630|840|1050|1260|
|每个池的最大并发登录数（请求数）<sup>4</sup>|420|630|840|1050|1260|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|
...
|最大日志大小 (GB) <sup>2</sup>|307|307|307|307|307|307|
|TempDB 最大数据大小 (GB)|224|256|288|320|512|768|
...
|每个池的最大数据 IOPS <sup>3</sup>|31,500|36,000|40,500|45,000|72,000|96,000|
|每个池的最大日志速率 (MBps)|70|80|80|80|80|80|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|1470|1680|1890|2100|3360|5040|
|每个池的最大并发登录数（请求数）<sup>4</sup>|1470|1680|1890|2100|3360|5040|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|
|每个数据库的最小/最大弹性池 vCore 选项|0, 0.25, 0.5, 1...7|0, 0.25, 0.5, 1...8|0, 0.25, 0.5, 1...9|0, 0.25, 0.5, 1...10|0, 0.25, 0.5, 1...10, 16|0, 0.25, 0.5, 1...10, 16, 24|
|副本数|4|4|4|4|4|4|
...
|最大日志大小 (GB) <sup>2</sup>|307|307|461|461|922|922|
|TempDB 最大数据大小 (GB)|128|192|256|320|384|448|
|[最大本地存储大小](resource-limits-logical-server.md#storage-space-governance) (GB)|4829|4829|4829|4829|4829|4829|
|存储类型|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|
|IO 延迟（近似）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|
|每个池的最大数据 IOPS <sup>3</sup>|18,000|27,000|36,000|45,000|54,000|63,000|
|每个池的最大日志速率 (MBps)|60|90|120|120|120|120|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|420|630|840|1050|1260|1470|
|每个池的最大并发登录数（请求数）<sup>4</sup>|420|630|840|1050|1260|1470|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|
...
|最大日志大小 (GB) <sup>2</sup>|922|922|922|1229|1229|1229|1229|
|TempDB 最大数据大小 (GB)|512|576|640|768|1024|1280|2560|
...
|每个池的最大数据 IOPS <sup>3</sup>|72,000|81,000|90,000|108,000|144,000|180,000|256,000|
...
|最大日志大小 (GB) <sup>2</sup>|171|213|256|299|341|384|
...
|存储类型|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|
|IO 延迟（近似）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|
|每个池的最大数据 IOPS <sup>3</sup>|12,499|15,624|18,748|21,873|24,998|28,123|
|每个池的最大日志速率 (MBps)|48|60|72|84|96|108|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|800|1,000|1,200|1,400|1,600|1,800|
|每个池的最大并发登录数（请求数）<sup>4</sup>|800|1,000|1,200|1,400|1,600|1,800|
...
|最大日志大小 (GB) <sup>2</sup>|427|512|683|1024|1024|
|TempDB 最大数据大小 (GB)|640|768|1024|2048|4096|
|[最大本地存储大小](resource-limits-logical-server.md#storage-space-governance) (GB)|13836|13836|13836|13836|13836|
|存储类型|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|
|IO 延迟（近似）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|
|每个池的最大数据 IOPS <sup>3</sup>|31,248|37,497|49,996|99,993|160,000|
|每个池的最大日志速率 (MBps)|120|144|192|264|264|
|每个池的最大并发辅助角色数（请求数）<sup>4</sup>|2,000|2,400|3,200|6,400|12,800|
|每个池的最大并发登录数（请求数）<sup>4</sup>|2,000|2,400|3,200|6,400|12,800|
...
|最大日志大小 (GB) <sup>2</sup>|230|230|230|230|
|TempDB 最大数据大小 (GB)|64|128|192|256|
...
|每个池的最大数据 IOPS <sup>3</sup>|15750|31500|47250|56000|
