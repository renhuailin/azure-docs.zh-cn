...
8. 在 `AppDelegate.cs` 中创建 `MSNotificationHubDelegate` 的实现：
...
10. 在 `AppDelegate.cs` 中，为 `AzureNotificationHubListener` 类实现 `DidReceivePushNotification` 方法：
