...
4. 在 `MainActivity` 类中删除 `NotificationHub` 和 `GoogleCloudMessaging` 的专用字段，并为 `Notifications` 添加一个字段：
    ...
    ```
5. 然后，在 `onCreate` 方法中删除 `hub` 字段和 `registerWithNotificationHubs` 方法的初始化。 然后，添加将初始化 `Notifications` 类实例的以下行。
