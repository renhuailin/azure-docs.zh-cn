...
## <a name="privacy-faq-for-developers"></a>隐私常见问题解答（面向开发人员）

**问：当应用程序将空间定位点放置在某处时，是否所有应用都有权访问它？**

**答:** 定位点按 Azure 帐户进行隔离。 只有你向帐户授予了访问权限的应用才能访问帐户中的定位点。

问：在我的应用或 SDK 中使用 ASA 时，适用哪些条款和条件？

答：以下[条款](https://www.nuget.org/packages/Microsoft.Azure.SpatialAnchors.WinRT/2.9.0/License)以及该许可中嵌入的任何条款均适用。

### <a name="i-want-to-release-an-app-or-sdk-that-includes-asa"></a>我想发布包含 ASA 的应用或 SDK
问：是否需要同意任何其他条款？

**答:** 不是。 使用 ASA 即表示你同意上述链接 ToU 和嵌入条款。 无需执行其他步骤。

问：Microsoft 是否要求在应用程序或 SDK 中向用户披露 ASA？

答：Microsoft 不要求披露，除非当地法律要求这样做来满足隐私要求或其他要求。  应与自己的法律顾问合作，以确定当地的法律是否要求披露 ASA 的使用或 Microsoft 的隐私惯例。

问：我的用户是否需要同意任何特定条款？

**答:** 不是。 合同关系存在于 Microsoft 和应用或 SDK 的开发者之间。  应与自己的法律顾问合作，以确定当地的法律是否要求同意使用 ASA 或 Microsoft 的隐私惯例。

问：我想将我的应用/SDK 使用 ASA 这一情况通知我的用户，建议使用哪个接口来通知用户？

答：“此应用正在使用 Microsoft 的 Azure 空间定位点。  有关 Azure 空间定位点的详细信息，请转到 [Azure 空间定位点 | Microsoft Azure](https://azure.microsoft.com/en-us/services/spatial-anchors/)”
