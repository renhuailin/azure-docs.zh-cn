...
- [获取任务计数](/rest/api/batchservice/job/gettaskcounts)可以获取一个作业中处于“活动”、“正在运行”和“已完成”状态的任务以及处于“已成功”或“已失败”状态的任务的聚合计数。 通过对每种状态的任务计数，可以更轻松地为用户展现作业进度，或检测可能影响作业的意外延迟或故障。

- [列出池节点计数](/rest/api/batchservice/account/listpoolnodecounts)获取每个池中处于不同状态（“正在创建”、“空闲”、“脱机”、“已占用”、“正在重启”、“正在重置映像”、“正在启动”、“其他”）的专用计算节点和低优先级计算节点的数目。 通过对每种状态的节点计数，你就可以确定是否有足够的计算资源来运行作业，并确定池可能存在的问题。
...
- 成功：执行结果为 `success` 的任务。 Batch 通过检查 [executionInfo](/rest/api/batchservice/task/get) 属性的 `TaskExecutionResult` 属性来确定任务是已成功还是已失败。
- 失败：执行结果为 `failure` 的任务。
...
- Idle：可用计算节点目前未运行任务。
- LeavingPool：节点正在离开池，可能是因为用户显式删除了它，或者是因为池正在重设大小或者正在进行向下自动缩放。
- Offline：Batch 无法用来计划新任务的节点。
- Preempted：由于 Azure 回收了 VM，低优先级节点从池中删除。 如果系统提供替代性的低优先级 VM 容量，则`preempted`节点可以重新初始化。
- Rebooting：节点正在重启。
- Reimaging：操作系统正在节点上重新安装。
- Running：节点正在运行一个或多个任务（不是启动任务）。
- Starting：Batch 服务正在节点上启动。
- StartTaskFailed：节点上的[启动任务](/rest/api/batchservice/pool/add#starttask)失败，已达到重试次数限制，并且已在启动任务上设置 `waitForSuccess`。 此节点不可用于运行任务。
- Unknown：节点失去与 Batch 服务的联系，其状态未知。
- Unusable：节点因错误而不能用于执行任务。
- WaitingForStartTask：节点上的启动任务已开始运行，但是设置了 `waitForSuccess`，启动任务尚未完成。
