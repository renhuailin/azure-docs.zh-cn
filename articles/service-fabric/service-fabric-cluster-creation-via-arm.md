...
你可以在此处找到适用于 [Windows](https://github.com/Azure-Samples/service-fabric-cluster-templates/tree/master/5-VM-Windows-1-NodeTypes-Secure-NSG) 以及 [Ubuntu](https://github.com/Azure-Samples/service-fabric-cluster-templates/tree/master/5-VM-Ubuntu-1-NodeTypes-Secure) 的默认模板。

以下命令可以创建 Windows 或 Linux 群集，具体取决于如何指定 OS 参数。 PowerShell/CLI 命令均在指定的 CertificateOutputFolder（请确保在运行此命令前已存在所指定的证书文件夹位置！）中输出证书。
