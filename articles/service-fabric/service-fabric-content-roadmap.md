...
* 运行状况查询（通过 [PowerShell](/powershell/module/servicefabric/?preserve-view=true&view=azureservicefabricps)、[CLI](service-fabric-sfctl.md)、[C# FabricClient API](/dotnet/api/system.fabric.fabricclient.healthclient) 和 [Java FabricClient API](/java/api/system.fabric) 或 [REST API](/rest/api/servicefabric)）。
