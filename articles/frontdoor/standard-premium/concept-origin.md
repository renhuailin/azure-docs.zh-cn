...
Azure Front Door 标准版/高级版源是指为客户端请求提供服务的应用程序的主机名或公共 IP。 Azure Front Door 标准版/高级版同时支持源组中的 Azure 和非 Azure 资源。 此外，还可在本地数据中心或通过其他云服务提供商托管该应用程序。 不得将源与数据库层或存储层混淆。 应将源视为应用程序后端的公共终结点。 如果要将源添加到 Azure Front Door 标准版/高级版源组，还必须添加以下信息：
...
* **间隔（秒）** ：定义向源发送运行状况探测的频率，或每个 Front Door 环境发送探测的时间间隔。
    ...
    >[!NOTE]
    >为了更快地进行故障转移，请将时间间隔设为较小值。 值越低，你的后端接收到的运行状况探测量越多。 例如，如果将时间间隔设置为 30 秒（并且假设全球有 100 个 Front Door POP），则每个后端每分钟将收到约 200 个探测请求。
