---
title: 对 Azure Front Door 标准版/高级版中的文件压缩进行故障排除
description: 了解如何排查 Azure Front Door 中的文件压缩问题。 本文介绍几种可能的原因。
services: frontdoor
author: duongau
...
---
# <a name="troubleshooting-azure-front-door-standardpremium-file-compression"></a>对 Azure Front Door 标准版/高级版文件压缩进行故障排除

> [!Note]
> 本文档适用于 Azure Front Door 标准/高级（预览版）。 正在寻找有关 Azure Front Door 的信息？ 请查看[此处](../front-door-overview.md)。

本文可帮助你排查 Azure Front Door 标准版/高级版文件压缩问题。
...
> [!TIP]
> 与部署新的终结点一样，AFD 配置变更也需要一些时间才能传播到整个网络。  通常情况下会在 90 分钟内应用更改。  如果这是首次为 CDN 终结点设置压缩，应考虑等待 1-2 个小时，以确保压缩设置传播到 POP。 
> 

### <a name="verify-the-request"></a>验证请求

首先，应仔细检查请求。 可以使用浏览器的[开发人员工具](https://developer.microsoft.com/microsoft-edge/platform/documentation/f12-devtools-guide/)来查看正在发出的请求。

* 验证请求是否被发送到终结点 URL `<endpointname>.z01.azurefd.net`，而不是源。
...
在 [Azure 门户](https://portal.azure.com)中导航到终结点，然后在“路由”面板中选择“配置”按钮。 验证压缩是否已启用。
...
**Via** HTTP 标头指明了由代理服务器正在将请求传递到的 web 服务器。  默认情况下，当请求包含 Via 标头时，Microsoft IIS Web 服务器不会压缩响应。  若要替代此行为，请执行以下操作：

* IIS 6：在 IIS 元数据库属性中设置 HcNoCompressionForProxies ="FALSE" 有关信息，请参阅 [IIS 6 压缩](/previous-versions/iis/6.0-sdk/ms525390(v=vs.90))。
* IIS 7 及更高版本：在服务器配置中将 noCompressionForHttp10 和 noCompressionForProxies 设置为 False。 有关详细信息，请参阅 [HTTP 压缩](https://www.iis.net/configreference/system.webserver/httpcompression)。
