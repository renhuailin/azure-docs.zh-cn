...
ms.locfileid: "111353311"
---
# <a name="understand-the-structure-and-syntax-of-bicep-files"></a>了解 Bicep 文件的结构和语法

本文介绍 Bicep 文件的结构。 本文演示了文件的不同部分，以及可在相应部分使用的属性。

本文适用对象为对 Bicep 文件有一定了解的用户。 其中提供了有关模板结构的详细信息。 若要通过分步教程来了解创建 Bicep 文件的过程，请参阅《[教程：创建和部署第一个 Azure 资源管理器 Bicep 文件](bicep-tutorial-create-first-bicep.md)》。

## <a name="template-format"></a>模板格式

Bicep 文件具有以下元素。 这些元素可按任意顺序显示。
...
例如，可以添加 SKU 参数以指定资源的不同大小。 可以使用模板函数来创建默认值，例如获取资源组位置。
...
有关可用的数据类型，请参阅[模板中的数据类型](data-types.md)。

有关详细信息，请参阅[模板中的参数](template-parameters.md)。
...
有关详细信息，请参阅[模板中的变量](template-variables.md)。
...
有关详细信息，请参阅[模板中的资源声明](resource-declaration.md)。
...
有关详细信息，请参阅[使用 Bicep 模块](bicep-modules.md)。
...
有关详细信息，请参阅[模板中的输出](template-outputs.md)。
...
有关 Bicep 的简介，请参阅 [Bicep 是什么？](bicep-overview.md)。
