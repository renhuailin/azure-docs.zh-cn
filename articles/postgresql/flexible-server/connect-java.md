---
title: 快速入门：将 Java和 JDBC 与 Azure Database for PostgreSQL 灵活服务器配合使用
description: 本快速入门介绍如何将 Java 和 JDBC 与 Azure Database for PostgreSQL 灵活服务器配合使用。
author: mksuni
ms.author: sumuth
...
ms.locfileid: "113288097"
---
# <a name="quickstart-use-java-and-jdbc-with-azure-database-for-postgresql-flexible-server"></a>快速入门：将 Java和 JDBC 与 Azure Database for PostgreSQL 灵活服务器配合使用

本主题演示如何创建示例应用程序，使其使用 Java 和 [JDBC](https://en.wikipedia.org/wiki/Java_Database_Connectivity) 在 [Azure Database for PostgreSQL 灵活服务器](./index.yml)中存储和检索信息。
...
> [!NOTE]
> 可以在[使用 Azure 门户创建 Azure Database for PostgreSQL 服务器](./quickstart-create-server-portal.md)中阅读有关创建 PostgreSQL 服务器的更多详细信息。
...
你在前面创建的 PostgreSQL 服务器有一个空数据库 postgres。 在本快速入门中，我们将按照以下步骤创建一个名为 `demo` 的新数据库：

1. 安装 [psql](https://www.postgresql.org/docs/current/static/app-psql.html) 的本地实例以连接到 Azure PostgreSQL 服务器。 
2. 运行以下 psql 命令，以连接到 PostgreSQL 服务器上的 postgres 数据库
...
3. 在出现提示时键入以下命令，以创建名为“demo”的新数据库：
