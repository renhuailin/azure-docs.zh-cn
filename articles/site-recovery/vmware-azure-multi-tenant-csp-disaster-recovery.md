...
1. 你可以通过 CSP 门户将新用户添加到租户订阅，如下所示：
    ...
    1. 现在，通过输入相关详细信息并选择权限，或者通过上传 CSV 文件中的用户列表来创建新用户。

    1. 创建新用户之后，请返回到 Azure 门户。 在“订阅”页上，选择相关的订阅。

    1. **(IAM) 选择 "访问控制**"，然后单击 "**角色分配**"。

    1. 单击 " **添加角色分配** " 添加具有相关访问级别的用户。 通过 CSP 门户创建的用户会显示在“角色分配”选项卡上。
