...
1. 在 Azure Sentinel 导航菜单中，选择“数据连接器”。 在连接器列表中，选择“安全事件”，然后选择细节窗格上的“打开连接器页” 。 然后按照“说明”选项卡下的屏幕说明进行操作（如此部分其余内容所述）。
...
1. 在要将其安全事件流式传输到 Azure Sentinel 的计算机上下载并安装 [Log Analytics 代理](../azure-monitor/agents/log-analytics-agent.md)（也称为 Microsoft Monitoring Agent 或 MMA）。
    ...
    1. 选择“在 Azure Windows 虚拟机上安装代理”，然后点击下方显示的链接。
    1. 对于要连接的每个虚拟机，在右侧显示的列表中选择其名称，然后选择“连接”。

    对于非 Azure Windows 计算机（物理、虚拟本地或其他云中的虚拟）：

    1. 选择“在非 Azure Windows 计算机上安装代理”，然后点击下方显示的链接。
    1. 选择“Windows 计算机”下方右侧显示的相应下载链接。
...
1. 在 Azure Sentinel 导航菜单中，选择“数据连接器”。 在连接器列表中，选择“Windows 安全事件（预览版）”，然后单击右下角的“打开连接器页面”按钮 。 然后按照“说明”选项卡下的屏幕说明进行操作（如此部分其余内容所述）。
...
1. 在 Azure Sentinel 门户中，单击“分析”，然后选择“规则模板”选项卡。选择“（预览版）匿名 RDP 登录检测”并将“状态”滑块移动到“已启用”    。
