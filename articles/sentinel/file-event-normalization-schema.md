...
从 [Azure Sentinel GitHub 存储库](https://aka.ms/AzSentinelFileEvent)部署[与源无关的分析器和特定于源的分析器](normalization.md#parsers)。
...
## <a name="normalized-content-for-process-activity-data"></a>进程活动数据的已规范化内容

以下 Azure Sentinel 分析规则适用于使用 Azure Sentinel 信息模型规范化的任何文件活动：

- SUNBURST 和 SUPERNOVA 后门哈希（规范化文件事件）
- 2021 年 3 月 IoC 匹配中披露的 Exchange Server 漏洞
- HAFNIUM UM 服务写入可疑文件
- NOBELIUM - 域、哈希和 IP IOC - 2021 年 5 月
- SUNSPOT 日志文件创建 
...
| <a name="eventoriginalresultdetails"></a>EventOriginalResultDetails  | 可选    | 字符串     | 描述事件的结果。 请注意，此值未规范化，旨在用作源提供的原始值。 目前，此架构没有相关的规范化字段，即 EventResultDetails |
...
| **EventProductVersion** | 可选    | 字符串     | 生成事件的产品的版本。 <br><br>示例： `12.1`<br><br>注意：此字段可能未在源记录中提供。 在这种情况下，此字段必须由分析器设置。           |
...
| ActingProcessId| 必需    | 整数        | 操作进程的进程 ID (PID)。<br><br>示例：`48610176`           <br><br>注意：类型定义为字符串以支持不同的系统，但在 Windows 和 Linux 上，此值必须是数字。 <br><br>如果你使用的是 Windows 或 Linux 计算机并使用了其他类型，请务必转换值。 例如，如果使用了十六进制值，请将其转换为十进制值。    |
...
| <a name="actoruserid"></a>ActorUserId    | 建议  | 字符串     |   操作者的唯一 ID。 具体的 ID 取决于生成事件的系统。 有关详细信息，请参阅[用户实体](normalization.md#the-user-entity)。  <br><br>示例： `S-1-5-18`    |
...
|**URL**     |  `https://1drv.ms/p/s!Av04S_*********we`       | 在以 URL 形式提供文件路径时使用。 URL 并不局限于使用 http 或 https 作为方案，任何有效值（包括 FTP 值）都是有效的。         |
|     |         |         |
