---
title: 可视化收集的数据
description: 了解如何使用 Azure Sentinel 快速查看和监视整个环境中发生的情况。
services: sentinel
author: yelevin
manager: rkarlin
...
ms.locfileid: "121782920"
---
# <a name="visualize-collected-data"></a>可视化收集的数据

在本文中，你将了解如何使用 Azure Sentinel 快速查看和监视整个环境中发生的情况。 将数据源连接到 Azure Sentinel 之后，可以即时可视化和分析数据，以了解所有已连接的数据源中发生的情况。 Azure Sentinel 提供工作簿，让你利用 Azure 中已提供的工具的强大功能，并提供内置的表和图表，用于分析日志与查询。 可以使用内置的工作簿，或者从头开始或基于现有的工作簿轻松创建新的工作簿。 
...
1. 选择要在其中创建该工作簿的订阅，并为其指定一个描述性的名称。 与其他任何元素一样，每个工作簿都是一个 Azure 资源，你可为其分配角色 (Azure RBAC) 以定义和限制哪些用户可以访问它。
1. 若要使其显示在要将可视化效果固定到的工作簿中，必须将其共享。 依次单击“共享”、“管理用户”。 
...
若要详细了解如何获取现成的检测，请参阅[获取内置分析](detect-threats-built-in.md)。

## <a name="next-steps"></a>后续步骤
本快速入门介绍了如何开始使用 Azure Sentinel。 请继续学习有关[如何检测威胁](detect-threats-built-in.md)的文章。

> [!div class="nextstepaction"]
> [创建自定义威胁检测规则](detect-threats-custom.md)，以便自动针对威胁做出响应。
