...
ms.locfileid: "121725121"
---
# <a name="azure-sentinel-process-event-normalization-schema-reference-public-preview"></a>Azure Sentinel 进程事件规范化架构参考（公共预览版）

进程事件规范化架构用于描述执行和终止进程的操作系统活动。 此类事件由操作系统以及安全系统报告，例如 EDR（终结点检测和响应）系统。 由 OSSEM 定义的进程是表示程序的运行实例的包含和管理对象。 请注意，进程不会运行。 相反，进程管理运行和执行代码的线程。
...
- “imProcessCreate”，用于需要进程创建信息的查询。 这是最常见的情况。
...
从 [Azure Sentinel GitHub 存储库](https://aka.ms/AzSentinelProcessEvents)部署[与源无关的分析器和特定于源的分析器](normalization.md#parsers)。

## <a name="add-your-own-normalized-parsers"></a>添加自己的规范化分析器

为[进程事件](normalization.md#the-process-entity)信息模型实现自定义分析器时，请使用以下语法命名 KQL 函数：`imProcess<Type><vendor><Product>`，其中 `Type` 为 `Create`、`Terminate` 或`Event`（如果分析器同时实现创建和终止事件）。

将 KQL 函数添加到 `imProcess<Type>` 和 `imProcess` 与源无关的分析器，以确保使用[进程事件](normalization.md#the-process-entity)模型的任何内容也使用新的分析器。
...
- “分析规则”：
    ...
    - 回收站中的恶意软件（规范化进程事件）
    - NOBELIUM - vbscript 的可疑 rundll32.exe 执行（规范化进程事件）
...
> [!NOTE]
> Log Analytics 还会添加与安全用例相关性较低的其他字段。 有关详细信息，请参阅 [Azure Monitor 日志中的标准列](../azure-monitor/logs/log-standard-columns.md)。
>
...
| **EventResult**         | 必需   | Enumerated |  描述事件的结果，规范化为以下支持的值之一： <br><br>- `Success`<br>- `Partial`<br>- `Failure`<br>- `NA`（不适用） <br><br>源可以只提供“EventResultDetails”字段的值，必须分析该字段才能获取“EventResult”值。 <br><br>请注意，进程事件通常只报告成功。 |
...
| “ActorUsernameType”              | 必需    | Enumerated |   指定 [ActorUsername](#actorusername) 字段中存储的用户名的类型。 有关详细信息，请参阅[用户实体](normalization.md#the-user-entity)。 <br><br>示例： `Windows`       |
| <a name="actoruserid"></a>“ActorUserId”    | 建议  | 字符串     |   Actor 的唯一 ID。 特定的 ID 取决于生成事件的系统。 有关详细信息，请参阅[用户实体](normalization.md#the-user-entity)。  <br><br>示例： `S-1-5-18`    |
| “ActorUserIdType”| 建议  | 字符串     |  [ActorUserId](#actoruserid) 字段中存储的 ID 的类型。 有关详细信息，请参阅[用户实体](normalization.md#the-user-entity)。 <br><br>示例： `SID`         |
...
| “TargetUsernameType”             | 进程创建事件所必需。   | Enumerated | 指定 [TargetUsername](#targetusername) 字段中存储的用户名的类型。 有关详细信息，请参阅[用户实体](normalization.md#the-user-entity)。          <br><br>  示例：`Windows`        |
|<a name="targetuserid"></a>“TargetUserId”   | 建议 | 字符串     | 目标用户的唯一 ID。 特定的 ID 取决于生成事件的系统。 有关详细信息，请参阅[用户实体](normalization.md#the-user-entity)。            <br><br> 示例： `S-1-5-18`    |
| “TargetUserIdType”               | 建议 | 字符串     | [TargetUserId](#targetuserid) 字段中存储的用户 ID 的类型。 有关详细信息，请参阅[用户实体](normalization.md#the-user-entity)。            <br><br> 示例：`SID`  |
...
| <a name="targetprocessname"></a>“TargetProcessName”              | 必需    | string     |目标进程的名称。 此名称通常是从映像或可执行文件派生的，该映像或可执行文件用于定义映射到进程虚拟地址空间的初始代码和数据。   <br><br>     示例：`C:\Windows\explorer.exe`     |
...
| “TargetProcessIsHidden”          | 可选     | Boolean    |   指示目标进程是否处于隐藏模式。  |
