...
ms.locfileid: "99806929"
---
# <a name="azure-sentinel-data-normalization-schema-reference"></a>Azure Sentinel 数据规范化架构参考
...
| 数据类型 | 物理类型 | 格式和值 |
| --------- | ------------- | ---------------- |
| **日期/时间** | 根据引入方法功能，按降序优先级使用：<ul><li>Log Analytics 内置的日期时间类型</li><li>使用 Log Analytics 日期时间数字表示形式的整数字段</li><li>使用 Log Analytics 日期时间数字表示形式的字符串字段</li></ul> | Log Analytics 日期时间表示形式。 <br></br>Log Analytics 日期和时间表示形式在性质上与 Unix 时间表示形式类似，但两者实际上是不同的。 请参阅这些转换准则。 <br></br>日期和时间应该经过时区调整。 |
...
| **用户** | String | 可以使用以下 3 个用户字段：<ul><li>用户名</li><li>用户 UPN</li><li>用户域</li></ul> |
| **用户 ID** | String | 目前支持以下 2 个用户 ID：<ul><li>用户 SID</li><li>Azure Active Directory ID</li></ul> |
| **设备** | String | 支持以下 3 个设备/主机列：<ul><li>ID</li><li>名称</li><li>完全限定的域名 (FQDN)</li></ul> |
| **国家/地区** | String | 根据以下优先级使用 ISO 3166-1 的字符串：<ul><li>Alpha-2 代码（例如，US 表示美国）</li><li>Alpha-3 代码（例如，USA 表示美国）</li><li>短名称</li></ul> |
| **区域** | String | 使用 ISO 3166-2 的国家/地区辖区名称 |
| **城市** | String | |
| **经度** | Double | ISO 6709 坐标表示形式（带符号的十进制数） |
| **纬度** | Double | ISO 6709 坐标表示形式（带符号的十进制数） |
| **哈希算法** | String | 支持以下 4 个哈希列：<ul><li>MD5</li><li>SHA1</li><li>SHA256</li><li>SHA512</li></ul> |
| **文件类型** | String | 文件类型的类型：<ul><li>扩展名</li><li>类</li><li>NamedType</li></ul> |
| 
...
|-|-|-|-|-|
| EventType | String | 交通 | 要收集的事件的类型 | 事件 |
| EventSubType | String | 身份验证 | 类型的附加说明（如果适用） | 事件 |
...
| DvcMacAddr | String | 06:10:9f:eb:8f:14 | 从中发送了事件的报告设备的网络接口的 MAC 地址。 | 设备，<br>Mac |
...
| EventReportUrl | String | https://192.168.1.1/repoerts/ae3-56.htm | 报告设备创建的完整报告的链接 | 事件 |
...
| EventSeverity | String | 低 | 如果报告的活动会造成安全影响，则指示影响的严重性。 | 事件 |
| EventOriginalUid | String | af6ae8fe-ff43-4a4c-b537-8635976a2b51 | 报告设备中的记录 ID。 | 事件 |
...
| DstGeoCity | String | Burlington | 与目标 IP 地址关联的城市 | 目标、<br>地域 |
| DstGeoCountry | 国家/地区（字符串） | USA | 与源 IP 地址关联的国家/地区 | 目标、<br>地域 |
| DstDvcHostname | 设备名称（字符串） |  victim_pc | 目标设备的设备名称 | 目标<br>设备 |
| DstDvcFqdn | String | victim_pc.contoso.local | 在其中创建了日志的主机的完全限定域名 | 目标、<br>设备 |
...
| DstMacAddr | String | 06:10:9f:eb:8f:14 | 连接或会话在其上终止的网络接口的 MAC 地址，最常见的称法是网络数据包中的目标 MAC | 目标、<br>MAC |
| DstDvcMacAddr | String | 06:10:9f:eb:8f:14 | 不直接与网络数据包关联的设备的目标 MAC 地址。 | 目标、<br>设备，<br>MAC |
| DstDvcDomain | String | CONTOSO | 目标设备的域。 | 目标、<br>设备 |
...
| DstUserSid | 用户 SID |  S-12-1445 | 与会话目标关联的标识的用户 ID。 通常，它是用来对服务器进行身份验证的标识。 有关详细信息，请参阅“数据类型”。 | 目标、<br>User |
| DstUserAadId | 字符串 (GUID) | ae92b0b4-cfba-4b42-85a0-fbd862f4df54 | 位于会话目标端的用户的 Azure AD 帐户对象 ID | 目标、<br>User |
| DstUserName | 用户名（字符串） | johnd | 与会话目标关联的标识的用户名。  | 目标、<br>User |
| DstUserUpn | 字符串 | johnd@anon.com | 与会话目标关联的标识的 UPN。 | 目标、<br>User |
| DstUserDomain | 字符串 | WORKGROUP | 位于会话目标中的帐户的域名或计算机名 | 目标、<br>User |
...
| DvcInboundInterface | String | eth0 | 如果中介设备（例如防火墙）报告了该值，则该值是该设备用来连接到源设备的网络接口。 | 设备 |
| DvcOutboundInterface | String  | 以太网适配器以太网 4 | 如果中介设备（例如防火墙）报告了该值，则该值是该设备用来连接到目标设备的网络接口。 | 设备 |
...
| NetworkIcmpType | String | 目标不可访问 | 对于 ICMP 消息，该值是 ICMP 消息类型的文本表示形式（RFC 2780 或 RFC 4443）。 | 网络 |
...
| NetworkRuleName | String | AnyAnyDrop | 确定 DeviceAction 时所依据的规则的名称或 ID | 网络 |
...
| SrcGeoCity | String | Burlington | 与源 IP 地址关联的城市 | 源、<br>地域 |
| SrcGeoCountry | 国家/地区（字符串） | USA | 与源 IP 地址关联的国家/地区 | 源、<br>地域 |
...
| SrcDvcOs | String | iOS | 源设备的 OS | 源、<br>设备 |
| SrcDvcModelName | String | Samsung Galaxy Note | 源设备的型号名称 | 源、<br>设备 |
| SrcDvcModelNumber | String | 10.0 | 源设备的型号 | 源、<br>设备 |
| SrcDvcType | String | 移动 | 源设备的类型 | 源、<br> 设备 |
| SrcIntefaceName | String | eth01 | 由源设备用来建立连接或会话的网络接口。 | 源 |
| SrcInterfaceGuid | String | 46ad544b-eaf0-47ef-827c-266030f545a6 | 所用网络接口的 GUID | 源 |
...
| SrcMacAddr | String | 06:10:9f:eb:8f:14 | 从中发起了连接或会话的网络接口的 MAC 地址。 | 源、<br>Mac |
| SrcDvcMacAddr | String | 06:10:9f:eb:8f:14 | 不直接与网络数据包关联的设备的源 MAC 地址。 | 源、<br>设备，<br>Mac |
| SrcPortNumber | 整数 | 2335 | 从中发起了连接的 IP 端口。 可能与包含多个连接的会话无关。 | 源、<br>端口 |
| SrcGeoRegion | 区域（字符串） | 佛蒙特州 | 与源 IP 地址关联的国家/地区中的区域 | 源、<br>地域 |
| SrcResourceId | String | /subscriptions/3c1bb38c-82e3-4f8d-a115-a7110ba70d05 /resourcegroups/contoso77/providers /microsoft.compute/virtualmachines /syslogserver1 | 生成消息的设备的资源 ID。 | 源 |
...
| SrcUserSid | 用户 ID（字符串） | S-15-1445 | 与会话源关联的标识的用户 ID。 通常，这是在客户端上执行操作的用户。 有关详细信息，请参阅“数据类型”。 | 源、<br>User |
| SrcUserAadId | 字符串 (GUID) | 16c8752c-7dd2-4cad-9e03-fb5d1cee5477 | 位于会话源端的用户的 Azure AD 帐户对象 ID | 源、<br>User |
| SrcUserName | 用户名（字符串） | bob | 与会话源关联的标识的用户名。 通常，这是在客户端上执行操作的用户。 有关详细信息，请参阅“数据类型”。 | 源<br>User |
| SrcUserUpn | 字符串 | bob@alice.com | 发起会话的帐户的 UPN | 源、<br>User |
| SrcUserDomain | 字符串 | 桌面 | 发起会话的帐户的域 | 源、<br>User |
| SrcZone | String | 点击 | 源的网络区域，由报告设备定义。 | 源 |
| NetworkProtocol | String | TCP | 连接或会话使用的 IP 协议。 通常为 TCP、UDP 或 ICMP | 网络 |
| CloudAppName | String | Facebook | HTTP 应用程序的目标应用程序的名称，由代理标识。 | 云 |
| CloudAppId | String | 124 | HTTP 应用程序的目标应用程序的 ID，由代理标识。 此值通常特定于所用的代理。 | 云 |
| CloudAppOperation | String | DeleteFile | 用户在 HTTP 应用程序的目标应用程序上下文中执行的操作，由代理标识。 此值通常特定于所用的代理。 | 云 |
...
| 文件路径 | String | C:\Malicious\ImNotMalicious.exe | 文件的完整路径，包括文件名 | 文件 |
| FileHashMd5 | String | 51BC68715FC7C109DCEA406B42D9D78F | 通过网络连接为协议传输的文件的 MD5 哈希值。 | 文件 |
| FileHashSha1 | String | 491AE3…C299821476F4 | 通过网络连接为协议传输的文件的 SHA1 哈希值。 | 文件 |
| FileHashSha256 | String | 9B8F8EDB…C129976F03 | 通过网络连接为协议传输的文件的 SHA256 哈希值。 | 文件 |
| FileHashSha512 | String | 5E127D…F69F73F01F361 | 通过网络连接为协议传输的文件的 SHA512 哈希值。 | 文件 |
| FileExtension |  String | exe | 通过网络连接为协议（例如 FTP 和 HTTP）传输的文件的类型。 | 文件
| FileMimeType | String | application/msword | 通过网络连接为协议（例如 FTP 和 HTTP）传输的文件的 MIME 类型 | 文件 |
...
| HttpContentType | String | multipart/form-data；boundary=something | HTTP/HTTPS 网络会话的 HTTP 响应内容类型头。 | Http |
| HttpReferrerOriginal | String | https://developer.mozilla.org/en-US/docs/Web/JavaScript | HTTP/HTTPS 网络会话的 HTTP referrer 头。 | Http |
| HttpUserAgentOriginal | String | Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36 | Http/HTTPS 网络会话的 HTTP 用户代理头。 | Http |
| HttpRequestXff | String | 120.12.41.1 | HTTP/HTTPS 网络会话的 HTTP X-Forwarded-For 头。 | Http |
| UrlCategory | String | 搜索引擎 | 定义的 URL 分组（也可能只是基于 URL 中的域），与该 URL 的内容相关（例如：成人、新闻、广告、暂停域等） | url |
...
| ThreatCategory | String | Trojan | 安全系统（例如 IPS 的 Web 安全网关）识别到的威胁的类别，与此网络会话相关联。 | 威胁 |
| ThreatId | String | Tr.124 | 安全系统（例如 IPS 的 Web 安全网关）识别到的威胁的 ID，与此网络会话相关联。 | 威胁 |
| ThreatName | String | EICAR 测试文件 | 识别到的威胁或恶意软件的名称 | 威胁 |
