...
## <a name="sap---system-applications-and-products-workbook"></a>SAP - 系统应用程序和产品工作簿

使用 [SAP - 系统应用程序和产品](sap-deploy-solution.md#deploy-sap-security-content)工作簿，对通过 SAP 数据连接器的数据进行可视化和监视。

例如：

:::image type="content" source="media/sap/sap-workbook.png" alt-text="SAP - 系统应用程序和产品工作簿。":::
...
### <a name="high-level-built-in-sap-solution-analytics-rules"></a>高级内置 SAP 解决方案分析规则
...
|**SAP - 高 - 数据在调试活动期间已更改**     | 标识调试活动期间对运行时数据的更改。         | 1. 激活调试 ("/h")。 <br>2. 选择要更改的字段并更新字段值。<br><br>**数据源**：SAPcon - 审核日志        |  执行、横向移动       |
...
|**SAP - 高 - 用户创建并使用新用户**     | 标识创建和使用其他用户的用户。        |  使用 SU01 创建一个用户，然后使用新建用户和相同的 IP 地址登录。<br><br>**数据源**：SAPcon - 审核日志 | 发现、预攻击、初始访问  |
|**SAP - 高 - 用户解锁并使用其他用户**     |标识由其他用户解锁和使用的用户。       |  使用 SU01 解锁用户，然后使用已解锁用户和相同的 IP 地址登录。<br><br>**数据源**：SAPcon - 审核日志、SAPcon - 更改文档日志 | 发现、预攻击、初始访问、横向移动  |
...
|**SAP - 高 - 执行敏感函数模块**     | 标识敏感 ABAP 函数模块的执行。     | 使用 SE37 直接运行敏感函数模块。 <br><br>**注意**：仅适用于生产系统。 <br><br>维护 [SAP - 敏感函数模块](#modules)播放列表中的敏感函数，并确保在 EUFUNC 表的后端激活表日志记录更改。 (SE13) <br><br>**数据源**：SAPcon - 表数据日志 | 发现、命令和控制 
|     |         |         |         |

### <a name="medium-level-built-in-sap-solution-analytics-rules"></a>中级内置 SAP 解决方案分析规则
...
|**SAP - 中 - 调试活动**     |  标识所有调试相关活动。       |在系统中激活调试 ("/h")、调试活动进程、向源代码添加断点等。 <br><br>**数据源**：SAPcon - 审核日志 |   发现      |
|**SAP - 中 - 通过 IP 多次登录**     |  标识来自同一 IP 地址的多位用户在计划的时间间隔内的登录。       |    以多位用户身份在同一 IP 地址登录。 <br><br>**数据源**：SAPcon - 审核日志 | 初始访问        |
|**SAP - 中 - 用户多次登录**     | 标识相同用户在计划的时间间隔内从多个终端的登录。  <br><br>仅通过 Audit SAL 方法使用，适用于 SAP 7.5 及更高版本。      |   以相同用户身份从不同的 IP 地址登录。   <br><br>**数据源**：SAPcon - 审核日志   |  预攻击、凭据访问、初始访问、集合       |
...
|**SAP - 中 - 敏感配置文件更改**     |标识敏感配置文件更改。  <br><br> 维护 [SAP - 敏感配置文件](#profiles)播放列表中的敏感配置文件。     |  使用 PFCG 更改角色。  <br><br>**数据源**：SAPcon - 更改文档日志、SAPcon - 审核日志   | 特权、升级、持久性、命令和控制 |
...
### <a name="low-level-built-in-sap-solution-analytics-rules"></a>低级内置 SAP 解决方案分析规则
...
|**SAP - 低 - 动态 RFC 目标**     |   使用动态目标标识 RFC 执行。 | 执行使用动态目标 (cl_dynamic_destination) 的 ABAP 报表。 例如，DEMO_RFC_DYNAMIC_DEST。   <br><br>**数据源**：SAPcon - 审核日志      |    集合、外泄     |
|     |         |         |         |

### <a name="informational-level-built-in-sap-solution-analytics-rules"></a>信息级别的内置 SAP 解决方案分析规则
