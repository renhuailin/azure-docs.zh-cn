...
下表提供了规范化数据值的指南，规范化字段需要这些数据值，并建议用于其他字段。

| 数据类型 | 物理类型 | 格式和值 |
| --------- | ------------- | ---------------- |
| **日期/时间** | 以下类型之一（根据所使用的引入方法功能，采用降序优先级）：<ul><li>Log Analytics 内置的日期时间类型</li><li>使用 Log Analytics 日期时间数字表示形式的整数字段</li><li>使用 Log Analytics 日期时间数字表示形式的字符串字段</li></ul> | Log Analytics 日期时间表示形式。 <br></br>Log Analytics 日期和时间表示形式与 Unix 时间表示形式类似，但两者实际上是不同的。 请参阅这些转换准则。 <br><br>必须针对时区调整日期和时间。 |
...
### <a name="removed-fields-in-version-02"></a>在 0.2 版中删除的字段
