...
|**别名**     |  在某些情况下，不同的用户要求某个字段具有不同的名称。 例如，在 DNS 术语中，用户需要一个名为 `query` 的字段，但更常见的情况是它有一个域名。 别名允许为指定的值使用多个名称，从而解决了此问题。 别名类与其别名化的字段相同。<br><br>请注意，Log Analytics 不支持别名。 若要实现别名，分析程序使用 `extend` 运算符创建原始值副本。        |
| | |
