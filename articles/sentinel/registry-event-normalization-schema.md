...
从 [Azure Sentinel GitHub 存储库](https://aka.ms/AzSentinelRegistry)部署[与源无关的分析器和特定于源的分析器](normalization.md#parsers)。
...
Azure Sentinel 提供了“通过 IFEO 注册表项持久化”的搜寻查询。 此查询适用于使用 Azure Sentinel 信息模型规范化的任何注册表活动数据。
...
| <a name="actingprocessname"></a>ActingProcessName              | 可选     | 字符串     |   操作进程映像文件的文件名。 通常被视为进程名称。  <br><br>示例： `C:\Windows\explorer.exe`  |
...
| **ParentProcessName**              | 可选     | 字符串     |  父进程映像文件的文件名。 被视为进程名称。    <br><br>示例： `C:\Windows\explorer.exe` |
| **ParentProcessId**| 必需    | 整数    | 父进程的进程 ID (PID)。   <br><br>     示例：`48610176`    |
