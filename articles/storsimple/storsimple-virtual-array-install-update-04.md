...
description: 介绍如何使用 StorSimple Virtual Array Web UI，通过 Azure 门户和修补程序方法应用更新 0.4。
services: storsimple
...
1. 在本地 Web UI 中，转到“维护” > “软件更新”。
    ...
    ![该屏幕截图显示从“维护”菜单中选择了“软件更新”。](./media/storsimple-virtual-array-install-update/update1m.png)

2. 在“更新文件路径”中，输入更新或修补程序的文件名。 也可以浏览到网络共享上的更新或修补程序安装文件。 单击“应用”。
   
    ![该屏幕截图显示“软件更新”页中的“更新文件路径”文本框。](./media/storsimple-virtual-array-install-update/update2m.png)

3. 显示一条警告。 假定这是单节点设备，应用更新后，设备将重新启动并且会出现停机。 单击选中图标。
   
   ![该屏幕截图显示一个关于停机时间警告的对话框。](./media/storsimple-virtual-array-install-update/update3m.png)

4. 更新开始。 成功更新设备后，该设备将重新启动。 本地 UI 在此期间不可访问。
   
    ![该屏幕截图显示更新的成功消息。](./media/storsimple-virtual-array-install-update/update5m.png)
...
5. 重新启动完成后，会转到“登录”页。 若要验证设备软件是否已更新，请在本地 Web UI 中，转到“维护” > “软件更新”。 针对 Update 0.4 显示的软件版本应为 **10.0.0.0.0.10289.0**。
   ...
   > 我们在本地 Web UI 和 Azure 门户中报告的软件版本稍有不同。 例如，针对同一版本，本地 Web UI 报告 **10.0.0.0.0.10289**，而 Azure 门户则报告 **10.0.10289.0**。
    ...
    ![该屏幕截图显示了具有当前软件版本的软件更新页。](./media/storsimple-virtual-array-install-update/update6m.png)
