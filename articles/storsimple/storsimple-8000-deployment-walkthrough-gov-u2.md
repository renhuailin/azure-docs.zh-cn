...
StorSimple Device Manager 服务可以管理多个 StorSimple 设备。 执行以下步骤创建 StorSimple Device Manager 服务的新实例。
...
在 Azure 政府门户中执行以下步骤，完成最基本的设备设置。
...
> [!IMPORTANT]
> StorSimple Device Manager 只能创建精简预配的卷。  但是不能创建部分预配的卷。
...
> * 如需 Windows Server 主机上的 MPIO 和 iSCSI 安装和配置说明，请转到 [为 StorSimple 设备配置 MPIO](./storsimple-8000-configure-mpio-windows-server.md)。 这些内容还将包括装载、初始化和格式化 StorSimple 卷的步骤。
...
在政府门户中执行以下步骤，在 StorSimple 设备上为单个卷创建按需手动备份。
