...
* **硬件问题** – 这些警报可向你通告硬件的运动状况。 通过这些警报，可以知道是否需要进行固件升级、网络接口是否存在问题，或者某个数据驱动器是否有问题。
...
为设备启用电子邮件通知后，每次发生关键警报时，通知列表成员都将收到电子邮件消息。 消息将从“storsimple-alerts-noreply\@mail.windowsazure.com”发送，并将描述警报条件。 收件人可以单击“取消订阅”将自己从电子邮件通知列表中删除。
...
2. 在“常规设置”边栏选项卡中，转到“警报设置”，并设置以下项：
   ...
   3. 在“其他电子邮件收件人”字段中，输入应接收警报通知的所有其他收件人的电子邮件地址。 用“someone\@somewhere.com”格式输入名称。 使用分号分隔电子邮件地址。 对于每个设备，最多可以配置 20 个电子邮件地址。 
...
5. 完成配置后，立即单击“保存”。 当系统提示你进行确认时，单击 **“是”**。
     ...
     ![警报测试通知电子邮件已发送 2](./media/storsimple-8000-manage-alerts/configure-alerts-email5.png)
...
| 警报文本 | 事件 | 详细信息/建议的操作 |
|:--- |:--- |:--- |
| 硬件组件环境温度传感器将状态报告为失败。  | 机箱类型：主机箱 | 当 StorSimple 的温度超出了可接受的范围时，将触发此警报。 检查环境外部温度或数据中心内 AC 通风管的气流。 当温度恢复正常时，警报会在经过一段时间后自动清除。 如果问题持续出现，请联系 Microsoft 支持。   |
