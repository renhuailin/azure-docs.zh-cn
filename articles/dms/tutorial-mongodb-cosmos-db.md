...
description: 使用 Azure 数据库迁移服务从本地 MongoDB 脱机迁移到用于 MongoDB 的 Azure Cosmos DB API。
services: dms
author: pochiraju
ms.author: rajpo
manager: craigg
...
ms.locfileid: "106443861"
---
# <a name="tutorial-migrate-mongodb-to-azure-cosmos-db-api-for-mongodb-offline"></a>教程：将 MongoDB 脱机迁移到用于 MongoDB 的 Azure Cosmos DB API

可以使用 Azure 数据库迁移服务，将数据库从 MongoDB 的本地或云实例一次性脱机迁移到用于 MongoDB 的 Azure Cosmos DB API。
...
在本教程中，你将在 MongoDB 中迁移 Azure 虚拟机中托管的数据集。 通过使用 Azure 数据库迁移服务，可以将数据集迁移到 MongoDB 的 Azure Cosmos DB API。 如果尚未设置 MongoDB 数据源，请参阅[在 Azure 中的 Windows VM 上安装和配置 MongoDB](/previous-versions/azure/virtual-machines/windows/install-mongodb)。

## <a name="prerequisites"></a>先决条件

要完成本教程，需要：

* [完成迁移前](../cosmos-db/mongodb-pre-migration.md)步骤，例如估计吞吐量和选择分区键。
...
* 确保虚拟网络的网络安全组 (NSG) 规则未阻止以下通信端口：53、443、445、9354 以及 10000-20000。 有关详细信息，请参阅[使用网络安全组筛选网络流量](../virtual-network/virtual-network-vnet-plan-design-arm.md)。
...
3. 在“创建迁移服务”界面上，为服务、订阅以及新的或现有资源组指定名称。
...
5. 选择现有虚拟网络或新建一个。
    ...
    有关如何在 Azure 门户中创建虚拟网络的详细信息，请参阅[使用 Microsoft Azure 门户创建虚拟网络](../virtual-network/quick-create-portal.md)一文。
...
创建服务后，在 Microsoft Azure 门户中找到并打开它。 创建新迁移项目。

1. 在 Azure 门户中，选择“所有服务”，搜索 Azure 数据库迁移服务，然后选择“Azure 数据库迁移服务”。

      ![显示如何查找 Azure 数据库迁移服务的所有实例的屏幕截图。](media/tutorial-mongodb-to-cosmosdb/dms-search.png)

2. 在“Azure 数据库迁移服务”屏幕上，搜索你创建的 Azure 数据库迁移服务实例名称，然后选择该实例。

3. 选择“+ 新建迁移项目”。

4. 在“新迁移项目”上指定项目名称，在“源服务器类型”文本框中选择“MongoDB”。 在“目标服务器类型”文本框中选择“CosmosDB (MongoDB API)”，然后为“选择活动类型”选择“脱机数据迁移”。 
...
1. 在“源详细信息”  屏幕上，指定源 MongoDB 服务器的连接详细信息。
   ...
   > [!IMPORTANT]
   > Azure 数据库迁移服务不支持将 Azure Cosmos DB 作为数据源。
   ...
   * **Azure 存储中的数据**：接受 Blob 容器 SAS URL。 如果 Blob 容器包含 MongoDB [bsondump 工具](https://docs.mongodb.com/manual/reference/program/bsondump/)生成的 BSON 转储，请选择“Blob 包含 BSON 转储”。 如果容器包含 JSON 文件，请勿选择此选项。
     ...
     可以在 Microsoft Azure 存储资源管理器中找到此 blob 容器 SAS 连接字符串。 为相关容器创建 SAS 将为你提供此请求格式的 URL。
     
     此外，根据 Azure 存储中的类型转储选项，记住以下信息：

     * 对于 BSON 转储，blOb 容器内的数据必须为 bsondump 格式。 将数据文件放入以 collection.bson 格式的包含数据库命名的文件夹中。 使用 collection.metadata.json 格式命名所有元数据文件。

     * 对于 JSON 转储，blob 容器中的文件必须放置到以包含数据库命名的文件夹中。 在每个数据库文件夹中，数据文件必须放入名为“data”，且采用 collection.json 格式命名的子文件夹中。 将所有元数据文件放入名为“metadata”，且同样采用 collection.json 格式命名的子文件夹中。 元数据文件必须采用由 MongoDB bsondump 工具所生成的相同格式。

    > [!IMPORTANT]
    > 不建议在 MongoDB 服务器上使用自签名证书。 如有必要，请使用连接字符串模式连接到服务器，并确保连接字符串具有引号 ("")。
    >
    >```
...
1. 在“迁移源详细信息”界面上，指定目标 Azure Cosmos DB 帐户的连接详细信息。 此帐户是将要接收所迁入 MongoDB 数据的 MongoDB 帐户的预配 Azure Cosmos DB API。
...
1. 在“映射到目标数据库”  屏幕上，映射源和目标数据库以进行迁移。
    ...
    如果 “Create” 显示在数据库名称旁边，则表明 Azure 数据库迁移服务没有找到目标数据库，因此该服务会为你创建该数据库。

    在迁移的这个时候，可以[预配吞吐量](../cosmos-db/set-throughput.md)。 在 Azure Cosmos DB 中，你可以在数据库级别预配吞吐量，也可为每个集合单独进行预配。 吞吐量以[请求单位](../cosmos-db/request-units.md)来度量。 深入了解 [Azure Cosmos DB 定价](https://azure.microsoft.com/pricing/details/cosmos-db/)。

    ![显示目标数据库映射的屏幕截图。](media/tutorial-mongodb-to-cosmosdb/dms-map-target-databases.png)

2. 选择“保存”  。
3. 在“集合设置”屏幕上，展开集合列表，然后查看要迁移的集合的列表。 

    Azure 数据库迁移服务会自动选择存在于源 MongoDB 实例上存在，但目标 Azure Cosmos DB 帐户上不存在的所有集合。 若要重新迁移已包含数据的集合，你需要在此窗格上明确选择集合。

    可以指定希望集合使用的 RU 数量。 Azure 数据库迁移服务建议使用根据集合大小设置的智能默认值。

    > [!NOTE]
    > 并行执行数据库迁移和收集。 如有必要，可以使用 Azure 数据库迁移服务的多个实例加快运行速度。
...
5. 在“迁移摘要”屏幕的“活动名称”文本框中指定迁移活动的名称。  
    ...
    ![显示迁移摘要的屏幕截图。](media/tutorial-mongodb-to-cosmosdb/dms-migration-summary.png)
...
在迁移活动界面上，选择“刷新”来更新显示，直到迁移状态显示为“已完成”。

> [!NOTE]
> 你可以选择活动，以便获取数据库级别和集合级别迁移指标的详细信息。

![显示活动状态已完成的屏幕截图。](media/tutorial-mongodb-to-cosmosdb/dms-activity-completed.png)
...
将 MongoDB 数据库中存储的数据迁移到用于 MongoDB 的 Azure Cosmos DB API 后，你可以连接到 Azure Cosmos DB 并管理该数据。 你还可以执行其他迁移后的优化步骤。 其中可能包括优化索引策略、更新默认的一致性级别或为 Azure Cosmos DB 帐户配置全局分发。 有关详细信息，请参阅[迁移后优化](../cosmos-db/mongodb-post-migration.md)。

## <a name="next-steps"></a>后续步骤

参阅 [Azure 数据库迁移指南](https://datamigration.microsoft.com/)，查看迁移指南的其他方案。
