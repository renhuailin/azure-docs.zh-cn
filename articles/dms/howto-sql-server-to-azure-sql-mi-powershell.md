...
* **AdventureWorks2016** 数据库的本地副本，可从 [此处](/sql/samples/adventureworks-install-configure?view=sql-server-2017)下载。
...
* 使用 [Install-Module PowerShell cmdlet](/powershell/module/powershellget/Install-Module?view=powershell-5.1) 从 PowerShell 库下载并安装 `Az.DataMigration` 模块（0.7.2 或更高版本）。
...
创建连接安全凭据作为 [PSCredential](/dotnet/api/system.management.automation.pscredential?view=powershellsdk-1.1.0) 对象。
