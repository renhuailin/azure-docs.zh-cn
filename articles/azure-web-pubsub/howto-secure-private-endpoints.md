---
title: 如何通过 Azure 专用终结点保护 VNet 与 Azure Web PubSub 服务之间的流量
description: 有关用于从虚拟网络安全访问 Azure Web PubSub 服务的专用终结点的概述。
author: yjin81
ms.author: yajin1
...
ms.locfileid: "108166867"
---
# <a name="use-private-endpoints-for-azure-web-pubsub-service"></a>针对 Azure Web PubSub 服务使用专用终结点

可针对 Azure Web PubSub 服务使用[专用终结点](../private-link/private-endpoint-overview.md)，以允许虚拟网络 (VNet) 中的客户端通过[专用链接](../private-link/private-link-overview.md)安全地访问数据。 专用终结点针对 Azure Web PubSub 服务使用 VNet 地址空间中的 IP 地址。 VNet 上的客户端和 Azure Web PubSub 服务之间的网络流量通过 Microsoft 主干网络上的专用链接进行传输，避免暴露给公共 Internet。
...
- 使用 [VPN](../vpn-gateway/vpn-gateway-about-vpngateways.md) 或 [ExpressRoutes](../expressroute/expressroute-locations.md) 通过专用对等互连从连接到 VNet 的本地网络安全地连接到 Azure Web PubSub 服务。

## <a name="conceptual-overview"></a>概念概述

:::image type="content" source="./media/howto-secure-private-endpoints/private-endpoint-overview.png" alt-text="Azure Web PubSub 服务的专用终结点概述。":::

专用终结点是用于[虚拟网络](../virtual-network/virtual-networks-overview.md) (VNet) 中的 Azure 服务的特殊网络接口。 为 Azure Web PubSub 服务创建专用终结点时，它将在 VNet 上的客户端和服务之间提供安全连接。 从 VNet 的 IP 地址范围为专用终结点分配 IP 地址。 专用终结点与 Azure Web PubSub 服务之间的连接使用安全的专用链接。
...
Azure Web PubSub 服务所有者可以通过 [Azure 门户](https://portal.azure.com)中 Azure Web PubSub 服务的“专用终结点”选项卡来管理同意请求和专用终结点。

> [!TIP]
> 如果只想限制通过专用终结点对 Azure Web PubSub 服务的访问，请[配置网络访问控制](howto-secure-network-access-control.md)以拒绝或控制通过公共终结点的访问。
...
创建专用终结点时，Azure Web PubSub 服务的 DNS CNAME 资源记录将更新为具有前缀 `privatelink` 的子域中的别名。 默认情况下，我们还会创建一个与 `privatelink` 子域对应的[专用 DNS 区域](../dns/private-dns-overview.md)，其中包含专用终结点的 DNS A 资源记录。
...
### <a name="create-a-private-endpoint-along-with-a-new-azure-web-pubsub-service-in-the-azure-portal"></a>在 Azure 门户中创建专用终结点以及新的 Azure Web PubSub 服务

1. 创建新的 Azure Web PubSub 服务时，请选择“网络”选项卡。选择“专用终结点”作为连接方法 。

    :::image type="content" source="./media/howto-secure-private-endpoints/portal-create-blade-networking-tab.png" alt-text="创建 Azure Web PubSub 服务 -“网络”选项卡。":::

1. 选择 **添加** 。 填写新专用终结点的订阅、资源组、位置、名称。 选择虚拟网络和子网。

1. 选择“查看 + 创建”  。

### <a name="create-a-private-endpoint-for-an-existing-azure-web-pubsub-service-in-the-azure-portal"></a>在 Azure 门户中为现有 Azure Web PubSub 服务创建专用终结点

1. 转到 Azure Web PubSub 服务。

1. 选择名为“专用终结点连接”的设置菜单。

1. 选择顶部“+ 专用终结点”按钮。

1. 填写新专用终结点的订阅、资源组、资源名称和区域。

1. 选择目标 Azure Web PubSub 服务资源。
