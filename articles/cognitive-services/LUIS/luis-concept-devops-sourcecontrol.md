...
- **主分支有自己的 LUIS 应用。** 此应用表示你的项目的解决方案的当前状态，其当前活动版本应始终映射到 `.lu` main 分支中的源。 应检查和测试此应用中对 `.lu` 源的所有更新，以便随时部署该应用以生成环境（例如生成环境）。 当对的更新 `.lu` 从功能分支合并到 main 中时，应在 LUIS 应用程序中创建新版本并增加 [版本号](#versioning)。
...
1. 根据你的分支策略，从主分支创建功能分支 (，通常是主分支或开发) 。
...
1. 仅[在 LUIS 门户中新建 LUIS 应用](./luis-how-to-start-new-app.md)（“开发分支应用”）以支持使用功能分支。
   ...
   * 如果正在开始处理新项目，则存储库中将不会有您的 `.lu` 主 LUIS 应用程序的源代码。 你完成功能分支中的工作后将通过从门户中导出开发分支应用来创建 `.lu` 文件，然后将其作为 PR 的一部分提交。
...
1. 更改获得批准后，将更新合并到主分支。 此时，你将使用 main 中更新的，创建 *主* LUIS 应用的新 [版本](./luis-how-to-manage-versions.md) `.lu` 。 有关设置版本名称的注意事项，请参阅[版本控制](#versioning)。
...
你的团队中工作的其他一些开发人员可能已对源进行了更新 `.lu` ，并在创建功能分支后将它们合并到主分支。 你可能想要将他们的更改合并到你的工作版本中，然后继续在自己的功能分支内进行自己的更改。 为此，可以使用与任何其他代码资产相同的方式，通过 [变基或合并到 main](https://git-scm.com/book/en/v2/Git-Branching-Rebasing) 。 由于 LUDown 格式的 LUIS 应用易于阅读，因此它支持使用标准合并工具进行合并。
...
获得批准后，可以将更改合并到主分支。 LUIS 应用的 LUDown 源没有特殊的注意事项：它易于阅读，因此它支持使用标准合并工具进行合并。 可以以其他源文件中的方式解决任何合并冲突。
...
- PR 合并前 PR 中的更新
- 已批准 PR 并将更改合并到主分支后，主分支 LUIS 应用。
...
主分支的 LUIS 应用应应用版本控制方案。 将 LUIS 应用程序的更新合并 `.lu` 到主分支时，会将更新的源导入到主分支的 LUIS 应用程序的新版本中。

建议为主 LUIS 应用版本使用数值版本控制方案，例如：
...
确定主 LUIS 应用的最新版本的版本号后，需要生成并测试新的应用版本，并将其发布到可在不同生成环境（例如质量保证或生产）中使用的终结点。 强烈建议自动执行持续集成 (CI) 工作流中的所有这些步骤。
...
使用为支持功能分支中的工作而创建的“开发分支”LUIS 应用时，你将在完成工作后导出你的应用，然后在 PR 中加入更新后的 `'lu`。 在 PR 合并到 main 后，应删除存储库中的分支和 "开发分支" LUIS 应用。 由于该应用仅用于支持功能分支中的工作，因此无需在该应用中应用特定的版本控制方案。

当 PR 中的更改合并到主副本时，即当应应用版本控制时，将对主副本的所有更新进行独立版本控制。
