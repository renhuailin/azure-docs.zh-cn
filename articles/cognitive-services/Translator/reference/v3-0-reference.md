...
## <a name="authentication-with-azure-active-directory-azure-ad"></a>使用 Azure Active Directory (Azure AD) 进行身份验证

 Translator v3.0 支持 Azure AD 身份验证，即 Microsoft 基于云的标识和访问管理解决方案。  使用授权标头，Translator 服务可以验证请求客户端是否有权使用资源并完成请求。

### <a name="prerequisites"></a>**先决条件**

* 简要了解如何[使用 Azure Active Directory 进行身份验证](/azure/cognitive-services/authentication?tabs=powershell#authenticate-with-azure-active-directory)。

* 简要了解如何[授权访问托管标识](/azure/cognitive-services/authentication?tabs=powershell#authorize-access-to-managed-identities)。

### <a name="headers"></a>**标头**

|标头|值|
|:-----|:----|
|授权| 此值是由 Azure AD 生成的访问持有者令牌。</br><ul><li> 持有者令牌提供身份验证证明，并验证客户端是否有权使用资源。</li><li> 身份验证令牌的有效期为 10 分钟，应在对 Translator 进行多次调用时重复使用。</br></li>请参阅上面的[使用访问令牌进行身份验证](#authenticating-with-an-access-token)。 </ul>|
|Ocp-Apim-Subscription-Region| 此值是 Translator 资源的区域。</br><ul><li> 如果资源为全局资源，则此值是可选项。</li></ul>|
|Ocp-Apim-ResourceId| 此值是 Translator 资源实例的资源 ID。</br><ul><li>你会发现资源 ID 位于 Azure 门户的“Translator 资源”→“属性”中。 </li><li>资源 ID 格式： </br>/subscriptions/<subscriptionId>/resourceGroups/<resourceGroupName>/providers/Microsoft.CognitiveServices/accounts/<resourceName>/</li></ul>|

##### <a name="translator-property-pageazure-portal"></a>Translator 属性页 - Azure 门户

:::image type="content" source="../media/managed-identities/resource-id-property.png" alt-text="屏幕截图：Azure 门户中的 Translator 属性页。":::

### <a name="examples"></a>**示例**

#### <a name="using-the-global-endpoint"></a>使用全局终结点
...
#### <a name="using-your-custom-endpoint"></a>使用自定义终结点
...
### <a name="examples-using-managed-identities"></a>使用托管标识的示例

Translator v3.0 还支持授权访问托管标识。 如果为 Translator 资源启用了托管标识，可以在请求头中传递托管标识生成的持有者令牌。

#### <a name="with-the-global-endpoint"></a>使用全局终结点
...
#### <a name="with-your-custom-endpoint"></a>使用自定义终结点
