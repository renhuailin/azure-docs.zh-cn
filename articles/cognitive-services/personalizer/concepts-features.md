...
* _用户角色_，例如 `Sports_Shopper`。 这不应是单个用户 ID。 
...
* 对于分类特征，无需定义可能的值，并且无需为数值预定义范围。

## <a name="supported-feature-types"></a>支持的特征类型

个性化体验创建服务支持字符串、数字和布尔类型的特征。 应用程序很可能会经常使用字符串功能，但有一些例外情况。

### <a name="how-choice-of-feature-type-affects-machine-learning-in-personalizer"></a>特征类型的选择如何影响个性化体验创建服务中的机器学习

* 字符串：对于字符串类型，键和值的每个组合都被视为独热特征（例如流派：“科幻”和流派：“记录”将为机器学习模型创建两个新的输入功能）。
* 数值：当数值应按比例对个性化设置结果产生影响时，应使用数值。 这取决于具体情况。 举个简单的例子，在个性化零售体验时，NumberOfPetsOwned 可能是数字特征，因为你可能希望拥有 2 或 3 只宠物的人对个性化结果的影响是拥有 1 只宠物的人的两到三倍。 基于数字单位但含义不是线性的特征（例如年龄、温度或人物高度）最好编码为字符串。 例如，DayOfMonth 将是一个包含“1”、“2”...“31”的字符串。 如果有多个类别，通常可以使用范围来改进特征质量。 例如，年龄可以编码为“Age”：“0-5”、“Age”：“6-10”等。
* 通过“false”值发送的布尔值表现的行为就像它们根本没有被发送一样。
...
可以根据自己的约定来命名特征命名空间，只要它们是有效的 JSON 键即可。 命名空间用于将特征整理成不同的集，以及区分名称相似的特征。 可以将命名空间视为添加到特征名称的“前缀”。 命名空间不能嵌套。


在以下 JSON 中，`user`、`environment`、`device` 和 `activity` 是特征命名空间。 

> [!Note]
> 目前，我们强烈建议为特征命名空间使用基于 UTF-8 并以不同字母开头的名称。 例如，`user`、`environment`、`device` 和 `activity` 以 `u`、`e`、`d` 和 `a` 开头。 目前，具有相同首字符的命名空间可能会导致用于机器学习的索引出现冲突。
...
* 它不能为 unicode 编码。
* 可以使用编号 < 256 的一些可打印符号作为命名空间名称。 
* 不能使用编号 < 32（不可打印）、32（空格）、58（冒号）、124（管道）和 126–140 的符号。
* 它不应以下划线“_”开头，否则将忽略该特征。
...
位置信息通常也受益于创建更广泛的分类。 例如，经纬度坐标（如纬度：47.67402° N，经度：122.12154° W）过于精确，会强制模型将纬度和经度作为不同的维度来学习。 尝试基于位置信息进行个性化设置时，将位置信息分组到更大的扇区可能会很有帮助。 实现此目标的一种简单方法是为经纬度数字选择适当的舍入精度，并通过将纬度和经度合并到一个字符串中来将它们合并为“区域”。 例如，在区域大约有几千米的情况下，表示纬度：47.67402° N，经度 122.12154° W 的好方法是“位置”：“34.3，12.1”。


#### <a name="expand-feature-sets-with-extrapolated-information"></a>使用推理出的信息扩展特征集

评估可从已有信息派生的未探索属性也可以获得更多的特征。 例如，在科幻电影列表个性化设置中，周末和工作日是否可能引发用户的不同行为？ 可将时间扩展为包含“周末”或“工作日”属性。 国家法定节假日是否会推动观众对某类电影的兴趣？ 例如，在相关的场合中，可以使用“万圣节”属性。 雨天是否会明显影响许多人选择观看电影？ 如果使用时间和地点，则天气服务可以提供此类信息，你可以将此信息添加为额外的特征。 
...
* 具有 _事件_ ID。 如果已有事件 ID，则应提交该 ID。 如果没有事件 ID，请不要发送事件 ID，个性化体验创建服务会为你创建一个，并将其返回到排名请求的响应中。 ID 与排名事件（而非用户）相关联。 如果创建 ID，GUID 是最佳选择。 
* 具有特征列表。
