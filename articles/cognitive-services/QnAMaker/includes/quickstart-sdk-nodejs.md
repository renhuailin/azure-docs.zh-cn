...
[QnA Maker](/javascript/api/@azure/cognitiveservices-qnamaker/) 使用两种不同的对象模型：
...
创作 QnA Maker 客户端是 [QnAMakerClient](/javascript/api/@azure/cognitiveservices-qnamaker/qnamakerclient) 对象，使用包含密钥的凭据向 Azure 进行身份验证。

创建该客户端以后，使用[知识库](/javascript/api/@azure/cognitiveservices-qnamaker/qnamakerclient#knowledgebase)创建、管理和发布知识库。

通过发送 JSON 对象来管理知识库。 对于即时操作，方法通常返回一个指示状态的 JSON 对象。 对于长时间运行的操作，响应是操作 ID。 使用操作 ID 调用 [client.operations.getDetails](/javascript/api/@azure/cognitiveservices-qnamaker/operations#getdetails-string--msrest-requestoptionsbase-) 方法，确定[请求状态](/javascript/api/@azure/cognitiveservices-qnamaker/operation)。
...
使用终结点和密钥实例化某个客户端。 使用密钥创建 ServiceClientCredentials 对象，并将其与终结点一起使用以创建 [QnAMakerClient](/javascript/api/@azure/cognitiveservices-qnamaker/qnamakerclient) 对象。
...
知识库为来自三个源的 [CreateKbDTO](/javascript/api/@azure/cognitiveservices-qnamaker/createkbdto) 对象存储问答对：

* 对于 **编辑内容**，请使用 [QnADTO](/javascript/api/@azure/cognitiveservices-qnamaker/qnadto) 对象。
    * 若要使用元数据和跟进提示，请使用编辑上下文，因为此数据是在单独的 QnA 对级别添加的。
* 对于 **文件**，请使用 [FileDTO](/javascript/api/@azure/cognitiveservices-qnamaker/filedto) 对象。 FileDTO 包括文件名以及用于访问该文件的公共 URL。
...
使用知识库信息调用 [create](/javascript/api/@azure/cognitiveservices-qnamaker/knowledgebase#create-createkbdto--servicecallback-operation--) 方法。 知识库信息基本上是一个 JSON 对象。
...
可以更新知识库，方法是：将知识库 ID 和包含 [add](/javascript/api/@azure/cognitiveservices-qnamaker/updatekboperationdto#add)、[update](/javascript/api/@azure/cognitiveservices-qnamaker/updatekboperationdto#update) 和 [delete](/javascript/api/@azure/cognitiveservices-qnamaker/updatekboperationdto#deleteproperty) DTO 对象的 [UpdateKbOperationDTO](/javascript/api/@azure/cognitiveservices-qnamaker/updatekboperationdto) 传递给 [update](/javascript/api/@azure/cognitiveservices-qnamaker/knowledgebase#update-string--updatekboperationdto--msrest-requestoptionsbase-) 方法。 DTO 也基本上是 JSON 对象。 使用 [wait_for_operation](#get-status-of-an-operation) 方法来确定更新是否成功。
...
使用 [download](/javascript/api/@azure/cognitiveservices-qnamaker/knowledgebase#download-string--models-environmenttype--msrest-requestoptionsbase-) 方法，将知识库作为 [QnADocumentsDTO](/javascript/api/@azure/cognitiveservices-qnamaker/qnadocumentsdto) 列表下载。 这不等同于“设置”页中 QnA Maker 门户的导出，因为此方法的结果不是 TSV 文件。
...
使用 [publish](/javascript/api/@azure/cognitiveservices-qnamaker/knowledgebase#publish-string--msrest-requestoptionsbase-) 方法发布知识库。 这样会通过知识库 ID 获取当前保存的已训练模型，并在某个终结点上将其发布。 检查 HTTP 响应代码，验证发布是否成功。
...
使用 [EndpointKeys.getKeys](/javascript/api/@azure/cognitiveservices-qnamaker/endpointkeys) 方法以获取 [EndpointKeysDTO](/javascript/api/@azure/cognitiveservices-qnamaker/endpointkeysdto) 类。
...
使用 [delete](/javascript/api/@azure/cognitiveservices-qnamaker/knowledgebase#deletemethod-string--msrest-requestoptionsbase-) 方法与知识库 ID 参数删除知识库。
...
某些方法（例如 create 和 update）可能需要很长的时间，系统不会等待此过程完成，而是返回一个 [operation](/javascript/api/@azure/cognitiveservices-qnamaker/operations)。 使用操作中的[操作 ID](/javascript/api/@azure/cognitiveservices-qnamaker/operation#operationid) 进行轮询（使用重试逻辑），确定原始方法的状态。
