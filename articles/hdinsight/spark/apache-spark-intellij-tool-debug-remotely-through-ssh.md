---
title: Azure Toolkit for IntelliJ：通过 SSH 调试 Spark 应用 - HDInsight
...
1. 在“生成工具”下拉列表中选择下列其中一项：
    ...
    * **SBT**：用于管理依赖项和生成 Scala 项目。
     ...
     :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-create-projectfor-debug-remotely.png" alt-text="Intellij 的“创建新项目”选项" border="true":::
...
1. 在下一个“新建项目”窗口中提供以下信息：
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-new-project.png" alt-text="Intellij 的“新建项目”选项选择 Spark 版本" border="true":::
...
1. 本地运行完成后，可以看到输出文件保存到当前的项目资源管理器的“数据” > “默认”中 。
    ...
    :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/spark-local-run-result.png" alt-text="Intellij 项目本地运行结果" border="true":::
...
1. 执行本地运行和本地调试时，工具已自动设置默认的本地运行配置。 打开右上角的“[HDInsight 上的 Spark] XXX”配置，可以看到已在“HDInsight 上的 Apache Spark”下创建了“[HDInsight 上的 Spark]XXX”  。 切换到“本地运行”选项卡。
    ...
    :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/local-run-configuration.png" alt-text="Intellij - 运行/调试配置 - 本地运行" border="true":::
...
1. 在“运行/调试配置”对话框中，选择加号 (+) 。 然后选择“HDInsight 上的 Apache Spark”选项。
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-add-new-Configuration.png" alt-text="Intellij 添加新配置" border="true":::

1. 切换到“在群集中远程运行”选项卡。为“名称”、“Spark 群集”  和“Main 类名”输入信息。 然后单击“高级配置(远程调试)”。 工具支持使用“执行器”进行调试。 numExectors 默认值为 5。 设置的值最好不要大于 3。

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-run-debug-configurations.png" alt-text="Intellij 运行调试配置" border="true":::

1. 在“高级配置(远程调试)”部分，选择“启用 Spark 远程调试”。  输入 SSH 用户名，然后输入密码或使用私钥文件。 若要执行远程调试，则需要设置它。 若只想使用远程运行，则无需设置它。

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-enable-spark-remote-debug.png" alt-text="Intellij 高级配置 - 启用 spark 远程调试" border="true":::

1. 现已使用提供的名称保存配置。 若要查看配置详细信息，请选择配置名称。 若要进行更改，请选择“编辑配置”。

1. 完成配置设置后，可以针对远程群集运行项目，或执行远程调试。

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/perform-remote-run-button.png" alt-text="Intellij 调试远程 Spark 作业“远程运行”按钮" border="true":::

1. 单击“断开连接”按钮，这样，提交日志就不显示在左面板中。 但是，它仍在在后端上运行。

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/spark-remote-run-result.png" alt-text="Intellij 调试远程 Spark 作业 - 远程运行结果" border="true":::

## <a name="perform-remote-debugging"></a>执行远程调试

1. 设置断点，然后单击“远程调试”图标。 与远程提交的区别是需要配置 SSH 用户名/密码。

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-debug-icon.png" alt-text="Intellij 调试远程 Spark 作业“调试”图标" border="true":::
...
1. 程序执行到达断点时，会在“调试程序”窗格中显示一个“驱动程序”选项卡和两个“执行程序”选项卡。   选择“恢复程序”图标继续运行代码，这会到达下一个断点。 需要切换到正确的“执行程序”选项卡，以找到目标执行程序来进行调试。 可以在相应的“控制台”选项卡上查看执行日志。
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-debugger-tab.png" alt-text="Intellij 调试远程 Spark 作业“调试”选项卡" border="true":::
...
1. 选择“恢复程序”图标继续运行。 代码在第二个断点处停止。 按预期方式捕获异常。
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-throw-error.png" alt-text="Intellij 调试远程 Spark 作业 - 引发错误" border="true":::

1. 再次选择“恢复程序”图标。 “HDInsight Spark 提交”窗口随即显示“作业运行失败”错误。

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-error-submission.png" alt-text="Intellij 调试远程 Spark 作业 - 错误提交" border="true":::

1. 要使用 IntelliJ 调试功能动态更新变量值，请再次选择“调试”。 “变量”窗格会再次出现。

1. 在“调试”选项卡上右键单击目标，选择“设置值”。  接下来，输入变量的新值。 然后按 **Enter** 保存该值。

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-set-value1.png" alt-text="Intellij 调试远程 Spark 作业 - 设置值" border="true":::

1. 选择“恢复程序”图标继续运行程序。 这一次不会捕获到异常。 可以看到，项目已成功运行，未出现任何异常。

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-debug-without-exception.png" alt-text="Intellij 调试远程 Spark 作业 - 无异常" border="true":::
