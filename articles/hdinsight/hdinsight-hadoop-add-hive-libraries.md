...
使用本文中的脚本操作，可以在使用适用于 WebHCat 和 HiveServer2 的 Hive 客户端时使用这些库。
...
* 在创建期间，包含 jar 文件的库的存储帐户 **必须** 链接到 HDInsight 群集。 它必须是默认的存储帐户，或者是通过“存储帐户设置”添加的帐户。
...
1. 按照[预配 Linux 上的 HDInsight 群集](hdinsight-hadoop-provision-linux-clusters.md)中的步骤开始预配群集，但不要完成预配。 也可以使用 Azure PowerShell 或 HDInsight .NET SDK 来使用此脚本创建群集。 有关使用这些方法的详细信息，请参阅[使用脚本操作自定义 HDInsight 群集](hdinsight-hadoop-customize-cluster-linux.md)。 对于 Azure 门户，请在“配置 + 定价”选项卡中选择“+ 添加脚本操作”。

1. 对于“存储”，如果包含 jar 文件库的存储帐户与用于群集的帐户不同，请填写“其他存储帐户”。
...
群集创建完成后，则可以使用通过此脚本从 Hive 添加的 jar，而无需使用 `ADD JAR` 语句。
