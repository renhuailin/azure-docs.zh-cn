...
>[!Important]
>此内容适用于用户。 如果你是管理员，可以在 [Azure Active Directory 管理文档](../index.yml)中查找有关如何设置和管理 Azure Active Directory (Azure AD) 环境的详细信息。
>
> 如果在登录帐户时遇到问题，请参阅[无法登录 Microsoft 帐户](https://support.microsoft.com/help/12429)以获取帮助。  此外，可以详细了解如何解决在尝试登录 Microsoft 帐户时收到“[该 Microsoft 帐户不存在](https://support.microsoft.com/help/13811)”消息的问题。

Microsoft Authenticator 应用可帮助你在使用双重验证的情况下登录到帐户。 双重验证可帮助你更加安全地使用帐户，因为密码可能被忘记、被盗或泄露。 双重验证使用第二个因素（如手机）加大其他人侵入你的帐户的难度。 可通过多种方式使用 Microsoft Authenticator 应用，包括：
...
验证器应用可与使用双重验证并支持基于时间的一次性密码 (TOTP) 标准的任何帐户配合使用。

组织可能要求你使用验证器应用来登录和访问组织数据和文档。 尽管用户名已显示在应用中，但在完成注册过程前，该帐户不会设置为验证方法。 有关详细信息，请参阅[添加工作或学校帐户](user-help-auth-app-add-work-school-account.md)。
