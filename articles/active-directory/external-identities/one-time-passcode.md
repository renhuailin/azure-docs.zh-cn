...
从10月2021开始，将为所有现有租户启用电子邮件一次性密码功能，并在默认情况下为新租户启用此功能。 此时，Microsoft 将不再支持通过创建非托管 ( "病毒" 或 "实时" ) Azure AD 用于 B2B 协作方案的帐户和租户来兑换邀请。 我们正在启用电子邮件一次性密码功能，因为它为你的来宾用户提供了无缝的回退身份验证方法。 但是，如果您选择不使用此功能，则可以禁用此功能。
