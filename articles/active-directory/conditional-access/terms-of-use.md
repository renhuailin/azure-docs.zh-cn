...
- 使用 [Microsoft Graph API](/graph/api/resources/agreement?view=graph-rest-beta)（目前提供预览版）创建和管理使用条款策略。
...
1. 如果要求最终用户在每台访问设备上接受使用条款，请将“要求用户在每台设备上同意”设置为“打开”。  如果启用此选项，则用户可能需要安装其他应用程序。 有关详细信息，请参阅[按设备实施的使用条款](#per-device-terms-of-use)。
...
1. 在“条件访问”下，使用“使用条件访问策略模板强制实施”列表选择用于强制实施使用条款策略的模板。 
   ...
   >条件访问策略控制（包括使用条款策略）不支持对服务帐户强制实施。 我们建议从条件访问策略中排除所有服务帐户。
    ...
    可以使用自定义条件访问策略将使用条款策略细化，向下细化到特定云应用程序或用户组。 有关更多信息，请参阅[快速入门：在访问云应用之前要求接受使用条款](require-tou.md)。
...
1. 在“编辑使用条款”窗格中，可以更改以下各项：
    - 名称 – 这是使用条款的内部名称，不会与最终用户共享
    - 显示名称 – 这是最终用户查看使用条款时可以看到的名称
