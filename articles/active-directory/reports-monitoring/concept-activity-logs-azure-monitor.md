...
* 审核 - [审核日志活动报告](concept-audit-logs.md)可以让你访问有关应用于租户的更改（如用户和组管理）或应用于租户资源的更新的信息。
...
下表列出了“美国西部”一个基本事件中心每月根据事件数据量估算的成本，根据用户登录行为等诸多因素，事件数据量可能因租户而异。若要针对应用程序的预期数据量进行准确的估算，请使用[事件中心定价计算器](https://azure.microsoft.com/pricing/details/event-hubs/)。
...
| 登录 | 100,000 | 18000 | 5,400,000 | 10.8 GB | 42188 | 364,504,320 | 23.9 美元 |  
...
| 日志类别 | 用户数 | 每日事件数 | 每月事件数（30 天） | 每月估算的成本（美元） |
|:-|--|--|--|-:|
| 审核和登录 | 100,000 | 16,500,000 | 495,000,000 | 1093.00 美元 |
| 审核 | 100,000 | 1,500,000 | 45,000,000 | 246.66 美元 |
| 登录 | 100,000 | 15,000,000 | 450,000,000 | 847.28 美元 |
...
**答**：登录活动日志和审核日志均可通过此功能进行路由，虽然与 B2C 相关的审核事件目前未包括在其中。 若要了解目前支持哪些类型的日志和哪些基于功能的日志，请参阅[审核日志架构](./overview-reports.md)和[登录日志架构](reference-azure-monitor-sign-ins-log-schema.md)。 
