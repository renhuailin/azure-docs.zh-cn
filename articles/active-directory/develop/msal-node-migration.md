...
MSAL Node 默认使用内存中的令牌缓存。 无需显式导入令牌缓存；其会作为 `ConfidentialClientApplication` 和 `PublicClientApplication` 对象的一部分予以公开。
...
出于安全原因，MSAL Node 不会公开刷新令牌。 而是由 MSAL 代你处理令牌刷新。 因此，你无需再为此生成逻辑。 尽管如此，如果你需要迁移并使用之前获取的刷新令牌，MSAL Node 可提供 `acquireTokenByRefreshToken`（其等效于 ADAL Node 的 `acquireTokenWithRefreshToken` 方法）。
