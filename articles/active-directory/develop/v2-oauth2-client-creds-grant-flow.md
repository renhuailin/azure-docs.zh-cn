...
OAuth 2.0 客户端凭据授权流允许 Web 服务（机密客户端）在调用其他 Web 服务时使用它自己的凭据（而不是模拟用户）进行身份验证。 为了进行更高级别的保证，Microsoft 标识平台还允许调用服务将证书（而不是共享机密）用作凭据。  由于使用的是应用程序自己的凭据，因此必须确保这些凭据的安全，即永远不要在源代码中发布该凭据，不要将其嵌入到网页中，或不要在广泛分布的本机应用程序中使用它。 
