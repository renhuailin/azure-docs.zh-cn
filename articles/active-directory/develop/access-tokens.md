...
仅当存在可以填充声明的值时，才提供声明。 应用不应依赖于所提供的声明。 示例包括 `pwd_exp` (不是每个租户都需要密码过期) 并且 `family_name` ( [客户端凭据] (v2-oauth2-client-creds-grant-flow.md) 流代表没有名称) 的应用程序。 始终会提供用于验证访问令牌的声明。
...
| `preferred_username` | String | 表示用户的主用户名。 可以是电子邮件地址、电话号码或未指定格式的一般用户名。 其值是可变的，可能随时改变。 由于此值是可变的，因此它不能用于做出授权决定。  它可用于用户名提示，在用户可读的 UI 中作为用户名。 需要 `profile` 范围才能接收此声明。 仅存在于 v2.0 令牌中。 |
