...
若要开始，请下载最新的 [Azure AD PowerShell 模块公共预览版](https://www.powershellgallery.com/packages/AzureADPreview)。

接着，运行 `Connect` 命令登录到 Azure AD 管理员帐户。 每次启动新会话都需要运行此命令。
...
本示例创建一个要求用户更频繁地在 Web 应用中进行身份验证的策略。 此策略对 Web 应用的服务主体设置访问/ID 令牌的生存期。

1. 创建令牌生存期策略。

    对于 Web 登录，此策略将访问/ID 令牌生存期设置为两小时。
...
## <a name="view-existing-policies-in-a-tenant"></a>查看租户中现有的策略
...
某些用户在运行 `Get-AzureADPolicy` cmdlet 后报告了 `Get-AzureADPolicy : The term 'Get-AzureADPolicy' is not recognized` 错误。 作为一种解决方法，请运行以下步骤卸载/重新安装 AzureAD 模块，然后安装 AzureADPreview 模块：
