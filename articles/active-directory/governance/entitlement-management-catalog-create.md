...
* 应用程序可以是 Azure AD 企业应用程序，包括 SaaS 应用程序，以及你自己的与 Azure AD 集成的应用程序。 若要详细了解如何为具有多个角色的应用程序选择适当的资源，请参阅[添加资源角色](entitlement-management-access-package-resources.md#add-resource-roles)。
...
属性是请求者在提交访问请求之前需要回答的必填字段。 他们对这些属性的回答将显示给审批者，并在 Azure Active Directory 的用户对象上加盖标记。 

> [!NOTE]
>在资源上设置的所有属性都需要一个答案，然后才能提交对包含该资源的访问包的请求。 如果请求者不提供答案，将不会处理请求。

若要要求访问请求具有属性，请使用以下步骤：

1. 单击左侧菜单中的“资源”，将显示目录中的资源列表。 

1. 单击要添加属性的资源旁边的省略号，然后选择“需要属性(预览)”。 

    ![添加资源 - 选择需要的属性](./media/entitlement-management-catalog-create/resources-require-attributes.png)
 
1.  选择属性类型：

    1. **内置**：包括 Azure Active Directory 用户配置文件属性。
    1. **目录架构扩展**：提供一种在 Azure Active Directory 中存储关于用户对象和其他目录对象附加数据的方法。 这包括组、租户详细信息和服务主体。 仅用户对象上的扩展属性可用于向应用程序发出声明。
    1. 如果选择“内置”，可以从下拉列表中选择一个属性。 如果选择“目录架构扩展”，可以在文本框中输入属性名称。

    > [!NOTE]
    > 只能为非管理员用户更新 User.mobilePhone 属性。 在[此处](/graph/permissions-reference#remarks-5)了解更多信息。 
...
1.  如果选择“多选”，请单击“编辑和本地化”按钮来配置答案选项。
    ...
    1. 选择响应选项的语言。 如果选择其他语言，则可以将响应选项本地化。
    1. 键入所需的任意多个响应，然后单击“保存”。

1. 如果希望在直接分配和自助服务请求期间可编辑属性值，请选择“是”。

    > [!NOTE]
    > ![添加资源 - 添加属性 - 使属性可编辑](./media/entitlement-management-catalog-create/attributes-are-editable.png)
    > - 如果在“属性值可编辑”字段中选择“否”，并且属性值为空，则用户将能够输入该属性的值 。 保存后，该值将不再可编辑。 
    > - 如果在“属性值可编辑”字段中选择“否”，并且属性值不为空，则用户将无法在直接分配和自助服务请求期间编辑预先存在的值 。
 
    ![添加资源 - 添加属性 - 问题](./media/entitlement-management-catalog-create/add-attributes-questions.png)

1.  如果要添加本地化，请单击“添加本地化”。

    1. 进入“为问题添加本地化”窗格后，为要本地化与选定属性相关的问题的语言选择语言代码。
    1. 使用配置的语言，在“本地化文本”框中键入问题。
    1. 添加所有所需的本地化后，单击“保存”。

    ![添加资源 - 添加属性 - 本地化](./media/entitlement-management-catalog-create/attributes-add-localization.png)

1.  在“需要属性(预览)”页上完成所有属性信息后，单击“保存” 。
...
可以从目录中删除资源。 只有当资源未在目录的任何访问包中使用时，才能从该目录中删除该资源。
