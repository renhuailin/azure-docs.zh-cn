...
转到“我的应用”门户。 使用凭据（或者已设置的具有访问权限的测试帐户的凭据）登录。 成功登录后，单击该应用的图标。 这样可以触发安装“我的应用”安全登录浏览器扩展。 如果用户已预定义凭据，应会自动对应用进行身份验证，否则需要在首次验证时指定用户名或密码。 
