...
3. 用户将令牌传递给应用程序代理。
4. 应用程序代理会验证令牌，并 (UPN) 中检索用户主体名称，然后连接器通过双重的经过身份验证的安全通道提取 UPN 和服务主体名称 () SPN。
5. 连接器执行 Kerberos 约束委派 (KCD 与本地 AD) 协商，模拟用户获取应用程序的 Kerberos 令牌。
...
如果为连接器计算机配置 SPNEGO，请确保与 SPNEGO 一起还配置了相应连接器组中的其他所有连接器。 应通过未配置 SPNEGO 的其他连接器来路由需要标准 Kerberos 令牌的应用程序。 某些 web 应用程序接受这两种格式，而无需对配置进行任何更改。 
