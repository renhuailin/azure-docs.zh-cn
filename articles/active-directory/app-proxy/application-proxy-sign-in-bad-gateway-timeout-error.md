---
title: 使用 Azure Active Directory 应用程序代理应用程序时出现无法访问此企业应用程序错误
description: 如何解决 Azure Active Directory 应用程序代理应用程序的常见访问问题。
...
若要查找代码，请查看错误消息左下方“状态代码”字段中的文本。 查找页面底部是否有任何其他提示。
...
1. 查看“连接器组”字段。 如果组中没有活动连接器，将看到一条警告。 如果未看到任何警告，则继续验证是否允许所有[必需端口](application-proxy-add-on-premises-application.md)。
...
1. 此处，有几种方法可进一步深入探讨：
   ...
   - 为该组下载新的连接器：从该页面可获取指向[下载新连接器](https://download.msappproxy.net/Subscription/d3c8b69d-6bf7-42be-a529-3fe9c2e70c90/Connector/Download)的链接。 将连接器安装在直接与后端应用程序相连的机器上。 通常，需要在应用程序所在的同一台服务器上安装连接器。 使用“下载连接器”链接将连接器下载到目标计算机上。 接下来，单击此连接器，并使用“连接器组”下拉列表确保其属于正确的组。
...
验证所有必需端口是否都已打开。 有关必需端口，请参阅[教程：添加本地应用程序以通过 Azure Active Directory 应用程序代理进行远程访问](application-proxy-add-on-premises-application.md)一文中的“打开端口”部分。 如果所有必需的端口都已打开，则继续下一部分。
...
如果上述方法未能解决该问题，则可能有几个不同的原因。 若要确定问题：
