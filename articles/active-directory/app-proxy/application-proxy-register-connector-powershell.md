---
title: 无提示安装 Azure Active Directory 应用程序代理连接器
description: 介绍如何执行 Azure Active Directory 应用程序代理连接器的无人参与安装，以对本地应用程序进行安全地远程访问。
...
ms.locfileid: "108185834"
---
# <a name="create-an-unattended-installation-script-for-the-azure-active-directory-application-proxy-connector"></a>为 Azure Active Directory 应用程序代理连接器创建无人参与安装脚本
...
要让[应用程序代理连接器](application-proxy-connectors.md)正常工作，必须使用应用程序管理员和密码将其注册到 Azure AD 目录。 通常在连接器安装期间出现弹出窗口对话框时输入此信息，但是可改为使用 PowerShell 自动执行此过程。
...
1. 创建包含目录管理用户名和密码的 Windows PowerShell 凭据对象 `$cred`。 运行以下命令，替换 *\<username\>* 和 *\<password\>* ：
