...
ms.locfileid: "121727652"
---
# <a name="what-is-risk"></a>什么是风险？

Azure AD 标识保护中的风险检测包括已识别的与目录中的用户帐户相关的任何可疑操作。
...
有两种类型的风险（用户和登录）和两种类型的检测或计算（实时和脱机）   。
...
| 异常令牌 | 脱机 | 此检测指示令牌中存在异常特征，例如活动时间和来自陌生 IP 地址的身份验证。 |
| 令牌颁发者异常 | 脱机 | 此风险检测指示存在具有已知攻击模式的异常活动，例如更新受信任的领域联合设置或更改签名证书。 |
