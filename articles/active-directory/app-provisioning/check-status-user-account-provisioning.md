...
Azure Active Directory (Azure AD) 包含一个 [用户帐户预配服务](user-provisioning.md) ，该服务有助于自动预配在 SaaS 应用和其他系统中预配用户帐户，以实现端到端的标识生命周期管理。 Azure AD 在 [此处](../saas-apps/tutorial-list.md)为包含用户预配教程的所有应用程序和系统提供预先集成的用户预配连接器。
...
* **目标系统** - Azure AD 预配服务要同步到的用户存储库。 这通常是 SaaS 应用程序 (示例： Salesforce、ServiceNow、G Suite、Dropbox for Business) ，但在某些情况下可以是本地系统，如 Active Directory (示例： Workday 入站同步 Active Directory) 。
