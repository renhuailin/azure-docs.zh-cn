...
* 可以在 Azure AD 中控制谁有权访问 Shibumi。
* 可让用户使用其 Azure AD 帐户自动登录到 Shibumi（单一登录）。
...
* Shibumi 支持 **SP** 和 **IDP** 发起的 SSO

* Shibumi 支持 **实时** 用户预配

## <a name="adding-shibumi-from-the-gallery"></a>从库中添加 Shibumi

若要配置 Shibumi 与 Azure AD 的集成，需要从库中将 Shibumi 添加到托管 SaaS 应用列表。

**若要从库中添加 Shibumi，请执行以下步骤：**
...
在本部分，我们基于名为 **Britta Simon** 的测试用户来配置并测试 Shibumi 的 Azure AD 单一登录。
若要正常使用单一登录，需要在 Azure AD 用户与 Shibumi 相关用户之间建立链接关系。

若要配置并测试 Shibumi 的 Azure AD 单一登录，需要完成以下构建基块：

1. **[配置 Azure AD 单一登录](#configure-azure-ad-single-sign-on)** - 使用户能够使用此功能。
2. **[配置 Shibumi 单一登录](#configure-shibumi-single-sign-on)** - 在应用程序端配置单一登录设置。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. **[创建 Shibumi 测试用户](#create-shibumi-test-user)** - 在 Shibumi 中创建 Britta Simon 的对应用户，并将其关联到其在 Azure AD 中的表示形式。
...
### <a name="create-shibumi-test-user"></a>创建 Shibumi 测试用户

在本部分，我们将在 Shibumi 中创建名为 Britta Simon 的用户。 Shibumi 支持默认已启用的实时用户预配。 此部分不存在任何操作项。 如果 Shibumi 中尚不存在用户，身份验证后会创建一个新用户。

### <a name="test-single-sign-on"></a>测试单一登录 

在本部分中，使用访问面板测试 Azure AD 单一登录配置。

在访问面板中单击“Shibumi”磁贴时，应会自动登录到设置了 SSO 的 Shibumi。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)（访问面板简介）。
