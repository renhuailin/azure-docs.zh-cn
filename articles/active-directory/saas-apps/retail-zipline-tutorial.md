...
还可以使用 Microsoft 访问面板在任何模式下测试此应用程序。 在单击访问面板中的 Retail Zipline 磁贴时，如果在配置时采用 SP 模式，则会重定向至应用程序登录页以启动登录流；如果在配置时采用 IDP 模式，则应自动登录到已为其设置了 SSO 的 Retail Zipline。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)（访问面板简介）。
