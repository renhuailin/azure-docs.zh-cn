...
## <a name="configure-and-test-azure-ad-sso-for-shopify-plus"></a>针对 Shopify Plus 配置并测试 Azure AD SSO

使用名为 B.Simon 的测试用户配置并测试 Shopify Plus 的 Azure AD SSO。 要正常使用 SSO，需要在 Azure AD 用户与 Shopify Plus 中的相关用户之间建立链接关系。

若要针对 Shopify Plus 配置并测试 Azure AD SSO，请执行以下步骤：
...
1. 在 Azure 门户的“Shopify Plus”应用程序集成页上，找到“管理”部分，选择“单一登录”  。
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 Shopify Plus 登录 URL，可在这里启动登录流。  

* 直接转到 Shopify Plus 登录 URL，从此处启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，你应会自动登录到为其设置了 SSO 的 Shopify Plus。 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 在“我的应用”中单击 Shopify Plus 磁贴时，如果是在 SP 模式下配置的，会重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，则应会自动登录到为其设置了 SSO 的 Shopify Plus。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
