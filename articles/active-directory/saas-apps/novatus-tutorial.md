...
* 可让用户使用其 Azure AD 帐户自动登录到 Novatus（单一登录）。
...
2. **[配置 Novatus 单一登录](#configure-novatus-single-sign-on)** - 在应用程序端配置单一登录设置。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. **[创建 Novatus 测试用户](#create-novatus-test-user)** - 在 Novatus 中有一个与 Azure AD 中的 Britta Simon 相对应的关联用户。
...
### <a name="configure-novatus-single-sign-on"></a>配置 Novatus 单一登录

若要在“Novatus”端配置单一登录，需要将下载的“证书(Base64)”以及从 Azure 门户复制的相应 URL 发送给 [Novatus 支持团队](mailto:jvinci@novatusinc.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
### <a name="create-novatus-test-user"></a>创建 Novatus 测试用户

本部分将在 Novatus 中创建一个名为 Britta Simon 的用户。 Novatus 支持默认启用的恰时用户预配。 此部分不存在任何操作项。 如果 Novatus 中不存在用户，则会在身份验证后创建一个新用户。
...
单击访问面板中的 Novatus 磁贴时，应当会自动登录到你为其设置了 SSO 的 Novatus。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)（访问面板简介）。
