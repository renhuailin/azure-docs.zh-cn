---
title: 教程：Azure Active Directory 单一登录 (SSO) 与 Standard for Success Accreditation 的集成 | Microsoft Docs
description: 了解如何在 Azure Active Directory 和 Standard for Success Accreditation 之间配置单一登录。
...
本教程介绍如何将 Standard for Success Accreditation 与 Azure Active Directory (Azure AD) 集成。 将 Standard for Success Accreditation 与 Azure AD 集成后，可以：
...
* 启用了 Standard for Success Accreditation 单一登录 (SSO) 的订阅。
...
使用名为 B.Simon 的测试用户，针对 Standard for Success Accreditation 配置并测试 Azure AD SSO。 若要使 SSO 正常工作，需要在 Azure AD 用户与 Standard for Success Accreditation 中的相关用户之间建立链接关系。

若要针对 Standard for Success Accreditation 配置并测试 Azure AD SSO，请执行以下步骤：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
    1. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 B. Simon 测试 Azure AD 单一登录。
    1. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 B. Simon 能够使用 Azure AD 单一登录。
1. **[配置 Standard for Success Accreditation SSO](#configure-standard-for-success-accreditation-sso)** - 在应用程序端配置单一登录设置。
    1. **[创建 Standard for Success Accreditation 测试用户](#create-standard-for-success-accreditation-test-user)** - 在 Standard for Success Accreditation 中设置 B.Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 Azure 门户的“Standard for Success Accreditation”应用程序集成页上，找到“管理”部分，然后选择“单一登录”  。
...
在本部分中，将授予 B.Simon 访问 Standard for Success Accreditation 的权限，以使其能够使用 Azure 单一登录。

1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”。 
1. 在应用程序列表中，选择“Standard for Success Accreditation”。
...
## <a name="configure-standard-for-success-accreditation-sso"></a>配置 Standard for Success Accreditation SSO

1. 打开新的 Web 浏览器窗口，并以具有超级用户访问权限的管理员身份登录到 Standard for Success Accreditation 站点。
...
1. 向下滚动到“单一登录设置”并单击“Microsoft Azure 单一登录”链接，然后执行以下步骤 。
    ...
    d. 在“证书指纹”文本框中，粘贴从 Azure 门户复制的“指纹值” 。

    e. 单击“保存”  。 

### <a name="create-standard-for-success-accreditation-test-user"></a>创建 Standard for Success Accreditation 测试用户
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 Standard for Success Accreditation 登录 URL，可从此处启动登录流。  

* 直接转至 Standard for Success Accreditation 登录 URL 并从那里启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，应会自动登录到为其设置了 SSO 的 Standard for Success Accreditation 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 在“我的应用”中单击 Standard for Success Accreditation 磁贴时，如果是在 SP 模式下配置的，会重定向到应用程序登录页，以便启动登录流；如果是在 IDP 模式下配置的，则应该会自动登录到为其设置了 SSO 的 Standard for Success Accreditation。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)。
