...
## <a name="add-factset-from-the-gallery"></a>从库中添加 FactSet
...
## <a name="configure-and-test-azure-ad-sso-for-factset"></a>配置并测试 FactSet 的 Azure AD SSO

使用名为 B.Simon  的测试用户配置并测试 FactSet 的 Azure AD SSO。 若要使 SSO 发挥作用，需要在 Azure AD 用户与 FactSet中的相关用户之间建立关联。

若要配置并测试 FactSet 的 Azure AD SSO，请执行以下步骤：
...
1. 在 Azure 门户中的“FactSet”应用程序集成页上，找到“管理”部分并选择“单一登录”。  
...
1. 在“设置 SAML 单一登录”页上，执行以下步骤  ：
    ...
    a. 在“标识符”文本框中键入 URL：`https://auth.factset.com`

    b. 在“回复 URL”文本框中键入 URL：`https://auth.factset.com/sp/ACS.saml2`
...
* 在 Azure 门户中单击“测试此应用程序”，你会自动登录到为其设置了 SSO 的 FactSet。

* 你可使用 Microsoft 的“我的应用”。 单击“我的应用”中的 FactSet 磁贴时，你会自动登录到为其设置了 SSO 的 FactSet。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)。

## <a name="next-steps"></a>后续步骤

配置 FactSet 后，就可以强制实施会话控制，从而实时防止组织的敏感数据遭到外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-aad)。
