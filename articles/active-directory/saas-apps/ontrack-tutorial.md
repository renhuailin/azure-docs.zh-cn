...
* 可让用户使用其 Azure AD 帐户自动登录到 OnTrack（单一登录）。
...
4. 在搜索框中键入“OnTrack”，在结果面板中选择“OnTrack”，然后单击“添加”按钮添加应用程序。   
     ...
     ![结果列表中的 OnTrack](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>配置和测试 Azure AD 单一登录

在本部分中，基于一个名为“Britta Simon”的测试用户使用 OnTrack 配置和测试 Azure AD 单一登录  。
若要使单一登录有效，需要在 Azure AD 用户与 OnTrack 相关用户之间建立链接关系。

若要配置和测试 OnTrack 的 Azure AD 单一登录，需要完成以下构建基块：

1. **[配置 Azure AD 单一登录](#configure-azure-ad-single-sign-on)** - 使用户能够使用此功能。
2. [配置 OnTrack 单一登录](#configure-ontrack-single-sign-on)  - 在应用程序端配置单一登录设置。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. [创建 OnTrack 测试用户](#create-ontrack-test-user)  - 在 OnTrack 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 [Azure 门户](https://portal.azure.com/)中的“OnTrack”  应用程序集成页上，选择“单一登录”  。
...
4. 在“设置 SAML 单一登录”页上，执行以下步骤  ：
    ...
    ![OnTrack 域和 URL 单一登录信息](common/idp-intiated.png)
...
5. OnTrack 应用程序需要特定格式的 SAML 断言，这要求向 SAML 令牌属性配置添加自定义属性映射。 以下屏幕截图显示了默认属性的列表。 单击“编辑”图标打开“用户属性”对话框。  
    ...
    ![屏幕截图显示“用户属性”对话框，在其右上角选择了“编辑”图标。](common/edit-attribute.png)
...
6. 除了上述属性，OnTrack 应用程序还要求在 SAML 响应中传递回更多的属性。 在“用户属性”对话框的“用户声明”部分执行以下步骤，以便添加 SAML 令牌属性，如下表所示：
    ...
    ![屏幕截图显示“用户声明”对话框，其中选择了“添加新声明”和“保存”操作。](common/new-save-attribute.png)
...
8. 在“设置 OnTrack”部分，根据要求复制相应 URL  。
...
### <a name="configure-ontrack-single-sign-on"></a>配置 OnTrack 单一登录

若要在 OnTrack 端配置单一登录，需要将下载的“联合元数据 XML”以及从 Azure 门户复制的相应 URL 发送给 [OnTrack 支持团队](mailto:CustomerService@insigniagroup.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”和“OnTrack”    。

    ![“企业应用程序”边栏选项卡](common/enterprise-applications.png)

2. 在应用程序列表中，选择“OnTrack”  。

    ![应用程序列表中的 OnTrack 链接](common/all-applications.png)
...
单击访问面板中的 OnTrack 磁贴时，应会自动登录到为其设置了 SSO 的 OnTrack。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)（访问面板简介）。
