...
* Whatfix 支持 SP 和 IDP 发起的 SSO。

> [!NOTE]
> 此应用程序的标识符是一个固定字符串值，因此只能在一个租户中配置一个实例。

## <a name="add-whatfix-from-the-gallery"></a>从库中添加 Whatfix
...
## <a name="configure-and-test-azure-ad-sso-for-whatfix"></a>配置并测试 Whatfix 的 Azure AD SSO

使用名为 B.Simon 的测试用户配置和测试 Whatfix 的 Azure AD SSO。 若要运行 SSO，需要在 Azure AD 用户与 Whatfix 相关用户之间建立链接关系。

若要配置并测试 Whatfix 的 Azure AD SSO，请执行以下步骤：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
    1. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 B. Simon 测试 Azure AD 单一登录。
    1. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 B. Simon 能够使用 Azure AD 单一登录。
1. [配置 Whatfix SSO](#configure-whatfix-sso) - 在应用程序端配置单一登录设置。
    1. [创建 Whatfix 测试用户](#create-whatfix-test-user) - 在 Whatfix 中创建 B.Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 Azure 门户的“Whatfix”应用程序集成页上，找到“管理”部分，并选择“单一登录”。
...
在本部分，你将通过授予 B.Simon 访问 Whatfix 的权限，支持其使用 Azure 单一登录。
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 Whatfix 登录 URL，你可以在那里启动登录流。  

* 直接转到 Whatfix 登录 URL，并从那里启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，你应会自动登录到为其设置了 SSO 的 Whatfix。 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 在“我的应用”中单击“Whatfix”磁贴时，如果是在 SP 模式下配置的，你会被重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，则应会自动登录到为其设置了 SSO 的 Whatfix。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)。

## <a name="next-steps"></a>后续步骤

配置 Whatfix 后，可以强制实施会话控制，实时防止组织的敏感数据外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-aad)。