...
* 在 Azure AD 中控制谁有权访问 Proofpoint on Demand。
* 让用户使用其 Azure AD 帐户自动登录到 Proofpoint on Demand。
...
* 已启用 Proofpoint on Demand 单一登录 (SSO) 的订阅。
...
1. 在“从库中添加”部分的搜索框中，键入“Proofpoint on Demand” 。
1. 从结果面板中选择“Proofpoint on Demand”，然后添加该应用。 在该应用添加到租户时等待几秒钟。

## <a name="configure-and-test-azure-ad-sso-for-proofpoint-on-demand"></a>配置并测试 Proofpoint on Demand 的 Azure AD SSO

使用名为 B.Simon 的测试用户配置和测试 Proofpoint on Demand 的 Azure AD SSO。 要使 SSO 正常工作，需要建立 Azure AD 用户与 Proofpoint on Demand 中相关用户的关联关系。

若要配置并测试 Proofpoint on Demand 的 Azure AD SSO，请执行以下步骤：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
    1. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 B. Simon 测试 Azure AD 单一登录。
    1. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 B. Simon 能够使用 Azure AD 单一登录。
1. **[配置 Proofpoint on Demand SSO](#configure-proofpoint-on-demand-sso)** - 在应用程序端配置单一登录设置。
...
1. 在 Azure 门户的“Proofpoint on Demand”应用程序集成页上，找到“管理”部分，选择“单一登录”  。
...
在本部分，你将通过授予 B.Simon 访问 Proofpoint on Demand 的权限，使其能够使用 Azure 单一登录。

1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”。 
1. 在应用程序列表中，选择“Proofpoint on Demand”。
...
### <a name="create-proofpoint-on-demand-test-user"></a>创建 Proofpoint on Demand 测试用户
...
* 在 Azure 门户中单击“测试此应用程序”。 这样将会重定向到 Proofpoint on Demand 登录 URL，可以从那里启动登录流。 

* 直接转到 Proofpoint on Demand 登录 URL，并从那里启动登录流。

* 你可使用 Microsoft 的“我的应用”。 在“我的应用”中单击“Proofpoint on Demand”磁贴时，应会自动登录到设置了 SSO 的 Proofpoint on Demand。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。

## <a name="next-steps"></a>后续步骤

配置 Proofpoint on Demand 后，可以强制实施会话控制，以实时防止组织的敏感数据发生外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。