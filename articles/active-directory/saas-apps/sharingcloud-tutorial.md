...
description: 了解如何在 Azure Active Directory 与 SharingCloud 之间配置单一登录。
...
1. 如果要在“IDP”发起的模式下配置应用程序，请在“基本 SAML 配置”部分中输入以下字段的值 ：
    ...
    a. 在“标识符”  文本框中，使用以下模式键入 URL：`https://<CustomerName>.sharingcloud.net/adfs/<CustomerName>/saml/federation.xml`

    b. 在“回复 URL”文本框中，使用以下模式键入 URL：`https://<CustomerName>.sharingcloud.net/social/complete/saml/`

1. 如果要在 SP  发起的模式下配置应用程序，请单击“设置其他 URL”  ，并执行以下步骤：

    在“登录 URL”文本框中，使用以下模式键入 URL：`https://<CustomerName>.sharingcloud.net/accounts/login/`

    > [!NOTE]
    > 这些不是实际值。 请使用实际的“标识符”、“回复 URL”和“登录 URL”更新这些值。 请联系 [SharingCloud 客户端支持团队](mailto:support@sharingcloud.com)获取这些值。 还可以参考 Azure 门户中的“基本 SAML 配置”部分中显示的模式。
...
若要在 SharingCloud 端配置单一登录，需要将下载的应用联合元数据 URL 发送给 [SharingCloud 支持团队](mailto:support@sharingcloud.com) 。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。

### <a name="create-sharingcloud-test-user"></a>创建 SharingCloud 测试用户

本部分将在 SharingCloud 中创建一个名为 Britta Simon 的用户。 SharingCloud 支持默认启用的实时用户预配。 此部分不存在任何操作项。 如果 SharingCloud 中不存在用户，则会在身份验证后创建一个新用户。
...
还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 在“我的应用”中单击 SharingCloud 磁贴时，如果是在 SP 模式下配置的，则会重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，则应会自动登录到为其设置了 SSO 的 SharingCloud。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。


## <a name="next-steps"></a>后续步骤

配置 SharingCloud 后，即可强制实施会话控制，实时防止组织的敏感数据发生外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。
