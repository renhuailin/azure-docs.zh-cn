...
从 Azure AD 应用程序库添加 PureCloud by Genesys，开始管理到 PureCloud by Genesys 的预配。 如果以前为 PureCloud by Genesys 设置过 SSO，则可以使用同一应用程序。 但建议你在最初测试集成时创建一个单独的应用。 若要详细了解如何从库中添加应用，可以单击[此处](../manage-apps/add-application-portal.md)。 
