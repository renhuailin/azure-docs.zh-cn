...
## <a name="add-contractworks-from-the-gallery"></a>从库中添加 ContractWorks
...
## <a name="configure-and-test-azure-ad-sso-for-contractworks"></a>配置并测试 ContractWorks 的 Azure AD SSO

使用名为 B.Simon 的测试用户配置和测试 ContractWorks 的 Azure AD SSO。 若要运行 SSO，需要在 Azure AD 用户与 ContractWorks 相关用户之间建立链接关系。

若要配置并测试 ContractWorks 的 Azure AD SSO，请执行以下步骤：
...
1. 在 Azure 门户的“ContractWorks”应用程序集成页上，找到“管理”部分，选择“单一登录”  。
...
1. 在“使用 SAML 设置单一登录”页上，输入以下字段的值： 
    ...
    在“标识符”文本框中键入 URL：`https://login.securedocs.com/saml/metadata`
...
本部分在 ContractWorks 中创建名为 B.Simon 的用户。 请在 [ContractWorks 支持团队](mailto:support@contractworks.com)的配合下，将用户添加到 ContractWorks 平台中。 使用单一登录前，必须先创建并激活用户。
...
* 在 Azure 门户中单击“测试此应用程序”。 这将重定向到 ContractWorks 登录 URL，你可以在其中启动登录流。  

* 直接转到 ContractWorks 登录 URL，从那里启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，你会自动登录到为其设置了 SSO 的 ContractWorks。 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 单击“我的应用”中的 ContractWorks 磁贴时，如果是在 SP 模式下配置的，你会被重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，则会自动登录到为其设置了 SSO 的 ContractWorks。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](../user-help/my-apps-portal-end-user-access.md)。

## <a name="next-steps"></a>后续步骤

配置 ContractWorks 后，可以强制实施会话控制，从而实时防止组织的敏感数据遭受外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-aad)。
