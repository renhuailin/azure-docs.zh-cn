...
## <a name="configure-and-test-azure-ad-sso-for-my-ibisworld"></a>配置并测试 My IBISWorld 的 Azure AD SSO

使用名为 B.Simon 的测试用户配置并测试 My IBISWorld 的 Azure AD SSO。  若要使 SSO 有效，需要在 Azure AD 用户与 My IBISWorld 相关用户之间建立关联。

若要配置并测试 My IBISWorld 的 Azure AD SSO，请执行以下步骤：
...
1. 在 Azure 门户中的“My IBISWorld”应用程序集成页上，找到“管理”部分并选择“单一登录”。
...
1. 在“基本 SAML 配置”部分，若要在 IDP 发起的模式下配置应用程序，请执行以下步骤：
    ...
    在“中继状态”文本框中，键入 URL：`RPID=http://fedlogin.ibisworld.com`，并将“登录 URL”文本框留空。

1. 如果要在 SP  发起的模式下配置应用程序，请单击“设置其他 URL”  ，并执行以下步骤：

    请联系 [My IBISWorld 支持团队](mailto:support@ibisworld.freshdesk.com)以获取 IBISWorld 的登录 URL，并将其设置到“登录 URL”文本框中。
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 My IBISWorld 登录 URL，你可以在那里启动登录流。  

* 直接转到 My IBISWorld 登录 URL，并从那里启动登录流。
...
配置 My IBISWorld 后，可以强制实施会话控制，实时防止组织的敏感数据外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-any-app)。