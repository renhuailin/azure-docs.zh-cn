...
* LinkedIn Sales Navigator 支持实时用户预配。
* LinkedIn Sales Navigator 支持自动用户预配。

> [!NOTE]
> 此应用程序的标识符是一个固定字符串值，因此只能在一个租户中配置一个实例。

## <a name="add-linkedin-sales-navigator-from-the-gallery"></a>从库中添加 LinkedIn Sales Navigator
...
## <a name="configure-and-test-azure-ad-sso-for-linkedin-sales-navigator"></a>配置并测试 LinkedIn Sales Navigator 的 Azure AD SSO
...
若要配置并测试 LinkedIn Sales Navigator 的 Azure AD SSO，请执行以下步骤：
...
1. **[配置 LinkedIn Sales Navigator SSO](#configure-linkedin-sales-navigator-sso)** - 在应用程序端配置单一登录设置。
    1. **[创建 LinkedIn Sales Navigator 测试用户](#create-linkedin-sales-navigator-test-user)** - 在 LinkedIn Sales Navigator 中创建 B.Simon 的对应用户，并将其关联到其在 Azure AD 中的表示形式。
...
1. 在 Azure 门户中的“LinkedIn Sales Navigator”应用程序集成页上，找到“管理”部分并选择“单一登录”  。
...
1. 在“帐户中心”  ，单击“设置”  下的“全局设置”  。 此外，请从下拉列表中选择“Sales Navigator”。 
    ...
    ![屏幕截图显示了“应用程序设置”，你可以在其中选择“销售导航”。](./media/linkedinsalesnavigator-tutorial/settings.png)

1. 单击“或单击此处从表单加载并复制单个字段”并执行以下步骤： 

    ![屏幕截图显示了“单一登录”，你可以在其中输入所述值。](./media/linkedinsalesnavigator-tutorial/values.png)
...
1. 转到“LinkedIn 管理设置”  分区。 单击“上传 XML 文件”选项，上传从 Azure 门户下载的 XML 文件。 
    ...
    ![屏幕截图显示了“配置 LinkedIn 服务提供商 SSO 设置”，你可以在其中上传 XML 文件。](./media/linkedinsalesnavigator-tutorial/metadata.png)

1. 单击“打开”  启用 SSO。 SSO 状态将从“未连接”  更改为“已连接” 

    ![屏幕截图显示了单一登录，你可以在其中启用“通过 SSO 对用户进行身份验证”。](./media/linkedinsalesnavigator-tutorial/authentication.png)
...
LinkedIn Sales Navigator 应用程序支持适时用户 (JIT) 预配，且进行身份验证后，会在应用程序中自动创建用户。 激活“自动分配许可证”以便为用户分配许可证。 
   ...
   ![创建 Azure AD 测试用户](./media/linkedinsalesnavigator-tutorial/provisioning.png)
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 LinkedIn Sales Navigator 登录 URL，可在其中启动登录流。  

* 直接转到 LinkedIn Sales Navigator 登录 URL，并从那里启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，你应会自动登录到为其设置了 SSO 的 LinkedIn Sales Navigator。 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 在“我的应用”中单击 LinkedIn Sales Navigator 磁贴时，如果是在 SP 模式下配置的，会重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，则应会自动登录到为其设置了 SSO 的 LinkedIn Sales Navigator。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](../user-help/my-apps-portal-end-user-access.md)。

## <a name="next-steps"></a>后续步骤

配置 LinkedIn Sales Navigator 后，就可以强制实施会话控制，从而实时保护组织的敏感数据免于外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-any-app)。