...
## <a name="add-darwinbox-from-the-gallery"></a>从库中添加 Darwinbox
...
## <a name="configure-and-test-azure-ad-sso-for-darwinbox"></a>配置并测试 Darwinbox 的 Azure AD SSO

使用名为 B.Simon 的测试用户配置和测试 Darwinbox 的 Azure AD SSO。 若要运行 SSO，需要在 Azure AD 用户与 Darwinbox 相关用户之间建立链接关系。

若要配置并测试 Darwinbox 的 Azure AD SSO，请执行以下步骤：
...
1. 在 Azure 门户的“Darwinbox”应用程序集成页上，找到“管理”部分，选择“单一登录”  。
...
1. 在“基本 SAML 配置”部分中，按照以下步骤操作：
   ...
   1. 在“标识符(实体 ID)”文本框中，使用以下模式键入 URL：`https://<SUBDOMAIN>.darwinbox.in/adfs/module.php/saml/sp/metadata.php/<CUSTOM_ID>`

    1. 在“登录 URL”文本框中，使用以下模式键入 URL：`https://<SUBDOMAIN>.darwinbox.in/` 

      > [!NOTE]
      > 这些不是实际值。 使用实际标识符和登录 URL 更新这些值。 请联系 [Darwinbox 客户端支持团队](https://darwinbox.com/contact-us.php)获取这些值。 还可以参考 Azure 门户中的“基本 SAML 配置”  部分中显示的模式。
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 Darwinbox 登录 URL，可以在其中启动登录流。 

* 直接转到 Darwinbox 登录 URL，并从那里启动登录流。

* 你可使用 Microsoft 的“我的应用”。 在“我的应用”中单击“Darwinbox”磁贴时，将会重定向到 Darwinbox 登录 URL。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)。
...
1. 打开 Darwinbox 移动应用程序。 立即单击“输入组织 URL”，在文本框中输入你的组织 URL，然后单击箭头按钮。
    ...
    ![显示“Darwinbox”移动应用的屏幕截图，其中选择了“输入组织 URL”，并突出显示了一个示例组织和箭头按钮。](media/darwinbox-tutorial/login.png)

1. 如果你有多域，则单击你的域。

    ![显示“选择域”屏幕的屏幕截图，其中已选择一个示例域。](media/darwinbox-tutorial/domain.png)

1. 将 Azure AD 电子邮件输入 Darwinbox 应用程序，然后单击“下一步”。

    ![屏幕截图显示“登录”屏幕并突出显示“下一步”按钮。](media/darwinbox-tutorial/email.png)

1. 将 Azure AD 密码输入 Darwinbox 应用程序，然后单击“登录”。

    ![屏幕截图显示“登录选项”屏幕并突出显示“下一步”按钮。](media/darwinbox-tutorial/account.png)

1. 最后，在登录成功后，将显示应用程序主页。

    ![Darwinbox 移动应用](media/darwinbox-tutorial/application.png)

## <a name="next-steps"></a>后续步骤

配置 Darwinbox 后，可以强制实施会话控制，实时防止组织的敏感数据遭到外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-aad)。