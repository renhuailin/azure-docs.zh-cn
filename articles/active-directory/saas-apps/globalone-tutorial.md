---
title: 教程：Azure AD SSO 与 EY GlobalOne 集成
...
ms.locfileid: "128568516"
---
# <a name="tutorial-azure-ad-sso-integration-with-ey-globalone"></a>教程：Azure AD SSO 与 EY GlobalOne 集成
...
## <a name="add-ey-globalone-from-the-gallery"></a>从库中添加 EY GlobalOne
...
若要配置并测试 EY GlobalOne 的 Azure AD SSO，请执行以下步骤：
...
1. [配置 EY GlobalOne SSO](#configure-ey-globalone-sso)，以在应用程序端配置 SSO 设置。
    1. **[创建 EY GlobalOne 测试用户](#create-ey-globalone-test-user)** ，以便在 EY GlobalOne 中拥有 B. Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 Azure 门户中的 EY GlobalOne 应用程序集成页上，找到“管理”部分，然后选择“单一登录”  。
...
## <a name="configure-ey-globalone-sso"></a>配置 EY GlobalOne SSO
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 EY GlobalOne 登录 URL，可在其中启动登录流。  

* 直接转到 EY GlobalOne 登录 URL，并从那里启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，应会自动登录到为其设置了 SSO 的 EY GlobalOne。 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 在“我的应用”中单击 EY GlobalOne 磁贴后：如果是在 SP 模式下配置的，会重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，则应会自动登录到已设置 SSO 的 EY GlobalOne。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](../user-help/my-apps-portal-end-user-access.md)。
