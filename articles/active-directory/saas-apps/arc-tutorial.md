...
## <a name="configure-and-test-azure-ad-sso-for-arc-publishing---sso"></a>配置并测试 Arc Publishing - SSO 的 Azure AD SSO

使用名为 **B.Simon** 的测试用户配置并测试 Arc Publishing - SSO 的 Azure AD SSO。 若要正常使用 SSO，需要在 Azure AD 用户与 Arc Publishing - SSO 中的相关用户之间建立链接关系。

若要配置并测试 Arc Publishing - SSO 的 Azure AD SSO，请执行以下步骤：
...
1. 在 Azure 门户中的“Arc Publishing - SSO”应用程序集成页上，找到“管理”部分并选择“单一登录”  。
...
1. 除上述属性以外，Arc Publishing - SSO 应用程序还要求在 SAML 响应中传回其他几个属性，如下所示。 这些属性也是预先填充的，但可以根据要求查看它们。
    ...
    > [!NOTE]
    > 此处使用 user.assignedroles 映射组属性。 这些是在 Azure AD 中创建的自定义角色，以将组名称映射回应用程序。 可在[此处](https://docs.microsoft.com/azure/active-directory/develop/howto-add-app-roles-in-azure-ad-apps#app-roles-ui)找到有关如何在 Azure AD 中创建自定义角色的更多指导。
...
* 在 Azure 门户中单击“测试此应用程序”。 这样将会重定向到 Arc Publishing - SSO 登录 URL，可在其中启动登录流。  

* 直接转到 Arc Publishing - SSO 登录 URL，并从那里启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，应会自动登录到为其设置了 SSO 的 Arc Publishing - SSO 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 在“我的应用”中单击 Arc Publishing - SSO 磁贴时，如果是在 SP 模式下配置的，会重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，则应会自动登录到为其设置了 SSO 的 Arc Publishing - SSO。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。


## <a name="next-steps"></a>后续步骤

配置 Arc Publishing - SSO 后，可以强制实施会话控制，实时保护组织的敏感数据免于外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。
