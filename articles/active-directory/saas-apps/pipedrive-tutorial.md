...
## <a name="add-pipedrive-from-the-gallery"></a>从库中添加 Pipedrive
...
## <a name="configure-and-test-azure-ad-sso-for-pipedrive"></a>配置并测试 Pipedrive 的 Azure AD SSO

使用名为 **B.Simon** 的测试用户配置并测试 Pipedrive 的 Azure AD SSO。 若要正常使用 SSO，需要在 Azure AD 用户与 Pipedrive 中的相关用户之间建立链接关系。

若要配置并测试 Pipedrive 的 Azure AD SSO，请执行以下步骤：
...
1. 在 Azure 门户中的“Pipedrive”应用程序集成页上，找到“管理”部分并选择“单一登录”  。
...
1. 单击“用户配置文件”并选择“设置”。  
    ...
    ![显示从“用户个人资料”菜单选择了“设置”的屏幕截图。](./media/pipedrive-tutorial/configure-1.png)

1. 向下滚动到“安全中心”，然后选择“单一登录”。 

    ![显示在“安全中心”中选择了“单一登录”的屏幕截图。](./media/pipedrive-tutorial/configure-2.png)

1. 在“Pipedrive 的 SAML 配置”部分执行以下步骤： 

    ![显示“Pipedrive 的 SAML 配置”部分的屏幕截图，其中突出显示了所有文本框。](./media/pipedrive-tutorial/configure-3.png)
...
1. 向下滚动到“公司”，然后选择“管理用户”。 
    ...
    ![显示从“公司”菜单选择了“管理用户”的屏幕截图。](./media/pipedrive-tutorial/user-1.png)

1. 单击“添加用户”。 
    
    ![显示在右侧选择了“添加用户”按钮的“管理用户”页的屏幕截图。](./media/pipedrive-tutorial/user-2.png)

1. 在“管理用户”部分执行以下步骤： 

    ![Pipedrive 配置](./media/pipedrive-tutorial/user-3.png)
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 Pipedrive 登录 URL，你可以从此处启动登录流。  

* 直接转到 Pipedrive 登录 URL，并从此处启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，你应会自动登录到为其设置了 SSO 的 Pipedrive。 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 当你在“我的应用”中单击 Pipedrive 磁贴时，如果是在 SP 模式下配置的，你会被重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，你应会自动登录到为其设置了 SSO 的 Pipedrive。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)。

## <a name="next-steps"></a>后续步骤

配置 Pipedrive 后，即可强制实施会话控制，实时防止组织的敏感数据发生外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-any-app)。