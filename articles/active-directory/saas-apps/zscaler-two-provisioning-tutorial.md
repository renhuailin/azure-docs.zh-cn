---
title: 教程：使用 Azure Active Directory 为 Zscaler Two 配置自动用户预配 | Microsoft Docs
description: 本教程介绍如何将 Azure Active Directory 配置为自动为 Zscaler Two 预配和取消预配用户帐户。
...
> [!NOTE]
> Azure AD 预配集成依赖于适用于企业帐户的 Zscaler Two SCIM API。

## <a name="add-zscaler-two-from-the-gallery"></a>从库添加 Zscaler Two

在使用 Azure AD 为 Zscaler Two 配置自动用户预配之前，需要从 Azure AD 应用程序库将 Zscaler Two 添加到托管的 SaaS 应用程序列表。
...
## <a name="assign-users-to-zscaler-two"></a>将用户分配到 Zscaler Two
...
* 建议先将一个 Azure AD 用户分配到 Zscaler Two 以测试自动用户预配配置。 稍后再分配更多的用户和组。

* 将用户分配到 Zscaler Two 时，需要在分配对话框中选择任何特定于应用程序的有效角色（如果有）。 具有“默认访问权限”  角色的用户排除在预配之外。

## <a name="set-up-automatic-user-provisioning"></a>设置自动用户预配

本部分介绍如何配置 Azure AD 预配服务以基于 Azure AD 中的用户和组分配在 Zscaler Two 中创建、更新和禁用用户和组。
...
4. 将“预配模式”  设置为“自动”  ：
    ...
    ![设置预配模式](./media/zscaler-two-provisioning-tutorial/provisioning-credentials.png)

5. 在“管理员凭据”部分中，输入 Zscaler Two 帐户的“租户 URL”和“机密令牌”，如下一步中所述    。

6. 若要获取“租户 URL”和“机密令牌”，请转到 Zscaler Two 门户中的“管理” > “身份验证设置”，然后选择“身份验证类型”下的 SAML       ：

    ![Zscaler Two 身份验证设置](./media/zscaler-two-provisioning-tutorial/secret-token-1.png)

    选择“配置 SAML”打开“配置 SAML”窗口   ：

    ![“配置 SAML”窗口](./media/zscaler-two-provisioning-tutorial/secret-token-2.png)

    选择“启用基于 SCIM 的预配”并复制“基本 URL”和“持有者令牌”，然后保存设置    。 在 Azure 门户中，将“基本 URL”粘贴到“租户 URL”框中，将“持有者令牌”粘贴到“机密令牌”框中     。

7. 在“租户 URL”和“机密令牌”框中输入值后，选择“测试连接”以确保 Azure AD 可以连接到 Zscaler Two    。 如果连接失败，请确保 Zscaler Two 帐户具有管理员权限，然后重试。

    ![测试连接](./media/zscaler-two-provisioning-tutorial/test-connection.png)

8. 在“通知电子邮件”框中输入应接收预配错误通知的人员或组的电子邮件地址  。 选择“发生故障时发送电子邮件通知”  ：

    ![设置通知电子邮件](./media/zscaler-two-provisioning-tutorial/notification.png)

9. 选择“保存”。 

10. 在“映射”部分下，选择“将 Azure Active Directory 用户同步到 ZscalerTwo”   ：

    ![同步 Azure AD 用户](./media/zscaler-two-provisioning-tutorial/user-mappings.png)

11. 在“属性映射”部分中，查看从 Azure AD 同步到 Zscaler Two 的用户属性  。 选为“匹配”属性的特性用于匹配 Zscaler Two 中的用户帐户以执行更新操作  。 选择“保存”，提交所有更改  。

    ![显示了 7 个映射的“属性映射”部分的屏幕截图。](./media/zscaler-two-provisioning-tutorial/user-attribute-mappings.png)

12. 在“映射”部分下，选择“将 Azure Active Directory 组同步到 ZscalerTwo”   ：

    ![同步 Azure AD 组](./media/zscaler-two-provisioning-tutorial/group-mappings.png)

13. 在“属性映射”部分中，查看从 Azure AD 同步到 Zscaler Two 的组属性  。 选为“匹配”属性的特性用于匹配 Zscaler Two 中的组以执行更新操作  。 选择“保存”，提交所有更改  。
...
15. 若要为 Zscaler Two 启用 Azure AD 预配服务，请在“设置”部分中将“预配状态”更改为“启用”    ：
    ...
    ![预配状态](./media/zscaler-two-provisioning-tutorial/provisioning-status.png)

16. 通过在“设置”部分的“范围”中选择所需的值，定义要预配到 Zscaler Two 的用户和/或组   。

    ![范围值](./media/zscaler-two-provisioning-tutorial/scoping.png)

17. 准备好预配时，选择“保存”  ：

    ![选择“保存”](./media/zscaler-two-provisioning-tutorial/save-provisioning.png)
