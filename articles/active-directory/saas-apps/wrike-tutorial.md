...
本教程介绍如何将 Wrike 与 Azure Active Directory (Azure AD) 集成。 将 Wrike 与 Azure AD 集成后，可以：

* 在 Azure AD 中控制谁有权访问 Wrike。
* 让用户使用其 Azure AD 帐户自动登录到 Wrike。
...
1. 在“从库中添加”部分的搜索框中，键入“Wrike” 。
1. 从结果面板中选择“Wrike”，然后添加该应用。 在该应用添加到租户时等待几秒钟。

## <a name="configure-and-test-azure-ad-sso-for-wrike"></a>配置并测试 Wrike 的 Azure AD SSO

使用名为 B.Simon 的测试用户配置并测试 Wrike 的 Azure AD SSO。 若要使 SSO 正常工作，需要在 Azure AD 用户与 Wrike 相关用户之间建立关联。

若要配置并测试 Wrike 的 Azure AD SSO，请执行以下步骤：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
    1. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 B. Simon 测试 Azure AD 单一登录。
    1. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 B. Simon 能够使用 Azure AD 单一登录。
1. **[配置 Wrike SSO](#configure-wrike-sso)** - 在应用程序端配置单一登录设置。
    1. **[创建 Wrike 测试用户](#create-wrike-test-user)** - 在 Wrike 中创建 B.Simon 的对应用户，并将其关联到其在 Azure AD 中的表示形式。
...
1. 在 Azure 门户中，在 B.Simon 应用程序集成页上，找到“管理”部分并选择“单一登录”  。
...
在本部分中，你将通过授予 B.Simon 访问 B.Simon 的权限，使其能够使用 Azure 单一登录。
...
## <a name="configure-wrike-sso"></a>配置 Wrike SSO
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 B.Simon 登录 URL，并在其中启动登录流。  

* 直接转到 Wrike 登录 URL，并从那里启动登录流。
...
配置 Wrike 后，可以强制实施会话控制，实时防止组织的敏感数据外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-aad)。