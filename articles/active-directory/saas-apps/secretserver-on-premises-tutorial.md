...
## <a name="add-secret-server-on-premises-from-the-gallery"></a>从库中添加 Secret Server (On-Premises)
...
## <a name="configure-and-test-azure-ad-sso-for-secret-server-on-premises"></a>为 Secret Server (On-Premises) 配置并测试 Azure AD SSO
...
若要配置并测试 Secret Server (On-Premises) 的 Azure AD SSO，请执行以下步骤：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
    1. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 B. Simon 测试 Azure AD 单一登录。
    1. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 B. Simon 能够使用 Azure AD 单一登录。
1. [配置 Secret Server (On-Premises) SSO](#configure-secret-server-on-premises-sso) - 在应用程序端配置单一登录设置。
    1. **[创建 Secret Server (On-Premises) 测试用户](#create-secret-server-on-premises-test-user)** - 在 Secret Server (On-Premises) 中创建 B.Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 Azure 门户中的“Secret Server (On-Premises)”应用程序集成页上，找到“管理”部分并选择“单一登录”。
...
1. 如果要在“IDP”发起的模式下配置应用程序，请在“基本 SAML 配置”部分中输入以下字段的值 ：
    ...
    a. 在“标识符”文本框中键入 URL：`https://secretserveronpremises.azure`
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 Secret Server (On-Premises) 登录 URL，你可以在其中启动登录流。  

* 直接转到 Secret Server (On-Premises) 登录 URL，并在其中启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，应该会自动登录到为其设置了 SSO 的 Secret Server (On-Premises)。 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 在“我的应用”中单击 Secret Server (On-Premises) 磁贴时，如果是在 SP 模式下配置的，会重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，则应会自动登录到为其设置了 SSO 的 Secret Server (On-Premises)。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。

## <a name="next-steps"></a>后续步骤

配置 Secret Server (On-Premises) 后，可以强制实施会话控制，从而实时保护组织的敏感数据免于外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。
