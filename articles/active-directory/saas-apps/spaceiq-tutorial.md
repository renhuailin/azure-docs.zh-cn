...
本教程介绍如何将 SpaceIQ 与 Azure Active Directory (Azure AD) 集成。 将 SpaceIQ 与 Azure AD 集成后，可以：

* 在 Azure AD 中控制谁有权访问 SpaceIQ。
* 让用户使用其 Azure AD 帐户自动登录到 SpaceIQ。
...
1. 在“从库中添加”部分的搜索框中，键入“SpaceIQ”。
1. 从结果面板中选择“SpaceIQ”，然后添加该应用。 在该应用添加到租户时等待几秒钟。

## <a name="configure-and-test-azure-ad-sso-for-spaceiq"></a>配置并测试 SpaceIQ 的 Azure AD SSO

使用名为 B.Simon 的测试用户配置并测试 SpaceIQ 的 Azure AD SSO。 若要使 SSO 正常工作，需要在 Azure AD 用户与 SpaceIQ 中的相关用户之间建立关联。

若要配置并测试 SpaceIQ 的 Azure AD SSO，请执行以下步骤：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
    1. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 B. Simon 测试 Azure AD 单一登录。
    1. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 B. Simon 能够使用 Azure AD 单一登录。
1. [配置 SpaceIQ SSO](#configure-spaceiq-sso) - 在应用程序端配置单一登录设置。
    1. [创建 SpaceIQ 测试用户](#create-spaceiq-test-user) - 在 SpaceIQ 中创建 B.Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 Azure 门户的“SpaceIQ”应用程序集成页上，找到“管理”部分，并选择“单一登录”。
...
4. 在“设置 SAML 单一登录”页上，执行以下步骤：
    ...
    b. 在“回复 URL”文本框中，使用以下模式键入 URL：`https://api.spaceiq.com/saml/<INSTANCE_ID>/callback`
...
在本部分，你将通过授予 B.Simon 访问 SpaceIQ 的权限，支持其使用 Azure 单一登录。
...
## <a name="configure-spaceiq-sso"></a>配置 SpaceIQ SSO

1. 打开一个新的浏览器窗口，并以管理员身份登录到 SpaceIQ 环境。

1. 登录后，单击右上角的拼图标志，然后单击“集成”

    ![帐户设置](./media/spaceiq-tutorial/setting.png) 

1. 在“所有预配和 SSO”下，单击 Azure 磁贴，添加 Azure 实例作为 IDP。

    ![SAML 图标](./media/spaceiq-tutorial/azure.png)

1. 在“SSO”对话框中，执行以下步骤：

    ![SAML 身份验证设置](./media/spaceiq-tutorial/configuration.png)
...
配置 SpaceIQ 后，可以强制实施会话控制，实时防止组织的敏感数据外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-aad)。