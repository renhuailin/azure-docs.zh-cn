...
## <a name="add-smartdraw-from-the-gallery"></a>从库中添加 SmartDraw
...
## <a name="configure-and-test-azure-ad-sso-for-smartdraw"></a>配置并测试 SmartDraw 的 Azure AD SSO

使用名为 **B.Simon** 的测试用户配置并测试 SmartDraw 的 Azure AD SSO。 若要正常使用 SSO，需要在 Azure AD 用户与 SmartDraw 中的相关用户之间建立链接关系。

若要配置并测试 SmartDraw 的 Azure AD SSO，请执行以下步骤：
...
1. 在“Azure 门户”中的“SmartDraw”应用程序集成页上，找到“管理”部分并选择“单一登录”  。
...
1. 单击“管理 SmartDraw 许可证”下的“单一登录”。 
    ...
    ![显示“管理 SmartDraw 许可证”对话框的屏幕截图，你可在其中选择“单一登录”。](./media/smartdraw-tutorial/single-sign-on.png)

1. 在“配置”页上执行以下步骤：

    ![显示可在其中输入所述值的“配置”页的屏幕截图。](./media/smartdraw-tutorial/configuration.png)
...
* 在 Azure 门户中单击“测试此应用程序”。 此操作会重定向到 SmartDraw 登录 URL，你可在其中启动登录流。  

* 直接转到 SmartDraw 登录 URL，并从那里启动登录流。

#### <a name="idp-initiated"></a>IDP 启动的：

* 在 Azure 门户中单击“测试此应用程序”后，你应会自动登录到为其设置了 SSO 的 SmartDraw。 

还可以使用 Microsoft“我的应用”在任何模式下测试此应用程序。 在“我的应用”中单击 SmartDraw 磁贴时，如果是在 SP 模式下配置的，会重定向到应用程序登录页来启动登录流；如果是在 IDP 模式下配置的，则应会自动登录到为其设置了 SSO 的 SmartDraw。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)。

## <a name="next-steps"></a>后续步骤

配置 SmartDraw 后，就可以强制实施会话控制，从而实时保护组织的敏感数据免于外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-aad)。