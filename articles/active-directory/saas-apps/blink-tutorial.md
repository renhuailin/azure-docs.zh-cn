...
* Blink 支持 SP 发起的 SSO。
* Blink 支持实时用户预配。
* Slack 支持[自动用户预配](blink-provisioning-tutorial.md)。
...
## <a name="configure-and-test-azure-ad-sso-for-blink"></a>配置并测试 Blink 的 Azure AD SSO

使用名为 **B.Simon** 的测试用户配置和测试 Blink 的 Azure AD SSO。 若要正常使用 SSO，需要在 Azure AD 用户与 Blink 相关用户之间建立链接关系。

若要配置并测试 Blink 的 Azure AD SSO，请执行以下步骤：
...
1. 在 Azure 门户的“Blink”应用程序集成页上，找到“管理”部分，选择“单一登录”  。
...
Blink 还支持自动用户预配，有关如何配置自动用户预配的更多详细信息，请参见[此处](./blink-provisioning-tutorial.md)。

## <a name="test-sso"></a>测试 SSO

在本部分，你将使用以下选项测试 Azure AD 单一登录配置。

* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 Blink 登录 URL，可以从那里启动登录流。

* 直接转到 Blink 登录 URL，并从那里启动登录流。

* 你可使用 Microsoft 的“我的应用”。 单击“我的应用”中的 Blink 磁贴时，会重定向到 Blink 登录 URL。 有关“我的应用”的详细信息，请参阅[“我的应用”简介](../user-help/my-apps-portal-end-user-access.md)。

## <a name="next-steps"></a>后续步骤

配置 Blink 后，即可强制实施会话控制，实时防止组织的敏感数据外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-aad)。