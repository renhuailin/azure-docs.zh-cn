...
本教程介绍如何将 itslearning 与 Azure Active Directory (Azure AD) 集成。 将 itslearning 与 Azure AD 集成后，可以：

* 在 Azure AD 中控制谁有权访问 itslearning。
* 让用户使用其 Azure AD 帐户自动登录到 itslearning。
...
1. 在“从库中添加”部分的搜索框中，键入 itslearning 。
1. 在结果面板中选择“itslearning”，然后添加该应用。 在该应用添加到租户时等待几秒钟。

## <a name="configure-and-test-azure-ad-sso-for-itslearning"></a>配置并测试 itslearning 的 Azure AD SSO

使用名为 B.Simon 的测试用户配置并测试 itslearning 的 Azure AD SSO。 若要正常使用 SSO，需要在 Azure AD 用户与 itslearning 中的相关用户之间建立链接关系。

若要配置并测试 itslearning 的 Azure AD SSO，请执行以下步骤：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
    1. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 B. Simon 测试 Azure AD 单一登录。
    1. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 B. Simon 能够使用 Azure AD 单一登录。
1. [配置 itslearning SSO](#configure-itslearning-sso) - 在应用程序端配置单一登录设置。
    1. [创建 itslearning 测试用户](#create-itslearning-test-user) - 在 itslearning 中创建 B.Simon 的对应用户，并将其链接到其在 Azure AD 中的表示形式。
...
1. 在 Azure 门户中的“itslearning”应用程序集成页上，找到“管理”部分并选择“单一登录”  。
...
4. 在“基本 SAML 配置”部分中，按照以下步骤操作：
    ...
    a. 在“标识符(实体 ID)”文本框中，键入 URL：`urn:mace:saml2v2.no:services:com.itslearning` 
...
在本部分，你将通过授予 B.Simon 访问 itslearning 的权限，使其能够使用 Azure 单一登录。
...
## <a name="configure-itslearning-sso"></a>配置 itslearning SSO
...
* 在 Azure 门户中单击“测试此应用程序”。 这会重定向到 itslearning 登录 URL，可从中启动登录流。 
...
配置 itslearning 后，可以强制实施会话控制，实时防止组织的敏感数据外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](/cloud-app-security/proxy-deployment-any-app)。