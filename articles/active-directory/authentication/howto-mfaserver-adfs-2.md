---
title: 将 Azure MFA 服务器与 AD FS 2.0 配合使用 - Azure Active Directory
...
>
> 如果使用基于云的 MFA，请参阅[使用 Azure AD 多重身份验证和 AD FS 来保护云资源](howto-mfa-adfs.md)。
>
...
10. 如果所有用户均已导入或将导入到该服务器并接受双重验证，请选中“需要 Azure 多重身份验证用户匹配”框。 如果大量用户尚未导入到该服务器并且/或者将免除进行多重身份验证，请使该框处于未选中状态。
    ...
    ![向 MFA 服务器添加基于窗体的网站](./media/howto-mfaserver-adfs-2/manual.png)
...
确保已将 Active Directory 中的用户导入服务器。 如果想将内部 IP 地址加入允许列表，以在从这些位置登录网站时不需要双重验证，请参阅[受信任的 IP 部分](#trusted-ips)。

![用于配置公司设置的注册表编辑器](./media/howto-mfaserver-adfs-2/reg.png)
...
4. 输入应允许使用的 IP 地址、IP 地址范围或子网。 如果输入的是子网，则选择相应的网络掩码，然后单击“确定”按钮。

![为 MFA 服务器配置受信任的 IP](./media/howto-mfaserver-adfs-2/trusted.png)
