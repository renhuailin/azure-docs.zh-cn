---
title: 将 Azure MFA 服务器与 AD FS 2.0-Azure Active Directory 配合使用
...
确保已将 Active Directory 中的用户导入服务器。 如果希望允许内部 IP 地址，以便在从这些位置登录网站时不需要进行双重验证，请参阅 " [受信任的 ip" 部分](#trusted-ips) 。
...
确保已将 Active Directory 中的用户导入服务器。 如果希望允许内部 IP 地址，以便在从这些位置登录网站时不需要进行双重验证，请参阅 "受信任的 Ip" 部分。
