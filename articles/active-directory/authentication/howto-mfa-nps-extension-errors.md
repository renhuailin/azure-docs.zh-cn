...
| **HTTPS_COMMUNICATION_ERROR** | NPS 服务器无法从 Azure AD MFA 接收响应。 验证防火墙是否双向打开，用于传入和传出 https://adnotifications.windowsazure.com 的流量 |
...
当对 NPS 扩展进行故障排除时，[Azure AD MFA NPS 扩展运行状况检查脚本](/samples/azure-samples/azure-mfa-nps-extension-health-check/azure-mfa-nps-extension-health-check/)执行基本的运行状况检查。 运行脚本，然后选择选项 3。
