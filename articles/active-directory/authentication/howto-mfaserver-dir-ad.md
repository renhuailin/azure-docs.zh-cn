---
title: Azure MFA 服务器与 Active Directory - Azure Active Directory
...
> [!NOTE]
> 不保证目录集成适用于 Active Directory 域服务以外的目录。

| 功能 | 说明 |
| --- | --- |
| 使用 Active Directory |选择“使用 Active Directory”选项可使用 Active Directory 进行导入和同步。  这是默认设置。 <br>注意：要正常完成 Active Directory 集成，请将计算机加入域并使用域帐户登录。 |
...
| 功能 | 说明 |
| --- | --- |
| 服务器 |输入运行 LDAP 目录的服务器的主机名或 IP 地址。  还可以以分号分隔形式指定备份服务器。 <br>注意：当绑定类型是 SSL (TLS) 时，需要输入完全限定的主机名。 |
...
![在 MFA 服务器中自定义目录集成属性](./media/howto-mfaserver-dir-ad/dirint3.png)
...
| 可分辨名称 |输入包含每条记录的可分辨名称的属性的属性名称。  在 Active Directory 中，它通常是 distinguishedName。 其他 LDAP 实现可能使用 entryDN 或类似值。  默认值为 distinguishedName。 <br><br>如果只包含可分辨名称的属性不存在，则可以使用 ads path 属性。  将自动剥除路径的“LDAP://\<server\>/”部分，仅留下对象的可分辨名称。 |
...
可以设置额外的多重身份验证服务器作为备份 RADIUS 代理、LDAP 代理，或将其用于 IIS 身份验证。 同步配置在所有代理之间共享。 但是，只有其中一个代理可以让多重身份验证服务器服务运行。 此选项卡允许选择应用于同步的多重身份验证服务器。
