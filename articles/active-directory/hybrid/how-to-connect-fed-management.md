...
2. 在“连接到 Azure AD”  页上，提供 Azure AD 的全局管理员凭据，并单击“下一步”  。
   ![此屏幕截图显示了“连接到 Azure AD”页，其中包含输入的示例凭据。](./media/how-to-connect-fed-management/RepairADTrust2.PNG)

3. 在“远程访问凭据”  页上，输入域管理员的凭据。

   ![此屏幕截图显示了“远程访问凭据”页，其中包含输入的示例凭据。](./media/how-to-connect-fed-management/RepairADTrust3.PNG)
    ...
    ![此屏幕截图显示了“已准备好进行配置”页，其中包含操作列表。](./media/how-to-connect-fed-management/RepairADTrust5.PNG)
...
> [!NOTE]
> 有关 AlternateID 和手动配置步骤的详细信息，请阅读[配置备用登录 ID](/windows-server/identity/ad-fs/operations/configuring-alternate-login-id)
...
2. 在“连接到 Azure AD”页上，输入 Azure AD 的全局管理员凭据，并单击“下一步”。  
   ...
   ![此屏幕截图显示了“连接到 Azure AD”页，其中包含输入的示例凭据。](./media/how-to-connect-fed-management/AddNewADFSServer2.PNG)
...
4. Azure AD Connect 会要求你提供在使用 Azure AD Connect 配置新的 AD FS 场时提供的 PFX 文件的密码。 单击“输入密码”  提供 PFX 文件的密码。
   ...
   ![此屏幕截图显示了“指定 SSL 证书”页，其中打开了“证书密码”窗口。](./media/how-to-connect-fed-management/AddNewADFSServer4.PNG)

    ![此屏幕截图显示了输入 PFX 文件密码后的“指定 SSL 证书”页。](./media/how-to-connect-fed-management/AddNewADFSServer5.PNG)
...
6. 单击“下一步”并完成最终“配置”页。   Azure AD Connect 完成将服务器添加到 AD FS 场后，将提供验证连接性的选项。
   ...
   ![此屏幕截图显示了“已准备好进行配置”页，其中包含单击“安装”后要完成的操作的列表。](./media/how-to-connect-fed-management/AddNewADFSServer7.PNG)

    ![此屏幕截图显示了“安装完成”页，其中显示了“已成功验证 Intranet 配置”消息。 ](./media/how-to-connect-fed-management/AddNewADFSServer8.PNG)
...
2. 提供 Azure 全局管理员凭据。
   ...
   ![此屏幕截图显示了“连接到 Azure AD”页，其中包含输入的示例用户名和密码。](./media/how-to-connect-fed-management/wapserver2.PNG)
...
6. 在“已准备好配置  ”页上，向导显示要执行的操作列表。
   ...
   ![此屏幕截图显示了“已准备好进行配置”页，其中包含要执行的操作的列表。](./media/how-to-connect-fed-management/WapServer7.PNG)
...
AD FS 支持丰富的声明语言，让你用来创建自定义声明规则。 有关详细信息，请参阅[声明规则语言的作用](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/dd807118(v=ws.11))。
