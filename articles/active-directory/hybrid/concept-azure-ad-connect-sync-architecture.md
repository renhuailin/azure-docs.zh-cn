...
![示意图显示了一个已连接的数据源，以及一个与名为“连接器”的行关联的同步引擎。](./media/concept-azure-ad-connect-sync-architecture/arch1.png)
...
![示意图显示了一个已连接的数据源和一个同步引擎，该引擎被分隔到连接器空间和 Metaverse 命名空间，并与一个名为“连接器”的行相关联。](./media/concept-azure-ad-connect-sync-architecture/arch2.png)
...
![示意图显示了一个导入对象，它从已连接的数据源引入到同步引擎中的连接器空间命名空间。](./media/concept-azure-ad-connect-sync-architecture/arch3.png)
...
![示意图显示了一个导出对象，它从 Metaverse 引入到连接器空间命名空间，然后再到已连接的数据源。](./media/concept-azure-ad-connect-sync-architecture/arch4.png)
...
![示意图显示了两个已连接的数据对象，它们由连接器关联到一个同步引擎的，该引擎有联接的对象和一个脱离的对象。](./media/concept-azure-ad-connect-sync-architecture/arch5.png)
...
![示意图显示了身份信息流，该信息从连接的数据流入连接器空间（导入），到 Metaverse，再到连接器空间（同步），然后流入连接的数据（导出）。](./media/concept-azure-ad-connect-sync-architecture/arch6.png)
...
![示意图显示了一个对象通过连接器在连接器空间与连接的数据之间的同步。](./media/concept-azure-ad-connect-sync-architecture/arch7.png)
