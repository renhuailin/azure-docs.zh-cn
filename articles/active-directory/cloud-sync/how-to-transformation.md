...
通过转换，你可以使用云同步更改如何将属性与 Azure Active Directory (Azure AD) 同步的默认行为。
...
1. 在 user 对象下查找 `ExtensionAttribute3` 的代码。
    ...
    ```
1. 编辑代码，以便将 company 属性映射到 `ExtensionAttribute3`。
   ...
   ```
 1. 将架构复制回 Graph 浏览器，将“请求类型”更改为“PUT”，然后选择“运行查询”。

    ![运行查询](media/how-to-transformation/transform-2.png)

 1. 现在，在 Azure 门户中，转到“云同步配置”，选择“重新启动预配”。

    ![重新启动预配](media/how-to-transformation/transform-3.png)

 1. 稍后，通过在 Graph 浏览器中运行以下查询来验证正在填充的属性：`https://graph.microsoft.com/beta/users/{Azure AD user UPN}`。
 1. 现在应看到如下值。

    ![显示值](media/how-to-transformation/transform-4.png)

## <a name="custom-attribute-mapping-with-function"></a>使用函数自定义属性映射
