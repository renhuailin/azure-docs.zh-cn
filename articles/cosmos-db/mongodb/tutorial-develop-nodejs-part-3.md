...
本教程的第 3 部分基于[第 2 部分](tutorial-develop-nodejs-part-2.md)，涵盖以下任务：
...
开始教程的此部分之前，请确保已完成教程[第 2 部分](tutorial-develop-nodejs-part-2.md)的步骤。
...
1. 在 Visual Studio Code 中，单击“停止”按钮 :::image type="icon" source="./media/tutorial-develop-nodejs-part-3/stop-button.png"::: 以停止 Node 应用。 
...
2. 在 Windows 命令提示符或 Mac Terminal 窗口中输入以下命令，生成 heroes 组件。 在此代码中，g=generate（生成），c=component（组件），heroes=组件名称，并且由于使用的是平面文件结构 (--flat)，因此没有为其创建子文件夹。
    ...
    :::image type="content" source="./media/tutorial-develop-nodejs-part-3/install-heros-component.png" alt-text="安装 hero 组件":::
...
4. 在同一文件夹中打开 app.module.ts 文件。请注意，该文件在第 5 行向声明添加了 `HeroesComponent`，并在第 10 行对其进行了导入操作。
    ...
    :::image type="content" source="./media/tutorial-develop-nodejs-part-3/app-module-file.png" alt-text="打开 app-module.ts 文件":::
...
5. 左键单击 `Hero` 一词，Visual Studio 会在代码块左侧显示一个灯泡图标。 
    ...
    :::image type="content" source="./media/tutorial-develop-nodejs-part-3/light-bulb.png" alt-text="Visual Studio Code 中的灯泡":::
...
2. 在 Visual Studio Code 中，单击左侧的“调试”按钮 :::image type="icon" source="./media/tutorial-develop-nodejs-part-2/debug-button.png":::，然后单击“启动调试”按钮 :::image type="icon" source="./media/tutorial-develop-nodejs-part-3/start-debugging-button.png":::。

3. 现在请打开 Internet 浏览器，导航到 localhost:3000，然后就会看到该应用在本地运行。

     :::image type="content" source="./media/tutorial-develop-nodejs-part-3/azure-cosmos-db-mongodb-mean-app.png" alt-text="在本地运行的 Hero 应用程序":::
