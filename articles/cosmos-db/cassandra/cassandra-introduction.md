...
ms.locfileid: "121782948"
---
# <a name="introduction-to-the-azure-cosmos-db-cassandra-api"></a>Azure Cosmos DB Cassandra API 简介
[!INCLUDE[appliesto-cassandra-api](../includes/appliesto-cassandra-api.md)]

Azure Cosmos DB Cassandra API 可以充当为 [Apache Cassandra](https://cassandra.apache.org) 编写的应用的数据存储。 这意味着通过使用现有的符合 CQLv4 的 [Apache 驱动程序](https://cassandra.apache.org/doc/latest/cassandra/getting_started/drivers.html?highlight=driver)，现有 Cassandra 应用程序现在可以与 Azure Cosmos DB Cassandra API 通信。 在许多情况下，只需更改连接字符串，就可以从使用 Apache Cassandra 切换为使用 Azure Cosmos DB Cassandra API。 
...
**吞吐量和存储弹性**：Azure Cosmos DB 可以通过 Azure门户、PowerShell 或 CLI 操作跨所有区域提供吞吐量，并可缩放预配的吞吐量。 可以根据需要[弹性缩放](scale-account-throughput.md)表的存储和吞吐量，其性能可以预测。
...
* 可以快速地着手生成以下特定于语言的应用，以便创建和管理 Cassandra API 数据库：
  - [Node.js 应用](manage-data-nodejs.md)
  - [.NET 应用](manage-data-dotnet.md)
  - [Python 应用](manage-data-python.md)
