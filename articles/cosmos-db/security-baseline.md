---
title: 适用于 Azure Cosmos DB 的 Azure 安全基线
description: Azure Cosmos DB 安全基线为实现 Azure 安全基准中指定的安全建议提供过程指南和资源。
author: msmbaldwin
...
ms.locfileid: "101659420"
---
# <a name="azure-security-baseline-for-azure-cosmos-db"></a>适用于 Azure Cosmos DB 的 Azure 安全基线

此安全基线将 [Azure 安全基准版本 1.0](../security/benchmarks/overview-v1.md) 中的指南应用到 Azure Cosmos DB。 Azure 安全基准提供有关如何在 Azure 上保护云解决方案的建议。
内容由 Azure 安全基准定义的 **安全控制** 和适用于 Azure Cosmos DB 的相关指南进行分组。 排除了不适用于 Azure Cosmos DB 的 **控件**。

 
若要查看 Azure Cosmos DB 完全映射到 Azure 安全基准，请参阅 [完整的 Azure Cosmos DB 安全基线映射文件](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)。
...
- [如何配置 Azure Cosmos DB 的专用终结点](how-to-configure-private-endpoints.md) 

- [如何创建具有安全配置的网络安全组](../virtual-network/tutorial-filter-network-traffic.md)

- [如何在 Cosmos DB 中配置 IP 防火墙](how-to-configure-firewall.md)
...
**Azure 策略内置定义-Microsoft.DocumentDB**：
...
- [配置跨域资源共享](how-to-configure-cross-origin-resource-sharing.md)
...
- [如何配置 Azure Cosmos DB 高级威胁防护](cosmos-db-advanced-threat-protection.md)
...
- [如何配置 Cosmos DB 高级威胁防护](cosmos-db-advanced-threat-protection.md)
...
**指导**：通过 Azure Monitor 引入日志来聚合 Azure Cosmos DB 生成的安全数据。 在 Azure Monitor 中，使用 Log Analytics 工作区来查询和执行分析，并使用存储帐户进行长期/存档存储。 或者，你可以将数据 SIEM 到 Azure Sentinel 或第三方安全事件和事件管理 (的) 。 

- [如何为 Azure Cosmos DB 启用诊断日志](/azure/cosmos-db/logging)
...
**指南**：启用 Azure Cosmos DB 的诊断设置，并将日志发送到 Log Analytics 工作区或 Azure 存储帐户。 Azure Cosmos DB 中的诊断设置用于收集资源日志。 这些日志是根据请求捕获的，它们也称为 **数据平面日志**。 数据平面操作的一些示例包括删除、插入和读取。 

你还可以启用 Azure 活动日志诊断设置，并将这些日志发送到用于 Azure Cosmos DB 日志的 Log Analytics 工作区。

- [如何启用 Azure Cosmos DB 的诊断设置](/azure/cosmos-db/logging)
...
- [如何在 Log Analytics 工作区中执行 Azure Cosmos DB 查询](monitor-cosmos-db.md)
...
**指导**：可以使用 Azure 门户中的“标识和访问控制(IAM)”窗格配置基于角色的访问控制 (RBAC)，并维护有关 Azure Cosmos DB 资源的库存。 角色适用于 Azure Active Directory (Azure AD) 中的用户、组、服务主体和管理标识。 对于个人和组，可使用内置角色或自定义角色。

Azure Cosmos DB 为 Azure Cosmos DB 中的常见管理方案提供内置的 Azure RBAC。 在 Azure AD 中有配置文件的个人可以将这些 Azure 角色分配给用户、组、服务主体或托管标识，以授予或拒绝对 Azure Cosmos DB 资源的资源和操作的访问权限。

还可以使用 Azure AD PowerShell 模块执行即席查询，以发现属于管理组的成员的帐户。

此外，Azure Cosmos DB 中的一些操作可以用 Azure AD 和特定于帐户的主密钥来控制。 使用“disableKeyBasedMetadataWriteAccess”帐户设置控制密钥访问。

- [了解 Azure Cosmos DB 中基于角色的访问控制](role-based-access-control.md)

- [使用 umentDB 命名空间 (Microsoft.DocAzure Cosmos DB 操作生成您自己的自定义角色) ](https://docs.microsoft.com/azure/role-based-access-control/resource-provider-operations#microsoftdocumentdb)

- [在 Azure AD 中创建新角色](../role-based-access-control/custom-roles.md)
...
- [将用户访问权限仅限于数据操作](how-to-restrict-user-data.md)
...
**指南**：默认或空白密码的概念与 Azure Active Directory (Azure AD) 或 Azure Cosmos DB 不存在。 Azure Cosmos DB 使用两种类型的密钥来验证用户身份并提供其数据和资源的访问权限：主密钥和资源令牌。 随时可以重新生成这些密钥。

- [了解 Azure Cosmos DB 中的数据的安全访问](secure-access-to-data.md)

- [如何重新生成 Azure Cosmos DB 密钥](https://docs.microsoft.com/azure/cosmos-db/manage-with-powershell#regenerate-keys)

- [如何使用 Azure AD 以编程方式访问密钥](certificate-based-authentication.md)
...
**指导**：不适用；Azure Cosmos DB 不支持管理员帐户。 所有访问都与 Azure Active Directory (Azure AD) 和 Azure RBAC () 基于角色的访问控制相集成。
...
**指南**： Azure Cosmos DB 使用两种类型的密钥来向用户授权，并且不支持在数据平面级别 (SSO) 单个 Sign-On。 可以通过 REST API 访问 Cosmos DB 的控制平面，并支持 SSO。 若要进行身份验证，请将请求的授权标头设置为从 Azure Active Directory (Azure AD) 获取的 JSON Web 令牌。

- [了解 Azure Database for Cosmos DB REST API](/rest/api/cosmos-db/)
...
此外，在环境中发生可疑或不安全活动时，还可以使用 Azure Active Directory (Azure AD) Privileged Identity Management (PIM) 来生成日志和警报。
...
- [如何配置和管理 Azure SQL Azure AD 身份验证](/azure/sql-database/sql-database-aad-authentication-configure)
...
**指导**：Azure Active Directory (Azure AD) 提供日志来帮助发现过时的帐户。 此外，可以使用 Azure 标识访问评审来有效管理组成员身份、对企业应用程序的访问和角色分配。 可以定期评审用户的访问权限，确保只有适当的用户才持续拥有访问权限。
...
**指南**：你可以创建 Azure Active Directory (的诊断设置 Azure AD) 用户帐户，将审核日志和登录日志发送到 Log Analytics 工作区，你可以在该工作区中配置所需的警报。
...
你还可以使用 Azure Active Directory (Azure AD) Identity Protection 和风险检测功能，配置对检测到的与用户标识相关的可疑操作的自动响应。 此外，还可以将日志引入 Azure Sentinel 以便进一步调查。
...
- [用 Azure 认知搜索为 Azure Cosmos DB 数据编制索引](https://docs.microsoft.com/azure/search/search-howto-index-cosmosdb?toc=/azure/cosmos-db/toc.json&amp;bc=/azure/cosmos-db/breadcrumb/toc.json)
...
**指导**：Azure Cosmos DB 为 Azure Cosmos DB 中的常见管理方案提供内置的 Azure 基于角色的访问控制 (Azure RBAC)。 在 Azure Active Directory (Azure AD) 中具有配置文件的个人可以将这些 Azure 角色分配给用户、组、服务主体或托管标识，以授予或拒绝对 Azure Cosmos DB 资源的资源和操作的访问权限。 角色分配范围仅限控制平面访问，包括对 Azure Cosmos 帐户、数据库、容器和套餐（吞吐量）的访问。

- [如何在 Azure Cosmos DB 中实现 Azure RBAC](role-based-access-control.md)
...
- [了解 Azure Cosmos DB 的静态加密](database-encryption-at-rest.md)

- [了解 Azure Cosmos DB 的静态加密密钥管理](/azure/cosmos-db/cosmos-db-security-controls)

- [如何配置 Azure Cosmos DB 帐户的客户托管密钥](how-to-setup-cmk.md)
...
- [Azure 安全中心提供支持的功能](https://docs.microsoft.com/azure/security-center/security-center-services?tabs=features-windows)
...
- [了解 Azure 基于角色的访问控制](../role-based-access-control/overview.md)
...
**指导**：使用 Azure Policy 为 Cosmos DB 实例定义和实施标准安全配置。 使用 **Microsoft.DocumentDB** 命名空间中的 Azure 策略别名创建自定义策略，以便审核或强制执行 Cosmos DB 实例的配置。 还可以为 Azure Cosmos DB 利用内置策略定义，例如：
- 为 Cosmos DB 帐户部署高级威胁防护
- Cosmos DB 应使用虚拟网络服务终结点
...
**指南**：如果对 Cosmos DB 或相关资源使用自定义 Azure 策略定义，请使用 Azure Repos 将自定义策略定义作为代码进行存储和管理。

- [将策略设计为代码工作流](../governance/policy/concepts/policy-as-code.md)

- [Azure Repos 文档](/azure/devops/repos/git/gitworkflow)
...
使用托管标识在 Azure Active Directory (Azure AD) 中为 Azure 服务提供一个自动托管的标识。 使用托管标识可以向支持 Azure AD 身份验证的任何服务（包括 Key Vault）进行身份验证，无需在代码中放入任何凭据。
...
**指南**：在支持 azure 服务的基础主机上启用了 Microsoft 反恶意软件 (例如，azure CosmosDB) ，但是不会在客户内容上运行。
...
- [了解 Azure Cosmos DB 自动备份](online-backup-and-restore.md)

- [如何还原 Azure Cosmos DB 中的数据](/azure/cosmos-db/how-to-backup-and-restore)
...
- [如何还原 Azure Key Vault 机密](https://docs.microsoft.com/powershell/module/az.keyvault/restore-azkeyvaultkey?view=azps-4.8.0&amp;preserve-view=true)
...
- [了解 Azure Cosmos DB 中的数据加密](database-encryption-at-rest.md)
