...
v2 SDK 在操作过程中使用 `DocumentClientException` 标记错误，v3 SDK 使用 `CosmosClientException`，后者公开了 `StatusCode`、`Diagnostics` 和其他与响应相关的信息。 使用 `ToString()` 时，所有完整信息都将被序列化：
...
v2 SDK 通过 `ResponseDiagnosticsString` 属性提供“仅限 Direct”诊断，v3 SDK 使用所有响应和异常中皆可用的 `Diagnostics`，后者更加丰富并且不局限于 Direct 模式。 它们不仅包括操作在 SDK 上花费的时间，还包括操作接触的区域：
...
`ConnectionPolicy` 中的某些设置已被重命名或替换：
...
|`ConnectionProtocol`|删除。 协议绑定到模式，模式为网关 (HTTPS) 或 Direct (TCP)。|
...
v2 SDK 通过 `Timestamp` 属性公开文档的时间戳，因为 `Document` 不再可用，用户可以将 `_ts` [系统属性](account-databases-containers-items.md#properties-of-an-item)映射到模型的属性中。

### <a name="openasync"></a>OpenAsync

对于使用 `OpenAsync()` 预热 v2 SDK 客户端的用例，可使用 `CreateAndInitializeAsync` 同时[创建和预热](https://devblogs.microsoft.com/cosmosdb/improve-net-sdk-initialization/) v3 SDK 客户端。
...
* 正在尝试为迁移到 Azure Cosmos DB 进行容量计划？
    ...
    * 如果你知道当前数据库工作负载的典型请求速率，请阅读[使用 Azure Cosmos DB Capacity Planner 估算请求单位](estimate-ru-with-capacity-planner.md)
