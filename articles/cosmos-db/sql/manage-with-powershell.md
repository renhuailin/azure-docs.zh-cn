...
以下指南介绍了如何使用 PowerShell 通过脚本来自动管理 Azure Cosmos DB Core (SQL) API 资源，其中包括 Cosmos 帐户、数据库、容器和吞吐量。 对于其他 API 的 PowerShell cmdlet，请参阅[适用于 Cassandra 的 PowerShell 示例](../cassandra/powershell-samples.md)、[适用于 MongoDB API 的 PowerShell 示例](../mongodb/powershell-samples.md)、[适用于 Gremlin 的 PowerShell 示例](../graph/powershell-samples.md)、[适用于 Table 的 PowerShell 示例](../table/powershell-samples.md)
