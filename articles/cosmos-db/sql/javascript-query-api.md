...
|`unwind(collectionSelector, [resultSelector], [options], [callback])`|执行与内层数组的自联接，并将来自双方的结果作为元组添加到结果投影。 例如，将 person 文档与 person.pets 进行联接将生成 [person, pet] 元组。 这类似于 .NET LINQ 中的 SelectMany。|
