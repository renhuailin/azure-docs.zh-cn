...
本文介绍使用 Azure Cosmos DB OLTP 连接器和 Azure Analysis Services 在 Power BI 中创建实时天气仪表板所需执行的步骤。 Power BI 仪表板将会显示图表，以便显示有关区域中的气温和降水量的准实时信息。
...
|方案 |设置 |
|---------|---------|
|1. 通过聚合在大型数据集上生成实时报表     | 选项 1：[Power BI 和带 DirectQuery 的 Azure Synapse Link](../synapse-link-power-bi.md)<br />  选项 2：[Power BI 和带 DirectQuery 的 Spark 连接器 + Azure Databricks + Azure Cosmos DB Spark 连接器。](https://github.com/Azure/azure-cosmosdb-spark/wiki/Connecting-Cosmos-DB-with-PowerBI-using-spark-and-databricks-premium)<br />  选项 3：Power BI 和带 DirectQuery 的 Azure Analysis Services 连接器 + Azure Analysis Services + Azure Databricks + Cosmos DB Spark 连接器。     |
|2. 在大型数据集上生成实时报表 (>= 10 GB)    |  选项 1：[Power BI 和带 DirectQuery 的 Azure Synapse Link](../synapse-link-power-bi.md)<br />  选项 2：[Power BI 和带 DirectQuery 的 Azure Analysis Services 连接器+ Azure Analysis Services](create-real-time-weather-dashboard-powerbi.md)       |
|3. 在大型数据集上生成即席报表 (< 10 GB)     |  [支持导入模式和增量刷新的 Power BI Azure Cosmos DB 连接器](create-real-time-weather-dashboard-powerbi.md)       |
|4. 生成定期刷新的即席报表   |  [采用导入模式的 Power BI Azure Cosmos DB 连接器（按计划定期刷新）](powerbi-visualize.md)       |
|5 生成即席报表（不刷新）    |  [采用导入模式的 Power BI Azure Cosmos DB 连接器](powerbi-visualize.md)       |


可以[使用 Azure Cosmos DB Power BI 连接器](powerbi-visualize.md)轻松设置方案 4 和 5。 本文将在下面介绍方案 2（选项 2）和方案 3 的设置。
