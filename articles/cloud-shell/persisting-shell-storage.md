...
安全起见，每个用户应预配自己的存储帐户。  对于 Azure 基于角色的访问控制 (Azure RBAC)，用户必须具有存储帐户级别的“参与者”访问权限或更高访问权限。
...
客户应选择一个主要区域，除非他们要求将其静态数据存储在特定区域中。 如果他们有此要求，则应使用次要存储区域。

### <a name="secondary-storage-regions"></a>次要存储区域
如果使用了次要存储区域，则关联的 Azure 存储帐户与要装载它们的 Cloud Shell 计算机位于不同的区域。 例如，Jane 可以将其存储帐户设置为位于加拿大东部（次要区域），但她安装的计算机仍位于主要区域。 她的静态数据位于加拿大，但在美国进行处理。
...
在 Cloud Shell 中，可运行一个名为 `clouddrive` 的命令，手动更新装载到 Cloud Shell 的文件共享。
...
备注：如果需要在文件中定义函数并从 PowerShell cmdlet 调用，则必须包含点运算符。 例如： .\MyFunctions.ps1
