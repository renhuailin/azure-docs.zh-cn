...
- 指向 Azure 内容分发网络 (CDN) 终结点 - 当你使用 Azure 存储和 Azure CDN 创建静态网站时，此功能非常有用。
- 指向同一区域中的另一个 DNS 记录集 - 别名记录可以引用相同类型的其他记录集。 例如，DNS CNAME 记录集可以是另一个 CNAME 记录集的别名。 如果希望有些记录集是别名，有些记录集不是别名，则这种安排会很有用。
...
为了解决此问题，可以使用别名记录。 与 CNAME 记录不同，别名记录在区域顶点创建，应用程序所有者可以使用它来将其区域顶点记录指向具有外部端点的流量管理器配置文件。 应用程序所有者指向用于其 DNS 区域中任何其他域的相同流量管理器配置文件。
