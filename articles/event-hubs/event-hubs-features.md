...
根据可配置的、基于时间的保留策略，从事件中心删除已发布的事件。 下面是几个要点：

- **默认** 值和可能的 **最短** 保持期为 **1 天 (24 小时)**。
- 对于事件中心 **标准版**，最长保留期为 **7 天**。 
- 对于 **专用** 的事件中心，最长保留期为 **90 天**。
- 如果更改保持期，则它将应用于所有消息，包括事件中心内已有的消息。 
