...
为启用了 BYOK 的命名空间设置诊断日志可提供有关操作所需的信息。 可以启用这些日志，稍后将其流式传输到事件中心，或通过 log analytics 进行分析或流式传输到存储，以执行自定义的分析。 若要了解有关诊断日志的详细信息，请参阅 [Azure 诊断日志概述](../azure-monitor/essentials/platform-logs-overview.md)。
