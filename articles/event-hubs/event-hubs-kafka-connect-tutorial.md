...
[Apache Kafka Connect](https://kafka.apache.org/documentation/#connect) 是一个框架，可通过 Kafka 群集连接到任何外部系统（例如 MySQL、HDFS）和文件系统并与之进行数据的导入/导出操作。 本教程详细介绍如何将 Kafka Connect 框架与事件中心配合使用。

> [!WARNING]
> 使用 Apache Kafka Connect 框架及其连接器不符合通过 Microsoft Azure 获得产品支持的条件。
>
