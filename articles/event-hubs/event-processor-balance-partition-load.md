...
每个事件处理器实例获取分区的所有权，并从上一个已知的[检查点](# Checkpointing)开始处理分区。 如果处理器发生故障（VM 关闭），则其他实例将通过查看上次修改时间来检测它。 其他实例尝试获取之前由非活动实例所拥有的分区所有权，检查点存储保证只有一个实例能够成功声明分区所有权。 因此，在任何给定时间点，最多有一个处理器从分区接收事件。
