---
title: Microsoft OPC 发布服务器命令行参数
description: 本文概述了 OPC 发布服务器命令行参数
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787427"
---
# <a name="command-line-arguments"></a>命令行参数

在以下内容中，可以使用所述的多个命令行参数来设置 OPC 发布服务器的全局设置。

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>版本 2.5 及更低版本的 OPC 发布服务器命令行参数

* 用法：opcpublisher.exe \<applicationname> [\<iothubconnectionstring>] [\<options>]
    
* applicationname：要使用的 OPC UA 应用程序名称，必需                       应用程序名称还用于在 IoT 中心设备注册表中用此名称注册发布服务器。
    
* iothubconnectionstring：IoT 中心所有者连接字符串，可选。 通常情况下，仅在首次启动应用程序时，指定 IoT 中心所有者连接字符串。 会加密连接字符串，并将其存储在该平台的证书存储中。
在后续调用中，将从此处读取并重复使用连接字符串。 如果在每次启动时指定连接字符串，则将删除为 IoT 中心设备注册表中的应用程序创建的设备并且每次都会重新创建。
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>版本 2.6 及更高版本的 OPC 发布服务器命令行参数
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
