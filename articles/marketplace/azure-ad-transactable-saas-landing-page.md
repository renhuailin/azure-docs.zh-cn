...
| “值” | 说明 |
| ------------ | ------------- |
| aud | 此令牌的目标受众。 在本例中，它应匹配你的应用程序 ID 并经过验证。 |
| preferred_username | 访问用户的主用户名。 可以是电子邮件地址、电话号码或其他标识符。 |
...
> [!NOTE]
> 来自 MSA 租户（租户 ID 为 ``9188040d-6c67-4c5b-b112-36a304b66dad``）的帐户返回的信息不会超过已使用 ID 令牌收集的信息。 因此可以跳过对这些帐户的 Graph API 调用。
