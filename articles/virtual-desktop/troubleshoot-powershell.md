...
description: 如何排查设置 Windows 虚拟桌面环境时 PowerShell 出现的问题。
author: Heidilohr
...
- Windows 虚拟桌面环境应已绑定到正确的 Azure Active Directory。
...
原因：Windows 虚拟桌面支持选择主机池、应用程序组和工作区的位置，以将服务元数据存储在特定的位置。 你的选项仅限于可使用此功能的位置。 此错误表示该功能在所选位置不可用。
...
- 若要排查设置 Windows 虚拟桌面环境和主机池时出现的问题，请参阅[环境和主机池创建](troubleshoot-set-up-issues.md)。
- 若要排查在 Windows 虚拟桌面中配置虚拟机 (VM) 时遇到的问题，请参阅[会话主机虚拟机配置](troubleshoot-vm-configuration.md)。
- 若要排查 Windows 虚拟桌面客户端连接问题，请参阅 [Windows 虚拟桌面服务连接](troubleshoot-service-connection.md)。
