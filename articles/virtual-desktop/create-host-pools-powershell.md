...
>[!IMPORTANT]
>为了帮助保护 Azure 中的 Azure 虚拟桌面环境，建议你不要在 VM 上打开入站端口 3389。 Azure 虚拟桌面不需要打开入站端口 3389，就能让用户访问主机池的 VM。 如果必须打开端口 3389 以进行故障排除，我们建议你使用[实时 VM 访问](../security-center/security-center-just-in-time.md)。 同时建议不要向 VM 分配公共 IP。
...
6. 卸载这些项后，这将删除与旧主机池的所有关联。 如果要将此主机重新注册到服务，请按照[将虚拟机注册到 Azure 虚拟桌面主机池](create-host-pools-powershell.md#register-the-virtual-machines-to-the-azure-virtual-desktop-host-pool)中的说明进行操作。
