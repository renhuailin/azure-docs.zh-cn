---
title: 排查远程桌面客户端问题 - Windows 虚拟桌面 - Azure
...
首先请检查所用的 Azure Active Directory 帐户。 如果已使用与要用于 Windows 虚拟桌面的帐户不同的 Azure Active Directory 帐户登录，则应注销或使用专用浏览器窗口。

如果使用的是 Windows 虚拟桌面（经典版），请使用[此文](./virtual-desktop-fall-2019/connect-web-2019.md)中的 Web 客户端链接连接到资源。
...
1. 确认 Web 客户端 URL 正确。
2. 确认所用的凭据适用于已绑定到该 URL 的 Windows 虚拟桌面环境。
...
## <a name="windows-client-blocks-windows-virtual-desktop-classic-feed"></a>Windows 客户端阻止 Windows 虚拟桌面（经典版）源

如果 Windows 客户端源不显示 Windows 虚拟桌面（经典版）应用，请按照以下说明进行操作：

1. 检查条件访问策略是否包含与 Windows 虚拟桌面（经典版）关联的应用 ID。
2. 检查条件访问策略是否阻止除 Windows 虚拟桌面（经典版）应用 ID 以外的其他所有身份进行访问。 如果是这样，则需要将应用 ID 9cdead84-a844-4324-93f2-b2e6bb768d07 添加到策略，以允许客户端发现源。
