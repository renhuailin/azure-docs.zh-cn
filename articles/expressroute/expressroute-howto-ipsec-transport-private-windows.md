...
ms.locfileid: "98017266"
---
# <a name="configure-ipsec-transport-mode-for-expressroute-private-peering"></a>配置 ExpressRoute 专用对等互连的 IPsec 传输模式

本文可帮助你通过 ExpressRoute 专用对等互连在传输模式下创建 IPsec 隧道。 隧道是在运行 Windows 和本地 Windows 主机的 Azure Vm 之间创建的。 本文中针对此配置的步骤使用组策略对象。 尽管可以创建此配置，而无需使用组织单位 (Ou) 和组策略对象 (Gpo) 。 Ou 和 Gpo 的组合将有助于简化安全策略的控制，并使你能够快速扩展。 本文中的步骤假定已有 Active Directory 配置，并且你熟悉如何使用 Ou 和 Gpo。

## <a name="about-this-configuration"></a>关于此配置

以下步骤中的配置使用单个 Azure 虚拟网络 (VNet) 与 ExpressRoute 专用对等互连。 但是，此配置可以跨越其他 Azure Vnet 和本地网络。 本文将帮助你定义可应用于一组 Azure Vm 或本地主机的 IPsec 加密策略。 这些 Azure Vm 或本地主机属于同一个 OU。 仅针对目标端口为 8080 的 HTTP 流量，在 Auzre VM（vm1 和 vm2）和本地 host1 之间配置加密。 可以根据要求创建不同类型的 IPsec 策略。
...
* **筛选器列表：** 筛选器列表是一个或多个筛选器的捆绑包。 一个列表可包含多个筛选器。 筛选器定义是根据以下条件阻止、允许还是保护通信： IP 地址范围、协议，甚至是特定端口。 每个筛选器匹配一组特定的条件；例如，从特定子网发送到特定计算机上特定目标端口的数据包。 当网络条件匹配这些筛选器中的一个或多个时，筛选器列表便被激活。 每个筛选器在特定筛选器列表内进行定义。 筛选器不能在筛选器列表之间共享。 但是，给定的筛选器列表可并入多个 IPsec 策略中。 
...
* 验证本地主机和 Azure Vm 之间是否有连接。

* 验证 Azure Windows Vm 和本地主机是否可以使用 DNS 正确解析名称。
...
1. 通过打开组策略管理 "管理单元来创建链接到 OU 的新 GPO。 然后找到该 GPO 将链接到的 OU。 在示例中，OU 名为“IPSecOU”。 

   [![9]][9]
2. 在“组策略管理”管理单元中，选择 OU 并右键单击。 在下拉列表中，选择 "**在此域中创建 GPO 并在此处链接**..."。

   [![10]][10]
3. 为 GPO 取一个直观的名称，以便可在之后轻松找到它。 选择 **"确定"** 以创建和链接 GPO。
...
1. 从下拉列表中，右键单击 **Active Directory 上的 "IP 安全策略**"，然后选择 " **管理 ip 筛选器列表和筛选器操作 ...**"。

   [![15]][15]
2. 在 "**管理筛选器操作**" 选项卡上，选择 " **添加**"。

   [![16]][16]

3. 在 **IP 安全筛选器操作向导** 上，选择 " **下一步**"。

   [![17]][17]
4. 为筛选器操作取一个直观的名称，以便可在之后找到它。 在本示例中，筛选器操作名为“myEncryption”。 还可以添加说明。 然后选择“下一步”  。

   [![18]][18]
5. “协商安全性”允许定义在 IPsec 无法与另一台计算机成功建立时的行为。 选择 " **协商安全**"，然后选择 " **下一步**"。

   [![19]][19]
6. 在 " **与不支持 IPsec 的计算机通信** " 页上，选择 " **不允许不安全的通信**"，然后选择 " **下一步**"。

   [![20]][20]
7. 在 " **IP 流量和安全** " 页上，选择 " **自定义**"，然后选择 " **设置 ...**"。

   [![21]][21]
8. 在“自定义安全方法设置”页上，选择“数据完整性和加密 (ESP): SHA1、3DES”。 选择“确定”。 

   [![22]][22]
9. 在“管理筛选器操作”页上，可以看到“myEncryption”筛选器已成功添加。 选择“关闭”。
...
1. 若要限定必须加密哪些类型的流量，请使用 IP 筛选器列表。 在 " **管理 IP 筛选器列表** " 选项卡中，选择 " **添加** " 以添加新的 IP 筛选器列表。

   [![24]][24]
2. 在“名称”字段中，键入 IP 筛选器列表的名称。 例如，“azure-onpremises-HTTP8080”。 然后选择“添加”。

   [![25]][25]
3. 在“IP 筛选器描述和镜像属性”页上，选择“镜像”。 镜像设置匹配两个方向的数据包，这使得可以进行双向通信。 然后，选择“下一步”。

   [![26]][26]
4. 在“IP 流量源”页上，从“源地址:”下拉菜单中，选择“特定的 IP 地址或子网”。 

   [![27]][27]
5. 指定 IP 流量的源地址 **Ip 地址或子网：** ，然后选择 " **下一步**"。

   [![28]][28]
6. 指定“目标地址:”IP 地址或子网。 然后选择“下一步”  。

   [![29]][29]
7. 在“IP 协议类型”页上，选择“TCP”。 然后选择“下一步”  。

   [![30]][30]
8. 在“IP 协议端口”页上，选择“从任意端口”和“至此端口:”。 在文本框中键入“8080”。 这些设置指定仅目标端口 8080 上的 HTTP 流量将被加密。 然后选择“下一步”  。
...
若要将同一类型的流量从本地主机加密到 Azure VM，需要另一个 IP 筛选器。 按照与设置第一个 IP 筛选器相同的步骤操作，并创建新的 IP 筛选器。 唯一的区别是源子网和目标子网。

1. 若要将新的 IP 筛选器添加到 IP 筛选器列表中，请选择“编辑”。

   [![33]][33]
2. 在 " **IP 筛选器列表** " 页上，选择 " **添加**"。
...
如果本地位置和 Azure 子网之间需要加密以保护应用程序。 您可以添加新的 IP 筛选器列表，而不是修改现有的 IP 筛选器列表。 将两个或多个 IP 筛选器列表关联到相同的 IPsec 策略将为你提供更大的灵活性。 您可以修改或删除 IP 筛选器列表，而不会影响其他 IP 筛选器列表。
...
1. 选择与 OU 相关联的“Active Directory 上的 IPSecurity 策略”。 右键单击，然后选择“创建 IP 安全策略”。
   ...
   [![37]][37]
2. 命名安全策略。 例如，“policy-azure-onpremises”。 然后选择“下一步”  。

   [![38]][38]
3. 选择 " **下一步** " 而不选中复选框。

   [![39]][39]
4. 验证是否选中了 " **编辑属性** " 复选框，然后选择 " **完成**"。
...
3. 规则提供选项来定义 IPsec 模式：隧道模式或传输模式。
   ...
   * 传输模式仅加密有效负载和 ESP 尾部；不会加密原始数据包的 IP 标头。 在传输模式下，数据包的 IP 源和 IP 目标不变。
   ...
   [![43]][43]
4. “网络类型”定义哪些网络连接与安全策略关联。 选择 " **所有网络连接**"，然后选择 " **下一步**"。

   [![44]][44]
5. 选择之前创建的 IP 筛选器列表，  **本地-HTTP8080**，然后选择 " **下一步**"。

   [![45]][45]
6. 选择之前创建的现有筛选器操作“myEncryption”。

   [![46]][46]
7. Windows 支持四种不同类型的身份验证：Kerberos、证书、NTLMv2 和预共享密钥。 由于我们使用的是已加入域的主机，因此请选择 **Active Directory 默认 (Kerberos V5 协议)**，然后选择 " **下一步**"。

   [![47]][47]
8. 新的策略会创建安全规则：“azure-onpremises-HTTP8080”。 选择“确定”。

   [![48]][48]

IPsec 策略要求目标端口 8080 上的所有 HTTP 连接使用 IPsec 传输模式。 由于 HTTP 是一种明文协议，因此启用安全策略后，可确保在通过 ExpressRoute 专用对等互连传输数据时对数据进行加密。 与具有高级安全性的 Windows 防火墙相比，配置 Active Directory 的 IPsec 策略更复杂。 但是，它允许对 IPsec 连接进行更多的自定义。

## <a name="8-assign-the-ipsec-gpo-to-the-ou"></a><a name="assigngpo"></a>8. 将 IPsec GPO 分配给 OU
...
PowerShell 脚本作为 HTTP 客户端在端口8080上生成 HTTP 请求：
...
如果 (HTTP 客户端) 本地运行 PowerShell 脚本，Azure VM 中的网络捕获将显示类似的跟踪。
