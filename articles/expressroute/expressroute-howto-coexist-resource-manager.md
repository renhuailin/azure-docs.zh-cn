...
1. 添加 VPN 客户端地址池。
   ...
   ```
2. 将 VPN [根证书](../vpn-gateway/vpn-gateway-howto-point-to-site-rm-ps.md#Certificates)上传到 Azure 用于 VPN 网关。 在此示例中，假定根证书存储在运行以下 PowerShell cmdlet 的本地计算机中，并且你在本地运行 PowerShell。 也可使用 Azure 门户来上传证书。
