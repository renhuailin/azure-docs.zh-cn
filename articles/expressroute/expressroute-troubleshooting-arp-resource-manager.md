...
description: 本页说明如何获取用于 ExpressRoute 线路)  (ARP 的地址解析协议
...
在进一步操作之前，请确保以下信息为 true：

* 配置了至少一个对等互连的有效 ExpressRoute 线路。 该线路必须由连接提供商进行完整的配置。 你或连接提供商必须至少在此线路上配置了 Azure 专用、Azure 公共或 Microsoft 对等互连。
...
* ARP 表中将有一个使用有效的 IP 地址和 MAC 地址的本地端条目。 这同样适用于 Microsoft 端。 
* 本地 IP 地址的最后一个八位字节将始终是奇数。
* Microsoft IP 地址的最后一个八位字节始终会是偶数。
* 对于所有三个对等互连 (主/辅助) ，同一 MAC 地址将显示在 Microsoft 端。 
...
如果发生本地或连接提供商的问题，ARP 表将显示以下两个情况之一。 你将看到本地 MAC 地址显示为 "未完成" 或 "仅在 ARP 表中看到 Microsoft 条目"。
