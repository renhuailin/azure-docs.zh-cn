...
技术配置文件提供带有内置机制的框架来与不同类型的参与方通信。 技术配置文件用于与 Azure Active Directory B2C (Azure AD B2C) 租户通信，以创建用户和读取用户配置文件。 技术配置文件可以自断言以实现与用户的交互。 例如，技术配置文件可收集用户的凭据进行登录，然后呈现注册页面或密码重置页面。

## <a name="types-of-technical-profiles"></a>技术配置文件的类型
...
- [Azure AD](active-directory-technical-profile.md)：为 Azure AD B2C 用户管理提供支持。
- [Azure AD 多重身份验证](multi-factor-auth-technical-profile.md)：通过使用 Azure AD 多重身份验证提供对验证电话号码的支持。
...
所有类型的技术配置文件都具有相同的概念。 它们首先读取输入声明并运行声明转换。 然后，它们与已配置的参与方（例如标识提供者、REST API 或 Azure AD 目录服务）进行通信。 在该过程完成后，技术配置文件返回输出声明并可能运行输出声明转换。 下图显示了如何处理技术配置文件中引用的转换和映射。 执行声明转换后，输出声明会立即存储在声明包中，无论技术配置文件与哪个参与方进行交互都是如此。
...
| 说明 | 0:1 | 技术配置文件的说明。 |
| 协议 | 1:1 | 用来与另一方通信的协议。 |
| 元数据 | 0:1 | 一组键和值，用于控制技术配置文件的行为。 |
| InputTokenFormat | 0:1 | 输入令牌的格式。 可能的值为 `JSON`、`JWT`、`SAML11` 或 `SAML2`。 `JWT` 值表示符合 IETF 规范的 JSON Web 令牌。 `SAML11` 值表示符合 OASIS 规范的 SAML 1.1 安全令牌。 `SAML2` 值表示符合 OASIS 规范的 SAML 2.0 安全令牌。 |
| OutputTokenFormat | 0:1 | 输出令牌的格式。 可能的值为 `JSON`、`JWT`、`SAML11` 或 `SAML2`。 |
...
| PersistedClaims | 0:1 | 对声明类型的之前定义的引用列表将持久保存在技术配置文件中。 |
| DisplayClaims | 0:1 | 对[自断言技术配置文件](self-asserted-technical-profile.md)提供的声明类型的以前定义引用的列表。 DisplayClaims 功能目前处于预览状态。 |
...
| IncludeInSso | 0:1 | 使用此技术配置文件时是应当对会话应用 SSO 行为，还是需要显式交互。 此元素仅在验证技术配置文件内使用的 SelfAsserted 配置文件中有效。 可能的值为 `true`（默认）或 `false`。 |
...
协议元素指定用于与另一方通信的协议。 Protocol  元素包含以下属性：

| Attribute | 必需 | 说明 |
| --------- | -------- | ----------- |
| 名称 | 是 | Azure AD B2C 支持的有效协议的名称，用作技术配置文件的一部分。 可能的值为 `OAuth1`、`OAuth2`、`SAML2`、`OpenIdConnect`、`Proprietary` 或 `None`。 |
| Handler | 否 | 当协议名称设置为 `Proprietary` 时，指定 Azure AD B2C 用来确定协议处理程序的程序集的名称。 |

## <a name="metadata"></a>元数据

Metadata 元素包含特定协议的相关配置选项。 相应的[技术配置文件](#types-of-technical-profiles)规范中介绍了支持的元数据列表。 Metadata 元素包含以下元素：
...
| 属性 | 必需 | 说明 |
| --------- | -------- | ----------- |
| 密钥 | 是 | 元数据密钥。 请参阅每种[技术配置文件类型](#types-of-technical-profiles)了解元数据项的列表。 |
...
为了与它所集成的服务建立信任，Azure AD B2C 以[策略密钥](policy-keys-overview.md)的形式存储机密和证书。 在技术配置文件执行期间，Azure AD B2C 从 Azure AD B2C 策略密钥中检索加密密钥。 然后 Azure AD B2C 使用密钥建立信任或对令牌进行加密或签名。 这些信任包括：

- [OAuth1](oauth1-technical-profile.md#cryptographic-keys)、[OAuth2](oauth2-technical-profile.md#cryptographic-keys) 和 [SAML](identity-provider-generic-saml.md) 标识提供者的联合身份验证。
- 保护与 [REST API 服务](secure-rest-api.md)的连接。
...
声明转换集合中的之前声明转换的输出声明可以是后续输入声明转换的输入声明。 通过这种方式，可以具有相互依赖的一系列声明转换。
...
InputClaims 元素从用于技术配置文件的声明包中提取声明。 例如，[自断言技术配置文件](self-asserted-technical-profile.md)使用输入声明来预填充用户提供的输出声明。 REST API 技术配置文件使用输入声明将输入参数发送到 REST API 终结点。 Azure AD 使用输入声明作为读取、更新或删除帐户的唯一标识符。
...
| PartnerClaimType | 否 | 指定的策略声明类型映射到的外部合作伙伴声明类型的标识符。 如果未指定 PartnerClaimType 属性，则指定的策略声明类型将映射到同名的合作伙伴声明类型。 当声明类型名称不同于另一方时，请使用此属性。 例如，如果第一个声明名称为“givenName”，而合作伙伴使用名为“first_name”的声明 。 |

## <a name="display-claims"></a>显示声明

“DisplayClaims”元素包含要呈现在屏幕上的用于从用户处收集数据的声明列表。 在显示声明集合中，可以包含对已创建的[声明类型](claimsschema.md)或[显示控件](display-controls.md)的引用。
...
OutputClaims 元素是在技术配置文件完成后返回到声明包的声明的集合。 可以在下一个业务流程步骤或输出声明转换中使用这些声明。 OutputClaims  元素包含以下元素：
...
| PartnerClaimType | 否 | 指定的策略声明类型映射到的外部合作伙伴声明类型的标识符。 如果未指定合作伙伴声明类型属性，则指定的策略声明类型将映射到同名的合作伙伴声明类型。 当声明类型名称不同于另一方时，请使用此属性。 例如，如果第一个声明名称为“givenName”，而合作伙伴使用名为“first_name”的声明 。 |

## <a name="output-claims-transformations"></a>输出声明转换

OutputClaimsTransformations 元素可能包含 OutputClaimsTransformation 元素的集合 。 输出声明转换用于修改输出声明或生成新的声明。 执行后，输出声明将被放回到声明包中。 可以在下一个业务流程步骤中使用这些声明。
...
验证技术配置文件用于验证[自断言技术配置文件](self-asserted-technical-profile.md#validation-technical-profiles)中的输出声明。 验证技术配置文件是来自任何协议（如 [Azure AD](active-directory-technical-profile.md) 或 [REST API](restful-technical-profile.md)）的普通技术配置文件。 验证技术配置文件返回输出声明，或返回错误代码。 错误消息在屏幕上呈现给用户，允许用户重试。

下图说明了 Azure AD B2C 如何使用验证技术配置文件来验证用户凭据。

![显示验证技术配置文件流的关系图。](./media/technical-profiles/validation-technical-profile.png)
...
SubjectNamingInfo 元素定义[依赖方策略](relyingparty.md#subjectnaminginfo)中的令牌中使用的使用者名称。 SubjectNamingInfo 元素包含以下属性：
...
假设你有一个 [REST API 技术配置文件](restful-technical-profile.md)，该配置文件具有单个终结点，其中你需要针对不同的场景发送不同的声明集。 创建具有共享功能的通用技术配置文件，例如 REST API 终结点 URI、元数据、身份验证类型和加密密钥。 创建包含通用技术配置文件的特定任务技术配置文件。 然后添加输入和输出声明，或覆盖与该技术配置文件相关的 REST API 终结点 URI。
...
- REST-ValidateProfile：包括 REST-API-Common 技术配置文件，并指定输入和输出声明 。
- REST-UpdateProfile：包括 REST-API-Common 技术配置文件，指定输入声明，并覆盖 `ServiceUrl` 元数据 。
...
### <a name="multilevel-inclusion"></a>多级包含
...
UseTechnicalProfileForSessionManagement 元素引用 [SSO 会话技术配置文件](custom-policy-reference-sso.md)。 **UseTechnicalProfileForSessionManagement** 元素包含以下属性：
...
用户旅程中的 [ClaimsProviderSelections](userjourneys.md#claims-provider-selection) 定义声明提供程序选项的列表及其顺序。 使用 EnabledForUserJourneys 元素可以筛选用户可用的声明提供程序。 **EnabledForUserJourneys** 元素包含以下值之一：
...
如果使用 OnClaimsExistence、OnItemExistenceInStringCollectionClaim 或 OnItemAbsenceInStringCollectionClaim，要求提供以下元数据  ：
