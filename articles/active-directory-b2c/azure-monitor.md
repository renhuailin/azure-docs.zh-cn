...
Azure AD B2C 使用 [Azure Active Directory 监视](../active-directory/reports-monitoring/overview-monitoring.md)。 若要在 Azure AD B2C 租户内启用 Azure Active Directory 中的诊断设置，请使用 [Azure Lighthouse](../lighthouse/overview.md) [委托资源](../lighthouse/concepts/architecture.md)，这样 Azure AD B2C（服务提供商）就可以管理 Azure AD（客户）资源。  完成本文中的步骤后，你将具有在 Azure AD B2C 门户中访问包含 [Log Analytics 工作区](../azure-monitor/logs/quick-create-workspace.md)的 azure-ad-b2c-monitor 资源组的权限。 还可以将日志从 Azure AD B2C 传输到 Log Analytics 工作区。

在此部署期间，授权 Azure AD B2C 目录中的某个用户或组在包含你的 Azure 订阅的租户中配置 Log Analytics 工作区实例。 若要创建授权，请将 [Azure 资源管理器](../azure-resource-manager/index.yml)模板部署到包含该订阅的 Azure AD 租户。

下图描绘了要在 Azure AD 和 Azure AD B2C 租户中配置的组件。

![资源组投影](./media/azure-monitor/resource-group-projection.png)

在此部署过程中，你将配置将托管 Log Analytics 工作区的 Azure AD B2C 租户和 Azure AD 租户。 应为 Azure AD B2C 帐户分配 Azure AD B2C 租户的[全局管理员](../active-directory/roles/permissions-reference.md#global-administrator)角色。 必须为用于运行此部署的 Azure AD 帐户分配 Azure AD 订阅的[所有者](../role-based-access-control/built-in-roles.md#owner)角色。另外务必还要确保在完成每个所述步骤时登录到相应的目录中。
...
接下来，你将创建一个 Azure 资源管理器模板，该模板授予你对之前创建的 Azure AD 资源组的 Azure AD B2C 访问权限（例如，azure-ad-b2c-monitor）。 使用“部署到 Azure”按钮从 GitHub 示例部署模板，这将打开 Azure 门户，并允许直接在门户中配置和部署模板。 对于这些步骤，请确保登录到 Azure AD 租户（而不是 Azure AD B2C 租户）。
...
5. 在“自定义部署”页面上，输入以下信息：

   | 字段   | 定义 |
   |---------|------------|
   | 订阅 |  选择包含 Azure 订阅的目录，其中创建了 azure-ad-b2c-monitor 资源组。 |
   | Region| 选择将部署资源的区域。  | 
   | Msp 套餐名称| 描述此定义的名称。 例如 Azure AD B2C 监视。  |
...
1. 确认是否选择了正确的目录和订阅。 在此示例中，已选择所有目录和所有订阅。
...
1. 在“监视”下，选择“诊断设置” 。
1. 如果资源上有现有的设置，则会看到已配置的设置列表。 如果要添加新设置，请选择“添加诊断设置”；如果要编辑现有设置，请选择“编辑”。 每个设置最多只能包含一个目标类型。
...
如果看到错误消息“若要为 Azure AD B2C 目录设置诊断设置以使用 Azure Monitor，需要设置委派资源管理”，请确保你的登录用户是[安全组](#32-select-a-security-group)成员，并[选择订阅](#4-select-your-subscription)。
...
1. 选择“库模板”。
1. 将“库模板”中的 JSON 替换为 [Azure AD B2C 基本工作簿](https://raw.githubusercontent.com/azure-ad-b2c/siem/master/workbooks/dashboard.json)中的内容：
1. 使用“应用”按钮应用该模板。
...
使用以下说明创建新的 Azure 警报，该警报将在请求总数与上一时段相比下降 25% 时发送[电子邮件通知](../azure-monitor/alerts/action-groups.md#configure-notifications)。 警报将每隔 5 分钟运行一次，并在过去 24 小时内查找下降情况。 警报是使用 Kusto 查询语言创建的。
