...
| RemoveMillisecondsFromDateTime| 否 | 指示是否将从 SAML 响应中的 datetime 值中删除 millisconds， (包括 IssueInstant、NotBefore、NotOnOrAfter 和 AuthnInstant) 。 可能的值：`false`（默认值）或 `true`。  |
