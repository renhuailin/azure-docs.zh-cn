...
1. 选择“新建”
    1. 输入 " **名称**" 和 " **显示名称**"。
    1. 输入 " **重定向 URI** " `https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com/oauth2/authresp` 。 将 `your-tenant-name` 替换为租户的名称。 
1. 单击“继续” 。
...
如果你希望用户使用 ID.me 帐户登录，则需要将该帐户定义为声明提供程序，Azure AD B2C 可通过终结点进行通信。 该终结点将提供一组声明，Azure AD B2C 使用这些声明来验证特定的用户是否已完成身份验证。
...
至此，已配置策略，以便 Azure AD B2C 知道如何与 ID.me 帐户通信。 请尝试上传该策略的扩展文件，这只是为了确认它到目前为止不会出现任何问题。
...
此时，标识提供者已设置，但不会出现在任何注册/登录屏幕中。 若要使其可用，请创建现有模板用户旅程的副本，然后对其进行修改，使其也具有 ID.me 标识提供者。
...
5. 重命名用户旅程的 ID。 例如，`SignUpSignInIdMe`。

### <a name="display-the-button"></a>显示按钮

**ClaimsProviderSelection** 元素类似于注册/登录屏幕上的标识提供者按钮。 如果为 ID.me 帐户添加 **claimsexchange** 元素，则当用户进入页面时，会显示一个新按钮。

1. 在创建的用户旅程中找到包含 `Order="1"` 的 **OrchestrationStep** 元素。
2. 在 **ClaimsProviderSelects** 下，添加以下元素。 将 **TargetClaimsExchangeId** 设置为适当的值，例如 `IdMeExchange`：
...
准备好按钮后，需将它链接到某个操作。 在此示例中，操作用于 Azure AD B2C 与 ID.me 帐户通信以接收令牌。
...
2. 添加以下 **ClaimsExchange** 元素，以确保为用于 **TargetClaimsExchangeId** 的 ID 使用相同的值：
    ...
    将 **TechnicalProfileReferenceId** 的值更新为之前创建的技术配置文件的 ID。 例如，`IdMe-OAUTH2`。
...
1. 在工作目录中创建 *SignUpOrSignIn.xml* 的副本并将其重命名。 例如，将其重命名为 *SignUpSignInIdMe.xml*。
1. 打开新文件，并将 **TrustFrameworkPolicy** 的 **PolicyId** 属性的值更新为唯一的值。 例如，`SignUpSignIdMe`。
1. 将 **PublicPolicyUri** 的值更新为策略的 URI。 例如 `http://contoso.com/B2C_1A_signup_signin_IdMe`
1. 更新 **DefaultUserJourney** 中的 **ReferenceId** 属性的值，使其与你 (SignUpSignIdMe) 创建的新用户旅程的 ID 匹配。
1. 保存更改，并上传文件。
1. 在“自定义策略”下，选择“B2C_1A_signup_signin” 。
1. 对于 " **选择应用程序**"，请选择前面注册的名为 *testapp1-template.json* 的 web 应用程序。 “回复 URL”应显示为 `https://jwt.ms`。
1. 选择 " **立即运行** "，然后选择 "ID.me" 以通过 ID.me 登录并测试自定义策略。
