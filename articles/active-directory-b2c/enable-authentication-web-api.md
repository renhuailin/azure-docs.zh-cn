...
description: 本文讨论如何使用 Azure Active Directory B2C 来保护 Web API。
...
ms.locfileid: "121723216"
---
# <a name="enable-authentication-in-your-own-web-api-by-using-azure-ad-b2c"></a>使用 Azure AD B2C 在自己的 Web API 中启用身份验证

若要授权访问 Web API，请仅为包含 Azure Active Directory B2C (Azure AD B2C) 颁发的有效访问令牌的请求提供服务。 本文介绍让 Azure AD B2C 授权访问 Web API 的过程。 完成本文中的步骤后，只有获取了有效访问令牌的用户才有权调用 Web API 终结点。  

## <a name="prerequisites"></a>先决条件

在开始之前，请阅读以下文章之一，其中讨论了如何为调用 Web API 的应用配置身份验证。 然后按照本文中的步骤将示例 Web API 替换为你自己的 Web API。   

- [在 ASP.NET Core 应用程序示例中配置身份验证](configure-authentication-sample-web-app-with-api.md)
- [在单页应用程序 (SPA) 示例中配置身份验证](configure-authentication-sample-spa-app.md)
...
1. 使用以下格式，在 HTTP 请求的身份验证头中将该访问令牌作为持有者令牌进行传递： 
...
- Web 应用程序、移动应用程序或 SPA 应用程序注册使你的应用可以通过 Azure AD B2C 登录。 此应用注册过程会生成应用程序 ID（也称为“客户端 ID”），可唯一标识你的应用程序（例如，应用 ID：1）  。

- Web API 注册使你的应用可以调用受保护的 Web API。 注册将公开 Web API 权限（范围）。 此应用注册过程会生成应用程序 ID，可唯一标识你的 Web API（例如，应用 ID：2） 。 向应用（应用 ID：1）授予对 Web API 范围（应用 ID：2）的权限。 

下图描述了应用程序注册和应用程序体系结构：

![使用 Web API 的应用的应用程序注册和应用程序体系结构的关系图。](./media/enable-authentication-web-api/app-with-api-architecture.png) 

## <a name="prepare-your-development-environment"></a>准备开发环境  

在接下来的步骤中，你将创建一个新的 Web API 项目。 选择编程语言：ASP.NET Core 或 Node.js。 确保有一台运行以下任一组件的计算机： 

# <a name="aspnet-core"></a>[ASP.NET Core](#tab/csharpclient)

* [Visual Studio Code](https://code.visualstudio.com/download)
* [用于 Visual Studio Code 的 C#](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp)（最新版本）
...
创建新的 Web API 项目。 首先，选择想要使用的编程语言：ASP.NET Core 或 Node.js 。
...
1. 在命令行界面中，运行 `npm init -y`。 此命令为 Node.js 项目创建默认的 package.json 文件。
...
在 Web API 项目中添加身份验证库。 身份验证库将分析 HTTP 身份验证头，验证令牌，并提取声明。 有关详细信息，请查看该库的文档。
...
找到 `ConfigureServices(IServiceCollection services)` 函数。 然后，在 `services.AddControllers();` 代码行之前添加以下代码片段：
...
找到 `Configure` 函数。 然后，在 `app.UseRouting();` 代码行之后添加以下代码片段：
...
- 匿名的 `/public` 终结点。 此终结点返回当前日期和时间。 使用此终结点通过匿名调用来调试 Web API。
...
`HelloController` 控制器使用 [AuthorizeAttribute](/aspnet/core/security/authorization/simple) 进行修饰，这会限制仅限经过身份验证的用户的访问。 
...
`/hello` 终结点首先调用 `passport.authenticate()` 函数。 该身份验证函数仅限已经过身份验证的用户的访问。 
...
将配置添加到配置文件。 该文件包含有关 Azure AD B2C 标识提供者的信息。 Web API 应用使用此信息来验证访问令牌，该访问令牌由 Web 应用当作持有者令牌传递。
...
|AzureAdB2C|ClientId| Web API 应用程序 ID。 在[上图](#app-registration-overview)中，它是使用“应用 ID：2”的应用程序。 若要了解如何获取 Web API 应用程序注册 ID，请参阅[先决条件](#prerequisites)。 |
|AzureAdB2C|SignUpSignInPolicyId|用户流或自定义策略。 若要了解如何获取用户流或策略，请参阅[先决条件](#prerequisites)。  |

# <a name="nodejs"></a>[Node.js](#tab/nodejsgeneric)

在项目根文件夹下，创建一个 config.json 文件，然后向其添加以下 JSON 代码片段：  
...
| 凭据 |clientID | Web API 应用程序 ID。 在[上图](#app-registration-overview)中，它是使用“应用 ID：2”的应用程序。 若要了解如何获取 Web API 应用程序注册 ID，请参阅[先决条件](#prerequisites)。 |
| 凭据 | 颁发者| 令牌颁发者 `iss` 声明值。 默认情况下，Azure AD B2C 会按以下格式返回令牌：`https://<your-tenant-name>.b2clogin.com/<your-tenant-ID>/v2.0/`。 将 `<your-tenant-name>` 替换为 Azure AD B2C [租户名称](tenant-management.md#get-your-tenant-name)的第一部分。 将 `<your-tenant-ID>` 替换为 [Azure AD B2C 租户 ID](tenant-management.md#get-your-tenant-id)。 |
...
最后，使用 Azure AD B2C 环境设置运行该 Web API。 

# <a name="aspnet-core"></a>[ASP.NET Core](#tab/csharpclient)

在命令行界面中，通过运行以下命令来启动 Web 应用：

```bush
 dotnet run
```

你应看到以下输出，这意味着你的应用已启动并正在运行，已准备好接收请求。

```
Now listening on: http://localhost:6000
```

若要停止该程序，请在命令行界面中选择 Ctrl+C。 可以使用 `node app.js` 命令重新运行该应用程序。

> [!TIP]
> 或者，若要运行 `dotnet run` 命令，可以使用 [Visual Studio Code 调试程序](https://code.visualstudio.com/docs/editor/debugging)。 Visual Studio Code 的内置调试程序有助于加速编辑、编译和调试循环。

打开浏览器并转到 `http://localhost:6000/public`。 在浏览器窗口中，应该会看到显示的以下文本，以及当前日期和时间。
...
## <a name="step-8-call-the-web-api-from-your-app"></a>步骤 8：从应用调用 Web API
