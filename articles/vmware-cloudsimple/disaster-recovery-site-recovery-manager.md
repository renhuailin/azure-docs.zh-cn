...
description: 介绍如何使用 VMware Site Recovery Manager 将 CloudSimple 私有云设置为灾难恢复站点。
author: Ajayan1008
ms.author: v-hborys
ms.date: 08/20/2019
ms.topic: article
...
你可以使用 CloudSimple 私有云作为本地 VMware 工作负荷 (DR) 站点的灾难恢复。
...
* 从你的私有云中 Active Directory (AD) 识别本地的用户。
* 在私有云中为所有用户设置 AD。
* 使用 Azure AD。
...
按照 VMware 文档，在本地环境中安装 vSphere 复制设备 (vRA) 。 安装包括以下高级步骤：
...
2. 在私有云中创建一个用户，用于 vSphere 复制和 SRM 安装。 下面的 [VCENTER UI：在私有 Cloud For vRA 中创建用户 & SRM 安装](#vcenter-ui-create-a-user-in-private-cloud-for-vra-and-srm-installation)。
3. 准备私有云环境以进行 vRA 安装。
...
#### <a name="vcenter-ui-create-a-user-in-private-cloud-for-vra-and-srm-installation"></a>vCenter UI：在私有云中创建 vRA 和 SRM 安装的用户
...
使用 soln-管理员凭据将日志记录到 vCenter 后，请遵循 VMware 文档，按照此 [vmware 文档](https://docs.vmware.com/en/Site-Recovery-Manager/6.5/com.vmware.srm.install_config.doc/GUID-F474543A-88C5-4030-BB86-F7CC51DADE22.html)中所述，在部署模型 "包含一个每个平台服务控制器的 vCenter 实例的双站点拓扑" 中执行 srm 服务器安装。 VMware 6.5 安装说明适用于 VMware 文档 [安装 Site Recovery Manager](https://docs.vmware.com/en/Site-Recovery-Manager/6.5/com.vmware.srm.install_config.doc/GUID-437E1B65-A17B-4B4B-BA5B-C667C90FA418.html)。
...
## <a name="multiple-replication-configuration"></a>多个复制配置
 ...
 [基于阵列的复制和 vSphere 复制技术可以同时与 SRM 一起使用](https://blogs.vmware.com/virtualblocks/2017/06/22/srm-array-based-replication-vs-vsphere-replication) 。 但是，必须将它们应用到不同的 Vm 集 (可以通过基于阵列的复制或 vSphere 复制来保护给定 VM，但不能同时使用这两) 种方法。 此外，CloudSimple 站点可以配置为多个受保护站点的恢复站点。 有关多站点配置的详细信息，请参阅 [SRM 多站点选项](https://blogs.vmware.com/virtualblocks/2016/07/28/srm-multisite/) 。
