...
接下来，通过 [az deployment group create](/cli/azure/deployment/group)部署模板 pf freebsd-设置。 在相同的路径下面下载 azuredeploy.parameters.json，然后定义你自己的资源值（如 `adminPassword`、`networkPrefix` 和 `domainNamePrefix`）。 
