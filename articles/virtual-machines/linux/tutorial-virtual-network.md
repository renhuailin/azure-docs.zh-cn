...
在创建虚拟网络之前，请先使用 [az group create](/cli/azure/group) 创建一个资源组。 以下示例在 eastus 位置创建名为 *myRGNetwork* 的资源组。
...
使用 [az network vnet create](/cli/azure/network/vnet) 命令创建虚拟网络。 在本例中，网络被命名为 *mvVNet* 并且为其分配了地址前缀 *10.0.0.0/16*。 另外，还将创建名为 *myFrontendSubnet*、前缀为 *10.0.1.0/24* 的子网。 本教程稍后会将前端 VM 连接到此子网。 
...
使用 [az network vnet subnet create](/cli/azure/network/vnet/subnet) 命令将新子网添加到虚拟网络。 在本例中，子网被命名为 *myBackendSubnet* 并且为其分配了地址前缀 *10.0.2.0/24*。 此子网用于所有后端服务。
...
使用 [az vm create](/cli/azure/vm) 命令创建 VM 时，默认的公共 IP 地址分配方法为动态。 使用 [az vm create](/cli/azure/vm) 命令创建虚拟机时，包含 `--public-ip-address-allocation static` 参数可以分配静态公共 IP 地址。 本教程不会演示此操作，但是，下一部分介绍了如何将动态分配的 IP 地址更改为静态分配的地址。 

### <a name="change-allocation-method"></a>更改分配方法

可以使用 [az network public-ip update](/cli/azure/network/public-ip) 命令更改 IP 地址分配方法。 在本示例中，前端 VM 的 IP 地址分配方法已更改为静态。
...
使用 [az network public-ip update](/cli/azure/network/public-ip) 命令更新分配方法。 在本例中，`--allocation-method` 将设置为 *static*。
...
使用 [az vm create](/cli/azure/vm) 命令创建名为 *myFrontendVM* 且使用 *myPublicIPAddress* 的 VM。
...
可以在创建 VM 时使用 [az vm create](/cli/azure/vm) 命令创建网络安全组。 执行此操作时，会将 NSG 与 VM 网络接口相关联，并自动创建一个 NSG 规则来允许任何源发到 *22* 端口上的流量。 在本教程的前面部分中，已在前端 VM 上自动创建了前端 NSG。 也会为端口 22 自动创建 NSG 规则。 

在某些情况下，预先创建 NSG 可能会有帮助，例如，在不应创建默认 SSH 规则时，或者应该将 NSG 附加到子网时。 

使用 [az network nsg create](/cli/azure/network/nsg) 命令创建网络安全组。
...
使用 [az network nsg rule create](/cli/azure/network/nsg/rule) 命令为端口 *80* 创建规则。
...
使用 [az network nsg rule create](/cli/azure/network/nsg/rule) 命令为端口 22 创建规则。 请注意，`--source-address-prefix` 参数指定了值 *10.0.1.0/24*。 此配置确保只允许来自前端子网的流量通过 NSG。
