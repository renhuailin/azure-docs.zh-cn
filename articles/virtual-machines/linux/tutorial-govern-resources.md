...
创建新组或找到现有组以后，请使用 [az role assignment create](/cli/azure/policy/assignment#az_policy_assignment_create) 命令将新的 Azure Active Directory 组分配到资源组的“虚拟机参与者”角色。
...
[Azure Policy](../../governance/policy/overview.md) 可帮助确保订阅中的所有资源符合企业标准。 订阅已经有多个策略定义。 若要查看可用的策略定义，请使用 [az policy definition list](/cli/azure/policy/definition#az_policy_definition_list) 命令：
...
若要锁定虚拟机和网络安全组，请使用 [az lock create](/cli/azure/resource/lock#az_resource_lock_create) 命令：
...
若要将标记应用于虚拟机，请使用 [az resource tag](/cli/azure/resource#az_resource_list) 命令。 资源上的任何现有标记都不会保留。
...
在解除锁定之前，不能删除锁定的网络安全组。 若要删除锁，请检索锁的 ID，并将其提供给 [az lock delete](/cli/azure/resource/lock#az_resource_lock_delete) 命令：
