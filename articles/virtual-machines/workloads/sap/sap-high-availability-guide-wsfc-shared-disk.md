...
- 可用性集和可用性区域中的 SAP 部署支持具有[高级 SSD](../../disks-types.md#premium-ssd) 磁盘的 [Azure 共享磁盘](../../disks-shared.md)。
- 不支持将 [Azure 超级磁盘](../../disks-types.md#ultra-disk)和 [Azure 标准磁盘](../../disks-types.md#standard-ssd)用作 SAP 工作负载的 Azure 共享磁盘。
- 请确保使用[高级 SSD 范围](../../disks-shared.md#disk-sizes)中指定的最小磁盘大小预配 Azure 高级磁盘，以便能够同时附加到所需数量的 VM（SAP ASCS Windows 故障转移群集通常为 2 个）。
 
SIOS 
- SIOS 解决方案在两个磁盘之间提供实时同步数据复制。
- 使用 SIOS 解决方案可以使用两个托管磁盘进行操作，而如果使用可用性集或可用性区域，托管磁盘将落在不同的存储群集上。 
- 支持在可用性区域中部署
...
-  具有高级 SSD 磁盘的 Azure 共享磁盘附带两个存储 SKU。
   - Azure 可用性集中的部署支持高级共享磁盘 (skuName - Premium_LRS) 的本地冗余存储 (LRS)。
   - Azure 可用性区域中的部署支持高级共享磁盘 (skuName - Premium_ZRS) 的区域冗余存储 (ZRS)。
...
以下是有关 Azure 高级共享磁盘的一些重要注意事项：

- 高级共享磁盘的 LRS
  - 具有高级共享磁盘的 LRS 的 SAP 部署将使用一个存储群集上的单个 Azure 共享磁盘来运行。 如果部署 Azure 共享磁盘的存储群集出现问题，SAP ASCS/SCS 实例将受到影响。

- 高级共享磁盘的 ZRS
  - 由于需要跨区域复制数据，ZRS 的写入延迟高于 LRS。
  - 不同区域中的可用性区域之间的距离不同，因此可用性区域之间的 ZRS 磁盘延迟也不相同。 请[对磁盘进行基准测试](../../disks-benchmarks.md)，以确定你所在区域的 ZRS 磁盘延迟。
  - 高级共享磁盘的 ZRS 在区域中的三个可用性区域之间同步复制数据。 如果其中一个存储群集出现任何问题，SAP ASCS/SCS 将继续运行，因为存储故障转移对于应用程序层为透明。
  - 有关更多详细信息，请参阅托管磁盘的 ZRS 的[限制](../../disks-redundancy.md#limitations)部分。
...
同时支持 Windows Server 2016 和 2019（使用最新的数据中心映像）。
