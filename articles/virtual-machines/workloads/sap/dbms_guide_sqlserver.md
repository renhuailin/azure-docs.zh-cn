...
> [!IMPORTANT]
> 本文档的范围涵盖 SQL Server 上的 Windows 版本。 SAP 不支持带有任何 SAP 软件的 Linux 版 SQL Server。 本文档不讨论 Microsoft Azure Platform 的“平台即服务”产品 - Microsoft Azure SQL 数据库。 本文讨论的是如何运行 SQL Server 产品（已知适用于 Azure 虚拟机中的本地部署），以及如何运用 Azure 的“基础结构即服务”功能。 这两种产品/服务的数据库性能与功能差异很大，不应混用。 另请参阅：<https://azure.microsoft.com/services/sql-database/>
> 
>
...
根据常规说明，操作系统、SQL Server 可执行文件和 SAP 2 层系统的 SAP 可执行文件应位于或安装在单独的 Azure 磁盘中。 通常情况下，SAP NetWeaver 工作负荷对大部分 SQL Server 系统数据库的使用率都不会很高。 尽管如此，SQL Server 的系统数据库（master、msdb 和 model）应与其他 SQL Server 目录一起放在单独的 Azure 磁盘上。 SQL Server tempdb 应位于 nonperisisted D:\ 驱动器或单独的磁盘上。


* 除 A 系列 VM、tempdb 数据、日志文件外，所有 SAP 认证的 VM 类型（请参阅 SAP 说明 [1928533]）都可放置在非持久性驱动器 D:\ 上。 
* 对于较旧的 SQL Server 版本，其中 SQL Server 默认安装了包含一个数据文件的 tempdb，建议使用多个 tempdb 数据文件。 请注意，D:\ 驱动器卷因 VM 类型而异。 有关不同 VM 的驱动器 D:\ 精确大小的信息，请参阅 [Azure 中 Windows 虚拟机的大小](../../sizes.md)。

这些配置使 tempdb 能够占用更多空间，更重要的是，占用比系统驱动器能够提供的更多的 IOPS 和存储带宽。 非持久性驱动器 D:\ 还提供更大的 I/O 延迟和吞吐量（A 系列 VM 除外）。 若要确定正确的 tempdb 大小，可以在现有系统上检查 tempdb 大小。 
...
- 单个 Azure 页 blob，这将备份大小限制为 1000 GB。 此限制也限制了可以实现的吞吐量。
...
[将 SQL Server 数据库备份到 Azure](../../../backup/backup-azure-sql-database.md) 页中提供对此备份方法的详细描述，该方法增加了集中备份配置、监视和管理的诸多优势。
...
### <a name="sql-server-clustering-using-windows-scale-out-file-server-or-azure-shared-disk"></a>使用 Windows 横向扩展文件服务器或 Azure 共享磁盘的 SQL Server 群集
...
- 不能定义用于将数据加密应用于数据库的线程数。 线程数主要取决于 SQL Server 数据和日志文件分布的磁盘卷数。 意味着卷（驱动器号）越不同，将并行执行加密的线程越多。 此类配置与之前的磁盘配置建议有点矛盾，该建议是在 Azure VM 中为 SQL Server 数据库文件构建一个或少量存储空间。 具有少量卷的配置将导致执行加密的线程数较少。 单个线程加密正在读取 64 KB 的盘区、对其进行加密，然后将记录写入事务日志文件，同时告知已加密盘区。 因此，事务日志上的负载适中。
