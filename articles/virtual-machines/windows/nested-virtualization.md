...
为主机创建新的 Windows Server 2016 或 Windows Server 2019 Azure VM。 确保为 VM 选择支持嵌套的大小，并且大小足以满足来宾 VM 的需求。 有关支持嵌套的虚拟机大小的列表，请参阅 [Azure 计算单位](../acu.md)一文。

可以在[可用产品(按区域)](https://azure.microsoft.com/regions/services/) 页上查看 VM 大小的区域可用性。
...
有关如何在来宾 VM 和 Azure VM 之间启用透明连接的说明，请参阅[此文档](/virtualization/hyper-v-on-windows/user-guide/nested-virtualization)。
