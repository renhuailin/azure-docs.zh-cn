...
收到设备后，可使用本地 Web UI 进行快速设置。 将数据从服务器复制到设备或从设备复制到服务器，然后将设备寄回 Azure。 就导入订单而言，在 Azure 数据中心内，数据自动从设备上传到 Azure。 通过 Azure 门户中的 Data Box 服务对整个过程进行端到端跟踪。

### <a name="q-when-should-i-use-data-box"></a>Q. 何时应使用 Data Box？
A. 如果需要将 40 - 500 TB 数据传入或传出 Azure，使用 Data Box 会很有利。 对于 < 40 TB 的数据大小，请使用 Data Box Disk，对于 > 500 TB 的数据大小，请注册 [Data Box Heavy](data-box-heavy-overview.md)。

### <a name="q-what-is-the-price-of-data-box"></a>Q. Data Box 的价格是多少？
A. 使用 Data Box 10 天会产生少许费用。 在 Azure 门户中创建订单并选择产品型号时，将显示设备的费用。 还有标准寄送费和 Azure 存储费用。 导出订单的定价模型与导入订单类似，但可能会有额外的流出量费用。 

有关详细信息，请转到 [Azure Data Box 定价](https://azure.microsoft.com/pricing/details/storage/databox/)和[流出量费用](https://azure.microsoft.com/pricing/details/bandwidth/)。 
...
### <a name="q-how-can-i-import-source-data-at-my-location-in-a-particular-country-to-an-azure-region-in-a-different-countryregion-or-export-data-from-an-azure-region-in-one-country-to-a-different-countryregion"></a>Q. 如何将自己位于特定国家的源数据导入到位于不同国家/地区的 Azure 区域，或将数据从一个国家的 Azure 区域导出到不同国家/地区？

Data Box 仅支持一个国家/地区（与目标相同）范围内的数据引入或引出，不跨越任何国际边界。 唯一的例外是在欧盟 (EU) 的订单，Data Box 可以在任何 EU 国家/地区之间寄送。

例如，在导入场景中，如果希望将位于加拿大的源数据移动到 Azure 美国西部存储帐户，可通过以下方式实现此目的：

1. 通过选择位于加拿大的存储帐户，在加拿大订购 Data Box。 该设备从位于加拿大的一个 Azure 数据中心寄送到创建订单时提供的送货地址（位于加拿大）。

2. 本地数据复制到 Data Box 后，将设备退回到加拿大的 Azure 数据中心。 然后，Data Box 上的数据会被上传到创建订单时选择的加拿大 Azure 区域中的目标存储帐户。

3. 接下来，你可以使用 AzCopy 之类的工具将数据复制到美国西部的存储帐户。 此步骤会产生 Data Box 计费中未包含的[标准存储](https://azure.microsoft.com/pricing/details/storage/)和[带宽费用](https://azure.microsoft.com/pricing/details/bandwidth/)。

#### <a name="q-does-data-box-store-any-customer-data-outside-of-the-service-region"></a>Q. Data Box 会在服务区域外存储任何客户数据吗？

A. 不是。 Data Box 不会在服务区域外存储任何客户数据。 客户对数据享有完全所有权，可以根据在创建订单时所选的存储帐户将数据保存到指定位置。  

除客户数据外，还有 Data Box 数据，包括与设备相关的安全项目、设备和服务的监视日志以及与服务相关的元数据。 在除巴西南部和东南亚以外的所有区域中，Data Box 数据都是通过异地冗余存储帐户在配对区域中进行存储和复制的，以防止数据丢失。  

考虑到巴西南部和东南亚的[数据驻留要求](https://azure.microsoft.com/global-infrastructure/data-residency/#more-information)，Data Box 数据存储在区域冗余存储 (ZRS) 帐户中，从而包含在单个区域中。 东南亚的所有 Data Box 数据存储在新加坡，巴西南部的数据存储在巴西。 

如果巴西南部和东南亚发生服务中断，客户可以从其他区域创建新订单。 将从创建新订单的区域提供新订单，客户负责 Data Box 设备的来回运输。

### <a name="q-how-can-i-recover-my-data-if-an-entire-region-fails"></a>Q. 如果整个区域发生故障，我如何恢复数据？

A. 在由于重大灾难而导致区域丢失的极端情况下，Microsoft 可能会启动区域故障转移。 在这种情况下，你不需要执行任何操作。 如果故障转移区域位于同一个国家/地区或商务区域内，会通过该区域履行订单。 但是，一些 Azure 区域在同一地理区域或商务区域内没有配对的区域。 如果其中任何一个区域发生灾难，你需要再次从可用的其他区域创建 Data Box 订单，并将数据复制到位于新区域中的 Azure。 有关详细信息，请参阅[业务连续性和灾难恢复 (BCDR)：Azure 配对区域](../best-practices-availability-paired-regions.md)。

### <a name="q-who-should-i-contact-if-i-come-across-any-issues-with-data-box"></a>Q. 如果遇到 Data Box 方面的任何问题，我应该与谁联系？
A. 如果有 Data Box 方面的任何问题，请[联系 Microsoft 支持部门](data-box-disk-contact-microsoft-support.md)。

### <a name="q-i-lost-my-data-box-is-there-a-lost-device-charge"></a>Q. 我丢失了 Data Box。 丢失的设备是否也要付费？
A. 是的。 设备丢失或损坏会产生费用。 [定价页](https://azure.microsoft.com/pricing/details/storage/databox/)和[产品服务条款](https://www.microsoft.com/licensing/product-licensing/products)中提到了这笔费用。
...
### <a name="q-i-couldnt-create-a-data-box-order-in-the-azure-portal-why"></a>Q. 我无法在 Azure 门户中创建 Data Box 订单。 为什么？
...
1. 订购 Data Box：几分钟（从门户）
2. 设备分配和准备：1-2 个工作日，取决于库存情况和其他待履行订单
3. 寄送：2-3 个工作日
4. 客户站点上的数据复制：取决于数据的性质、大小和文件数
5. 退货：2-3 个工作日
6. 在数据中心处理设备：1-2 个工作日，视其他待履行订单而定
7. 将数据上传到 Azure：处理完成且连接设备后，立即开始。 上传时间取决于数据的性质、大小和文件数。

### <a name="q-i-ordered-a-couple-of-data-box-devices-i-cant-create-any-additional-orders-why"></a>Q. 我订购了多台 Data Box 设备。 无法再创建任何订单。 为什么？
A. 每个订阅在每个商务区域（所选的国家和地区的组合）最多允许有五个有效订单。 如果需要订购更多设备，请联系 Microsoft 支持部门提高订阅限制。
...
A. Data Box 服务在所选的国家和地区组合中不可用。 更改此组合可能会允许使用 Data Box 服务。 有关该服务适用区域的列表，请转到 [Data Box 的适用区域](data-box-overview.md#region-availability)。
...
### <a name="q-i-have-filled-up-my-data-box-with-data-and-need-to-order-another-one-is-there-a-way-to-quickly-place-the-order"></a>Q. Data Box 已填满数据，需要再订购一台。 是否有某种方法可让我快速下单？
...
A. Data Box 前面的电源按钮下有两个 LED 灯。 最下面的灯是系统故障指示灯，用于指示系统是否正常。
...
A. 如果无法在 Azure 门户中访问解锁密码，请检查订阅和存储帐户的权限。 请确保具有资源组级别的参与者或所有者权限。 至少需要 Data Box 操作员角色权限才能查看访问凭据。
...
A.  可以在 Azure 门户中跟踪 Data Box 的订单状态。 创建订单时，系统会提示你提供通知电子邮件。 如果已提供，则在订单状态发生任何变化时，你都会收到电子邮件通知。 详细了解如何[配置通知电子邮件](data-box-portal-ui-admin.md#edit-notification-details)。

### <a name="q-how-do-i-return-the-device"></a>Q. 如何退回设备？ 
A.  Microsoft 将在电子墨水显示屏上显示发货标签。 如果发货标签未显示在电子墨水显示屏上，请转到“概述”>“下载发货标签”。 下载并打印标签，将标签插入设备上的透明塑料标签中，然后将设备放在运输承运商位置。 
...
A.  如果使用的是 SMB 客户端，你可以使用 `Robocopy`、`Diskboss` 等 SMB 复制工具，甚至使用 Windows 文件资源管理器的拖放操作，将数据复制到设备上。 
...
### <a name="q-my-device-was-delivered-but-the-device-seems-to-be-damaged-what-should-i-do"></a>Q. 我的设备已送达，但似乎已损坏。 我该怎么办？
A. 如果设备到达时已损坏或存在被篡改的证据，请不要使用该设备。 请[联系 Microsoft 支持部门](data-box-disk-contact-microsoft-support.md)并尽快退回设备。 还可以新建针对更换设备的 Data Box 订单。 在这种情况下，更换设备不收费。
...
A. 是的。 Microsoft 还提供自管理寄送选择。 在下 Data Box 订单时，可以选择“自我管理的发货”选项。 有关详细信息，请参阅[Data Box 自管理寄送](data-box-portal-customer-managed-shipping.md)。
...
A. 我们设法让你尽快收到 Data Box 设备。 我们优先从离你的存储帐户位置最近的数据中心发货，但会从任何有可用库存的 Azure 数据中心发运设备。 你的 Data Box 应退回到发货标签中显示的发货地点。
...
### <a name="q-i-have-finished-prepare-to-ship-for-my-import-order-and-shut-down-the-device-can-i-still-add-more-data-to-the-data-box"></a>Q. 我已完成导入订单的“准备交付”，并关闭了设备。 是否仍能向 Data Box 添加更多数据？
...
A. 如果设备无法启动，请转到 Azure 门户中的订单。 下载发货标签，并将其贴在设备上。 有关详细信息，请转到[下载发货标签](data-box-portal-admin.md#download-shipping-label)。
...
A.  将数据复制到 Data Box 中时，数据将上传到 Azure 存储帐户中的以下某个路径，具体取决于数据是块 Blob、页 Blob 还是 Azure 文件：
...
A.  如果容器名称包含大写字母，这些名称会自动转换为小写。 如果名称存在其他方面的不合规情况（包含特殊字符、采用其他语言，等等），则上传将会失败。 有关命名共享、容器和文件的更多指南，请转到：
...
### <a name="q-what-happens-to-my-data-after-i-return-the-data-box"></a>Q. 退回 Data Box 后，我的数据会发生什么情况？
...
A.  不是。 如果你选择了 Microsoft 管理的发货，则无法传输数据。 Azure 数据中心目前不接受客户或 UPS 以外的承运人发送 Data Box。
