...
克隆操作在某些情况下很有用。 例如，你已使用 Data Box 传输一些数据。 随着生成的数据越来越多，你需要使用另一个 Data Box 设备将这些数据传输到 Azure 中。 在这种情况下，只需克隆相同的顺序即可。
...
如果 Data Box 的电子墨迹显示工作不正常，则可能需要下载发货标签，并且不显示 "退货发货" 标签。 Data Box Heavy 上没有电子墨迹显示，因此此工作流不会应用于 Data Box Heavy。
...
你可能需要更改接收订单状态电子邮件的用户。 例如，当设备已妥投或提货时，需要告知某个用户。 完成数据复制时可能需要告知另一个用户，使该用户在从源中删除数据之前，可以验证数据是否在 Azure 存储帐户中。 在这种情况下，可以编辑通知详细信息。
...
2. 单击“下载订单历史记录”。  下载的历史记录包含一条电信跟踪日志记录。 Data Box Heavy 设备上有两个与两个节点对应的日志集。 如果向下滚动到该日志的底部，则可看到以下内容的链接：
    
   - **复制日志** -将数据从 Data Box 复制到 Azure 存储帐户的文件列表从复制到 Azure 存储帐户 (将订单) 或从存储帐户复制到 Data Box (导出顺序) 。
   - **审核日志** -包含有关如何打开 Data Box 的信息，以及在 Data Box 在 Azure 数据中心外时访问它的共享。
...
|订单状态 |描述 |
|---------|---------|
|已订购     | 已成功下单。 <br>如果设备有货，Microsoft 会确定要发货的设备，并准备设备。 <br> 如果设备不可用，则会在设备可用时处理此订单。 订单可能需要花费几天到几个月的时间来进行处理。 如果订单无法在90天内完成，则会取消订单，并收到通知。         |
...
|已取消            |订单已取消。 <br> 你取消了订单，或者服务在发生错误后取消了订单。 如果订单无法在90天内完成，则该订单也将取消，并收到通知。     |
...
|复制已完成，但出现错误| 数据复制已完成，但在复制期间发生错误。 <br> 使用 Azure 门户中提供的路径查看 Azure 存储帐户中的复制日志。 请参阅 [下载完成但出现错误时复制日志的示例](./data-box-logs.md#upload-completed-with-errors)。   |
|复制已完成，但出现警告| Azure 存储帐户中的数据复制已完成，但数据发生非严重错误。 <br> 请使用 Azure 门户中提供的路径查看复制日志。 记下 "非严重" 错误。 请参阅 [下载完成时复制日志的示例，但会出现警告](./data-box-logs.md#upload-completed-with-warnings)。   |
|复制失败并出现错误| 从 Azure 存储帐户复制数据失败，订单已终止。 不会发送设备。 <br> 使用 Azure 门户中提供的路径查看 Azure 存储帐户中的复制日志。 请参阅 [下载失败并出现错误时的复制日志示例](./data-box-logs.md#upload-completed-with-errors)。   |
|已发运     |订单已发货。 可以使用门户中你的订单上显示的跟踪 ID 来跟踪货物。        |
|已交货     |货物已交付到订单中指定的地址。        |
|已提货     |承运人已提取并扫描了你的回寄设备。         |
...
如果你使用的是自管理的装运，则在复制完成后并且收到设备之前，你会看到以下状态 (，而不是上表中提到的状态) ：
