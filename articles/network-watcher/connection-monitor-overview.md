...
1. 在左侧的 " **监视** " 部分中，选择 " **连接监视器**"。
1. 你会看到在连接监视器中创建的所有连接监视器。 若要查看在经典连接监视器体验中创建的连接监视器，请转到“连接监视器”选项卡。
    
  :::image type="content" source="./media/connection-monitor-2-preview/cm-resource-view.png" alt-text="显示连接监视器中创建的连接监视器的屏幕截图" lightbox="./media/connection-monitor-2-preview/cm-resource-view.png":::
...
根据你在测试配置中选择的协议，连接监视器将为源-目标对运行一系列检查。 根据所选测试频率运行检查。
...
* **警告** – 
     * 如果指定了阈值，并且连接监视器观察到失败百分比超过80% 的阈值，则测试将被标记为 "警告"。
...
* **顶级筛选器** –按文本、实体类型 (连接监视器、测试组或测试) 时间戳和范围搜索列表。 范围包括订阅、区域、源和目标类型。 请查看下图中的框 1。
* **基于状态的筛选** 器–按连接监视器、测试组或测试的状态进行筛选。 请查看下列图像中的框 2。
* **基于警报的筛选器** - 按连接监视器资源上触发的警报进行筛选。 请查看下图中的框 3。

  :::image type="content" source="./media/connection-monitor-2-preview/cm-view.png" alt-text="显示如何在连接监视器中筛选连接监视器、测试组和测试的视图的屏幕截图 " lightbox="./media/connection-monitor-2-preview/cm-view.png":::
    
例如，若要查看连接监视器中源 IP 为10.192.64.56 的所有测试：
1. 将视图更改为“测试”。
1. 在搜索字段中键入“10.192.64.56”
1. 在顶层筛选器的“范围”中，选择“源” 。

仅显示在源 IP 为10.192.64.56 的连接监视器中失败的测试：
1. 将视图更改为“测试”。
1. 对于基于状态的筛选器，请选择“失败”。
1. 在搜索字段中键入“10.192.64.56”
1. 在顶层筛选器的“范围”中，选择“源” 。
...
在连接监视器体验之前创建的连接监视器中，所有四个指标都可用：% 探测失败、AverageRoundtripMs、ChecksFailedPercent (预览) 和 RoundTripTimeMs (预览) 。 在连接监视器体验中创建的连接监视器中，数据仅适用于标记有 *(Preview)* 的指标。

  :::image type="content" source="./media/connection-monitor-2-preview/monitor-metrics.png" alt-text="显示连接监视器中指标的屏幕截图" lightbox="./media/connection-monitor-2-preview/monitor-metrics.png":::
...
1. 从连接监视器中，[使用 Azure 门户](connection-monitor-preview-create-using-portal.md#)创建连接监视器 
1. 从连接监视器中，在仪表板中使用 "配置警报" 
...
连接监视器有助于诊断连接监视器和网络中的问题。 混合网络中的问题通过你之前安装的 Log Analytics 代理进行检测。 Azure 中的问题由网络观察程序扩展进行检测。 
