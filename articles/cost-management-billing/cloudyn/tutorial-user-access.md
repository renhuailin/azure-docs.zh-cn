...
当你注册了 Azure 协议或帐户时，拥有管理员权限的帐户会在 Cloudyn 中进行创建，以便你可以执行本教程中的所有步骤。 本教程介绍了对 Cloudyn 数据的访问，包括用户管理和实体管理。 您将学习如何：
...
如果希望从 Cloudyn 中删除或导出个人数据，需要创建支持票证。 在创建支持票证后，它将像普通请求（数据主体请求）一样执行。 然后，Microsoft 将采取提示操作来删除该帐户并删除任何客户或个人数据。
