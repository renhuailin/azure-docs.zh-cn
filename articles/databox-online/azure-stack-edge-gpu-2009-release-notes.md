...
本文适用于映射到软件版本号 **2.1.1377.2170** 的 **Azure Stack Edge Pro 2010** 版本。
...
|**4.**|遏制|在限制期间，如果不允许新写入设备，NFS 客户端完成的写入会失败并出现 "权限被拒绝" 错误。| 错误如下所示：<br>`hcsuser@ubuntu-vm:~/nfstest$ mkdir test`<br>mkdir：无法创建目录 "test"：权限被拒绝|
