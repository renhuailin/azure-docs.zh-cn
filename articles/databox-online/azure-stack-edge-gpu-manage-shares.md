...
若要将数据传输到 Azure，需在 Azure Stack Edge Pro 上创建共享。 在 Azure Stack Edge Pro 设备上添加的共享可以是本地共享，也可以是用于将数据推送到云的共享。
...
1. 在 Azure 门户中，转到 Azure Stack Edge 资源，然后转到“云存储网关”>“共享”。 在命令栏上选择“+ 添加共享”。
    ...
    ![选择“添加共享”](media/azure-stack-edge-gpu-manage-shares/add-share-1.png)
...
6. 此步骤取决于你是创建 SMB 共享还是 NFS 共享。
    - **如果创建 SMB 共享** - 在“所有特权本地用户”字段中，选择“新建”或“使用现有”。 如果创建新的本地用户，请提供 **用户名**、**密码** 和确认密码。 这样就会为本地用户分配权限。 在此处分配权限以后，即可使用文件资源管理器修改这些权限。
        ...
        ![添加 SMB 共享](media/azure-stack-edge-gpu-manage-shares/add-smb-share.png)

        如果针对此共享数据勾选“仅允许读取操作”，则可以指定只读用户。
    - **如果创建 NFS 共享** - 需要提供能够访问共享的 **受允许客户端的 IP 地址**。

        ![添加 NFS 共享](media/azure-stack-edge-gpu-manage-shares/add-nfs-share.png)
...
1. 在 Azure 门户中，转到 Azure Stack Edge 资源，然后转到“云存储网关”>“共享”。 在命令栏上选择“+ 添加共享”。
    ...
    ![选择“添加共享 2”](media/azure-stack-edge-gpu-manage-shares/add-local-share-1.png)

2. 在“添加共享”中指定共享设置。 提供共享的唯一名称。
    
    共享名称只能包含数字、大小写字母和连字符。 共享名称必须为 3 到 63 个字符长，并且必须以字母或数字开头。 每个连字符的前后必须为非连字符字符。
...
7. 选择“创建”。 
    ...
    ![查看更新的“共享”边栏选项卡](media/azure-stack-edge-gpu-manage-shares/add-local-share-3.png)
    
    选择该共享，以查看其 Edge 计算模块的本地装入点。

    ![查看本地共享详细信息](media/azure-stack-edge-gpu-manage-shares/add-local-share-4.png)
...
1. 在 Azure 门户中，转到 Azure Stack Edge 资源，然后转到“云存储网关”>“共享”。 从共享列表中选择要装载的共享。 “用于计算”列会将所选共享的状态显示为“已禁用”。
    ...
    ![选择共享](media/azure-stack-edge-gpu-manage-shares/mount-share-1.png)

2. 选择“装载”。

    ![选择装载](media/azure-stack-edge-gpu-manage-shares/mount-share-2.png)

3. 出现确认提示时，选择“是”。 此时会装载共享。

    ![确认装载](media/azure-stack-edge-gpu-manage-shares/mount-share-3.png)

4. 装载共享以后，转到共享列表。 此时会看到“用于计算”列将共享状态显示为“已启用”。

    ![装载的共享](media/azure-stack-edge-gpu-manage-shares/mount-share-4.png)

5. 再次选择共享，查看该共享的本地装入点。 Edge 计算模块使用共享的这个本地装入点。

    ![共享的本地装入点](media/azure-stack-edge-gpu-manage-shares/mount-share-5.png)

## <a name="unmount-a-share"></a>卸载共享

在 Azure 门户中执行以下步骤，以卸载共享。

1. 在 Azure 门户中，转到 Azure Stack Edge 资源，然后转到“云存储网关”>“共享”。 从共享列表中选择要卸载的共享。 需确保卸载的共享不被任何模块使用。 如果共享被某个模块使用，则相应模块会出问题。

    ![选择共享 2](media/azure-stack-edge-gpu-manage-shares/unmount-share-1.png)

2.  选择“卸载”。

    ![选择卸载](media/azure-stack-edge-gpu-manage-shares/unmount-share-2.png)

3. 出现确认提示时，选择“是”。 此时会卸载共享。

    ![确认卸载](media/azure-stack-edge-gpu-manage-shares/unmount-share-3.png)

4. 卸载共享以后，转到共享列表。 此时会看到“用于计算”列将共享状态显示为“已禁用”。

    ![卸载的共享](media/azure-stack-edge-gpu-manage-shares/unmount-share-4.png)
...
1. 在共享列表中，选择并单击要删除的共享。
    ...
    ![选择共享 3](media/azure-stack-edge-gpu-manage-shares/delete-share-1.png)

2. 单击 **“删除”** 。

    ![单击“删除”](media/azure-stack-edge-gpu-manage-shares/delete-share-2.png)

3. 当系统提示你进行确认时，单击 **“是”**。

    ![确认删除](media/azure-stack-edge-gpu-manage-shares/delete-share-3.png)
...
1.  在 Azure 门户中转到“共享”。 选择并单击要刷新的共享。
    ...
    ![选择共享 4](media/azure-stack-edge-gpu-manage-shares/refresh-share-1.png)

2.  单击“刷新”。 

    ![单击“刷新”](media/azure-stack-edge-gpu-manage-shares/refresh-share-2.png)
 
3.  当系统提示你进行确认时，单击 **“是”**。 此时会启动一个作业来刷新本地共享的内容。

    ![确认刷新](media/azure-stack-edge-gpu-manage-shares/refresh-share-3.png)
...
![更新的时间戳](media/azure-stack-edge-gpu-manage-shares/refresh-share-4.png)
 
失败时会引发警报。 警报中详细描述了原因，以及问题的建议解决方法。 警报中还会提供某个文件的链接，该文件包含失败的完整摘要，其中指出了无法更新或删除哪些文件。

## <a name="sync-pinned-files"></a>同步已固定文件 

若要自动同步已固定文件，请在 Azure 门户中执行以下步骤：
 
1. 选择现有的 Azure 存储帐户。 

2. 转到“容器”，然后选择“+ 容器”以创建容器 。 将此容器命名为“newcontainer”。 将公共访问级别设置为“容器”。

    ![已固定文件的自动同步 1](media/azure-stack-edge-gpu-manage-shares/image-1.png)
...
3. 选择容器名称并设置以下元数据：  
    ...
    ![已固定文件的自动同步 2](media/azure-stack-edge-gpu-manage-shares/image-2.png)
 
4. 在设备上新建共享。 通过选择现有的容器选项将其映射到已固定容器。 将共享标记为只读。 创建新用户并指定该共享的用户名和相应的密码。  

    ![已固定文件的自动同步 3](media/azure-stack-edge-gpu-manage-shares/image-3.png)
 
5. 在 Azure 门户中，浏览到所创建的容器。 上传要固定到 newcontainer 的文件，其元数据设置为固定。 

6. 选择 Azure 门户中的“刷新数据”，以便设备下载特定 Azure 存储容器的固定策略。  

    ![已固定文件的自动同步 4](media/azure-stack-edge-gpu-manage-shares/image-4.png)
 
7. 访问在设备上创建的新共享。 已上传到存储帐户的文件现在已下载到本地共享。 

    每当设备断开或重新连接时，它都会触发刷新。 刷新将仅关闭那些已更改的文件。 
...
1. 在资源中转到“概述”。 在共享列表中，选择并单击与需要同步的存储帐户相关联的共享。
    ...
    ![选择包含相关存储帐户的共享](media/azure-stack-edge-gpu-manage-shares/sync-storage-key-1.png)

2. 单击“同步存储密钥”。 当系统提示确认时，单击“是”。

     ![选择“同步存储密钥”](media/azure-stack-edge-gpu-manage-shares/sync-storage-key-2.png)
