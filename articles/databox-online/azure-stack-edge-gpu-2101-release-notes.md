...
|**共.** |Azure Arc + Kubernetes 群集 |默认情况下， `yamls` 从 Git 存储库删除资源时，不会从 Kubernetes 群集中删除相应的资源。  |若要在从 git 存储库删除资源时允许删除资源，请 `--sync-garbage-collection` 在 Arc OperatorParams 中设置。 有关详细信息，请参阅 [删除配置](../azure-arc/kubernetes/use-gitops-connected-cluster.md#additional-parameters)。 |
|**全天候.**|NFS |使用设备上的 NFS 共享装入来写入数据的应用程序应使用独占写入。 这可确保写入操作写入磁盘。| |
...
|**15.**|Kubernetes |如果在设备上配置了计算后，为 IoT Edge 提供自己的证书，并将这些证书添加到 Azure Stack Edge 设备上，则不会提取新证书。|若要解决此问题，你应该在设备上配置计算之前上传证书。 如果已配置计算，请 [连接到设备的 PowerShell 接口，并运行 IoT Edge 命令](azure-stack-edge-gpu-connect-powershell-interface.md#use-iotedge-commands)。 重新启动 `iotedged` 和 pod `edgehub` 。|
...
|**20**|Internet Explorer|如果启用了增强的安全功能，则可能无法访问本地 web UI 页。 | 禁用增强的安全性，并重新启动浏览器。|
