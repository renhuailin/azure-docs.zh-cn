...
Azure 资源管理器提供一个管理层，用于在 Azure 订阅中创建、更新和删除资源。 Azure Stack Edge Pro 设备支持用相同的 Azure 资源管理器 API 在本地订阅中创建、更新和删除虚拟机 (VM)。 通过此支持，你能够以与云一致的方式管理设备。 
...
Azure 资源管理器提供一致的管理层来调用 Azure Stack Edge Pro 设备 API 并执行创建、更新和删除 VM 等操作。 下图详细介绍了 Azure 资源管理器的体系结构。
...
在开始之前，请确保用于通过 Azure 资源管理器连接到设备的客户端使用的是 TLS 1.2。 有关详细信息，请转到[在访问 Azure Stack Edge Pro 设备的 Windows 客户端上配置 TLS 1.2](azure-stack-edge-j-series-configure-tls-settings.md)。

## <a name="step-1-configure-azure-stack-edge-pro-device"></a>步骤 1：配置 Azure Stack Edge Pro 设备 
...
证书可确保通信受信任。 在 Azure Stack Edge Pro 设备上，会自动生成自签名的设备、blob 和 Azure 资源管理器证书。 此外，也可以选择自带签名的 blob 和 Azure 资源管理器证书。
...
1. 首先，需要为签名链创建根证书。 有关详细信息，请参阅[创建签名链证书](azure-stack-edge-gpu-manage-certificates.md#create-signing-chain-certificate)的步骤。
...
2. 终结点证书必须导出为包含私钥的 .pfx 文件。 有关详细步骤，请参阅[将证书导出为包含私钥的 .pfx 文件](azure-stack-edge-gpu-manage-certificates.md#export-certificates-as-pfx-format-with-private-key)。
...
1. 你将在客户端上安装 Azure PowerShell 模块（将与设备一起使用）。

    a. 以管理员身份运行 PowerShell。 需访问 PowerShell 库。 


    b. 若要从 PowerShell 库安装所需的 Azure PowerShell 模块，请运行以下命令：
...
下面显示的示例输出指示已成功安装 2.5.0 版本的 AzureRM 模块。
...
现在，将在设备的本地 Web UI 上定义的 Azure 一致 VIP 添加到：
...
2. 将以下条目添加到 hosts 文件，替换为设备的适当值： 
    ...
    > [!IMPORTANT]
    > hosts 文件中的条目应与提供用于在后面的步骤中连接到 Azure 资源管理器的条目完全匹配。 确保此处的 DNS 域条目均采用小写形式。
...
2. 将环境设置为 Azure Stack Edge Pro，将用于 Azure 资源管理器调用的端口设置为 443。 可以通过两种方式定义环境：
    ...
    - 针对执行的每个 cmdlet 定义内联环境。 这可确保通过正确的环境进行所有 API 调用。 默认情况下，调用将通过 Azure 公用环境进行，但是你希望这些调用通过为 Azure Stack Edge Pro 设备设置的环境进行。
...
2. 调用本地设备 API 来对与 Azure 资源管理器的连接进行身份验证。 
    ...
    2. 可以通过 `login-AzureRMAccount` 或 `Connect-AzureRMAccount` 命令进行连接。 
        ...
        1. 若要登录，请键入以下命令。 此实例中的租户 ID 采用硬编码 - c0257de7-538f-415c-993a-1b87a031879d。 使用以下用户名和密码。
            ...
            - **密码** - [设置 Azure 资源管理器的密码](azure-stack-edge-j-series-set-azure-resource-manager-password.md)，然后使用此密码登录。 
...
> [!IMPORTANT]
> 与 Azure 资源管理器的连接每 1.5 小时或每当 Azure Stack Edge Pro 设备重启时过期一次。 如果发生这种情况，所执行的任何 cmdlet 都将返回错误消息，表示不再连接到 Azure。 需重新登录。

## <a name="switch-environments"></a>切换环境

运行 `Disconnect-AzureRmAccount` 命令以切换到其他 `AzureRmEnvironment`。 

如果在不使用 `Disconnect-AzureRmAccount` 的情况下使用 `Set-AzureRmEnvironment` 和 `Login-AzureRmAccount`，则实际上不会切换环境。  
