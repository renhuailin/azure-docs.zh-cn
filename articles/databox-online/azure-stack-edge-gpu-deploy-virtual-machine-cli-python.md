...
有关工作流图表的详细说明，请参阅[使用 Azure PowerShell 在 Azure Stack Edge Pro 设备上部署 VM](azure-stack-edge-gpu-deploy-virtual-machine-powershell.md)。 有关如何连接到 Azure 资源管理器的信息，请参阅[使用 Azure PowerShell 连接到 Azure 资源管理器](azure-stack-edge-gpu-connect-resource-manager.md)。
...
3. 你已在 Azure Stack Edge Pro 设备和客户端的受信任存储中创建并安装了所有证书。 按照[步骤 2：创建和安装证书](azure-stack-edge-gpu-connect-resource-manager.md#step-2-create-and-install-certificates)中所述过程操作。
...
4. 使用 `az login` 命令登录到 Azure Stack Edge Pro 环境。 你可以以用户或[服务主体](../active-directory/develop/app-objects-and-service-principals.md)身份登录到 Azure Stack Edge Pro 环境。
   ...
   有关详细信息，请参阅 [Azure 资源管理器密码](/azure/azure-stack-edge-gpu-set-azure-resource-manager-password)。
