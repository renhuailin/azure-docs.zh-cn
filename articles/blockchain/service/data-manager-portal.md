...
* 为 Azure 区块链服务事务节点创建一个区块链数据管理器实例。
* 添加区块链应用程序

## <a name="prerequisites"></a>先决条件

* 完成[快速入门：使用 Azure 门户创建区块链成员](create-member.md)或[快速入门：通过 Azure CLI 创建 Azure 区块链服务区块链成员](create-member-cli.md)。 使用区块链数据管理器时，建议使用 Azure 区块链服务“标准”层。
...
区块链数据管理器实例连接并监控 Azure 区块链服务事务节点。 只有能够访问事务节点的用户可以创建连接。 实例从事务节点捕获所有原始块和原始事务数据。 区块链数据管理器发布“RawBlockAndTransactionMsg”消息，此消息是从 web3.eth [getBlock](https://web3js.readthedocs.io/en/v1.2.0/web3-eth.html#getblock) 和 [getTransaction](https://web3js.readthedocs.io/en/v1.2.0/web3-eth.html#gettransaction) 查询返回的信息的超集。

出站连接将区块链数据发送到 Azure 事件网格。 创建实例时，可以配置单个出站连接。 区块链数据管理器支持对任意给定的区块链数据管理器实例使用多个事件网格主题出站连接。 可将区块链数据发送到一个或多个目标。 要添加另一个目标，只需向实例添加其他出站连接即可。
...
1. 选择 **添加** 。
    ...
    设置 | 说明
    --------|------------
    名称 | 为连接的区块链数据管理器输入唯一的名称。 区块链数据管理器名称可以包含小写字母和数字，并且最大长度为 20 个字符。
...
区块链数据管理器需要智能合同 ABI 和部署的字节码文件来添加应用程序。
...
在要停止捕获区块链事件并将数据发送到出站连接时，请停止区块链管理器实例。 实例停止后，区块链数据管理器不会产生费用。 有关详细信息，请参阅[定价](https://azure.microsoft.com/pricing/details/blockchain-service)。
