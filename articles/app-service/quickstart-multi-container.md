...
在 Cloud Shell 终端中，使用 [az webapp create](/cli/azure/webapp#az_webapp_create) 命令在 `myAppServicePlan` 应用服务计划中创建一个多容器 [Web 应用](overview.md#app-service-on-linux)。 不要忘记将 \<app_name> 替换为唯一的应用名称（有效字符是 `a-z`、`0-9` 和 `-`）。
