---
title: 扩展 ASE v1 中的应用
description: 在应用服务环境中缩放 Web 应用。 本文档仅供使用旧版 v1 ASE 的用户使用。
author: ccompy
...
![显示缩放服务计划和工作线程池服务计划详情查看位置的屏幕截图。][1]
...
如果 ASE 具有足够的容量，那么此步骤非常简单。  转到包含你想要扩展的站点的应用服务计划，并选择“扩展”。  这会打开 UI，可以在其中为 ASP 手动设置缩放或设置自动缩放规则。  要手动缩放应用，只需将*“缩放依据”_设置为_“手动输入的实例计数”**。  从此处将滑块拖到所需的数量，或者在滑块旁边的框中输入所需的数量。  

![显示可以为 ASP 设置缩放或为 ASP 配置自动缩放规则的屏幕截图。][2] 

ASE 中的 ASP 自动缩放规则与一般运行方式相同。  可以选择“缩放依据”下面的*“CPU 百分比”_，根据 CPU 百分比创建 ASP 的自动缩放规则，或使用_“计划和性能规则”**创建更复杂的规则。   若要查看有关配置自动缩放的更完整详细信息，请参阅此处的指南：[在 Azure 应用服务中缩放应用][AppScale]。 
