...
门户上可能会显示 Azure 应用服务针对高 CPU 消耗量提供的建议。 也可将监视器设置为监视某些[指标](web-sites-monitor.md)。 在 [Azure 门户仪表板](../azure-monitor/essentials/metrics-charts.md)上检查 CPU 使用率时，请检查 CPU 的最大值，这样才不会错过峰值。
...
如果应用程序消耗过多的内存，门户中会显示 Azure 应用服务针对高内存消耗量提供的建议。 可将监视器设置为监视某些[指标](web-sites-monitor.md)。 在 [Azure 门户仪表板](../azure-monitor/essentials/metrics-charts.md)上检查内存使用率时，请务必检查内存的最大值，这样才不会错过峰值。
