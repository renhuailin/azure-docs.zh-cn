...
正文包含的字段集将因通知类型而异。 下面是两个示例消息正文，从中可以大致了解消息正文在一般情况下的外观及其可包含的内容。
...
`Twin.Update` 通知的正文是一个 JSON 补丁文档，其中包含对数字孪生的更新。

例如，假设已使用以下补丁更新了数字孪生。

:::code language="json" source="~/digital-twins-docs-samples/models/patch-component-2.json":::

相应的通知（如果由服务（例如，更新某个数字孪生的 Azure 数字孪生）同步执行）将包含如下所示的正文：
...
正文是受影响的数字孪生，以 JSON 格式表示。 此正文的架构是“数字孪生资源 7.1”。

对于创建事件，有效负载将反映创建资源后的孪生状态，因此，它应包含系统生成的所有元素，就如同一个 `GET` 调用一样。

下面是包含组件但不包含顶级属性的某个 [IoT 即插即用 (PnP)](../iot-pnp/overview-iot-plug-and-play.md) 设备的正文示例。 应该省略对设备没有意义的属性（例如报告的属性）。
...
下面是数字孪生的另一个示例。 此示例基于[模型](concepts-models.md)，且不支持组件：
...
正文是关系的有效负载，也采用 JSON 格式。 其所用格式与通过 [DigitalTwins API](/rest/api/digital-twins/dataplane/twins) 对关系发出的 `GET` 请求的格式相同。 
...
对于 `Relationship.Delete`，正文与 `GET` 请求相同，它会在执行删除之前获得最新状态。
...
* [如何：使用 Azure 数字孪生 API 和 SDK](how-to-use-apis-sdks.md)