...
本文的此版本通过使用 Azure PowerShell 逐个手动完成这些步骤。
...
1. 如果选择使用 Azure Cloud Shell：
   1. 有关详细信息，请参阅 [Azure Cloud Shell 概述](../cloud-shell/overview.md)。
...
1. 如果这是你第一次将 Azure 数字孪生与此订阅一起使用，则必须注册 **DigitalTwins** 资源提供程序。
...
> [!IMPORTANT]
> **DigitalTwins** PowerShell 模块为预览版时，必须使用 cmdlet 单独安装它 `Install-Module` 。 此 PowerShell 模块正式发布后，它会包含在将来的 Az PowerShell 模块发行版中，并在 Azure Cloud Shell 中默认提供。
...
* 一个 [Azure 资源组](../azure-resource-manager/management/overview.md)。 如果还没有现有资源组，可以使用 [AzResourceGroup](/powershell/module/az.resources/new-azresourcegroup) cmdlet 创建一个资源组。
...
> [!TIP]
> 你可以随时通过运行 `Get-AzDigitalTwinsInstance` 和管道到来查看这些属性和实例的所有属性 `Select-Object -Property *` 。
...
使用 [AzAdUser](/powershell/module/az.resources/get-azaduser) cmdlet 确定应该为其分配角色的用户的 Azure AD 帐户的 **ObjectId** 。

```azurepowershell-interactive
Get-AzADUser -UserPrincipalName <Azure-AD-user-principal-name-of-user-to-assign>
```

使用以下命令来分配角色。 它必须由在 Azure 订阅中具有 [足够权限](#prerequisites-permission-requirements) 的用户运行。 命令要求为应分配角色的用户的 Azure AD 帐户传入 **ObjectId** 。 它还要求你使用之前选择的相同订阅 ID、资源组名称和 Azure 数字孪生实例名称。
