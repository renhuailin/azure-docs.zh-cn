...
| **AzureAttestation** | Azure 证明。<br/><br/>注意：目前无法通过 Azure 门户配置此标记。 | 出站 | 否 | 是 | 
...
| **AzureDevOps** | Azure Dev Ops。<br/><br/>注意：目前无法通过 Azure 门户配置此标记。| 入站 | 否 | 是 |
...
| **AzureMonitor** | Log Analytics、Application Insights、AzMon 和自定义指标（GiG 终结点）。<br/><br/>注意：对于 Log Analytics，还需要提供 Storage 标记 。 如果使用了 Linux 代理，亦必须有“GuestAndHybridManagement”。 | 出站 | 否 | 是 |
...
| AzureSphere | 此标记或此标记所涵盖的 IP 地址可用于限制对 Azure Sphere 安全服务的访问。 </br> 注意：目前无法通过 Azure 门户配置此标记。 | 推送、请求和匿名 | 否 | 是 | 
| AzureStack | Azure Stack Bridge 服务。 </br> 此标记表示每个区域的 Azure Stack Bridge 服务终结点。 </br> *注意：此标记当前不可通过 Azure 门户进行配置。* | 出站 | 否 | 是 |
...
| **AzureUpdateDelivery** | 用于访问 Windows 更新。 <br/><br/>注意：此标记提供对 Windows 更新元数据服务的访问。 若要成功下载更新，还必须启用 AzureFrontDoor.FirstParty 服务标记并使用定义的协议和端口配置出站安全规则，如下所示： <ul><li>AzureUpdateDelivery：TCP，端口 443</li><li>AzureFrontDoor.FirstParty：TCP，端口 80</li></ul>目前无法通过 Azure 门户配置此标记| 出站 | 否 | 否 |  
...
| EOPExternalPublishedIPs | 此标记表示用于安全与合规中心 PowerShell 的 IP 地址。 请参阅[使用 EXO V2 模块连接到安全与合规中心 PowerShell，了解更多详情](/powershell/exchange/connect-to-scc-powershell)。 <br/><br/> 注意：目前无法通过 Azure 门户配置此标记。 | 推送、请求和匿名 | 否 | 是 |
...
| **PowerBI** | 活动版本。 注意：目前无法通过 Azure 门户配置此标记。 | 推送、请求和匿名 | 否 | 否|
| PowerPlatformInfra | 此标记表示由基础结构用来托管 Power Platform 服务的 IP 地址。 注意：目前无法通过 Azure 门户配置此标记。 | 出站 | 否 | 否 |
...
| **WindowsAdminCenter** | 允许 Windows Admin Center 后端服务与 Windows Admin Center 的客户安装通信。 注意：目前无法通过 Azure 门户配置此标记。 | 出站 | 否 | 是 |
...
### <a name="tags-supported-in-the-classic-deployment-model"></a>经典部署模型中支持的标记
...
### <a name="use-the-service-tag-discovery-api"></a>使用服务标记发现 API
...
> [!NOTE]
> 
> - 跨所有 Azure 区域在 API 结果中传播新服务标记数据最长需要 4 周时间。 
> - 必须完成身份验证，并具有一个对当前订阅拥有读取权限的角色。 
> - API 数据表示可以与 NSG 规则一起使用的那些标记，这些标记表示可下载的 JSON 文件中当前包含的一部分标记。 
