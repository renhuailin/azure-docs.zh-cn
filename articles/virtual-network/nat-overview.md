...
连接到 NAT 的公共 IP 地址分别为 UDP 和 TCP 提供多达 64,000 个并发流。 你可以从一个 IP 地址开始，然后使用公共 IP 地址和/或公共 IP 前缀，扩展到 16 个 IP 地址。  NAT 网关资源将使用与该资源关联的所有 IP 地址，以便从已配置相同 NAT 网关资源的所有子网进行出站连接。

NAT 允许创建从虚拟网络到 Internet 的流。 在活动流的响应中仅允许来自 Internet 的返回流量。

与负载均衡器出站 SNAT 不同，NAT 对虚拟机实例的哪个专用 IP 可以建立出站连接不设限制。  主要和辅助 IP 配置可与 NAT 建立出站 Internet 连接。
