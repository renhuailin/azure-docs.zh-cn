...
1. 现在，我们将使用 Azure CLI 对 Cassandra 托管实例所需的 VNet 和子网应用一些特殊权限。 首先，我们需要为现有 VNet 查找 `Resource ID`。 复制此命令的输出值（即 `Resource ID`）供稍后使用。
...
1. 现在，我们应用特殊权限，并将前一个命令的输出作为 scope 参数传递：
   ...
   ```
    > [!NOTE]
    > 上面的 `assignee` 和 `role` 值分别是固定的服务主体和角色标识符。 
