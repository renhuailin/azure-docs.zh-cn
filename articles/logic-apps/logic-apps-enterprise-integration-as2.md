...
> [!IMPORTANT]
> 原始 AS2 连接器已被弃用，因此除非你需要跟踪功能，否则请改用 AS2 (v2) 连接器。 除跟踪外，v2 连接器可提供更优性能，并保留与原始版本相同的功能，它是 Azure 逻辑应用运行时固有的，在消息大小、吞吐量和延迟方面有显著的性能提升。 另外，v2 连接器不要求与集成帐户建立连接。 但要求执行先决条件中所述的操作，确保将集成帐户关联到你计划在其中使用连接器的逻辑应用。
