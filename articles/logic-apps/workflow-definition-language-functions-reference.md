...
| [result](../logic-apps/workflow-definition-language-functions-reference.md#result) | 返回指定作用域内操作内顶级操作的输入和输出，例如 `For_each` 、 `Until` 和 `Scope` 。 |
...
返回字符串或 XML (JSON) 类型值、对象或对象数组的 JavaScript 对象表示法。
...
> [!IMPORTANT]
> 如果没有定义输出结构的 XML 架构，则函数可能会返回结果，其中结构与预期的格式大不相同，具体取决于输入。
>  
> 此行为使此功能不适用于输出必须符合定义良好的协定的方案，例如，在关键业务系统或解决方案中。
...
| 返回值 | 类型 | 说明 |
| ------------ | ---- | ----------- |
| <*JSON-result*> | JSON 本机类型、对象或数组 | 输入字符串或 XML 中的 JSON 本机类型值、对象或对象数组。 <p><p>-如果传入的 XML 具有根元素中的单个子元素，则该函数为该子元素返回单个 JSON 对象。 <p> -如果传入的 XML 包含根元素中的多个子元素，则函数将返回一个数组，其中包含这些子元素的 JSON 对象。 <p>-如果字符串为 null，则该函数返回空对象。 |
||||
...
此示例使用 `json()` 和 `xml()` 函数将根元素中具有一个子元素的 XML 转换为一个名 `person` 为的该子元素的 JSON 对象：
...
此示例使用 `json()` 和 `xml()` 函数将根元素中具有多个子元素的 XML 转换为一个名为的数组 `person` ，其中包含这些子元素的 JSON 对象：
...
* 如果只有 substring 值为空，则函数返回字符串长度减1。
...
返回指定范围内操作中的顶级操作的结果，如 `For_each` 、 `Until` 或 `Scope` 操作。 `result()`函数接受单个参数，即作用域的名称，并返回一个数组，该数组包含该范围内第一级操作的信息。 这些操作对象包含与函数返回的属性相同的属性 `actions()` ，例如操作的开始时间、结束时间、状态、输入、相关 id 和输出。

> [!NOTE]
> 此函数 *只* 从作用域操作中的第一层操作返回信息，而不从更深层嵌套的操作（如 switch 或 condition 操作）返回信息。

例如，您可以使用此函数从失败的操作获取结果，以便您可以诊断和处理异常。 有关详细信息，请参阅[获取失败的上下文和结果](../logic-apps/logic-apps-exception-handling.md#get-results-from-failures)。
...
| 参数 | 必选 | 类型 | 说明 |
| --------- | -------- | ---- | ----------- |
| <*scopedActionName*> | 是 | 字符串 | 要从该范围内的顶级操作中输入和输出的范围操作的名称 |
||||

| 返回值 | 类型 | 说明 |
| ------------ | ---- | ----------- |
| <*array-object*> | 数组对象 | 一个数组，该数组包含指定范围内的每个顶级操作的输入和输出的数组 |
||||

*示例*

此示例 `For_each` 通过使用操作中的函数，从循环内的中的每个迭代中返回 HTTP 操作的输入和输出 `result()` `Compose` ：
...
> 否则，你将收到一个错误，这与其他语言中类似的函数不同，其中的结果是来自 *startIndex* 到字符串末尾的子字符串。 *Length* 参数是可选的，如果未提供此参数，则 **substring ()** 函数将以 *startIndex* 开头的所有字符都作为字符串的末尾。
...
该示例还使用 [参数 () ](#parameters) 函数获取中的 xml 字符串 `'items'` ，并使用 [xml () ](#xml) 函数将字符串转换为 xml 格式。
...
在此示例中，假设你有包含 XML 文档命名空间的 XML 字符串 `xmlns="https://contoso.com"` ：

```xml
<?xml version="1.0"?><file xmlns="https://contoso.com"><location>Paris</location></file>
```

这些表达式使用 XPath 表达式 `/*[name()="file"]/*[name()="location"]` 或 `/*[local-name()="file" and namespace-uri()="https://contoso.com"]/*[local-name()="location"]` ，查找与节点匹配的节点 `<location></location>` 。 这些示例显示了逻辑应用设计器或表达式编辑器中使用的语法：
