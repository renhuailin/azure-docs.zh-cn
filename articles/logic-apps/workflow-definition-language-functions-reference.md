...
例如，你使用的是返回图像或视频文件的 HTTP 操作。 可以使用 `binary()` 将值转换为 base64 编码的内容信封模型。 然后，可以在其他操作（如 `Compose`）中重复使用内容信封。
...
> [!NOTE]
> `count` 参数值必须是不超过 100,000 的正整数。 `startIndex` 和 `count` 的总和不得超过 2,147,483,647。
