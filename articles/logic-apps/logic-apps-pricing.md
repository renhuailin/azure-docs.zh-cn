...
例如，假设一个工作流从轮询触发器开始，该触发器通过定期对终结点进行出站调用来获取记录。 无论触发器是触发还是跳过（例如，当触发器检查终结点但未找到任何数据或事件时），出站调用都将作为单次执行进行计量和计费。 触发器状态控制是否已创建和运行工作流实例。 现在，假设此操作还支持并启用了分块或分页。 如果此操作必须执行 10 次调用才能完成所有数据的获取，尽管进行了多次调用，但仍会将此操作作为单次执行来进行计量和计费。
