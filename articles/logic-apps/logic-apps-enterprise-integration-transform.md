...
1. 创建一个逻辑应用并[将它链接到包含映射的集成帐户](./logic-apps-enterprise-integration-create-integration-account.md "了解如何将集成帐户链接到逻辑应用")。
2. 将“请求”触发器添加到逻辑应用  
   ![“显示 Microsoft 托管 API”下拉列表的屏幕截图，其中已选择“请求”触发器。 下拉列表位于使用 Visual Studio 企业集成 SDK 创建的逻辑应用中。](./media/logic-apps-enterprise-integration-transforms/transform-1.png)    
3. 通过首先选择“添加操作”来添加“转换 XML”操作   
   ![屏幕截图显示在“请求”触发器屏幕中选择了“添加操作”按钮。](./media/logic-apps-enterprise-integration-transforms/transform-2.png)   
4. 在搜索框中输入“转换”一词，以便在所有操作中筛选出要使用的操作  
   ![屏幕截图显示如何在“显示 Microsoft 托管 API”下拉列表中搜索“转换 XML”操作以便可以将其添加到“请求”触发器。](./media/logic-apps-enterprise-integration-transforms/transform-3.png)  
5. 选择“转换 XML”操作   
...
7. 选择要用于执行转换的“映射”的名称。 该映射必须已处于集成帐户中。 在前面的步骤中，已向逻辑应用授予了对包含映射的集成帐户的访问权限。      
   ![屏幕截图显示“请求”触发器的“转换 XML”屏幕中的“内容”和“映射”字段。](./media/logic-apps-enterprise-integration-transforms/transform-4.png) 
8. 保存工作  
    ![屏幕截图显示逻辑应用设计器中的“保存”按钮。](./media/logic-apps-enterprise-integration-transforms/transform-5.png) 
