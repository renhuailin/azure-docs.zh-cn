...
description: 了解如何通过在 Azure 数据工厂或 Synapse Analytics 管道中使用复制活动，将数据从 Oracle Responsys 复制到支持的接收器数据存储。
...
ms.locfileid: "124827101"
---
# <a name="copy-data-from-oracle-responsys-using-azure-data-factory-or-synapse-analytics-preview"></a>使用 Azure 数据工厂或 Synapse Analytics 从 Oracle Responsys 复制数据（预览版）
[!INCLUDE[appliesto-adf-asa-md](includes/appliesto-adf-asa-md.md)]

本文概述如何使用 Azure 数据工厂或 Synapse Analytics 管道中的复制活动从 Oracle Responsys 复制数据。 它是基于概述复制活动总体的[复制活动概述](copy-activity-overview.md)一文。
...
## <a name="create-a-linked-service-to-oracle-responsys-using-ui"></a>使用 UI 创建一个到 Oracle Responsys 的链接服务

使用以下步骤在 Azure 门户 UI 中创建一个到 Oracle Responsys 的链接服务。
...
2. 搜索 Oracle 并选择 Oracle Responsys 连接器。

   :::image type="content" source="media/connector-oracle-responsys/oracle-responsys-connector.png" alt-text="Oracle Responsys 连接器的屏幕截图。":::    


1. 配置服务详细信息，测试连接，然后创建新的链接服务。

   :::image type="content" source="media/connector-oracle-responsys/configure-oracle-responsys-linked-service.png" alt-text="Oracle Responsys 的链接服务配置的屏幕截图。":::
...
| clientSecret | 与 Responsys 应用程序关联的客户端密码。 可选择将此字段标记为 SecureString，将其安全地存储在服务中，或在 Azure Key Vault 中存储密码，让服务复制活动在执行数据复制时从中进行拉取（详见[在 Key Vault 中存储凭据](store-credentials-in-key-vault.md)）。 | 是 |
