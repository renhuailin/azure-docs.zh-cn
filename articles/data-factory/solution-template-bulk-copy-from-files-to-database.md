...
4. 你会看到所创建的管道，如以下示例中所示：
    ...
    > [!NOTE]
    > 如果在上述 **步骤 2** 中选择 " **Azure Synapse Analytics (以前的 SQL DW)** 作为数据目标，则必须根据 azure Synapse Analytics Polybase 的要求，输入与 azure Blob 存储的连接进行过渡。 如以下屏幕截图所示，模板会自动为 Blob 存储生成存储路径。 检查是否在管道运行后创建了容器。
