...
| 查询 | 使用自定义 SQL 查询读取数据。 例如 `SELECT * FROM mytable` 或 `SELECT * FROM "MyTable"`。 请注意，在 PostgreSQL 中，如果未加引号，则将实体名称视为不区分大小写。 | 否（如果指定了数据集中的 tableName 属性） |
