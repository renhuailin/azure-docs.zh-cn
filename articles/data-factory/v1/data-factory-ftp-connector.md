...
本文介绍如何使用 Azure 数据工厂中的复制活动从 FTP 服务器移动数据。 它基于 [数据移动活动](data-factory-data-movement-activities.md) 一文，其中概括介绍了如何使用复制活动移动数据。
...
>[!NOTE]
>FTP 连接器支持通过无加密或显式 SSL/TLS 加密访问 FTP 服务器;它不支持隐式 SSL/TLS 加密。
...
| properties | 说明 | 必须 |
| --- | --- | --- |
| folderPath |文件夹的子路径。 对于字符串中的特殊字符，请使用转义符 "\"。 有关示例，请参阅“链接服务和数据集定义示例”。<br/><br/>可将此属性与 **partitionBy** 组合在一起，基于切片开始和结束的日期时间构成文件夹路径。 |是 |
...
* 类型为文件[共享](#dataset-properties)的输入[数据集](data-factory-create-datasets.md)
