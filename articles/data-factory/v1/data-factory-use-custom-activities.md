...
- [数据转换活动](data-factory-data-transformation-activities.md)使用 Azure HDInsight、Azure Batch 和 Azure 机器学习工作室（经典）等计算服务转换数据。
...
若要创建 .NET 自定义活动，请使用实现 **IDotNetActivity** 接口的类创建 **.NET 类库** 项目。 此接口仅包含一个方法：[Execute](/dotnet/api/microsoft.azure.management.datafactories.runtime.idotnetactivity)，其签名是：
...
8. 将 **IDotNetActivity** 接口的 **执行** 方法实现（添加）到 **MyDotNetActivity** 类并将以下示例代码复制到该方法。
    ...
    以下示例将计算与数据切片关联的每个 blob 中搜索词（“Microsoft”）的出现次数。
...
| [Twitter 情绪分析示例](https://github.com/Azure/Azure-DataFactory/tree/master/SamplesV1/TwitterAnalysisSample-CustomC%23Activity) |调用 Azure 机器学习工作室（经典）模型和进行情绪分析、评分、预测等。 |
