...
| linkedServiceName |活动使用的链接服务的名称。 <br/><br/>活动可能需要你指定链接到所需计算环境的链接服务。 |对 HDInsight 活动、ML 工作室（经典）活动和存储过程活动是必需的。 <br/><br/>对其他活动均非必需 |
...
| [机器学习工作室（经典版）](#ml-studio-classic) | [ML 工作室（经典）批量执行活动](#ml-studio-classic-batch-execution-activity)、[ML 工作室（经典）更新资源活动](#ml-studio-classic-update-resource-activity) |
...
创建 ML 工作室（经典）链接服务，用于向数据工厂注册工作室（经典）批处理计分终结点。 可以在此链接服务中运行的两个数据转换活动：[机器学习工作室（经典）批处理执行活动](#ml-studio-classic-batch-execution-activity)、[机器学习工作室（经典）更新资源活动](#ml-studio-classic-update-resource-activity)。
...
[ML 工作室（经典）批处理执行活动](#ml-studio-classic-batch-execution-activity) | 借助 Azure 数据工厂，可轻松创建管道，这些管道使用已发布的工作室（经典）Web 服务进行预测分析。 使用 Azure 数据工厂管道中的批处理执行活动，可调用工作室（经典）Web 服务来预测批处理中的数据。
[ML 工作室（经典）更新资源活动](#ml-studio-classic-update-resource-activity) | 随着时间的推移，需要使用新的输入数据集重新训练 ML 工作室（经典）评分试验中的预测模型。 完成重新训练后，需使用重新训练的机器学习模型更新评分 Web 服务。 可以使用更新资源活动，通过新的训练模型更新 Web 服务。
...
可以在 ML 工作室（经典）批处理执行活动 JSON 定义中指定以下属性。 活动的类型属性必须是：**AzureMLBatchExecution**。 必须先创建工作室（经典）链接服务，然后指定该服务的名称作为 linkedServiceName 属性的值。 将活动类型设置为 AzureMLBatchExecution 时，**typeProperties** 节支持以下属性：
...
## <a name="ml-studio-classic-update-resource-activity"></a>ML 工作室（经典版）更新资源活动
可以在 ML 工作室（经典）更新资源活动 JSON 定义中指定以下属性。 活动的类型属性必须是：**AzureMLUpdateResource**。 必须先创建工作室（经典）链接服务，然后指定该服务的名称作为 linkedServiceName 属性的值。 将活动类型设置为 AzureMLUpdateResource 时，**typeProperties** 节支持以下属性：
