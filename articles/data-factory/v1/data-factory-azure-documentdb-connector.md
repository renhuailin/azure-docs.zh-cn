...
| nestingSeparator |指示嵌套文档的特殊字符 |任意字符。 <br/><br/>Azure Cosmos DB 是 JSON 文档的 NoSQL 存储，其中允许存在嵌套结构。 Azure 数据工厂允许用户通过 nestingSeparator 来表示层次结构，以上示例中是 "."。 对于分隔符，复制活动将生成具有三个子元素的 "Name" 对象，这些元素在表定义中以 "Name. First"、"Name. 中间" 和 "Name. Last" 开头。 |否 |
...
| writeBatchSize |向 Azure Cosmos DB 服务发送创建文档的并行请求数。<br/><br/>向/从 Cosmos DB 复制数据时，可使用此属性对性能进行微调。 当增加 writeBatchSize 时，由于会向 Cosmos DB 发送更多的并行请求，因此可以获得更好的性能。 但是，您需要避免可能引发错误消息的限制： "请求速率很大"。<br/><br/>限制由多个因素决定，包括文档大小、文档中的术语数、目标集合的索引策略等。对于复制操作，可以使用更好的集合 (例如 S3) 以 (2500 请求单位/秒) 提供最大吞吐量。 |Integer |否（默认值：5） |
...
设置 "external"： "true" 并指定 externalData 策略信息 Azure 数据工厂服务：表在数据工厂外部且不由数据工厂中的活动生成。
...
```
Azure Cosmos DB 是 JSON 文档的 NoSQL 存储，其中允许存在嵌套结构。 Azure 数据工厂允许用户通过 **nestingSeparator** 来表示层次结构，这是本示例中的 "."。 对于分隔符，复制活动将生成具有三个子元素的 "Name" 对象，这些元素在表定义中以 "Name. First"、"Name. 中间" 和 "Name. Last" 开头。
