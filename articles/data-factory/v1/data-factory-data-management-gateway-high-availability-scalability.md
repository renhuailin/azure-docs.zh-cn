...
包含多个节点的网关可在所有节点间使数据存储凭据保持同步。 如果存在节点到节点连接问题，则凭据可能无法进行同步。为使用网关的本地数据存储设置凭据时，系统会在调度程序/工作器节点上保存凭据。 调度程序节点与其他辅助角色节点同步。 此过程称为“凭据同步”。节点之间的信道可以通过公共 SSL/TLS 证书进行加密。  
...
- 针对 TLS/SSL 证书支持受 Windows Server 2012 R2 支持的任何密钥大小。
- 不支持使用 CNG 密钥的证书。
...
- 如果所有网关节点均需 HTTP 代理，请在 diahost.exe.config 和 diawp.exe.config 中设置代理并使用服务器管理器，以确保所有节点具有相同的 diahost.exe.config 和 diawip.exe.config。请参阅[配置代理设置](data-factory-data-management-gateway.md#configure-proxy-server-settings)部分了解更多详情。 
- 若要在网关配置管理器中更改节点到节点通信的加密模式，请在门户中保留一个节点并删除其他所有节点。 然后，在更改加密模式后重新添加节点。
- 如果选择加密节点到节点信道，请使用正式的 TLS 证书。 自签名证书可能导致连接问题，因为相同的证书在其他计算机上的证书验证机构中可能不受信任。 
