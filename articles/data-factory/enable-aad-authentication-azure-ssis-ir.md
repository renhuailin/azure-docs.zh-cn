---
title: 为 Azure SSIS 集成运行时启用 Azure Active Directory 身份验证
description: 本文介绍如何实现使用 Azure 数据工厂的指定系统/用户分配的托管标识进行 Azure Active Directory 身份验证以创建 Azure-SSIS 集成运行时。
...
本文介绍如何实现使用 Azure 数据工厂 (ADF) 的指定系统/用户分配的托管标识进行 Azure Active Directory (Azure AD) 身份验证，并使用该验证代替传统的身份验证方法（如 SQL 身份验证）以执行以下操作：

- 创建 Azure-SSIS 集成运行时 (IR)，而该运行时会代表你在 Azure SQL 数据库服务器/托管实例中预配 SSIS 目录数据库 (SSISDB)。
...
> [!NOTE]
> - 在此场景中，只能在创建和后续启动 Azure-SSIS IR 过程中使用 ADF 的指定系统/用户分配的托管标识进行 Azure AD 身份验证，而 Azure-SSIS IR 可以预配并连接到 SSISDB。 在 SSIS 包的执行过程中，Azure-SSIS IR 仍通过使用在 SSISDB 预配期间创建的完全托管帐户进行 SQL 身份验证来连接到 SSISDB。
>
> - 如果已使用 SQL 身份验证创建了 Azure SSIS IR，则此时不能通过 PowerShell 将其重新配置为使用 Azure AD 身份验证，但你可以通过 Azure 门户/ADF 应用执行此操作。 

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

## <a name="enable-azure-ad-authentication-on-azure-sql-database"></a>对 Azure SQL 数据库启用 Azure AD 身份验证
...
3.  将 ADF 的指定系统/用户分配的托管标识添加到组中。 可以按照[数据工厂的托管标识](./data-factory-service-identity.md)一文中的说明操作，获取 ADF 的指定系统/用户分配的托管标识的对象 ID（例如 765ad4ab-XXXX-XXXX-XXXX-51ed985819dc，但不要将应用程序 ID 用于此目的）。
...
10. 如果 SSISDB 是使用 SQL 身份验证创建的，并且你想要改为对 Azure-SSIS IR 使用 Azure AD 身份验证来访问它，请首先确保向主数据库授予权限的步骤已成功完成。 然后，右键单击“SSISDB”数据库并选择“新建查询” 。
...
## <a name="enable-azure-ad-authentication-on-azure-sql-managed-instance"></a>对 Azure SQL 托管实例启用 Azure AD 身份验证
...
6.  如果 SSISDB 是使用 SQL 身份验证创建的，并且你想要改为对 Azure-SSIS IR 使用 Azure AD 身份验证来访问它，请首先确保向主数据库授予权限的步骤已成功完成。 然后，右键单击“SSISDB”数据库并选择“新建查询” 。
...
## <a name="run-ssis-packages-using-azure-ad-authentication-with-the-specified-systemuser-assigned-managed-identity-for-your-adf"></a>使用 ADF 的指定系统/用户分配的托管标识进行 Azure AD 身份验证以运行 SSIS 包

在 Azure-SSIS IR 上运行 SSIS 包时，可以使用 ADF 的指定系统/用户分配的托管标识进行 Azure AD 身份验证，以连接到各种 Azure 资源。 当前，支持在以下连接管理器上使用 ADF 的指定系统/用户分配的托管标识进行 Azure AD 身份验证。
