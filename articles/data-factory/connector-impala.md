---
title: 从 Impala 复制数据
description: 了解如何通过在 Azure 数据工厂或 Synapse Analytics 管道中使用复制活动，将数据从 Impala 复制到支持的接收器数据存储。
...
ms.locfileid: "124767691"
---
# <a name="copy-data-from-impala-using-azure-data-factory-or-synapse-analytics"></a>使用 Azure 数据工厂或 Synapse Analytics 从 Impala 复制数据

[!INCLUDE[appliesto-adf-asa-md](includes/appliesto-adf-asa-md.md)]

本文概述如何使用 Azure 数据工厂或 Synapse Analytics 管道中的复制活动从 Impala 复制数据。 本文基于总体概述复制活动的[复制活动概述](copy-activity-overview.md)一文。
...
## <a name="create-a-linked-service-to-impala-using-ui"></a>使用 UI 创建一个到 Impala 的链接服务
...
2. 搜索 Impala 并选择 Impala 连接器。
   ...
   :::image type="content" source="media/connector-impala/impala-connector.png" alt-text="Impala 连接器的屏幕截图。":::    


1. 配置服务详细信息，测试连接，然后创建新的链接服务。

   :::image type="content" source="media/connector-impala/configure-impala-linked-service.png" alt-text="Impala 的链接服务配置的屏幕截图。":::
...
| password | 使用 UsernameAndPassword 时用户名所对应的密码。 将此字段标记为 SecureString 以安全地存储它，或[引用 Azure Key Vault 中存储的机密](store-credentials-in-key-vault.md)。 | 否 |
