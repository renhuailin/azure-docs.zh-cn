...
完成编辑后，动态内容编辑器会自动在内容中转义字符。 例如，content editor 中的以下内容是包含两个表达式函数的字符串内插。 
...
| 参数 | 必选 | 类型 | 说明 |
| --------- | -------- | ---- | ----------- |
| <*format*> | 否 | String | 表示返回的 GUID 的单一[格式说明符](/dotnet/api/system.guid.tostring)。 默认情况下，格式为“D”，但可以使用“N”、“D”、“B”、“P”或“X”。 |
|||||
