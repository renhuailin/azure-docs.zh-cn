...
打开滑块后，系统将提示您选择要使用的集成运行时配置。 如果选择了 "AutoResolveIntegrationRuntime"，则将启动具有8个常规计算（默认为60分钟的时间）的群集。 如果你想要在会话超时之前允许更多的空闲团队，可以选择更高的 TTL 设置。 有关数据流集成运行时的详细信息，请参阅数据流 [性能](concepts-data-flow-performance.md#ir)。
...
> [!NOTE]
> 用户从其 ADF 浏览器 UI 启动的每个调试会话都是一个新会话，其中包含自己的 Spark 群集。 您可以使用以上调试会话的 "监视" 视图来查看和管理每个工厂的调试会话。 每隔一小时向你收费，其中每个调试会话执行包括 TTL 时间。
