...
### <a name="how-to-perform-activity-level-errors-and-failures-in-pipelines"></a>如何处理管道中的活动级错误和失败
...
* [以可视方式监视管道](https://docs.microsoft.com/azure/data-factory/monitor-visually)
 ...
 * 批计数 (n) 用于构造 n 个队列。 稍后我们将更详细地讨论如何构造这些队列。
 ...
 * 考虑到队列的构造方式，客户可以通过设置多个 *foreach*（其中每个 foreach 的项具有相似的处理时间）来提高 foreach 性能。 这可以确保并行（而不是按顺序）处理长时间运行。
...
* 如果每个复制活动最多需要 2 分钟才能开始，而问题主要出在 VNet 联接（相较于Azure IR），这可能是复制性能问题。 若要查看故障排除步骤，请参阅[复制性能改进](https://docs.microsoft.com/azure/data-factory/copy-activity-performance-troubleshooting)。
* 可以使用生存时间功能来减少数据流活动的群集启动时间。 请查看[数据流集成运行时](https://docs.microsoft.com/azure/data-factory/control-flow-execute-data-flow-activity#data-flow-integration-runtime)。
...
* 如果遇到 SHIR 的容量问题，请升级 VM 以增加节点来平衡活动。 如果收到有关自承载 IR 一般性故障或错误、自承载 IR 升级或自承载 IR 连接问题的错误消息（可能会生成长队列），请参阅[排查自承载集成运行时问题](https://docs.microsoft.com/azure/data-factory/self-hosted-integration-runtime-troubleshoot-guide)。
...
**分辨率**
* 如果通过连接器收到了来自任何源或目标的错误消息（可能会生成长队列），请参阅[连接器故障排除指南](https://docs.microsoft.com/azure/data-factory/connector-troubleshoot-guide)
* 如果收到有关映射数据流的错误消息（可能会生成长队列），请参阅[数据流故障排除指南](https://docs.microsoft.com/azure/data-factory/data-flow-troubleshoot-guide)。
* 如果收到有关 Databricks、自定义活动或 HDI 等其他活动的错误消息（可能会生成长队列），请参阅[活动故障排除指南](https://docs.microsoft.com/azure/data-factory/data-factory-troubleshoot-guide)
* 如果收到有关运行 SSIS 包的错误消息（可能会生成长队列），请参阅 [Azure-SSIS 包执行故障排除指南](https://docs.microsoft.com/azure/data-factory/ssis-integration-runtime-ssis-activity-faq)和 [Integration Runtime 管理故障排除指南](https://docs.microsoft.com/azure/data-factory/ssis-integration-runtime-management-troubleshoot)。
