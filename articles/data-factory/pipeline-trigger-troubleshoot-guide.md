---
title: 在 ADF 中对管道业务流程和触发器进行故障排除
description: 使用不同方法来解决 ADF 中的管道触发器问题
author: ssabat
...
ms.locfileid: "97521820"
---
# <a name="troubleshoot-pipeline-orchestration-and-triggers-in-adf"></a>在 ADF 中对管道业务流程和触发器进行故障排除
...
#### <a name="issue"></a>问题
在某些情况下，在专用终结点上运行 ADF 和 Azure Function App。 你正在尝试获取与 Azure Function App 进行交互的管道。 你已尝试三种不同的方法，但一个返回错误 `Bad Request` ，另两个方法返回 `103 Error Forbidden` 。

#### <a name="cause"></a>原因 
ADF 目前不支持 Azure Function App 专用终结点连接器。 这应该是 Azure Function App 拒绝调用的原因，因为它将被配置为仅允许来自专用链接的连接。
...
#### <a name="issue"></a>问题  
如果要复制的特定文件夹下的文件包含具有不同架构（如可变列数、不同分隔符、引号字符设置或某些数据问题）的文件，则在此错误中 ADF 管道将最终运行：
...
#### <a name="resolution"></a>解决方法
创建复制数据活动时，请选择 "二进制复制" 选项。 通过这种方式，可将数据从一个 Data Lake 大容量复制或迁移到另一（带有 **binary** 选项），ADF 不会打开要读取架构的文件，而只是将每个文件视为二进制文件并将其复制到另一个位置。
...
#### <a name="issue"></a>问题
通常需要以5分钟为间隔监视 ADF 管道。 可以使用终结点从数据工厂查询和筛选管道运行。 
...
- [外部-从 ADF 发送通知](https://www.mssqltips.com/sqlservertip/5962/send-notifications-from-an-azure-data-factory-pipeline--part-2/)
