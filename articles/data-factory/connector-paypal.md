---
title: 从 PayPal 复制数据（预览版）
description: 了解如何通过在 Azure 数据工厂或 Synapse Analytics 管道中使用复制活动，将数据从 PayPal 复制到支持的接收器数据存储。
...
ms.locfileid: "124824815"
---
# <a name="copy-data-from-paypal-using-azure-data-factory-or-synapse-analytics-preview"></a>使用 Azure 数据工厂或 Synapse Analytics（预览版）从 PayPal 复制数据
[!INCLUDE[appliesto-adf-asa-md](includes/appliesto-adf-asa-md.md)]

本文概述如何使用 Azure 数据工厂或 Synapse Analytics 管道中的复制活动从 PayPal 复制数据。 它是基于概述复制活动总体的[复制活动概述](copy-activity-overview.md)一文。
...
| clientSecret | 与 PayPal 应用程序关联的客户端密码。 将此字段标记为 SecureString 以安全地存储它，或[引用存储在 Azure Key Vault 中的机密](store-credentials-in-key-vault.md)。 | 是 |
