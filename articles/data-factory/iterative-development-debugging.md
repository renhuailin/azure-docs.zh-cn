...
数据流设计器中的数据预览和数据流的管道调试旨在充分利用少量的数据示例。 但是，如果需要在管道中测试逻辑或对大量数据进行数据流处理，请使用更多的内核和最少的常规用途计算，增加在调试会话中使用的 Azure Integration Runtime 的大小。
...
> [!NOTE]
> 如果有一个管道，该管道包含并行执行的数据流或需要使用大型数据集进行测试的数据流，则选择 "使用活动运行时"，以便数据工厂可以使用在数据流活动中选择的 Integration Runtime。 这使得数据流能在多个群集上执行，还可适应并行数据流执行。
