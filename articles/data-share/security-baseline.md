...
- [如何配置 Azure 活动日志的诊断设置](/azure/azure-monitor/platform/activity-log)
...
- [在 Azure 存储帐户中存储资源日志](/azure/azure-monitor/platform/resource-logs#send-to-azure-storage)
...
**指南**： Azure 数据共享支持 Azure Active Directory (Azure AD) 的 SSO 身份验证。 通过使用组织的预先存在的标识为服务启用 SSO，减少用户必须管理的标识和凭据的数量。
...
**指南**：启用 Azure AD 多重身份验证，并遵循 Azure 安全中心的标识和访问建议。
...
- [Azure 数据共享适用于通用 Azure 内置角色 ](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#general)
...
**指南**：使用托管标识，通过 Azure Active Directory (Azure AD) 中的自动托管标识提供 Azure 服务。 使用托管标识可以向支持 Azure AD 身份验证的任何服务（包括 Key Vault）进行身份验证，无需在代码中放入任何凭据。
