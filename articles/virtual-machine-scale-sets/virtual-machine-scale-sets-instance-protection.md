...
description: 了解如何保护 Azure 虚拟机规模集实例以防止进行横向缩减和规模集操作。
author: avirishuv
ms.author: avverma
ms.topic: conceptual
...
使用 [az vmss update](/cli/azure/vmss#az_vmss_update) 将横向缩减保护应用到规模集实例。
...
1. 选择要保护的实例的名称。
1. 选择“保护策略”选项卡。
1. 在“保护策略”边栏选项卡中，选择“防止进行规模集操作”选项。
1. 选择“保存”。 
...
防止对实例进行规模集操作时，也会防止对实例进行自动缩放启动的横向缩减。 设置 "protectFromScaleSetActions": true 时，不能指定 "protectFromScaleIn": false
...
使用 [az vmss update](/cli/azure/vmss#az_vmss_update) 将规模集操作保护应用到规模集实例。
