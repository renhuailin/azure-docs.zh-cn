...
横向缩减策略功能使用户能够通过三种横向缩减配置来配置虚拟机的横向缩减顺序： 
...
请注意，跨可用性区域或容错域的均衡不会在可用性区域或容错域之间移动实例。 均衡的实现方式是删除不平衡的可用性区域或容错域中的虚拟机，直到虚拟机的分布变得均衡。
...
创建一个资源组，然后创建一个将横向缩减策略设置为 OldestVM 的新规模集。
...
下面的示例在创建新规模集时添加横向缩减策略。 首先创建一个资源组，然后创建一个横向缩减策略为 OldestVM 的新规模集。 
