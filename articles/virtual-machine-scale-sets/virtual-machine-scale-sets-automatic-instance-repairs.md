...
启用自动实例修复策略之前，请确保规模集实例具有配置的应用程序终结点，以发出应用程序运行状况状态。 如果实例返回状态 200 ("确定") 此应用程序终结点上，则实例将被标记为 "正常"。 在所有其他情况下，实例被标记为 "不正常"，包括以下方案：
...
此功能目前仅适用于最多为200实例的规模集。 规模集可以部署为单个放置组或多个放置组，但如果为规模集启用了自动实例修复，则实例计数不能超过200。
...
2. 如果端点使用状态200进行响应 (确定) ，则实例将被标记为 "正常"。 在所有其他情况下 (包括) 无法访问终结点，则实例将标记为 "不正常"。
...
结合使用 [AzVmss](/powershell/module/az.compute/get-azvmss?view=azps-3.7.0) cmdlet 和参数 *InstanceView* ，查看 *ServiceState* 以进行自动实例修复。
...
使用 Set-AzVmssOrchestrationServiceState cmdlet 可更新 *serviceState* 以实现自动实例修复。 一旦将规模集选择为自动修复功能，就可以使用此 cmdlet 来挂起或恢复规模集的自动修复。
