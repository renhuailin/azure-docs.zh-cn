...
description: 了解如何在 Azure 内容分发网络中优化大型文件下载。 本文包括多种方案。
services: cdn
documentationcenter: ''
author: asudbring
manager: danielgi
...
大型文件的交付面临多项挑战。 首先，大型文件的平均下载时间可能很长，因为应用程序可能无法逐次下载所有数据。 在某些情况下，应用程序可能先下载文件的最后一部分，然后再下载第一部分。 当仅请求文件的一小部分或者用户暂停下载时，下载会失败。 下载还可能会延迟，直至内容分发网络 (CDN) 从源服务器检索整个文件。 
...
CDN 会在收到区块后进行缓存。 无需在 CDN 缓存上缓存整个文件。 文件或字节范围的后续请求将从 CDN 缓存提供。 如果未在 CDN 上缓存所有区块，将使用预提取从源请求区块。 此优化依赖于源服务器的功能来支持字节范围请求;如果源服务器不支持字节范围请求，则下载大于8mb 大小的数据的请求将会失败。 
...
- 用户可向 CDN 发送范围请求，系统会将其视为普通文件进行处理。 仅当文件类型有效且字节范围在 10 MB 到 150 GB 之间时，才会应用优化。 如果所请求的平均文件大小小于 10 MB，则请改用常规 Web 分发。
