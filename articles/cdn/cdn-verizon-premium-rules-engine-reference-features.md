...
| [Cache-Key 查询字符串](https://docs.vdms.com/cdn/Content/HRE/F/Cache-Key-Query-String.htm) | 确定 cache-key 是否会包括与请求关联的查询字符串参数。 <br> 这是一个相对路径，用于唯一标识缓存的资产。  在检查缓存内容时，边缘服务器使用此相对路径。  默认情况下，缓存项将不包含查询字符串参数。 |
...
| [H.264 支持视频搜寻参数](https://docs.vdms.com/cdn/Content/HRE/F/H.264-Support-VSP-HPD.htm) | 使用 HTTP 渐进式下载时，将重写分配给控制通过 H.264 媒体进行搜寻的参数的名称。 |
...
| [调试缓存响应标头](https://docs.vdms.com/cdn/Content/HRE/F/Debug-Cache-Response-Headers.htm) | 确定是否在响应中包括 [X-EC-Debug 响应标头](https://docs.vdms.com/cdn/Content/Knowledge_Base/X_EC_Debug.htm)，用于提供所请求资产的缓存策略的相关信息。 |
...
### <a name="origin"></a><a name="origin"></a>原点
...
| [代理特殊标头](https://docs.vdms.com/cdn/Content/HRE/F/Proxy-Special-Headers.htm) | 定义一组[特定于 CDN 的请求头](https://docs.vdms.com/cdn/Content/Knowledge_Base/Request-Format.htm#RequestHeaders)，这些标头将从边缘服务器转发给源服务器。 |
...
| [QUIC](https://docs.vdms.com/cdn/Content/HRE/F/QUIC.htm) | 确定是否会就 CDN 服务支持 QUIC 通知客户端。 |
| [流式处理优化](https://docs.vdms.com/cdn/Content/HRE/F/Streaming-Optimization.htm) | 调整缓存配置以优化实时流的性能，并减少源服务器上的负载。 |
| [User 变量](https://docs.vdms.com/cdn/Content/HRE/F/User-Variable.htm) | 向传递到定制流量处理解决方案的用户定义变量赋值。 |
...
[Web 应用程序防火墙](https://docs.vdms.com/cdn/Content/HRE/F/Web_Application_Firewall.htm)功能确定请求是否会被 Web 应用程序防火墙屏蔽。
