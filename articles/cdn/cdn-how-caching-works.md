...
> [!IMPORTANT]
> 默认情况下，针对 DSA 进行了优化的 Azure CDN 终结点将忽略缓存指令标头，绕过缓存。 对于 **Verizon 提供的标准 Azure CDN** 和 **Akamai 提供的标准 Azure CDN** 配置文件，可以使用 [CDN 缓存规则](cdn-caching-rules.md)启用缓存，来调整 Azure CDN 终结点处理这些标头的方式。 仅对于 **Verizon 提供的高级 Azure CDN** 配置文件，可以使用 [规则引擎](./cdn-verizon-premium-rules-engine.md)来启用缓存。
...
- 在从客户端发往 CDN POP 的 HTTP 响应中使用时：
     ...
     - Verizon 推出的 Azure CDN 标准版/高级版和 Microsoft 推出的 Azure CDN 标准版遵循 [RFC 7234 - 超文本传输协议 (HTTP/1.1)：缓存 (ietf.org)](https://tools.ietf.org/html/rfc7234#section-5.2.2.8) 中 Cache-Control 指令的缓存行为 。
