...
> 
> 有关 Azure Blob 存储的详细信息，请参阅 [Blob 存储简介](../storage/blobs/storage-blobs-introduction.md)。
...
> [!NOTE] 
> 缓存规则仅适用于 **Verizon 提供的标准 Azure CDN** 和 **Akamai 提供的标准 Azure CDN** 配置文件。 对于 **Verizon 提供的高级 Azure CDN** 配置文件，必须在 **管理** 门户中使用 [Azure CDN 规则引擎](./cdn-verizon-premium-rules-engine.md)来获得类似的功能。
...
[Azure PowerShell](/powershell/azure/) 是管理 Azure 服务的最快捷且最强大的方式之一。 使用 `Get-AzStorageBlob`cmdlet 来获取对 blob 的引用，并设置 `.ICloudBlob.Properties.CacheControl` 属性。 
...
要使用 .NET 代码指定 Blob 的 `Cache-Control` 标头，请使用[适用于 .NET 的 Azure 存储客户端库](../storage/blobs/storage-quickstart-blobs-dotnet.md)来设置 [CloudBlob.Properties.CacheControl](/dotnet/api/microsoft.azure.storage.blob.blobproperties.cachecontrol) 属性。
...
使用 [Azure 命令行接口](/cli/azure) (CLI)，可以从命令行管理 Azure blob 资源。 若要在使用 Azure CLI 上传 Blob 时设置 cache-control 标头，可使用 `-p` 开关设置 *cacheControl* 属性。 以下示例说明如何将 TTL 设置为 1 小时（3600 秒）：
