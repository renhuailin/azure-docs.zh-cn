...
了解 PowerShell 函数，这些函数用于部署、预配 IoT Edge for Linux on Windows 虚拟机及获取其状态。
...
| installPackages | 无 | **开关参数**。 切换后，该函数将尝试安装 Moby 和 IoT Edge 包，而不只是验证包是否存在。 |
