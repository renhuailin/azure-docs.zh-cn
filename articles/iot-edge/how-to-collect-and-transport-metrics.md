...
[![指标监视体系结构](./media/how-to-collect-and-transport-metrics/arch.png)](./media/how-to-collect-and-transport-metrics/arch.png#lightbox)
...
|  2️ | [指标收集器模块](https://aka.ms/edgemon-metric-collector)是 Microsoft 提供的 IoT Edge 模块，用于收集工作负荷模块指标并将其传输到设备外。 指标收集采用拉取模型。 你可以通过配置收集频率、终结点和筛选器来控制从模块传出的数据。 有关详细信息，请参阅本文后面的[指标收集器配置部分](#metrics-collector-configuration)。 |
...
|  6️ | 若要通过 IoT 中心路由指标，则需要设置一个（一次性）云工作流。 该工作流将处理来自指标收集器模块的消息，并将这些消息发送到 Log Analytics 工作区。 即使是通过此可选路径收到的指标，该工作流也能使用[特选可视化](how-to-explore-curated-visualizations.md)和[警报](how-to-create-alerts.md)功能。 如需详细了解如何设置此云工作流，请参阅[通过 IoT 中心路由指标](#route-metrics-through-iot-hub)部分。 |
...
| 环境变量名称 | 说明 |
|-|-|
| `ResourceId` | 与设备通信的 IoT 中心的资源 ID。 有关检索资源 ID 的步骤，请参阅[资源 ID](#resource-id) 部分。  <br><br>  **必需** <br><br> 默认值：none |
...
## <a name="route-metrics-through-iot-hub"></a>通过 IoT 中心路由指标
...
此方法需进行额外设置，才能将 IoT 中心接收的指标消息传递到 Log Analytics 工作区。 如未完成该等设置，集成的其他部分（如[特选可视化](how-to-explore-curated-visualizations.md)和[警报](how-to-create-alerts.md)）将无法正常工作。
