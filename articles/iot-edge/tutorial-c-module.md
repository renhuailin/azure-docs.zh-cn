...
本教程演示如何在 Visual Studio Code 中使用 C 开发模块，以及如何将其部署到 IoT Edge 设备。 如果要使用 Windows 容器开发模块，请改为参阅[使用 Windows 容器开发 C IoT Edge 模块](tutorial-c-module-windows.md)。
...
3. 在命令面板中，键入并运行 **Azure IoT Edge:New IoT Edge solution** 命令。 按命令面板中的提示创建解决方案。
   ...
   | 为模块提供 Docker 映像存储库 | 映像存储库包含容器注册表的名称和容器映像的名称。 容器映像是基于你在上一步中提供的名称预先填充的。 将 localhost:5000 替换为 Azure 容器注册表中的“登录服务器”值 。 可以在 Azure 门户的容器注册表的“概述”页中检索登录服务器。 <br><br> 最终的映像存储库看起来类似于 \<registry name\>.azurecr.io/cmodule。 |
