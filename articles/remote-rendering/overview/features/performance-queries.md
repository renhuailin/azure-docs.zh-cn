...
| NetworkLatency | 近似平均往返网络延迟（毫秒）。 在上图中，此值对应于红色箭头的总和。 该值是通过 `FrameStatistics` 的 `LatencyPoseToReceive` 值减去实际服务器渲染时间来计算的。 尽管该近似值不准确，但它提供了网络延迟的某个指示，与客户端上计算的延迟值不相关。 |
...
类 `ServiceStatistics` 是一个 C# 类，它包装了帧统计信息和性能评估查询，并提供了方便的功能来以聚合值或预构建字符串的形式返回统计信息。 要在客户端应用程序中显示服务器端统计信息，最简单方法是使用以下代码。
