...
>| AppCpuUsage | 应用程序 CPU 使用率 (预览)  | 百分比 | 针对分配给此应用的 CPU 的 JVM 进程的最近 CPU 使用率，在 [0.0，1.0] 之间的双精度类型值。 如果值为 0.0，则表示在所观察的最近一段时间内，任何 CPU 都未运行 JVM 进程中的线程，而如果值为 1.0，则表示在所观察的最近一段时间内，所有 CPU 都一直在主动运行 JVM 中的线程。 JVM 中的线程包括应用程序线程以及 JVM 内部线程。|
...
>| 名称 | Spring Actuator 指标名称 | 计价单位 | 详细信息 |
>|------|-----------------------------|------|---------|
>| CPU 使用率       | cpu-usage      | 百分比      | 相对于所有系统 CPU 资源，进程的 CPU 使用率的百分比 [0-100]。 |
