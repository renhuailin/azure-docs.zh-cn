...
以下视频介绍了如何使用 Azure Key Vault 管理机密。
...
> [!Important]
> 每个密钥保管库必须具有唯一的名称。 在以下示例中，将 \<your-keyvault-name> 替换为 Key Vault 的名称。
...
6. 要测试应用，请访问公共终结点或测试终结点。
    ...
    你将看到消息 `Successfully got the value of secret connectionString from Key Vault https://<your-keyvault-name>.vault.azure.net/: jdbc:sqlserver://SERVER.database.windows.net:1433;database=DATABASE;`。
...
6. 访问公共终结点或测试终结点以测试应用。
    ...
    你将看到消息 `Successfully got the value of secret test from Key Vault https://<your-keyvault-name>.vault.azure.net: success`。
