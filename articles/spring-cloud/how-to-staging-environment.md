...
* 用于 Azure CLI 的 [Azure Spring Cloud 扩展](/cli/azure/azure-cli-extensions-overview)

本文使用基于 Spring Initializr 构建的应用程序。 如果要对本示例使用其他应用程序，需要在该应用程序面向公众的部分进行一项简单的更改，将过渡部署与生产部署区分开来。
...
1. 使用带有[此配置](https://start.spring.io/#!type=maven-project&language=java&platformVersion=2.3.4.RELEASE&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=hellospring&name=hellospring&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.hellospring&dependencies=web,cloud-eureka,actuator,cloud-starter-sleuth,cloud-starter-zipkin,cloud-config-client)的 Spring Initializr 为示例应用生成代码。
...
1. 从左侧窗格中打开“应用”窗格，查看服务实例的应用。

   ![打开的“应用”窗格的屏幕截图。](media/spring-cloud-blue-green-staging/app-dashboard.png)

1. 可选择某个应用并查看详细信息。

   ![应用的详细信息的屏幕截图。](media/spring-cloud-blue-green-staging/app-overview.png)

1. 打开“部署”以查看应用的所有部署。 该网格显示了生产和过渡部署。

   ![此屏幕截图显示了列出的应用部署。](media/spring-cloud-blue-green-staging/deployments-dashboard.png)

1. 选择 URL 以打开当前部署的应用程序。
    
   ![此屏幕截图显示了已部署的应用程序的 URL。](media/spring-cloud-blue-green-staging/running-blue-app.png)

1. 选择“状态”列中的“生产”以查看默认应用 。
    
   ![此屏幕截图显示了默认应用的 URL。](media/spring-cloud-blue-green-staging/running-default-app.png)

1. 选择“状态”列中的“过渡”，查看过渡应用 。
    
   ![此屏幕截图显示了过渡应用的 URL。](media/spring-cloud-blue-green-staging/running-staging-app.png)

>[!TIP]
> * 确认测试终结点以斜线 (/) 结尾，从而确保正确加载 CSS 文件。  
> * 如果浏览器要求在查看该页之前输入登录凭据，请使用 [URL decode](https://www.urldecoder.org/) 来解码测试终结点。 URL 解码会返回格式为“https://\<username>:\<password>@\<cluster-name>.test.azureapps.io/gateway/green”的 URL。 使用此格式来访问终结点。
...
1. 在过渡环境中验证更改后，可将其推送到生产环境。 在“应用” > “部署”页上，选择当前在“生产”中的应用程序。

1. 选择绿色部署的“注册状态”之后的省略号，然后选择“设置为生产”。 

   ![此屏幕截图显示了用于将过渡生成设置为生产的选项。](media/spring-cloud-blue-green-staging/set-staging-deployment.png)

1. 确认应用的 URL 显示你所做的更改。

   ![此屏幕截图显示了当前在生产环境中的应用的 URL。](media/spring-cloud-blue-green-staging/new-production-deployment.png)
...
若要从 Azure 门户中删除过渡部署，请转到过渡部署页并选择“删除”按钮。
