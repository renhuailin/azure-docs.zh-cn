...
要缩放缓存，请在 [Azure 门户](https://portal.azure.com)中[找到“缓存”](cache-configure.md#configure-azure-cache-for-redis-settings)，并从“资源菜单”选择“缩放”。

![缩放](./media/cache-how-to-scale/redis-cache-scale-menu.png)

在左侧，从“选择定价层”和“选择”中选择想要选择的定价层。
...
当缓存缩放到新的定价层，系统会在左边的“Azure Redis 缓存”边栏选项卡中显示“缩放”状态。
...
修改 `Size`、`Sku` 或 `ShardCount` 属性后，可以在 PowerShell 中使用 [Set-AzRedisCache](/powershell/module/az.rediscache/set-azrediscache) cmdlet 缩放 Azure Redis 缓存实例。 如何将名为 `myCache` 的缓存缩放为 2.5 GB 缓存的演示示例如下。
...
若要使用 Azure CLI 缩放 Azure Cache for Redis 实例，请调用 `azure rediscache set` 命令并传入所需配置更改，包括新大小、sku 或群集大小，具体取决于所需的缩放操作。
...
* 将 **标准** 缓存扩展为更大大小或更大层，或者将 **高级** 缓存扩展为更大大小时，通常将保留所有数据。 将标准或高级缓存缩放到更小的大小时，数据可能会丢失，具体取决于与缩放后的新大小相关的缓存中的数据量。 如果缩小时数据丢失，则使用 [allkeys lru](https://redis.io/topics/lru-cache) 逐出策略逐出密钥。
...
* **标准** 和 **高级** 缓存在缩放操作期间保持可用。 但是，缩放标准和高级缓存时，以及从基本缓存缩放到标准缓存时，可能会发生连接故障。 这些连接故障预期为很小的故障，Redis 客户端通常可以立即重新建立连接。
...
向两个缓存之间添加跨域复制链接后，便无法在群集中启动缩放操作或更改分片数。 若要发布这些命令，必须取消链接缓存。 有关详细信息，请参阅[配置异地复制](cache-how-to-geo-replication.md)。
...
* 不能从较高的定价层缩放到较低的定价层。
  * 不能从 **高级** 缓存向下缩放到 **标准** 或 **基本** 缓存。
  * 不能从 **标准** 缓存向下缩放到 **基本** 缓存。
* 可从 **基本** 缓存缩放到 **标准** 缓存，但不能同时更改大小。 如果需要不同大小，则可以在后续执行缩放操作，将其缩放为所需大小。
