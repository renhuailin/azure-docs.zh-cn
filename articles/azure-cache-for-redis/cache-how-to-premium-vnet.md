...
> [!IMPORTANT]
> Azure Cache for Redis 现在支持 Azure 专用链接，它可简化网络体系结构并保护 Azure 中终结点之间的连接。 可以通过专用终结点从虚拟网络连接到 Azure Cache 实例，该终结点在虚拟网络中的子网内分配有一个专用 IP 地址。 所有层上均提供 Azure 专用链接，包括 Azure Policy 支持和简化的 NSG 规则管理。 要了解详细信息，请参阅[专用链接文档](cache-private-link.md)。 要将 VNet 注入的缓存迁移到专用链接，请参阅[此处](cache-vnet-migration.md)。
>
