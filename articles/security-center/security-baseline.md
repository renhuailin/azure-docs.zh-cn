...
ms.locfileid: "107285714"
---
# <a name="azure-security-baseline-for-security-center"></a>Azure 安全中心安全基线

此安全基线将 [Azure 安全基准 1.0 版](../security/benchmarks/overview-v1.md)中的指南应用于 Azure 安全中心。 Azure 安全基准提供有关如何在 Azure 上保护云解决方案的建议。 内容按“安全控件”分组，此类控件按适用于 Azure 安全中心的 Azure 安全基准和相关的指南定义。 

> [!NOTE]
> 排除了不适用于安全中心或 Microsoft 为其责任方的控制。 若要查看安全中心如何完全映射到 Azure 安全基准，请参阅[完整的安全中心安全基线映射文件](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/security-center-security-baseline-v1.1.xlsx)。
...
- [使用 Log Analytics 代理的防火墙要求](/azure/azure-monitor/platform/log-analytics-agent#firewall-requirements)
...
或者，也可以将安全中心生成的数据发送到 Azure Sentinel 或第三方 SIEM。
...
- [将活动日志发送到 Log Analytics 工作区](/azure/azure-monitor/platform/activity-log#send-to-log-analytics-workspace)
...
或者，也可以启用与 Azure安全中心相关的数据和由 Azure 安全中心生成的数据，并将这些数据载入到 Azure Sentinel 中。 Azure Sentinel 支持 playbook，便于对与安全相关的问题进行自动威胁响应。
...
- [Azure Monitor 中的日志警报](/azure/azure-monitor/platform/alerts-unified-log)
...
**指南**：创建有关使用专用于 Azure 平台或特定于 Azure 安全中心产品/服务的管理帐户的标准操作过程。 使用 Azure 安全中心标识和访问管理来监视 Azure Active Directory (Azure AD) 中管理帐户的数量。 安全中心还具有“安全管理员”内置角色，该角色允许用户更新安全策略以及关闭警报和建议，确保定期评审和协调具有此角色分配的所有用户。
...
指南：启用 Azure Active Directory (Azure AD) 多重身份验证以访问 Azure 安全中心和 Azure 门户，并遵循任何安全中心标识和访问建议。
...
**指导**：Azure Active Directory (Azure AD) 提供日志来帮助发现过时的帐户。 此外，请使用 Azure AD 标识和访问评审来有效管理组成员身份、对企业应用程序的访问以及角色分配。 可定期评审与 Azure 安全中心相关的用户访问权限，确保相应人员继续拥有访问权限。
...
**指导**：使用单独的订阅和管理组对各个安全域（如环境类型和数据敏感度级别）实现隔离。 你可以限制对应用程序和企业环境所需 Azure 资源的访问级别。 可以通过 Azure Active Directory (Azure AD) RBAC 来控制对 Azure 资源的访问。
...
- [安全地将数据发送到 Log Analytics](/azure/azure-monitor/platform/data-security#sending-data-securely-using-tls-12)
...
### <a name="46-use-azure-role-based-access-controls-to-control-access-to-resources"></a>4.6：使用 Azure 基于角色的访问控制来控制对资源的访问 
...
- [Azure Log Analytics 工作区的权限](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#log-analytics-reader)
...
- [如何在密钥保管库中启用软删除和清除保护](https://docs.microsoft.com/azure/storage/blobs/soft-delete-blob-overview?tabs=azure-portal)

- [Azure 存储 Blob 的软删除](https://docs.microsoft.com/azure/storage/blobs/soft-delete-blob-overview?tabs=azure-portal)
