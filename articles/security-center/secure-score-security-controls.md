...
安全分数以百分比值的形式显示在 Azure 门户页面中，但基础值也清楚显示：
...
你可以通过 Azure 门户或以编程方式查看总体安全分数以及每个订阅的评分，如以下部分中所述：

- [从门户获取安全功能分数](#get-your-secure-score-from-the-portal)
- [从 REST API 获取安全功能分数](#get-your-secure-score-from-the-rest-api)
- [从 Azure 资源关系图中获取安全分数 (ARG) ](#get-your-secure-score-from-azure-resource-graph-arg)

### <a name="get-your-secure-score-from-the-portal"></a>从门户获取安全功能分数

安全中心会在门户中突出显示你的分数：这是 "安全中心概述" 页中的第一个主要磁贴。 选择此磁贴将转到 "专用安全分数" 页，您可以在其中看到按订阅细分的分数。 选择单个订阅查看按优先级排列的建议的详细列表，以及修正这些建议将对订阅分数产生的潜在影响。

概括而言，安全分数显示在安全中心门户页面中的以下位置。
...
可以通过安全功能分数 API（当前为预览版）访问分数。 通过 API 方法，可灵活地查询数据，久而久之构建自己的安全功能分数报告机制。 例如，可以使用 [安全评分 API](/rest/api/securitycenter/securescores) 来获取特定订阅的分数。 此外，你可以使用 [安全分数控制 API](/rest/api/securitycenter/securescorecontrols) 列出你的订阅的安全控制和当前分数。
...
### <a name="get-your-secure-score-from-azure-resource-graph-arg"></a>从 Azure 资源关系图中获取安全分数 (ARG) 

使用 Azure 资源关系图，可以通过强大的筛选、分组和排序功能，在你的云环境中即时访问资源信息。 这是以编程方式或从 Azure 门户中查询 Azure 订阅中的信息的一种快速且有效的方式。 [了解有关 Azure 资源关系图的详细信息](https://docs.microsoft.com/azure/governance/resource-graph/)。
...
1. 使用以下示例输入 Kusto 查询 (以获取指南) 。
    ...
    - 此查询将返回订阅 ID、当前分数（以磅为单位），并返回订阅的最大分数。 
    ...
    - 此查询返回所有安全控件的状态。 对于每个控件，您将获得不正常资源的数量、当前评分和最大分数。 
...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-lboi"; width=55%>- 应对 SQL 数据库启用高级数据安全<br>- 应修正 Azure 容器注册表映像中的漏洞<br>- 应修正 SQL 数据库中的漏洞<br>- 应通过漏洞评估解决方案修正漏洞<br>- 应对 SQL 托管实例启用漏洞评估<br>- 应对 SQL Server 启用漏洞评估<br>- 应在虚拟机上安装漏洞评估解决方案<br>- 容器映像只应从受信任的注册表部署 (预览) <br>- 应在群集上安装和启用适用于 Kubernetes 的 Azure 策略外接程序， (预览) </td>
  </tr>
  <tr>
  ...
  <tr>
    <td class="tg-lboi"><strong><p style="font-size: 16px"> 启用高级威胁防护（最大分数 0）</p></strong>Azure 安全中心的可选 Azure Defender 威胁防护计划为你的环境提供全面的防御。 当安全中心检测到环境中的任何区域遭到威胁时，会生成警报。 这些警报会描述受影响资源的详细信息、建议的修正步骤，在某些情况下还会提供触发逻辑应用作为响应的选项。<br>每个 Azure Defender 计划都是单独的可选产品/服务，可以使用此安全控制中的相关建议启用此功能。<br><a href="/azure/security-center/threat-protection">详细了解安全中心的威胁防护</a>。</td>
