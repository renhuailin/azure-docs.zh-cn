...
## <a name="display-azure-security-center-recommendations-in-microsoft-cloud-app-security"></a>在 Microsoft Cloud App Security 中显示 Azure 安全中心建议<a name="azure-mcas"></a>

Microsoft Cloud App Security 是一个云访问安全代理 (CASB)，它支持各种部署模式，包括日志集合、API 连接器和反向代理。 它提供了丰富的显示效果、数据旅程控制和成熟分析服务，用于跨所有 Microsoft 和第三方云服务发现和防范网络威胁。

如果已启用 Microsoft Cloud App Security，并且从安全中心的设置中选择了集成，则安全中心的强化建议将显示在 Cloud App Security 中，而无需进行其他配置。

> [!NOTE]
> 安全中心将安全相关的客户数据存储在其资源所在的地理区域。 如果 Microsoft 尚未在该资源所在的地理位置部署安全中心，则数据将存储在美国。 启用 Microsoft Cloud App Security 后，将根据 Microsoft Cloud App Security 的地理位置规则存储此信息。 有关详细信息，请参阅[非区域性服务的数据存储](https://azuredatacentermap.azurewebsites.net/)。


## <a name="stream-security-alerts-from-other-microsoft-services"></a>流式传输来自其他 Microsoft 服务的安全警报<a name="alerts-other"></a>

### <a name="display-azure-waf-alerts-in-security-center"></a>在安全中心显示 Azure WAF 警报<a name="azure-waf"></a>
...
### <a name="display-azure-ddos-protection-alerts-in-security-center"></a>在安全中心显示 Azure DDoS 防护警报<a name="azure-ddos"></a>
...
如果已启用 Azure DDoS 防护，则无需进行额外的配置，就会将 DDoS 警报流式传输到安全中心。 有关 DDoS 防护生成的警报详细信息，请参阅[警报的引用表](alerts-reference.md#alerts-azureddos)。
