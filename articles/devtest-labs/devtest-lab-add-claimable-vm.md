---
title: 创建和管理可认领 VM
description: 了解如何使用 Azure 门户在 Azure 开发测试实验室中添加可认领虚拟机，并查看认领/取消认领虚拟机要遵循的流程。
ms.topic: how-to
ms.date: 06/26/2020
...
在用户认领 VM 后，开发测试实验室将启动该计算机，并将其上移到实验室用户的“我的虚拟机”列表中。 这意味着，实验室用户现在将拥有此计算机的所有者权限。 此步骤所需的时间可能不同，具体取决于启动时间以及在认领事件期间执行的任何其他自定义操作。 一旦认领后，该计算机在可认领池中将不再可用。  
...
用户在取消认领 VM 后，将不再拥有该特定实验室 VM 的所有者权限，并且该 VM 将可被任何其他实验室用户申领，而其状态显示已退回到池。 
