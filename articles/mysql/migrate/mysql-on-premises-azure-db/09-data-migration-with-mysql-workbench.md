---
title: 从本地 MySQL 到 Azure Database for MySQL 的迁移指南：通过 MySQL 工作台迁移数据
...
ms.locfileid: "112082678"
---
# <a name="mysql-on-premises-to-azure-database-for-mysql-migration-guide-data-migration-with-mysql-workbench"></a>从本地 MySQL 到 Azure Database for MySQL 的迁移指南：通过 MySQL 工作台迁移数据
...
在开始向 Azure Database for MySQL 导入进程之前，请查看服务器参数。 可使用 [Azure 门户](/azure/mysql/howto-server-parameters)检索和设置服务器参数，或者通过调用 [Azure PowerShell for MySQL cmdlet](/azure/mysql/howto-configure-server-parameters-using-powershell) 进行更改。
...
- 若要用 mysql 工具执行相同操作，请将 [CA 根证书](https://www.digicert.com/CACerts/BaltimoreCyberTrustRoot.crt.pem) 下载到 c:\\temp（使用此目录）。

    > [!NOTE]
    > 该证书可能变更。 有关证书的最新信息，请参阅[配置应用程序中的 SSL 连接性以安全连接到 Azure Database for MySQL](/azure/mysql/howto-configure-ssl)。
...
- `innodb\_buffer\_pool\_size` – 在迁移期间从门户的定价层纵向扩展服务器到 32 vCore 内存优化 SKU，以增大 innodb\_buffer\_pool\_size。 只能通过纵向扩展 Azure Database for MySQL 服务器的计算来增大 innodb\_buffer\_pool\_size。 参阅 [Azure Database for MySQL 中的服务器参数](/azure/mysql/concepts-server-parameters#innodb_buffer_pool_size)以了解该层的最大值。 内存优化 32 vCore 系统中的最大值为 `132070244352`。

- `innodb\_io\_capacity` & `innodb\_io\_capacity\_max` - 将参数更改为 `9000` 以提高 IO 利用率，从而优化迁移速度。

- `max\_connections` - 如果使用生成多个线程的工具来提高吞吐量，请增加连接来支持该工具。 默认值为 `151`，最大值为 `5000`。
...
- 选择“开始导出”，请注意 MySQL 工作台会调用 `mysqldump` 工具。

- 打开新创建的导出脚本。

- 查找所有 `DEFINER` 语句，并将其更改为有效的用户或完全删除。
...
- 查找 `SET GLOBAL` 语句并将其更改为有效的用户或将其完全删除。

- 确保未将 `sql\_mode` 设置为 `NO\_AUTO\_CREATE\_USER`。

- 删除 `hello\_world` 函数。

- 在 MySQL 工作台中，创建与 Azure Database for MySQL 的新连接。
...
可以更改 Azure Database for MySQL 目标实例上的以下参数。 可以通过 Azure 门户或使用 [Azure PowerShell for MySQL cmdlet](/azure/mysql/howto-configure-server-parameters-using-powershell) 来设置这些参数。
