...
> [!Note]
> Azure Database for MySQL Flexible 服务器目前不支持基于 GTID 的复制。 
...
若要跳过从源服务器（托管在本地、虚拟机中或其他云提供程序托管的数据库服务中）复制表的操作，需支持 `replicate_wild_ignore_table` 参数。 （可选）使用 [Azure 门户](how-to-configure-server-parameters-portal.md)或 [Azure CLI](how-to-configure-server-parameters-cli.md) 在副本服务器（在 Azure 中托管）上更新此参数。
