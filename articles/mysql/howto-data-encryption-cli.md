...
description: 了解如何通过 Azure CLI 为 Azure Database for MySQL 设置和管理数据加密。
author: mksuni
ms.author: sumuth
ms.service: mysql
ms.topic: how-to
...
---
# <a name="data-encryption-for-azure-database-for-mysql-by-using-the-azure-cli"></a>通过 Azure CLI 进行 Azure Database for MySQL 数据加密

了解如何通过 Azure CLI 为 Azure Database for MySQL 设置和管理数据加密。
...
* 在已创建的 Azure Key Vault 中，创建要用于 Azure Database for MySQL 数据加密的密钥。
...
1. 可通过两种方式获取 Azure Database for MySQL 的托管标识。
   ...
   ### <a name="update-an-existing-the-azure-database-for-mysql-server-to-get-a-managed-identity"></a>更新现有的 Azure Database for MySQL 服务器以获取托管标识。

   ```azurecli-interactive
   az mysql server update --name  <server name>  -g <resource_group> --assign-identity
   ```

2. 为“主体”（即 MySQL 服务器的名称）设置“密钥权限”（“获取”、“包装”、“解包”）。
...
1. 使用 Azure Key Vault 中创建的密钥为 Azure Database for MySQL 启用数据加密。
...
除了 Azure 门户之外，还可以使用新服务器和现有服务器的 Azure 资源管理器模板在 Azure Database for MySQL 服务器上启用数据加密。

### <a name="for-a-new-server"></a>对于新服务器

使用预先创建的 Azure 资源管理器模板之一来预配启用了数据加密的服务器：[数据加密示例](https://github.com/Azure/azure-mysql/tree/master/arm-templates/ExampleWithDataEncryption)
