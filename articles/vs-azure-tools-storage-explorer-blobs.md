...
[Azure Blob 存储](./storage/blobs/storage-quickstart-blobs-dotnet.md)是用于存储大量非结构化数据（例如文本或二进制数据）的服务，这些数据可通过 HTTP 或 HTTPS 从世界各地进行访问。
...
[共享访问签名 (SAS)](./storage/common/storage-sas-overview.md) 用于对存储帐户中的资源进行委托访问。
