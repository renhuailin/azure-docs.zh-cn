...
热修补是在新的 Windows Server Azure Edition 虚拟机 (VM) 上安装更新的一种新方式，安装后无需重新启动。 本文介绍有关适用于 Windows Server Azure Edition VM 的热补丁的信息，热补丁具有以下优势：
...
> [!NOTE]
> 在预览阶段，只能使用[此链接](https://aka.ms/AzureAutomanageHotPatch)在 Azure 门户中开始使用。
...
1.  通过 Azure 门户创建 VM
    * 预览期间，你将需要使用[此链接](https://aka.ms/AzureAutomanageHotPatch)开始创建。
1.  提供 VM 详细信息
    * 确保在“映像”下拉列表中选择了“Windows Server 2019 Datacenter: Azure Edition”
...
在 Windows Server 2019 Datacenter: Azure Edition VM上启用热补丁后，大多数每月安全更新都以热补丁的方式提供，这些热补丁不要求重新启动。 在计划基线或计划外基线月份发送的最新累积更新则要求重新启动 VM。 还可能会定期提供额外的“关键”或“安全”类修补程序，这可能需要重新启动 VM。

VM 每隔数天自动评估，并在任意 30 天周期内多次评估，以确定适用于该 VM 的补丁。 这种自动评估确保能够尽早发现任何缺失的补丁。

在发布每月补丁后的 30 天内，将按照[可用性优先原则](../virtual-machines/automatic-vm-guest-patching.md#availability-first-patching)安装补丁。 只会根据 VM 的时区在 VM 的非高峰期安装补丁。 VM 在非高峰期必须正在运行，才能自动安装补丁。 如果 VM 在定期评估期间已关闭，则会评估 VM，并在 VM 打开后的下一次定期评估期间自动安装适用的补丁。 下一次定期评估通常会在几天内发生。
...
* 热修补是一种在 Azure 中的 Windows Server 2019 Datacenter: Azure Edition VM 上安装更新的新途径，在安装后不需要重新启动。 热修补的工作方式是在不需要重新启动进程的情况下，修补正在运行的进程的内存中代码。
...
* 当你在 Windows Server 2019 Datacenter: Azure Edition 中使用热补丁时，你的 VM 将具有更高的可用性（重启次数更少）和更快的更新（安装包更小，安装速度更快，并且需要重新启动）。 该过程会导致 VM 始终处于最新且安全的状态。
...
* 当前不支持从 Windows Server 的现有版本（即 Windows Server 2016 或 2019 的非 Azure 版本）进行升级。 今后将支持升级到未来版本的 Windows Server Azure Edition。
...
* 在预览期间，Windows Server Azure Edition 的许可证是免费的。 但是，为 VM 设置的任何底层基础结构的成本（存储、计算、网络等）仍会向你的订阅计费。
