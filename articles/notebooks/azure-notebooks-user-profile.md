...
用户 ID 必须包含四到十六个字母、数字和连字符。 不能使用其他字符，且用户 ID 不能以连字符开头或结尾，也不能在一行中使用多个连字符。 因为用户 Id 在所有 Azure Notebooks 帐户中是唯一的，所以你可能会看到消息 "用户 ID 已在使用中"。 如果尝试使用 Microsoft 商标作为用户 ID，还会显示 (消息。 ) 在这些情况下，请选择其他用户 ID。
