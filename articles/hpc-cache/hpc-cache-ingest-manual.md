...
本文详细说明了如何手动将数据复制到 Blob 存储容器以用于 Azure HPC 缓存。 它使用多线程并行操作来优化复制速度。

有关将数据移到 Azure HPC 缓存的 Blob 存储的详细信息，请参阅[将数据移到 Azure Blob 存储](hpc-cache-ingest.md)。

## <a name="simple-copy-example"></a>简单复制示例

可以通过针对预定义的文件或路径集在后台一次性运行多个复制命令，在客户端上手动创建多线程复制。

Linux/UNIX ``cp`` 命令包含用于保留所有权和 mtime 元数据的 ``-p`` 参数。 可以选择性地将此参数添加到以下命令。 （添加该参数会增加客户端为了修改元数据而向目标文件系统发送的文件系统调用数。）
...
例如，如果目录包含 1000 个文件（按顺序从 `0001` 到 `1000` 编号），则可以使用以下表达式创建 10 个并行线程，其中每个线程复制 100 个文件：
...
## <a name="copy-data-with-unstructured-file-names"></a>复制包含非结构化文件名的数据
...
针对单个目标文件系统装入点运行足够多的并行线程后，在某个时间点，添加更多的线程便不能再提高吞吐量。 （将会根据每秒文件数或每秒字节数来测量吞吐量，具体取决于数据类型。）更糟糕的是，过多的线程有时会导致吞吐量下降。

如果发生这种情况，则可以使用相同的远程文件系统装载路径，将客户端装入点添加到其他 Azure HPC 缓存装载地址：
