...
> [!TIP]
> 请参阅[管理存储目标](manage-storage-targets.md)，了解如何删除或暂停存储目标，或让它们将缓存数据写入后端存储。
...
缓存中采用某些自定义 DNS 设置类型的存储目标还提供一个控件用于刷新其 IP 地址。 （这种配置非常罕见。）在[管理存储目标](manage-storage-targets.md#update-ip-address-custom-dns-configurations-only)中了解如何刷新 DNS 设置。
...
* 有关停止、删除和刷新各个存储目标的信息，请参阅[管理存储目标](manage-storage-targets.md)。
