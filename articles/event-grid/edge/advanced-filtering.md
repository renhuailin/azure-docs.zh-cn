...
* `Value` - 运行筛选器所依据的引用值（或）`Values` - 运行筛选器所依据的引用值集。
...
对于支持多个值的运算符（如 `NumberIn`、`NumberNotIn`、`StringIn` 等），每个值都被视为一个 `OR` 条件。 因此，`StringBeginsWith("a", "b", "c")` 将匹配以 `a`、`b` 或 `c` 开头的任何字符串值。

> [!CAUTION]
> NOT 运算符 - `NumberNotIn` 和 `StringNotIn` 对 `Values` 字段中给定的每个值的行为方式类似于 AND 条件。
>
> 如果不这样做，会使筛选器成为“全部接受”筛选器并违背筛选的目的。

## <a name="floating-point-rounding-behavior"></a>浮点舍入行为

事件网格使用 `decimal` .NET 类型处理所有数字值。 在事件订阅 JSON 中指定的数值不受浮点舍入行为限制。

## <a name="case-sensitivity-of-string-filters"></a>字符串筛选器是否区分大小写

所有字符串比较都不区分大小写。 目前没有方法更改此行为。

## <a name="allowed-advanced-filter-keys"></a>允许的高级筛选键

`Key` 属性可以是已知的顶级属性，也可以是具有多个点的 json 路径，其中每个点表示单步执行嵌套 json 对象。

与 JSONPath 规范不同，事件网格对于密钥中的 `$` 字符没有任何特殊含义。
...
自定义事件架构中的 `Key` 没有任何限制，因为事件网格不会对有效负载强制实施任何信封架构。

## <a name="numeric-single-value-filter-examples"></a>数值单值筛选器示例
