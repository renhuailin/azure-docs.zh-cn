...
description: IoT Edge 上的事件网格的概念。
author: VidyaKukke
manager: rajarv
ms.author: vkukke
...
从事件网格的角度看，事件处理程序是发送事件的位置。 处理程序将执行进一步的操作来处理事件。 事件网格支持多个处理程序类型。 可以使用受支持的 Azure 服务或你自己的 Webhook 作为处理程序。 根据处理程序的类型，事件网格遵循不同机制，以保证事件的传递。 如果目标事件处理程序是 HTTP Webhook，则会重试该事件，直到处理程序返回状态代码 `200 – OK`。 对于 Edge 中心，如果传递事件时未发生任何异常，则认为该事件成功。
...
使用自定义主题时，必须始终在数组中发布事件。 对于低吞吐量方案，数组将只有一个值。 对于大容量用例，建议在每次发布时对多个事件进行批处理，以实现更高的效率。 批的大小最大可达 1 MB。 每个事件还不应超过 1 MB（预览版）。