...
事件网格提供持久传送。 它为每个订阅 **至少传递一次** 消息。 事件会立即发送到每个订阅的已注册终结点。 如果终结点未确认收到事件，事件网格会重试传送事件。

> [!NOTE]
> 事件网格不保证事件传递的顺序，因此订户可能会按顺序接收它们。 
...
> [!NOTE]
> 如果没有为终结点配置 Dead-Letter，则出现上述错误时将删除事件，因此，如果不想删除这些类型的事件，请考虑配置死信。
