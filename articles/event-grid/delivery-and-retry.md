...
如果订阅的终结点返回的错误是与配置相关的错误，不能通过重试解决 (例如，如果终结点已被删除) ，EventGrid 将对事件执行死信，或在死信未配置时删除事件。

下面是不会进行重试的终结点类型：

| 终结点类型 | 错误代码 |
| --------------| -----------|
| Azure 资源 | 400请求错误，413请求实体太大，403被禁止 | 
| Webhook | 400错误请求，413请求实体太大，403被禁止，找不到404，未授权401 |
 
> [!NOTE]
> 如果没有为终结点配置 Dead-Letter，则出现上述错误时将删除事件。 如果不想删除这些类型的事件，请考虑配置死信。

如果订阅的终结点返回的错误不在上面的列表中，则 EventGrid 将使用如下所述的策略执行重试：
...
事件网格已进行所有重试尝试后会将事件发送到死信位置。 如果事件网格收到 400 (错误请求) 或 413 (请求实体太大) 响应代码，它会立即为死信计划事件。 这些响应代码指示事件传送将永远不会成功。
...
不在以上集中的所有其他代码 (200-204) 被视为失败，并将在需要时重试 () 。 某些代码已关联到下面所述的特定重试策略，所有其他代码遵循标准的指数退让模型。 请务必注意，由于事件网格体系结构的高度并行化特性，重试行为是不确定的。 

| 状态代码 | 重试行为 |
| ------------|----------------|
| 400 错误的请求 | 不重试 |
| 401 未授权 | 在5分钟或更多的 Azure 资源终结点后重试 |
| 403 禁止访问 | 不重试 |
| 404 未找到 | 在5分钟或更多的 Azure 资源终结点后重试 |
