...
Azure 提供三种协助在整个解决方案中传递事件或消息的服务。 这些服务分别为：
...
事件网格是启用事件驱动、反应编程的事件底板。 它使用发布-订阅模式。 发布服务器发出事件，但不知道如何处理事件。 订阅服务器决定需要处理的事件。
...
有关详细信息，请参阅[事件中心概述](../event-hubs/event-hubs-about.md)。
...
有关详细信息，请参阅[服务总线概述](../service-bus-messaging/service-bus-messaging-overview.md)。
...
:::image type="content" source="./media/compare-messaging-services/overview.png" alt-text="显示事件中心、服务总线和事件网格如何连接在一起的图像。":::
