...
description: Azure Migrate 安全基线为实现 Azure 安全基准中指定的安全建议提供了过程指南和资源。
author: msmbaldwin
ms.topic: conceptual
ms.date: 12/04/2020
ms.author: mbaldwin
...
ms.locfileid: "101094813"
---
# <a name="azure-security-baseline-for-azure-migrate"></a>适用于 Azure Migrate 的 Azure 安全基线

此安全基线将 [Azure 安全基准版本 1.0](../security/benchmarks/overview-v1.md) 中的指南应用于 Azure Migrate。 Azure 安全基准提供有关如何在 Azure 上保护云解决方案的建议。
内容按“安全控件”分组，这些控件根据适用于 Azure Migrate 的 Azure 安全基准和相关指南定义。 排除了不适用于 Azure Migrate 的“控件”。

 
若要查看 Azure Migrate 如何完全映射到 Azure 安全基准，请参阅[完整的 Azure Migrate 安全基线映射文件](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)。
...
指南：Azure Migrate 使用 Azure Active Directory (Azure AD) 作为默认标识和访问管理服务。 你应该使 Azure AD 标准化，以便控制组织在以下资源中的标识和访问管理：

- Microsoft 云资源，如 Azure 门户、Azure 存储、Azure 虚拟机（Linux 和 Windows）、Azure Key Vault、PaaS 和 SaaS 应用程序。

- 贵组织的资源，例如 Azure 上的应用程序，或公司网络资源。
...
指南：Azure Migrate 使用 Azure Active Directory 来提供对 Azure 资源、云应用程序和本地应用程序的标识和访问管理。 此内容包括企业标识（例如员工）以及外部标识（如合作伙伴和供应商）。 这样，单一登录 (SSO) 便可以管理和保护对本地和云中的组织数据和资源的访问。 将所有用户、应用程序和设备连接到 Azure AD，实现无缝的安全访问和更好的可见性和控制。
...
指南：Azure Migrate 使用 Azure Active Directory，后者支持通过多重身份验证进行的强身份验证控制，并且支持强无密码方法。
...
指南：Azure Migrate 已与提供以下数据源的 Azure Active Directory 集成：
...
指南：Azure Migrate 使用 Azure RBAC，通过限制向哪些帐户授予对其所属的订阅和管理组的特权访问权限，来隔离对业务关键型系统的访问。
...
指南：Azure Migrate 使用 Azure Active Directory 帐户来定期管理其资源、审阅用户帐户和访问权限分配，以确保帐户及其访问权限有效。 可使用 Azure AD 访问评审来审查组成员身份、对企业应用程序的访问权限和角色分配。 Azure AD 报告提供日志来帮助发现过时的帐户。 你还可使用 Azure AD Privileged Identity Management 来创建访问评审报表工作流以便于执行评审。
...
指南：Azure Migrate 已与 Azure Active Directory 集成，以管理其资源。 使用 Azure AD 的权利管理功能可自动执行访问请求工作流，包括访问权限分配、审查和过期。 还支持两阶段或多阶段审批。
...
指南：Azure Migrate 通过与 Azure 基于角色的访问控制 (RBAC) 集成来管理其资源。 使用 Azure RBAC，可通过角色分配来管理 Azure 资源访问。 可以将这些角色分配给用户、组服务主体和托管标识。 某些资源具有预定义的内置角色，可以通过工具（例如 Azure CLI、Azure PowerShell 或 Azure 门户）来清点或查询这些角色。 通过 Azure RBAC 分配给资源的特权应始终限制为角色所需的特权。 这是对 Azure AD Privileged Identity Management (PIM) 的实时 (JIT) 方法的补充，应定期进行审查。
...
指南：Azure Migrate 中的服务器迁移工具将数据从正在迁移的服务器磁盘中复制到 Azure 订阅中的存储帐户和托管磁盘。 在将数据处理写入到订阅中的存储帐户或托管磁盘之前，数据处理是暂时性的，并且不会保留在 Azure Migrate 中

通过使用 Azure 基于角色的访问控制 (Azure RBAC) 和基于网络的访问控制以及 Azure 服务（如用于托管磁盘中的存储服务加密）中的特定控制限制访问，保护存储在存储帐户和托管磁盘上的敏感数据。 
...
指南：Azure Migrate 不会保留任何敏感数据。 Azure Migrate 中保留的所有数据都使用 Microsoft 托管密钥进行静态加密。

Azure Migrate 中的服务器迁移工具将数据从正在迁移的服务器磁盘中复制到 Azure 订阅中的存储帐户和托管磁盘。 磁盘可能包含也可能不包含敏感数据。 在将数据处理写入到订阅中的存储帐户或托管磁盘之前，数据处理是暂时性的，并且不会保留在 Azure Migrate 中。 存储帐户和托管磁盘上的复制数据使用 Microsoft 托管密钥进行静态加密。 对于高度敏感的数据，你可以选择使用存储帐户和托管磁盘上的客户托管密钥实现额外的静态加密。

- [将 VMware 虚拟机迁移到具有服务器端加密 (SSE) 和客户托管密钥 (CMK) 的 Azure](how-to-migrate-vmware-vms-with-cmk-disks.md)

- [通过客户托管密钥配置存储帐户加密](../storage/common/customer-managed-keys-configure-key-vault.md)
...
Azure Migrate 不允许在其资源上运行应用程序或安装软件。 
...
配置方式
- [配置条件访问以阻止访问 Azure 资源管理器](../role-based-access-control/conditional-access-azure-management.md)
...
指南：Azure Migrate 当前不生成 Azure 资源日志。

自动可用的活动日志包含针对 Azure Migrate 资源的所有写入操作（PUT、POST、DELETE），但读取操作 (GET) 除外。 活动日志可用于在进行故障排除时查找错误，或监视组织中的用户如何对资源进行修改。
