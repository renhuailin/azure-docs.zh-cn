...
1. 在“评估服务器” > “评估类型”中，选择“Azure VMware 解决方案 (AVS)（预览版）”  。
...
- 跨 AVS 节点的利用率：跨所有节点的预计的 CPU、内存和存储利用率。
    ...
    - 当前，内存、重复数据删除和压缩设置为 100% 内存利用率、1.5 重复数据删除和压缩，这将在以后的发行版中作为用户定义的输入，从而进一步允许用户微调所需的大小。
