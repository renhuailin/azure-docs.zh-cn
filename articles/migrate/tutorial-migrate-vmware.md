...
11. 在“计算”中，查看 VM 名称、大小、OS 磁盘类型和可用性配置（如果在上一步中选定）。 VM 必须符合 [Azure 要求](migrate-support-matrix-vmware-migration.md#azure-vm-requirements)。
...
2. 如果计算机具有 Linux OS，请在迁移的计算机上安装 Azure VM [Linux](../virtual-machines/extensions/agent-linux.md) 代理。 在迁移期间，我们会自动为 Windows VM 安装 VM 代理。
