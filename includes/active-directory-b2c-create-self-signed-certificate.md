...
ms.locfileid: "102095283"
---
如果还没有证书，可以使用自签名证书。 自签名证书是未由证书颁发机构签署 (CA) 的安全证书，并且不提供由 CA 签名的证书的安全保障。 
...
1. 选择证书，然后选择 "**操作**" "  >  **所有任务**" "  >  **导出**"。
1. 选择“是” > “下一步” > “是，导出私钥” > “下一步”   。
1. 接受“导出文件格式”的默认值。
1. 提供证书的密码。

为了 Azure AD B2C 接受 .pfx 文件密码，必须在 Windows 证书存储导出实用工具中将密码加密为 TripleDES-SHA1 选项，而不是 AES256-SHA256。

# <a name="macos"></a>[macOS](#tab/macos)

在 macOS 上，使用密钥链 Access 中的 [证书助手](https://support.apple.com/guide/keychain-access/aside/glosa3ed0609/11.0/mac/11.0) 生成证书。

1. 按照如何 [在 Mac 上的密钥链 Access 中创建自签名证书](https://support.apple.com/guide/keychain-access/kyca8916/mac)的说明进行操作。
