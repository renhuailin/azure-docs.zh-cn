...
- 如果与 Azure 存储容器关联的共享上传的 blob 与创建时为该共享定义的 blob 类型不匹配，则不会更新此类 blob。 例如，如果在设备上创建块 blob 共享，将该共享与具有页 blob 的现有云容器相关联，刷新该共享以下载文件，然后修改已作为页 blob 存储在云中的某些已刷新文件，你将看到上传失败。
- 不支持在共享中创建文件后重命名该文件。
