...
分区数是在创建时指定的，必须介于 1 和每个定价层[允许的最大分区计数](../articles/event-hubs/event-hubs-quotas.md#basic-vs-standard-vs-premium-vs-dedicated-tiers)之间。 

建议至少选择在特定事件中心的应用程序峰值负载期间，与持续预期所需[吞吐量单位 (TU)](../articles/event-hubs/event-hubs-scalability.md#throughput-units)、[处理单位 (PU)](../articles/event-hubs/event-hubs-scalability.md#processing-units) 或[容量单位 (CU)](../articles/event-hubs/event-hubs-dedicated-overview.md) 数量相同的分区数。

应该根据在每个层中具有按定价单位（TU、PU 或 CU）计量的吞吐容量的单个分区进行计算。 可以缩放命名空间的 TU 或 PU 或者专用群集的 CU，而不管分区计数如何。 例如，当命名空间设置为 1 TU 容量时，具有 32 个分区的标准层或具有 1 个分区的事件中心会产生完全相同的费用。 
