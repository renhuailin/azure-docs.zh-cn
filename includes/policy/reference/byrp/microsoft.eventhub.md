...
|[事件中心命名空间应使用客户托管的密钥进行加密](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2Fa1ad735a-e96f-45d2-a7b2-9a4932cab7ec) |Azure 事件中心支持选择将静态数据与 Microsoft 托管的密钥加密 (默认) 或客户托管的密钥。 如果选择使用客户管理的密钥加密数据，则可以分配、旋转、禁用和撤消对事件中心将用于加密命名空间中的数据的密钥的访问权限。 请注意，事件中心仅支持对专用群集中的命名空间的客户托管密钥进行加密。 |Audit、Disabled |[1.0.0](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Event%20Hub/EventHub_CustomerManagedKeyEnabled_Audit.json) |
