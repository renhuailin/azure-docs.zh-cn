...
| Azure 中虚拟设备上的分层卷的大小上限 |30 TB（对于 StorSimple 8010） <br></br> 64 TB（对于 StorSimple 8020） |StorSimple 8010 和 StorSimple 8020 是 Azure 中的虚拟设备，分别使用标准存储和高级存储。 |
| 物理设备上本地固定卷的大小上限 |9 TB（对于 StorSimple 8100） <br></br> 24 TB（对于 StorSimple 8600） |StorSimple 8100 和 StorSimple 8600 是物理设备。 |
| iSCSI 连接的最大数目 |512 | |
...
| 可以并行处理以实现备份、还原或克隆的卷的最大数目 |16 |<ul><li>如果卷数超过 16 个，它们会在处理槽可用时依序处理。</li><li>在完成操作之前，无法为克隆或还原的分层卷创建新备份。 对于本地卷，在卷联机后可以备份。</li></ul> |
