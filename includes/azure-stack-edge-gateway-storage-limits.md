...
---
本部分介绍了应用于 Azure Stack Edge 服务的 Azure 存储服务限制，以及 Azure 文件存储、Azure 块 Blob 和 Azure 页 Blob 的必需命名约定。 请仔细查看存储限制并遵循所有建议。
...
> [!IMPORTANT]
> 如果有任何文件或目录超出了 Azure 存储服务限制，或者不符合 Azure 文件存储/Blob 命名约定，则这些文件或目录不会通过 Azure Stack Edge 服务引入到 Azure 存储中。