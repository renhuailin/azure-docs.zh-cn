...
使用传输层安全性 (TLS) 标准来保护 IoT 设备和 IoT 中心之间的 Internet 连接安全。 Azure IoT 支持 [TLS 1.2](https://tools.ietf.org/html/rfc5246)、TLS 1.1 和 TLS 1.0（按此顺序）。 对 TLS 1.0 的支持仅为向后兼容性提供。 检查 [IoT 中心内的 TLS 支持](../articles/iot-hub/iot-hub-tls-support.md)，以了解如何将中心配置为使用 TLS 1.2，因为它可提供最大安全性。
